...
**Project and package discovery is an adapter capability, and one adapter serves every Node-hosted tool.** What a program is on disk — the project directory, the files beneath it, the directories its dependencies are found in — is read by a `host/` package — the Node host adapter above, as far as project and package discovery goes — which the language server and the command-line compiler share rather than each answering for itself, the language server being a second Node-hosted tool over the same adapter and not an adapter of its own; the compiler core receives packages as records (Packages §4.1) and never spells `node_modules`. One program per project directory: the directory holding a `hexagon.json`, a manifest nested beneath it being a package of its own (Packages §2.2) and so a program of its own here, and a directory holding none admitted by the host as a project under an implicit empty manifest (Packages §2.5) — after ownership is settled, so that a folder opened inside a package never becomes a project of its own. An editor folder opened inside a package belongs to that package's project, and two open folders see each other only through an installed dependency; a file two programs hold has one identity, its open buffer's text reaching both, and each program keeps its own analysis and its own failures. npm's layout is the first lookup mechanism (Packages §4.1); a second is another supplier of directories, and no compiler pass changes for it. A diagnostic is anchored at the file and span that drew it, a dependency's file included, its other locations carried as related information; where several programs hold one file, their reports are merged — identical ones once, different ones each — and the file is cleared only when no program still reports on it.
