...
describe("nothing but declaration text changes", () => {
  /**
   * #228's own claim: the face is a `.d.ts` matter and the emitted JavaScript
   * does not know about it. The program's own lines are therefore pinned
  ...
  test("the emitted program is unchanged, and names no face", () => {
    ...
    expect(javascript).toContain('import { rows as __rowsForeign } from "./rows.js";');
