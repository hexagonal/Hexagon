...
describe("constraint position: the same reading, one namespace over", () => {
  ...
  test("the thirteen pre-registered names are never reached", () => {
  ...
  test("a renamed alias over one exported constraint names both repairs", () => {
    ...
    expect(messages([
      ...
      // The repair is a module import, so it names a module and carries no
      // path (Modules §10's row, Packages §3.4) — one `moduleImportLine`, the
      // same one the resolver's sibling message writes.
  ...
  test("a name that is nothing at all carries the family signpost", () => {
    ...
    expect(messages([
      ["/main.hex", "module Main\n\n" + "export fun size<a: Nope>(x: a): Int = 1\n"],
    ])).toContain(
