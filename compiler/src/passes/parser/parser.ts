...
/**
 * Foreign Enums §2.4's closed list, verbatim. Every non-member — a float
 * (`NaN` and signed zero separate `Object.is` from `===`, which §4's `switch`
 * rests on), an interpolated string, an expression, a bare name — is refused
 * with this one sentence, because what the author wrote is never the point:
 * what may stand there is.
 */
const LITERAL_ENUM_MEMBER =
 ...
 */
const ALIAS_SLOT = "<Alias>";

/**
 * The alias a refused import head is told to write: the specifier's basename,
 * each `-`, `_`, or `.`-separated segment upper-cased at its start and joined
 * (Modules §3.1, §10). `geometry` → `Geometry`, `search-params` →
 * `SearchParams`; where the derivation yields no uppercase-start identifier
 * the message names the slot instead.
 *
 * The derivation reads the *specifier*, never the file system: the parser has
 * no module graph, and the rewrite has to be spellable from the one line the
 * author wrote. It is deliberately not a general identifier sanitiser — a
 * segment that is not already identifier-shaped fails the whole derivation
 * rather than being mangled into something the author never chose.
 */
function derivedAlias(specifier: string): string {
...
class Parser {
  ...
  #parseVisibleDeclaration(
    ...
    if (this.#atExternEnumHead()) {
      ...
      // ordinary declaration prefix here, not the block's per-declaration one,
      // because the head stands alone and is a declaration of this module.
      const externToken = this.#advance();
      return this.#parseExternEnum(externToken.span, exported, itemStart);
    }
   ...
   * recover a binding under, and minting the derived one would put a name in
   * scope that the source never wrote.
   */
  #refuseNamedList(start: LaidOut.Token): Parsed.ErrorItem {
    this.#advance();
    let first: string | undefined;
    while (!this.#at("RightBrace") && !this.#at("Eof")) {
      const token = this.#current();
      if (token.kind !== "NonUpperName" && token.kind !== "UpperName") break;
      this.#advance();
      first ??= token.text;
      if (this.#atContextual("as")) {
        this.#advance();
        if (this.#at("NonUpperName") || this.#at("UpperName")) this.#advance();
      }
      if (!this.#at("Comma")) break;
      this.#advance();
    }
    if (this.#at("RightBrace")) this.#advance();
  ...
  /**
   * JavaScript's namespace head, refused under the Rewrite Rule (Modules
   * §3.1, §10; #762). `*` no longer appears in the import grammar at all, so
   * the one place a JS author's muscle memory fires gets a redirect rather
   * than a bare "expected an alias".
   *
   * The span returned covers `* as` — the two tokens the head spends,
   * whitespace between them included — so the caller can report against it
   * once the rewrite is spellable: the message names the specifier, and the
   * specifier is three tokens further on. With no `as` to pair with there is
   * no two-token span and the star's own stands, the same restraint the
   * lexer's JavaScript-block-comment redirect shows when it finds no closer.
   */
  #refuseJavaScriptNamespaceHead(): Source.Span {
    const star = this.#advance();
    if (!this.#atContextual("as")) return star.span;
    const as = this.#advance();
    return spanFrom(star.span, as.span);
  }

  #parseExtern(): Parsed.Item {
    const start = this.#advance();
    if (this.#at("Import")) {
      this.#advance();
      const specifier = this.#parseImportSpecifier();
    ...
    if (this.#atContextual("enum")) {
      return this.#parseExternEnum(start.span, false, undefined);
    }
    if (!this.#atContextual("from")) {
      // Three continuations, since #773: the block's `from`, the module effect's
      // `import`, and the literal enum's `enum` — the FFI's one module-free
      // `extern` head (Part 4 §2.2). A bare `extern` opening an indented block
      // is not a fourth (Foreign Enums §2.4), and this is the sentence that says
      // so.
      this.#error("expected `from`, `import` or `enum` after `extern`");
      this.#synchronize(itemEnds);
      return { kind: "ErrorItem", span: spanFrom(start.span, this.#previous().span) };
    }
    this.#advance();
  ...
  #parseExternDeclaration(
    ...
    // FFI Part 5 §11 (#982): `static` and `new` describe a foreign class's
    // constructor object, which only an `extern class` block names. At block
    // level there is no class for either to reach, whatever grammar follows.
    if (this.#atContextual("static") || this.#atContextual("new")) {
      const label = this.#current();
      this.#errorAt(
        label.span,
        label.kind === "NonUpperName" && label.text === "new"
    ...
    if (annotation.kind === "Function") {
      // §13's second row. The rewrite is the callable the *type* describes, so
      // it spells that type's own arrow — never `->!`, which would overwrite a
      // colour the author already wrote — and names its parameters, which a
      // function type does not carry: `x`, `y`, … in order, the placeholders a
      // reader fills in.
      const written = this.#writtenAnnotation(annotation);
      const slots = annotation.parameters.map((parameter, at) =>
        `${PARAMETER_PLACEHOLDERS[at] ?? `x${at + 1}`}: ${
          this.#writtenAnnotation(parameter) ?? "T"
        }`
      );
      const result = this.#writtenAnnotation(annotation.result);
      ...
      this.#errorAt(
        annotation.span,
        "extern callable declarations use `fun`; " +
          (written === undefined ? "this binding's type" : `a binding of type \`${written}\``) +
          " is callable — write " +
          (result === undefined
            ? `\`fun ${localName.text}(…) ${arrow} T\``
  ...
  /**
   * FFI Part 5's instance member rows (#982): `method`, `get`, `set`, standalone
   * in an `extern from` block. Each is a callable row of the `fun` shape — the
   * signature is read by `#parseExternSignature` — whose first parameter is the
   * JavaScript receiver; what the keyword adds is the calling convention the
   * emitter reproduces and the arity each convention fixes.
   */
  #parseExternMember(
    ...
    this.#advance();
    const foreignName: Parsed.Name = {
      text: foreignText,
      startClass: nameToken.kind === "UpperName" ? "upper" : "non-upper",
      span: nameToken.span,
    };
    ...
    if (!aliased && (nameToken.kind === "Wildcard" || foreignText === "true" || foreignText === "false")) {
      ...
      this.#errorAt(
        nameToken.span,
        `foreign member \`${foreignText}\` uses the reserved \`__\` prefix; bind it ` +
          `with an alias: \`${member} ${foreignText} as ${alias}\``,
      );
    } else if (localName.startClass !== "non-upper") {
      this.#errorAt(
        localName.span,
        `foreign member \`${foreignText}\` is not a legal Hexagon term name; bind it with an alias: ` +
          `\`${member} ${foreignText} as ${lowerInitial(localName.text)}\``,
      );
    }
    if (this.#at("Less")) {
      this.#errorAt(this.#current().span, "generic extern declarations are not part of Hexagon v1");
    ...
    if (isStatic) {
      ...
      this.#errorAt(
        ...
        `an extern \`${member}\` takes its receiver as an explicit first parameter; write \`${head}(${
          member === "set" ? "subject: Subject, value: Value" : "subject: Subject"
        }) ->! ${member === "set" ? "Unit" : result}\``,
      );
    } else if (member === "get" && parameters.length > 1) {
      // §3.1: a property read takes its subject and nothing else.
      this.#errorAt(
        parameters[1]!.span,
        "a property read takes no arguments beyond its subject; for a receiver call, use `method`",
      );
    } else if (member === "set" && parameters.length !== 2) {
      // §4.1: the subject first, the assigned value last.
      const subject = this.#writtenText(parameters[0]!.span, parameters[0]!.span) ?? "subject: Subject";
      this.#errorAt(
        (parameters[2] ?? parameters[0]!).span,
        `an extern \`set\` takes the subject and the assigned value: \`${head}(${subject}, value: Value) ->! Unit\``,
      );
    }
  ...
  /**
   * A callable extern row's signature — parameters, outer arrow, result — shared
   * by `fun` and FFI Part 5's instance members (#982), which are callable rows
   * of the same shape (Part 4 §4.5's partition). What differs is read by the
  ...
  #parseExternSignature(
    ...
    // can name the type the author wrote (`let version: String`) rather than a
    // placeholder — and because that rewrite is a `let`, which keeps `:`, the
    // arrow seat below stays silent on this path: this row's repair is the
    // other keyword, not an arrow it would not have.
    const missingParameterList = !this.#at("LeftParen");
    const parameterOpen = missingParameterList ? undefined : this.#current().span;
    const parsedParameters = missingParameterList
    ...
    // that claims nothing back: it is the colon row's fixit, the
    // missing-result row's advice, and the recovery here, one answer three
    // times. Purity is reachable from a written `->` and from nowhere else.
    let effect: Parsed.ArrowEffect | undefined = "constant";
    ...
    if (arrow === undefined) {
      ...
      if (separator.kind === "Colon") {
        ...
        // arrow seat can have no other reading — an extern declaration has no
        // body to begin. What it spells is retained only where the spelling
        // is not a claim — `=>?` — because a refused arrow may not hand a row
        // the purity it did not write.
    ...
    if (missingParameterList && member === undefined) {
      this.#errorAt(
        ...
        "extern `fun` declares a callable and requires a parameter list; for a " +
          `foreign value, write \`let ${localName.text}: ` +
          `${this.#writtenAnnotation(returnAnnotation) ?? "Type"}\``,
      );
    }
    return {
      parameters,
  ...
  /**
   * One annotation as its author spelled it, for the rewrites that quote a type.
   *
   * A **synthesized** annotation was never written: it is the placeholder a
   * failed parse left behind, and it carries the *name's* span, so quoting it
   * would put the row's own name where its result type belongs (`write \`fun
   * parse(text: String) ->! parse\``). The caller's placeholder is the honest
   * answer there, exactly as it is at a member header (#867).
   */
  #writtenAnnotation(annotation: Parsed.TypeAnnotation): string | undefined {
    if (annotation.kind === "NamedType" && annotation.synthesized === true) return undefined;
    return this.#writtenText(annotation.span, annotation.span);
  }

  /** Whether the token just consumed was of this kind — the closer a rewrite needs. */
  #previously(kind: TokenKind): boolean {
    return this.#index > 0 && this.#previous().kind === kind;
  }
  ...
  /**
   * `[export] extern enum T = "up" as Up | "down" as Down` — the **literal
   * form** of a foreign enum (Foreign Enums §2.4), and the FFI's one module-free
   * `extern` head (Part 4 §2.2).
   *
   * The item produced is a `Union`: the form *is* a closed nominal union inside
   * Hexagon — its local type name, its constructor namespace, matching,
   * exhaustiveness, reachability and derivation are the ordinary union's, and
   * §2.4 says so in as many words. What marks it is `externEnum` on the item and
   * a `literal` on each constructor: the runtime values are the declaration's
   * own rather than Unions §6's tagged objects, which only emission and the
   * `.d.ts` face read.
   *
   * Type parameters are the one thing the ordinary union head has that is
   * refused here rather than parsed, because a foreign enum is monomorphic
   * (§2.1). The list is read and refused, so the members after it still parse
   * and the author gets one report.
   *
   * `opaque` never reaches this method. `#atOpaqueSubject` admits `record` and
   * `union` alone, so `export opaque extern enum` draws that method's own
   * refusal — "`opaque` applies to `record` and `union` declarations" — and the
   * declaration below it is still read here, one diagnostic and no cascade. A
   * bare `opaque extern enum` is not a head at all: `#atOpaqueHead` does not
   * admit `extern`, so the line falls out of the item grammar exactly as it did
   * before this form existed.
   *
   * `externSpan` is the `extern` token, already consumed by the caller; the
   * current token is the contextual `enum`.
   */
  #parseExternEnum(
    externSpan: Source.Span,
    exported: boolean,
    itemStart?: Source.Span,
  ): Parsed.Item {
    const start = itemStart ?? externSpan;
    this.#advance();
    const nameToken = this.#takeName(
      "UpperName",
      "`extern enum` requires an uppercase type name",
    );
    if (nameToken === undefined) {
      this.#synchronize(itemEnds);
      return { kind: "ErrorItem", span: spanFrom(start, this.#previous().span) };
    ...
    if (this.#at("LeftParen")) {
      // §2.1's monomorphism, refused at the list rather than at the `(`: the
      // whole list is consumed so the `=` and the members below still parse.
      const opening = this.#advance();
      while (!this.#at("RightParen") && !this.#at("Eof") && !itemEnds.has(this.#current().kind)) {
        this.#advance();
      }
      const closing = this.#expect("RightParen", "expected `)` after type parameters");
      this.#errorAt(
        spanFrom(opening.span, closing?.span ?? opening.span),
    ...
    // A member begins at its leading `|`, or at the value where none precedes —
    ...
    while (!itemEnds.has(this.#current().kind)) {
      const value = this.#parseForeignLiteral();
      if (value === undefined) {
        this.#synchronize(itemEnds);
        return { kind: "ErrorItem", span: spanFrom(start, this.#previous().span) };
      }
      ...
      const name = parsedName(constructor);
      if (this.#at("LeftParen")) {
        // §2.1: "The body permits nullary members only." The list is consumed
        // so the members after it still parse, and the rewrite names the
        // declaration that *does* carry structure.
        const opening = this.#advance();
        let depth = 1;
  ...
  /**
   * One literal `extern enum` member value (Foreign Enums §2.4) — a string, an
   * integer with its sign folded in, `true`, `false`, `null`, or `undefined`.
   *
   * Four of those spellings are read here and nowhere else in the grammar.
   * `true`/`false` are hard keywords whose only other role is the #147 redirect
   * to `True`/`False`; §2.4 is the one position where they name JavaScript
   * booleans, and no redirect fires. `null` and `undefined` are ordinary
   * `NonUpperName`s everywhere else — Hexagon admits no ambient nullish value
   * (FFI Part 2 §2.2) — and name the JavaScript values only here.
   *
   * The sign is part of the integer as it is in a pattern (Pattern Matching
   * §2.5): a member list contains no operators, so there is no unary minus to
   * collide with.
   */
  #parseForeignLiteral():
    ...
    | undefined
  {
    ...
    if (token.kind === "String") {
      ...
      if (token.parts.some(({ kind }) => kind === "Interpolation")) {
        this.#errorAt(token.span, LITERAL_ENUM_MEMBER);
        return undefined;
      }
      const value = token.parts
        .map((part) => part.kind === "Text" ? part.value : "")
        .join("");
      return { literal: { kind: "String", value }, span: token.span };
    }
    if (token.kind === "Integer" || (token.kind === "Minus" && this.#peek(1).kind === "Integer")) {
    ...
    if (token.kind === "True" || token.kind === "False") {
      this.#advance();
      return {
        literal: { kind: "Bool", value: token.kind === "True" },
        span: token.span,
      };
    }
    if (token.kind === "NonUpperName" && (token.text === "null" || token.text === "undefined")) {
      this.#advance();
      return {
        literal: token.text === "null" ? { kind: "Null" } : { kind: "Undefined" },
        span: token.span,
      };
    }
    this.#errorAt(token.span, LITERAL_ENUM_MEMBER);
...
function isKeywordToken(token: LaidOut.Token): boolean {
  return (keywordKinds as readonly string[]).includes(token.kind);
}
