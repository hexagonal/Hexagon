 ...
 * "the offending call" is reachable for an arrow that fails inside one — and
 * the span a report falls back to when nothing else places it.
 */
interface SeatBody {
  readonly type: Mono;
...
/** One failed arrow, and which of Effects §9's contract rows reports it. */
interface SeatFailure {
  ...
  /** The contract's own arrow node, for its span. */
  readonly arrow: FunctionMono;
  /**
   * Set on a **bounds conflict** (§13.2's conflict bullet): the body's colour
   * was caught between a supplied arrow impure at this instantiation and this
   * invoked one. The contract's invoked arrow becomes a related location, and a
   * linked row drops "unconditionally" — the effect is one the contract handed
   * the body, not one the body raised.
   */
...
interface SeatBounds {
  /** The invoked arrow that demanded pure of it. */
  upper?: SeatFailure;
  /** The supplied arrow that demanded impure of it. */
  lower?: SeatFailure;
  /**
...
class Checker {
   ...
   */
  readonly #absorbedCalls: { readonly effect: Mono; readonly span: Source.Span }[] = [];
  /**
  ...
  /**
   * The tower's members, by the symbol each declares *(#808)* — the resolved
   * answer to the spelling `towerMemberSpelling` guesses at.
   *
   * Derived once from `#constraintsByIdentity` on first ask, and only from the
   * five rung identities: a `Vector.add` resolves to a symbol this map does not
   * hold, and lifts nothing. The value is the rung's declared name, which is
   * what `#supportsTarget` asks the instance table in.
   */
  #towerMemberRungs: Map<Resolved.SymbolId, TowerMember> | undefined;
  /**
   ...
   * the receiver *does* honor is skipped, which is what makes the owned set
   * disjoint from the honored one and keeps §6's refusal out of it.
   */
  #ownedTowerMember(type: Mono, name: string): MemberCandidate | undefined {
    const actual = this.#prune(type);
    if (
    ...
    for (const [identity, members] of TOWER_MEMBERS) {
      if (honored.has(identity) || !members.has(name)) continue;
      const declaration = this.#constraintsByIdentity.get(identity);
      if (declaration === undefined) continue;
      const candidate = constraintMemberCandidates(declaration)
        .find(({ member, subjectFirst }) => subjectFirst && member === name);
   ...
   * reports come from, and nothing here should take them over.
   */
  #checkDotSeats(
    expression: Resolved.CallExpr,
    calleeType: Mono,
   ...
   *
   * `subject` is the **delayed bind** *(#821)*: the face a tower member call
   * would run at, handed to the subject seats as their *expectation* while the
   * subject variable itself stays unbound. An expectation is not an annotation
   * (Functions §4.3), so an operand that cannot enter the face elaborates at its
   * own type and the caller decides the lift afterwards — with every operand in
   * hand, which is what a subject established "together, after their operands
   * are in" (§2.2) means. Binding first, as the lift once did, decided the
   * stand-down off the receiver alone.
   */
  #dotCallSeats(
    expression: Resolved.CallExpr,
    callee: Resolved.AccessExpr,
    ...
    subject?: { readonly face: Mono; readonly seats: ReadonlySet<number> },
    ...
    // What a seat expects while it elaborates: its parameter, or — at a subject
    // seat of a tower member call under a face — the face itself, the subject
    // variable it is written in being still unbound (#821). The two coincide
    // once the lift fires; until it does, only the face can carry the
    // expectation down into a nested operation, which is what makes
    // `a.add(b.multiply(c))` run at the face throughout.
    const expectation = (index: number): Mono | undefined =>
      subject !== undefined && subject.seats.has(index)
        ? subject.face
        : parameters?.[index];
    ...
    for (const [index, argument] of expression.arguments.entries()) {
      ...
      seats[index + 1] = this.#inferExpr(argument, level, expectation(index + 1));
    }
   ...
   */
    ...
    const { seats, pass } = this.#dotCallSeats(
      ...
      candidate.identity === POW_IDENTITY && candidate.member === "pow" &&
        ...
        ? { index: 1, base: 0 }
        : undefined,
      home === undefined || subjectSeats === undefined
        ? undefined
        : { face: home, seats: subjectSeats },
    );
    // **The delayed bind** *(#821, Method Syntax §2.2)*. The subject seats are
    // established together, after their operands are in — so §5.1's stand-down
    // is decided over *every* operand, receiver and arguments alike, and not
    // over the one the dot happened to hold first. Where all of them reach the
    // face the lift fires exactly as it always did; where one declines, the
    // subject is left to the operands and the note (§6) is what the consuming
    // seat's refusal will say.
  ...
  #declinedSubject(
    ...
    for (const index of [...subjectSeats].sort((left, right) => left - right)) {
      ...
      if (this.#operandReaches(seat, home)) continue;
  ...
  #baseConstraintHomeClause(identity: string): string {
    ...
    const name = home === undefined ? undefined : this.#moduleName(home);
    if (name === undefined) return "";
    return ` in module \`${name}\``;
  }

  /**
   * `moduleBaseName` for **this** module's package: the one seat where the
   * checker's reports learn how their own reader spells another module.
   *
   * Every sentence this class draws is read inside the package whose module is
   * being checked, so this is the only spelling any of them may print, and
   * routing them through one method is what keeps a new report from acquiring
   * the self-qualifying spelling by writing the plain function out again.
   */
  #moduleName(path: string): string | undefined {
    return moduleBaseName(path, this.#packageName);
  }
    ...
    switch (expression.kind) {
      ...
      case "Call": {
        ...
        const callee = calleeIsLambda
          ...
          : this.#inferExpr(expression.callee, level);
        // *(#808.)* The lift, at a tower member call spelled bare, qualified
        // through its constraint, or as a pipe stage (the rewrite arrives here
        // as an ordinary call). The home is what the subject seats expect while
        // they elaborate — `let r: BigInt = Num.add(i, j)` is `BigInt` addition
        // of two injected `Int`s, never the injected `Int` sum — and *(#821)*
        ...
        // A subject seat's expectation is the home, the subject variable it is
        // written in being still unbound (#821) — the two coincide once the lift
        // fires, and until it does only the home can carry the expectation down
        // into a nested operation.
        const memberSeat = (index: number): Mono | undefined =>
          memberHome !== undefined && memberSubjectSeats?.has(index) === true
            ? memberHome
            : calleeParameters?.[index];
        if (!calleeIsLambda) {
          for (const [index, argument] of expression.arguments.entries()) {
            if (deferredLambdas.has(index)) continue;
            arguments_[index] = this.#inferExpr(argument, level, memberSeat(index));
          }
          if (deferredLambdas.size > 0) {
        ...
        // **The delayed bind** *(#821)*: the subject seats are established
        // together, after their operands are in, so §5.1's stand-down is decided
        // over every one of them. The lift's own unification is silent by
        // construction and the parameter list is the same array either way, so
        // the sweep below is the one it always was.
        const memberDeclined = memberHome === undefined ||
            memberSubjectSeats === undefined
          ? undefined
          : this.#declinedSubject(
            arguments_,
            expression.arguments,
            memberSubjectSeats,
            memberHome,
  ...
  #inferExceptionPattern(pattern: Resolved.Pattern, level: number): void {
    ...
    if (pattern.symbol === undefined) {
      // A `catch` arm is **no seat of the door's** (Pattern Matching §2.2): it
      // matches the open `Exn`, which has no constructor set to read a head
      // out of. Exception constructors are bare or qualified here as
      // everywhere (Exceptions §5.2), so a head scope did not bind is refused
      // with whatever qualified spelling this module does hold — Modules §10's
      // bare-constructor row, in the one pattern seat the door never reaches.
      const qualified = pattern.qualifications?.[0];
      this.#diagnostics.add({
        severity: "error",
        message: qualified === undefined
          ? `unknown constructor \`${pattern.text}\``
          : `no bare \`${pattern.text}\`; write \`${qualified}\``,
        primary: pattern.nameSpan,
      });
      this.#brokenPatterns.add(pattern);
      return;
  ...
  #routeClause(
    ...
    const home = this.#moduleName(path);
    if (home === undefined) return undefined;
    ...
    // binds. The taken-spelling half is what tier 3 *is* — a bare spelling this
    // module binds otherwise, which is why the door (which reads scope first)
    // did not open — and the reader is owed that sentence rather than left to
    // find it.
    ...
    // the compiler tier itself can offer it. No requesting package passed here:
    // `home` came from `#moduleName` and is already this reader's own spelling,
    // and eliding a second time is how a name would lose two segments.
  ...
  #constraintRouteClause(
    ...
    const declaring = this.#moduleName(path) ?? path;
    const plural = names.length > 1;
    const spelled = englishList(names.map((name) => `\`${alias}.${name}\``));
    // `declaring` is already this reader's spelling — see `#routeClause`.
    ...
    return {
      split: (pattern) => {
        ...
        if (
          ...
          this.#exceptions.has(pattern.symbol)
        ) {
  ...
  #unionColumn(
    ...
    return {
      ...
      split: (pattern) => {
        ...
        if (pattern.kind !== "Constructor" || pattern.symbol === undefined) {
          return distinctHead(pattern);
        }
  ...
  #assumedColumn(patterns: readonly Resolved.Pattern[]): CoverageColumn {
    ...
    for (const pattern of patterns) {
      if (pattern.kind !== "Constructor" || pattern.symbol === undefined) continue;
    ...
    return {
      signature: [...heads.values()],
      split: (pattern) => {
        if (pattern.kind !== "Constructor" || pattern.symbol === undefined) return undefined;
        const head = heads.get(constructorHeadKey(pattern.symbol));
   ...
   * unification cannot fail. It is performed *before* the arguments elaborate,
   * which is what makes the home their expected type.
   */
   ...
   * span**. Without it `i.pow(2n)` and `Pow.pow(i, 2n)` took a bare "type
   * mismatch" where `i ** 2n` named the door (#783's second finding).
   */
  #argumentPass(
    ...
    // The face this call's lift stood down from, once `standDown` has said so
    // (#821), and the operands that ran at it. Set after the pass is built,
    ...
    const eager = (index: number): void => {
      ...
      if (powerSeat !== undefined && powerSeat.index === index) {
        // The base is read **here**, not when the seat was described: the
        // arguments elaborate after the pass is built, so a base captured then
        // would be the placeholder rather than the operand.
        this.#checkExponent(expression, actual, actuals[powerSeat.base] ?? ERROR);
        return;
      }
  ...
  #checkCallArguments(
    ...
    this.#argumentPass(parameters, arguments_, expressions, span, powerSeat).finish();
  }
  ...
  #walkSeat(
     ...
     * variable (§13.2).
     */
    const bounds = new Map<Variable, SeatBounds>();
    ...
    // earliest.
    let conflict: { readonly failure: SeatFailure; readonly order: number } | undefined;
    for (const [variable, bound] of bounds) {
      if (bound.upper !== undefined && bound.lower !== undefined) {
        if (conflict === undefined || bound.order < conflict.order) {
          conflict = { failure: { ...bound.upper, handed: true }, order: bound.order };
        }
        continue;
      }
      ...
      if (bound.unconditional) settle.add(variable);
    }
    if (failure === undefined && conflict !== undefined) failure = conflict.failure;
  ...
  #compareArrow(
    ...
    instantiation: EffectConstant,
    body: SeatBody,
    bounds: Map<Variable, SeatBounds>,
    ...
    const boundsOf = (variable: Variable): SeatBounds => {
    ...
    if ((sign === "co" || sign === "inv") && !demanded.impure) {
      const failure: SeatFailure = {
        ...
        place,
        span: this.#seatSpan(body, bodyRaw, true),
        colour: bodyRaw,
        arrow: left,
      };
      if (solved.kind === "Effect") {
        if (solved.impure) record(failure);
      } else if (solved.kind === "Variable") {
        boundsOf(solved).upper ??= failure;
      }
    }
    ...
    if ((sign === "contra" || sign === "inv") && demanded.impure) {
      const failure: SeatFailure = {
        ...
        place,
        span: this.#seatSpan(body, bodyRaw, false),
        colour: bodyRaw,
        linked,
        arrow: left,
      };
   ...
   * body's colour, and the two directions name different acts:
   *
   * - an **invoked** arrow reports "at the offending call — the first in source
   *   order where several offend", which is a call the body wrote at that
   *   colour. It may stand inside a lambda nested in the member's body, which
   *   is why the calls are searched rather than the frame walked.
   * - a **supplied** arrow reports "at the demand that narrowed it, the first
   *   in source order where several do", which is the unification that pinned
   *   the colour.
   *
   * Each falls back to the other, and both to `undefined` — where no call and
   * no demand carries the colour the caller places the report at the contract's
   * own failing arrow instead.
   */
  #seatSpan(body: SeatBody, colour: Mono, invoked: boolean): Source.Span | undefined {
    const pin = this.#colourPin(colour);
    const call = this.#offendingCall(body, colour);
    return invoked ? (call ?? pin) : (pin ?? call);
  }
  ...
  #reportStandDown(
    ...
    // Through the forwarding forms, so the parenthesized and piped spellings of
  ...
  #missingInstanceMessage(requirement: Requirement, type: Mono): string {
    ...
    const rider = this.#towerFaceRider(requirement, type);
    const clause = this.#legalHomesClause(requirement, type);
    const fixit = this.#derivationFixit(requirement, type);
    if (fixit === undefined) {
      return clause === undefined ? `${head}${rider}` : `${head}${clause}${rider}`;
    }
   ...
   * *(#808)*.
   *
   * The lift admits `n - m` at a written `Int` face and `i / j` at a written
  ...
  #legalHomesClause(requirement: Requirement, type: Mono): string | undefined {
    ...
    if (this.#sealedConstraint(requirement.identity)) {
      ...
      const seat = this.#moduleName(constraintHome.path);
    ...
    const constraintSeat = constraintHome.path === undefined
      ? undefined
      : this.#moduleName(constraintHome.path);
    const subjectSeat = subjectHome.path === undefined
      ? undefined
      : this.#moduleName(subjectHome.path);
  ...
  #derivationFixit(
    ...
    const seat = this.#moduleName(home.path) ?? home.path;
    return {
      replaces: true,
  ...
  #handWrittenHashRefusal(item: Resolved.HonorItem): string | undefined {
    ...
    const elsewhere = here !== undefined && there !== undefined && there !== here
      ? ` in module \`${this.#moduleName(there) ?? there}\``
      : "";
  ...
  #userNominalIterableFailure(
    ...
    return `\`${this.#display(type)}\` is not iterable. Define \`honor Iterable<${subject}>\` in ` +
      `module \`${this.#moduleName(declaringPath) ?? declaringPath}\`, which declares ` +
      `\`${declaration.name}\`. ` +
  ...
  #materializePattern(pattern: Resolved.Pattern): Typed.Pattern {
    ...
    if (pattern.kind === "Constructor") {
      // A head Pattern Matching §2.2's door refused never resolved to a
      // constructor, so there is none to materialize: the program has already
      // been reported and the node materializes as the `_` §7.3's fourth tier
      // reads it as, which is also what keeps the typed tree total.
      if (pattern.symbol === undefined) {
        return { kind: "Wildcard", span: pattern.span };
      }
      const { open: _open, qualifications: _qualifications, ...node } = pattern;
      return {
        ...node,
