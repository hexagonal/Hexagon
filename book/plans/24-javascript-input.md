...
5. `Array(a)` is a captured snapshot copied at every crossing; `Vector(a)` is a stable
   persistent value. Never describe `Array(a)` as borrowed or zero-copy.
...
9. Representation-direct callbacks retain ordinary function identity; a callback naming
   a captured collection crosses as a per-crossing copying wrapper without it.
10. Explicit collection conversions are shallow: they change the outer collection only.
