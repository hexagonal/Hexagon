...
- **Dec equality is numerical; display retains places.** The `d` suffix selects
  ...
  through `Dec.fromFloat`/`fromFloatEven`, which round the binary value to required places. Ordinary inspection uses `show`; `unscaled` returns the stored BigInt.
...
- Establishes representation-direct values, `Nullable(a)` as the explicit nullish
  door, `Array(a)` (with `JsMap`/`JsSet`) as a captured foreign collection copied at
  every crossing, and top-level `Seq(a)` as an adapted persistent sequence.
...
- Literal patterns admit bare integers, `BigInt`, `Float`, and `String`, each matching
  by its type's `==` equality; a `Float` literal follows SameValueZero (`0.0` matches `-0.0`, `NaN` equals
  itself), and the named special values (`Float.nan`, the infinities) are tested in
  guards. An `n`-suffixed pattern is monomorphic `BigInt`, supports exact arbitrary-precision
  positive and negative values, and shares coverage identity with an equal bare integer
  at a BigInt position (`0` and `0n` are one case); patterns never widen their position.
  `True`/`False` are
...
- Top-level `Seq(a)` receives one persistent memoizing adapter; hidden nested adapters
  and adapter-requiring callbacks are rejected; a callback naming a captured collection
  is accepted through a per-crossing copying wrapper, without identity.
- Foreign receiver calls, properties, and classes lower to subject-first companion
  functions; representation-direct callbacks retain function identity, while a
  callback naming a captured collection crosses as a per-crossing copying wrapper with
  no identity promise.
