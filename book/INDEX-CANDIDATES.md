...
| --- | --- | --- |
| implied type | Chapter 22, opening definition | `Iterable.Item`; multiple type members, scope, coherence, and restrictions |
...
| Array, foreign | Chapter 25, “`Array` is borrowed; `Vector` is persistent” | Readonly borrow, stability contract, and explicit conversions |
...
| constrained export | Chapter 26, opening and “Fundamental types receive direct functions” | Generic function and public dictionaries |
...
| constraint dictionary | Chapter 26, opening definition | JavaScript object representation, parameters, handles, and factories; see **dictionary** |
| dictionary | Chapter 12, “Concrete code remains concrete” | Chapter 26, **constraint dictionary** |
...
| erasure | Chapter 23, “Types erase when values need nothing extra” | Type aliases, nominal constructors, pipes, and dot calls |
| enum, foreign | Chapter 25, “Foreign enums become closed unions” | `extern enum`; TypeScript enums, frozen objects, symbols, and singleton values |
| exhaustiveness | Chapter 10, “`match` handles every alternative” | Chapter 11, “Exhaustiveness protects future changes” |
| exception | Chapter 20, opening and “Exception declarations add constructors to `Exn`” | See also **Exn**, **JsError**, and **Result** |
| Exn | Chapter 20, opening and “Exception declarations add constructors to `Exn`” | Open exception type; see **exception** |
| extern declaration | Chapter 25, opening and “A boundary declaration is a contract” | Foreign modules, members, classes, and callbacks |
...
| implicit rethrow | Chapter 20, “Missing cases propagate automatically” | See **exception** |
...
| JsError | Chapter 20, “Foreign failures enter through `JsError`” | Foreign JavaScript throwables |
| JsValue | Chapter 25, “Foreign failure stays foreign until decoded” | Explicit decoding of uncertain foreign values |
...
| opacity | Chapter 14, “Opaque exports hide representation” | Opaque records and unions; Chapter 24 TypeScript brands |
| Nullable | Chapter 25, “`Nullable` is the foreign nullish door” | `NullableCase`; conversion to and from `Option` |
...
| persistent collection | Chapter 21, opening definition | `Vector`, `Map`, and `Set` |
...
| primitive types | Chapter 2, opening definition — types whose values are already JavaScript primitive values | Chapter 23, native values; Chapter 24, TypeScript faces; see **Bool** and **Unit** for the rows defined elsewhere |
...
| representation-direct | Chapter 25, “A boundary declaration is a contract” | See also **extern declaration** |
...
| trusted boundary | Chapter 25, “A boundary declaration is a contract” | Explicit conversion and decoding |
