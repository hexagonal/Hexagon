...
was above. If a Float value and a Dec value meet in one expression, that is an error:
neither becomes the other quietly. A binding ends an expression, so its value is settled
there: after `let rate = quantity * 1.5`, `rate` is a Float, and `rate * price` is an
error. So does the receiver of a dot call, which is settled before the dot is: write
`quantity * 1.5 * price` rather than `(quantity * 1.5).multiply(price)`. Only a literal
is read this way; a Float value never quietly becomes a Dec.
