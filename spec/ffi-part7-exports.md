...
- **Two stated exceptions to export correspondence**: Part 8 §3.4's zero-entry-point constrained export, whose doctrine record (Part 8 §17.2) is not modified here, and a term named `then`, withheld from both files (§7) *(#1014)*.
...
### 1.1 The contested vocabulary: a face survives its own file *(#662)*

Some faces are written in TypeScript's standard-library vocabulary — `Iterable` (the §2.3 pin), `ReadonlyArray` (Part 1 §4.1's `Array(a)` row), `ReadonlyMap` and `ReadonlySet` (Part 10), and `Error` (§6's exception faces and the `isHexError` guard); `IterableIterator` joins when #659 lands. These spellings are the **contested vocabulary**: TypeScript resolves names file-locally before its library, so a type-space binding under one of them in a module's own `.d.ts` captures every face there written in it. Four classes, all measured *(#662)*: a non-generic capture makes the shipped file fail the consumer's compile (`TS2315`); a *same-arity* capture (`export union Iterable(a)`) compiles clean everywhere with every `Seq` face silently meaning the user's union — a wrong published contract with zero errors; an `export record Error` leaves every exception face an intersection with the user's type, silently stripping the real `Error` members and degrading `isHexError`'s predicate; and an *exception itself named* `Error` captures its own face into a self-reference — `export type Error = Error & {…}` — which no TypeScript accepts (`TS2456`, a circular alias). Lowercase library spellings (`boolean`, `number`, `string`, `void`, `unknown`, `never`) are outside the vocabulary permanently — nominal names are parser-gated uppercase.

The repair has two halves, and neither moves a user's spelling:

1. **Compiler-chosen spellings never contest the vocabulary.** The contested spellings join the collision universe every probe in this part reads — §2.1's generated alias, §2.4's minted locals and yielding aliases — so nothing the compiler mints can take a lib spelling a face needs. (The universe's over-claim asymmetry extends unchanged: over-claiming here costs at most an unneeded qualification below, which is cosmetic.)

2. **User-written spellings never move — Part 1 §10 stands absolute — and the face steps around them.** Where the file's universe — the same flat one §2.4 defines, deliberately read without per-namespace subtlety — contains a contested spelling, every face in that file written in that spelling qualifies through TypeScript's own global-scope reference: `globalThis.Iterable<a>`, `globalThis.Error & {…}`. The bindings that make qualification *necessary* are the type-space ones — an exported type under the name, a carried named import's local (both measured to capture); a value-space-only member of the universe — a constructor sharing the spelling, a namespace alias — triggers a qualification the file did not strictly need, which is this rule's instance of §2.4's licensed over-claim: one flat universe, no second drifting quantity, at the cost of a harmless qualified spelling. The qualified spelling **is** the library type — assignability, inference, and member access are byte-for-byte those of the bare spelling, consumers never write the qualifier themselves, and the reference works on every toolchain that can consume these declarations at all (verified through real `tsc` at 4.7, 5.0, 5.6, and current). The qualifier itself is beyond contest, and not by the uppercase gate: `export let globalThis: Int = 1` is legal Hexagon and emits a `declare const globalThis` into the same file, and the qualified faces still resolve to the library types — measured — because TypeScript resolves `globalThis` in type position specially rather than through the file's bindings. **Collision-only:** a file whose universe contains no contested spelling emits the bare vocabulary it always did, byte-identical — the house rule that a probe which has not had to move emits the text it always emitted, read at the face.

Fixed here:

- **No published name moves, in either emitted file, for any reason.** (A term named `then` is withheld, not moved — §7.) The emitted JavaScript and the `.d.ts` therefore agree on every export name by construction — the agreement is absolute, not maintained by machinery. §2.4's rule that the two files plan their *internal* aliases independently stands on its unobservability license, unchanged.
- **The vocabulary is defined by the faces, not by TypeScript's library.** It holds exactly the spellings this corpus's face rules can emit, and grows only when a face rule is added — each arrival spec-ruled, versioned, and semantically inert in emitted text (qualification appears where contested; nothing renames). TypeScript growing its library moves nothing: a global no face spells captures nothing, because the capture needs both sides.
- **Qualification is per file and per spelling, not per program**: each module's `.d.ts` qualifies exactly the contested spellings its own type space binds.
- **A namespace import binding is not a capture route.** A TS namespace import does not occupy the plain type-name space — measured with a control *(#662)* — so a source-written `import Iterable …` contests nothing and no yielding is triggered on its account; it stays in the universe on the flat-check convention (Part 12 §11.1), whose only cost is the harmless over-claim above. The claim is this declaration file's alone: the `.js` rules the opposite way, because an ESM namespace import occupies the value-name space like any binding (§1.2 rule 2, measured).
- **This repair does not touch the emitted JavaScript.** The declaration file's defect and its qualification live here alone; runtime bindings, exception identity (`name`/`$hex`, #488), and every consumer's imports are exactly what they were. The emitted JavaScript has a capture of its own — the same disease in the other file, with its own vocabulary and its own qualification — and §1.2 is its rule *(#666)*.

Constant qualification — writing `globalThis.` on every vocabulary face unconditionally — is rejected for the same reason the corpus commits to readable emission: it would spend every shipped file's resemblance to hand-written TypeScript on a hazard only self-colliding files have. Correction record §14.6 carries the origin, the measurements, and the rejected alternatives, including the superseded export-rename design.

### 1.2 The runtime vocabulary: the emitted JavaScript survives its own module *(#666)*

JavaScript resolves names file-locally before the global scope, exactly as TypeScript does (§1.1) — and the emitted `.js` writes runtime text in the global vocabulary: the exception helper's `new Error(…)` and `Object.assign` (Exceptions §7.2), the range and `Seq` machinery's `Symbol.iterator` and its inbound adapter's `Boolean`, `TypeError`, and `String`, the memoizer's `WeakMap`, the derived-instance helpers' `Math`, `Number`, `String`, `Array`, the match lowering's `RangeError` (its unreachable-default arm), the widening seats' `BigInt(…)`, the `JsMap.fromSeq`/`JsSet.fromSeq` lowerings' and the `JsMap`/`JsSet` capture copy's `Map` and `Set` constructors (FFI Part 10 §6.5, §2), and Unit's own spelling `undefined` (Products §2.6). A module-scope binding under one of those spellings captures every such reference in its module. Four severity classes, all measured *(#666)*: `export record Error` (or `Object`) beside a declared exception makes **every raise throw `TypeError: Error is not a constructor`**; `export record Symbol` beside any `for` loop makes the loop throw before its first iteration — the range's `[Symbol.iterator]` reads the record constructor; **silently wrong values**, the worst class, with two measured instances — `let undefined = 1` (lowercase, so legal at module *or function* scope) makes every Unit in scope become `1`, and `export exception Boolean` beside a `Seq` boundary makes `Seq.length` of a three-element sequence answer `0`, the adapter's `Boolean(__next.done)` reading the exception constructor; and a binding JavaScript itself refuses (`export let eval`, or an export named `await` whose minted import line spells it verbatim) is a **load-time `SyntaxError`** — the module never runs at all. Module-import aliases capture like declarations: `import Error` — a module named `Error` — lowers to a namespace import that poisons the *importing* module's helper. Uppercase spellings are module-level-only exposures — `let` requires a non-uppercase-start name, so no function local can bind them. `console` sits in the vocabulary by the feeder rule below though it is not user-capturable today: its one reference (the debug probe's module-evaluation capture) is written into a stdlib module no user binding can reach — in the list on purpose, so a future inlining of the probe is covered before it happens.

**The runtime vocabulary** is defined by the emitter's references, not by JavaScript's global object: it holds exactly the global spellings the emitted runtime text can write — helper bodies and inline expression seats alike — single-sourced in the implementation beside §1.1's vocabulary, and grown only with the text that references them. The two vocabularies are siblings, not copies: `Iterable` names no JavaScript value and `Math` appears in no face, so each file guards its own. A conformance **tripwire** renders every helper and scans every emitted-reference seat, asserting the referenced globals are a subset of the list — the capturable set moves when a helper moves into module scope, and this pins that movement to a conscious edit rather than a silent re-draw. JavaScript growing its global object moves nothing: a global the emitter never writes captures nothing, because the capture needs both sides.

The repair is §1.1's move in value space. **No published name moves, in either emitted file, for any reason** — §1.1's own guarantee, now true of both repairs — and for a vocabulary capture not even an internal spelling moves; internal seats move only where JavaScript refuses the user's spelling outright (rule 4), and those seats publish nothing:

1. **Compiler-minted spellings never contest the vocabulary** — §1.1's first half, transferred. The `.js` emitter's generated internal names are immune by construction, Lexer §3.2's prefix, so the population that can contest is closed and small: the **minted import locals** — the named imports emission itself decides (a prelude term, a companion operation reached by a dot call (Method Syntax §8.2), a member seat (Constraints §6.1), a specialization edition (Part 8): Modules §11.2's liberty; and an extern class's constructor object — the importer's `__class_<Type>` import and the binding module's own import of the class, FFI Part 5 §7) — and among those, exactly the ones whose local mirrors a spelling outside the prefix — the imported export's, or, for a class's constructor object, the foreign class name (many are minted under the prefix already, `__trieEmpty`-style, and immune by construction). Where a mirrored spelling lands in the runtime vocabulary or JavaScript's reserved words — a companion export named `undefined` is legal Hexagon — the minted local is aliased at rule 4's seat (`import { undefined as __undefined }`, subject to Lexer §3.2's collision probe) and every reference follows; a class's constructor object takes its fixed `__class_<Type>` instead (FFI Part 5 §7). The seat is shared with rule 4; the *trigger* differs by authorship: a minted local aliases for either spelling-class, since nothing user-written is at stake — a source-written import local aliases only where JavaScript refuses the spelling (rule 4), and where it takes a vocabulary spelling the binding is the user's, so the module qualifies around it instead (rule 2).

2. **Contested references qualify through reserved imports of the globals themselves.** Where a module's top-level bindings — declarations and module-import aliases, one flat quantity read before rendering, §2.4's discipline, and read over the module's **source** bindings: rule 1's aliasing runs after it, so a minted local never enters the trigger and the quantity stays pre-rendering — contain a runtime-vocabulary spelling, the module imports that global's reserved capture from the program's runtime module (below), and every compiler-written reference to the contested spelling uses it: `new __global_Error(__message)`, `__global_Object.assign(…)`, `[__global_Symbol.iterator]`. A capture is spelled `__global_<Name>`, Lexer §3.2's `global` category, fixed and never suffixed; the category keeps the family apart from rule 1's and rule 4's `__<name>` locals by construction, so neither ever probes past a capture. The namespace alias is named in that list deliberately, against §1.1's grain: a source `import String` lowers to `import * as String`, which occupies JavaScript's value-name space like any binding — measured, `import Error` beside a declared exception emits `import * as Error` above the helper's `new Error(__message)`, which then reads the namespace object — where a TypeScript namespace import leaves the plain type-name space alone (§1.1's measured control). The triggers diverge on exactly this binding form, the mirror of `Iterable` naming no JavaScript value. This is §1.1's qualification in value space, with the substitution the language forces: TypeScript resolves type-position `globalThis` specially, immune to the file's bindings (measured, §1.1), but JavaScript protects *no* spelling of the global scope — value-position `globalThis` is an ordinary identifier (`export let globalThis: Int = 1` emits `const globalThis = 1;` verbatim, and a function local may bind it too — measured) — so the protected spelling is manufactured from the one namespace no user binding or import local can enter, Lexer §3.2's reserved prefix, and arrives as an import binding. The import line doubles as a manifest: `import { __global_Error, __global_Object } from "./hex.js"` names exactly the spellings this module contests — the specifier path-adjusted from the module's own emitted location, so a module emitted into a subdirectory (a dotted name, or another package's directory, Packages §6) writes `"../hex.js"` one level down, `"../../hex.js"` two, and so on (#829; Part 1 §8.3's rule for the `.d.ts` side, applied to the JavaScript side). Per module and per spelling, on the spelling alone: the trigger reads what the module *binds*, never what its emission happens to reference, because the reference set is exactly the quantity that re-draws silently — a trigger that read it would fail precisely when the hazard moves. A module binding no runtime-vocabulary spelling emits the bare text it always did, byte-identical.

3. **Unit takes its immune spelling where contested.** A module that binds `undefined` anywhere — module scope or function scope — spells Unit `void 0` throughout. Not because no qualifier could rescue it: a reserved import of `globalThis.undefined` works, measured, and §14.7 records that so it is not re-invented — but `void 0` owes no import, is immune at every scope with no per-module machinery, and is JavaScript's own idiom for exactly this value. Everywhere else Unit stays `undefined`, the readable spelling.

4. **Unsafe value-binding names have stable descriptive locals.** A lawful Hexagon value-binding name that JavaScript refuses as a local in a strict-mode module takes Lexer §3.2's `__<sourceName>` spelling and collision allocation. This covers JavaScript reserved words and the strict-mode binding restrictions on `eval` and `arguments`: absent a collision, `null`, `await`, `eval`, and `arguments` lower to `__null`, `__await`, `__eval`, and `__arguments`. The rule applies wherever such a source binding becomes a JavaScript local, including parameters and nested bindings, and to emission-decided import locals with an unsafe spelling (`import { await as __await }`). All references follow the binding's allocated name. An exported value retains its source export name (`export { __null as null }`), and its declaration-file local uses the same allocated spelling. Only an actual occupied or reserved generated name introduces `_1`, `_2`, …; resolver symbol IDs and unrelated prelude additions cannot change the spelling. Rule 1 uses the same allocation for compiler-minted import locals whose spelling would capture runtime vocabulary. Source-written bindings with otherwise legal JavaScript names retain rules 2 and 3's treatment. These aliases carry values, not type names: the reserved words and strict-mode-restricted names are lowercase, while Hexagon type names are parser-gated uppercase. An extension of the unsafe-name set must preserve that distinction. This replaces the former `__binding<symbolId>` spelling; it introduces no new generated-name category.

**The runtime module takes the reserved seat.** Part 1 §8.3 reserves the runtime's code home in so many words — *if a later pass ever materializes shared runtime JavaScript (the helper-dedup direction), `hex.js` is its designated home, so the type home and the code home converge on one module identity instead of inventing a second*. The globals capture is not the helper dedup that parenthetical anticipated, but it is materialized shared runtime JavaScript, and the reservation's stated rationale — one module identity rather than a second — is exactly what applies, so it takes `hex.js` (§8.3 carries the matching amendment). One per program, emitted exactly when some module of the program is contested, carried on its own reachability channel like every emission-decided import, and named by the **same stem §8.3's probe settles**: whatever filename the runtime declaration seat won (`hex`, `hex1`, …), the code home follows it — one probe, one identity, a user module never renamed on its account; §8.3's probe input is the emitted filenames — the declared module names, known once every header is parsed (#829) — so the stem is defined before any module is emitted, and for a program owing `hex.js` and no `hex.d.ts`. **The declaration file does not grow.** No generated `.d.ts` imports the runtime module's exports and TypeScript never type-checks the emitted `.js`, so declaring `__global_Error` there would only surface reserved names in every consumer's `Hex` namespace; the additions are omitted, and `hex.d.ts` keeps exactly its four faces — and with them the fixed-four-names ground on which it emits the bare vocabulary, and the preview's inline namespace, both byte-identical. Each artifact is emitted exactly when owed, independently. Its text is the capture no contested module can perform for itself, one binding per vocabulary member, the full vocabulary always — its bytes depend on the vocabulary alone, never on which program or module asked:

```js
const __global_Error = globalThis.Error, __global_Object = globalThis.Object, /* … one per member … */;
export { __global_Error, __global_Object, /* … */ };
```

Inside it nothing shadows — the module binds only reserved names — and the reason the captures cannot live in the contested file itself is JavaScript's own scoping: `const` shadows its whole scope, so a leading `const __global_Error = Error;` beside a later user `Error` binding reads the temporal dead zone and the module dies at load. The capture must be made in a scope the user's bindings cannot reach, and an import from one is exactly that. Unlike §8.3's type-only artifact, `hex.js` is the corpus's first **executable** program-scoped artifact — it must reach every consumer that runs emitted code, so every host's execution set carries it on the same footing as a prelude module (a host that materializes only source-derived modules loses every contested program at its first import).

Fixed here, restated as the invariant the pair of sections now guarantees: **no published name moves, in either emitted file, for any reason — and a vocabulary capture moves no internal spelling either.** (A term named `then` is withheld from both files, never moved — §7.) What moves internally is exactly the set of *user* spellings JavaScript refuses to bind at all (rule 4), together with the compiler-minted locals of rule 1 — at seats that publish nothing. The `.d.ts` and the `.js` answer their captures with the same sentence — *the compiler's own references step around the user's binding, through a spelling no user binding can touch* — `globalThis.X` where TypeScript provides that spelling, `__global_X` where the lexer's reservation must manufacture it. The preview keeps §1.1's rule untouched and owes nothing new: nothing here adds to any `.d.ts` at all — the shipped runtime declaration module and the preview's inline `Hex` namespace are generated from the same four faces and both stay byte-identical — and the JavaScript pane renders the entry module's true emitted text, import lines included (measured); what a host owes the *Run* path is the execution-set obligation above, a loading concern, not a preview one.
...
through which a declaration file reaches the runtime's branded interfaces `Hex.Vector<a>`, `Hex.Map<k, v>`, `Hex.Set<a>`, and `Hex.Range`. *(#1071.)* The four types are declared by their companions' public intrinsic `type` rows (Intrinsics §3.3), so a **face** names each by its Hexagon name — `Vector<a>`, `Map<k, v>`, `Set<a>`, `Range` — through §2.4's sink like any module-owned type, as a type-only named import from the companion's declaration file (`import type { Vector } from "./Hex/Vector.js"`); that file's declaration seat exports the name as an alias of the interface (`export type Vector<a> = Hex.Vector<a>;`), which is where this namespace import appears. The import is type-only and adds no emitted JavaScript dependency. The compiler controls the alias: it tries `Hex`, then `Hex_1`, `Hex_2`, … and takes the first candidate colliding with no top-level identifier the module's items can put in that `.d.ts`, regardless of TypeScript namespace, nor any spelling in §1.1's contested vocabulary (§2.4). Only the generated alias is renamed; user exports never are (Part 1 §10; Part 12 §11.1).

**A compiler-chosen `.d.ts` spelling takes an underscore before its numeric suffix** (#619) — the idiom the emitted JavaScript already uses (#425) — and every probe in this part counts the same way: this alias, §2.4's minted locals, §5's brands. The unsuffixed spelling is unchanged everywhere; the suffix stays collision-only, so a probe that has not had to move emits the text it always did. The underscore is single on purpose: Lexer §3.2 reserves the *leading* `__` for generated names, and Part 7's probed spellings are faces a TypeScript reader looks at, not hygiene names — `Hex__1` would echo the reserved prefix without being under it, so the doubled form is refused.

The `Hex` import is the only **namespace** import the compiler *synthesizes*: a source-level module import's line is the module's own, carried into the `.d.ts` where a face reaches through the alias and left out where none does (§2.4). It is not, however, the only import the compiler writes — a face mentioning a type owned by another *Hexagon module* may take a type-only **named** import of it, minted by the compiler where nothing the source wrote names that type here (§2.4; correction records §14.2, §14.3).
...
Two of those rows **pin** a nominal type: prelude `Bool` faces as `boolean`, prelude `Seq(a)` as `Iterable<a>`. A pin is not just another face-table row — `Nullable` has no declaration to bypass, while a pinned type is a module-owned nominal whose faces deliberately do not name it. *(#1071, #1076.)* The captured `Array(a)`, `JsMap(k, v)`, and `JsSet(a)` are pinned once their companions declare them by public intrinsic `type` rows (Intrinsics §3.3): each one's boundary meaning is the native collection itself — the captured value is a native array, `Map`, or `Set` (Part 1 §2.2) — so its face is TypeScript's read-only view of that native type and nothing Hexagon owns remains to name: `ReadonlyArray<a>`, `ReadonlyMap<k, v>`, `ReadonlySet<a>`. Their seats export `export type Array<a> = ReadonlyArray<a>;` and its two siblings. The `Array` seat's name shadows TypeScript's global `Array` only inside its own file, and no face spells the global — Part 1 §4.1 faces the captured array as `ReadonlyArray` — so nothing is captured; should a face ever need the global, `Array` joins §1.1's contested vocabulary. The criterion *(#622)*: **a type is pinned when its entire boundary meaning is a JS-native notion that JavaScript's own syntax consumes** — the primitive `if` tests, the protocol `for...of` drives — nothing beyond the notion promised outbound or demanded inbound. Nominal law inside Hexagon, the native notion at the border; Part 1 §8.2's `Seq` carve-out states the same criterion from the face side ("the one type whose face is the protocol rather than a brand over it"). `Vector`, `Map`, and `Set` carry the iterable protocol too and are not pinned: satisfying a protocol as a courtesy is not being exhausted by it — their boundary contract is their identity, and their faces name it (`Vector<a>`, §2.1). `Range` is the sharp case: its outbound face exposes iteration and nothing else (Part 1 §8.1), and the criterion's *inbound* leg alone excludes it — a `Range` position demands a genuine `Range`, where a pinned type accepts anything satisfying the notion. Part 3 §14.2's `Stream(a)` (outbound face `IterableIterator<a>`) is the standing candidate: its membership — including the one-spelling question its inbound door raises — is settled with its implementation (#659).

A pin governs the type **everywhere** it reaches a `.d.ts` *(#622; correction record §14.5)*. Every rendered face spells the native notion, settled before §2.4's sink is consulted — no rung sees a pinned type, and no import line is owed for one. And the home module's declaration seat exports the type's own name **as an alias of the same spelling**, at the type's own parameters, the doc comment riding it:

```ts
export type Seq<a> = Iterable<a>;
```

§5's brand does not arise — the exclusion is recorded there. The name and every face then denote one type, so the consumer who reaches for the exported name — the corpus-taught first move — writes a spelling interchangeable with every signature. Faces still spell the native notion rather than the alias, deliberately: the native spelling is self-contained — TypeScript's own vocabulary, meaningful in every file with no import owed — and the alias is transparent, so spelling it through the name would re-enter the naming machinery the pin exists to bypass and buy nothing. Self-contained held one exposure — a module's own export taking a TS-lib spelling captured every face written in it — repaired by §1.1: the contested vocabulary joins the compiler's probe universes, and a face a user binding contests anyway qualifies through `globalThis`, collision-only *(#662; correction record §14.6)*.
...
2. *(Retired with the named import — Modules §3, #762. No source form binds an imported name under a local, so this rung has no occurrence to answer; the numbering below is kept so the records in §14 read true. A type an importer names bare now reaches the sink through its own declaration — `type Point = Geo.Point` is rung 1's, a transparent alias whose expansion the faces carry — or through the companion fallback, whose bare occurrence is rung 5's, as the fallback's has always been.)*
...
3. **The occurrence is qualified through a *module* import's alias** (Modules §3.1) — **`Alias.Name`**, the spelling the source wrote at that seat:
   ...
   **A qualifier is readable only in the module that wrote it** — it names a binding, and a binding reference is meaningless where the binding is not. It is worse than meaningless where a type crosses a module boundary carrying one — a type alias's expansion travels with the qualifiers its *writer* used (§1) — because the importer may bind that same alias to a different module, and the face would then name a real type that is the wrong one, with nothing to report it. Rung 3 therefore answers only for a qualifier **this module itself wrote** — the alias *binding*, not its spelling. A qualifier that arrived on a type from another module is not an occurrence here however it is spelled, and falls to rung 5; that a module of its own binds an alias of the same spelling makes no difference, and is exactly the case where reading the spelling would publish another module's type under this one's name. No derived declaration loses anything: each is emitted by the module that wrote its signature.
   ...
   The prelude never reaches this rung. Its modules carry no `import` lines at all (Modules §5.5), so no prelude signature can hold a qualifier, and a prelude identity arrives at rung 4 — or through an expansion at rung 5 — bare.

4. **The prelude's type inventory** (Modules §5.5) — the entry's own name, or a probed local where this module has taken the bare spelling. The resolver publishes, per module, the prelude's importable type inventory: exported unions, records, extern types, and public intrinsic `type`-row types that are not pinned (§2.3; Intrinsics §3.3, #1071), each with its owning member's specifier, in normative prelude order. Type aliases have no entry, because faces carry their expansion (§1, Modules §11.4).
...
**The file's imports are exactly what its answers owe.** Rung 1 owes nothing: the declaration it names is one this file already carries, save in the shapes fenced above. Rungs 3 through 5 each owe the line their answer is spelled through (rung 2 being retired), and a line no answer owes is not written. This is candidates-then-filter, the architecture of prelude instance evidence (#153) and the synthesized term import (#263): the resolver decides availability, emission decides what is imported, and neither is inferred from the other. Three consequences are worth stating outright:
...
A written dot is not enough, and three things keep one from counting: §2.3's pins, applied ahead of the sink; an earlier rung, rung 1's own declaration outranking rung 3 at a qualified seat as much as at a bare one (rung 2 being retired); and an occurrence that reaches **no rendered face** — a qualifier in an unexported binding's signature, or in a type written inside a body, spells nothing this file publishes. The rule is stated on the emitted line rather than on that list, so a rung or a pin added later cannot leave the list stale.
...
**Scope.** The **TypeScript faces** of the shipped `.d.ts`, and nothing else in that file: the generated `Hexagon:` face documentation renders its types on the Hexagon side, by its own renderer, and is not this section's (§2.2, Part 8). The inspection-only TypeScript preview also keeps bare *names* — the collection companions' `Vector`, `Map`, and `Set` among them since #1071: it is one pane of text with nothing to import from (the constraint behind Part 1 §8.3 obligation 6's inline namespace), and the divergence is recorded here rather than left to be rediscovered. Bare names, not bare vocabulary: §1.1's `globalThis` qualification imports nothing, so the preview carries it exactly as the shipped file does (§14.6).
...
- The runtime constructor **may be the representation-honest identity function** — `Point` already *is* the POJO (Products §5.4). Emitter's choice; the exported function's existence and identity stability are the contract. Where a field type names a captured foreign collection the exported constructor is §7 occasion 4's stable export wrapper, which walks that field on entry *(#876)*; the identity licence does not reach it.
...
An exported non-opaque union exports its type and **every constructor**, exactly as it does between Hexagon modules (Modules §3.1) — a payload constructor whose payload names a captured foreign collection as §7 occasion 4's stable export wrapper *(#876)*. The constructor shapes follow the representation (Unions §6) — payload constructors are functions, nullary constructors are shared constants, whatever the union's mix:
...
**This brand form is uniform across everything Hexagon exports opaquely** — `opaque record`, `opaque union`, exported extern `type`, exported extern class types (Part 4 §12.3; Part 5 §6.1), and an exported `extern enum` in its object-reading form (`ffi-foreign-enums.md` §7.2 — the literal form faces as the literal union instead) — **save the §2.3-pinned types, which never take a brand** *(#622)*. The brand's one reading — "a nominal Hexagon-governed value; obtain and use it through the exported functions" — asserts identity, and a pin marks precisely the type whose boundary contract is a JS-native notion rather than an identity: outbound it crosses as the notion, inbound it accepts what satisfies the notion — whether by a representation that carries the protocol from birth (`Seq`, Part 3 §9.4) or by a per-crossing shim (`Stream`'s, were it to join — Part 3 §14.2) is each type's own mechanism, not the criterion's. A brand there would assert the one thing the boundary deliberately declines to demand, and it measured as exactly that — an exported type uninhabited from outside and assignable with the published face in neither direction. A pinned type's declaration seat is §2.3's alias of the pinned face instead; §2.3 owns the rule. For every unpinned opaque export the mechanism below is unchanged: one mechanism, one reading.
...
- **One brand symbol per exported type**, declared `declare const <name>Brand: unique symbol` and **not exported** — the symbol is unnameable outside the declaration file, which is what makes the type unforgeable in TS. The `<name>Brand` spelling is representative; per-type uniqueness within the file and stability of the *type's* name are the contract, and identifier collisions are the emitter's ordinary renaming problem: a moved brand takes §2.1's underscore suffix, `<name>Brand_1` — a brand already looks like a type, so it aliases like one — and the probes stay outside Lexer §3.2's reserved `__` prefix, a brand being a face, not a hygiene name.
...
4. *(#876)* **an exported Hexagon function whose signature names a captured foreign collection** anywhere Part 1 §5.4's walk reaches — a parameter or result of type `Array(a)`, a record parameter with an `Array` field, a callback parameter over arrays. This is the **stable export wrapper of occasion 4** (the per-crossing artifact a *callback value* receives is Part 6 §5.5's *conversion wrapper*, a different thing under a different term; the extern-side twin is Part 4 §4.3's copying wrapper). The wrapper runs the walk over each such parameter on entry and over the result on exit (Part 1 §5.4), so a JavaScript caller's array becomes Hexagon's copy and Hexagon's array leaves as a copy, and calls the internal function. **The public name binds the wrapper; the internal edition — whose name, wherever the two-edition split exists, is a reserved `__` spelling — is exported under that internal name, appearing in no `.d.ts`, and Hexagon importers bind it** — the convention the trailing-evidence generic edition already uses (the edit note below) — so a cross-module Hexagon call is an ordinary Hexagon-to-Hexagon call and never copies (Part 1 §2.2). The internal name is a reserved `__` spelling (Lexer §3.2): **compiler linkage outside the published foreign contract**, absent from the `.d.ts`. A JavaScript module that imports it anyway reaches the unwalked function, and that act is a **boundary-contract violation** standing with fabricated brands and dictionaries in Part 1 §3.1 — recorded at Part 1 §5.4; the `.d.ts` omission alone is not the protection, the contract is. Exported **constructors** are functions for this occasion (Part 1 §5.4 item 4): a union or record constructor whose payload names a captured collection takes the wrapper. Where occasion 4 meets a constrained export it follows the published face: each fundamental specialization with a captured position takes the wrapper, and so does the **base-name generic edition** when Part 8 §4.1 triggers it — that edition is published (Part 8 §4.2, §8.1), so its public name binds the wrapper over the source positions while its evidence parameters are not walked (Part 9 §9, §10) — and the internal trailing-evidence edition, which Hexagon importers bind, takes none. Where occasion 1 and occasion 4 meet on one signature — `f(xs: Seq(Int), ys: Array(Int))` — occasion 4 governs the linkage: Hexagon importers bind the internal edition, which is harmless to occasion 1 because a Hexagon caller always supplies a genuine `Seq`, for which occasion 1's door is the identity pass-through. A polymorphic export with a captured position — `first(xs: Array(a)): a` — walks the array and carries `a` by identity (Part 1 §5.4's type-variable clause).

*(Edit note, 2026-08-02, defect 12's implementation.)* **Where occasion 1 meets a constrained export: it follows the published face.** A constrained export's face is its fundamental specializations (§8, Part 8) and, when Part 8 §4.1 triggers it, the base-name generic edition; the internal trailing-evidence edition ships under an internal name that appears in no `.d.ts` and exists as Hexagon-to-Hexagon plumbing (the published base-name edition, when it exists, is that function exported directly under Part 9 §9's common case, or under #876 its stable export wrapper). So each specialization with a top-level `Seq(a)` parameter takes occasion 1's stable wrapper — those are the bindings a JavaScript caller is invited to hand an `Iterable<a>` — and the internal edition takes none: a door there would tax every cross-module Hexagon constrained call to serve a caller the published face says does not exist. JavaScript reaching the internal name is off the face entirely and stays Part 1 §3.1's territory, as it always was.

**The wrapper is allocated once with the ESM binding, not per reference or call, so its JS identity is stable** — a JS consumer storing, comparing, or deregistering the export observes one function forever — and a wrapper of occasions 1 and 4 faces as the export it stands in for: its `name` is the public name and its `length` the arity that name's published face declares (Part 6 §1), so the emitter's own spelling for the wrapper never reaches a consumer. No wrapper validates its inputs (§1). (Occasion 4's stable export wrapper is stable in exactly this sense; Part 6 §5.5's conversion wrappers are per-crossing values and go under a different term for that reason.)

**An exported non-function binding whose type names a captured collection is refused** *(#876; Part 1 §5.4 item 4)* — `export let weekdays: Array(String) = …`, or a record value with an `Array` field. An ESM value binding is one live binding, read by JavaScript and by every Hexagon importer alike; a wrapper cannot sit on a value, and a copy would be the very thing all of them share, so a JavaScript consumer's mutation of it would change a value Hexagon retains. The diagnostic names the type and offers the rewrites: **export a function** whose result takes occasion 4's outbound walk — `export let weekdays(): Array(String) = …`, or a nullary function over a module-private value — or export a `Vector` (or another identity-safe type) whose element types themselves satisfy Part 1 §5.4. The compiler's own value exports — **public dictionary handles** (Part 9 §3) — are not under this refusal: a handle whose dictionary type names a captured collection is emitted with its affected members as stable copying wrappers, beside the unwrapped internal dictionary Hexagon uses (Part 9 §3.4; Part 1 §5.4 item 4). That two-object shape is Part 9's specifically defined treatment and does not lift this refusal for ordinary exported values. **Deferred, not rejected:** a separate internal binding for Hexagon importers beside a foreign-facing copy — the shape occasion 4 already has for functions — would lift this refusal for values; it is recorded as the upgrade path and is not part of v1.

Exported extern bindings re-export per Part 4 §7; their `.d.ts` faces follow this part's rules (extern types as §5 brands, extern callables as ordinary function declarations against their declared faces). An exported extern class declaring a `new` or `static` member also exports its constructor object under `__class_<Type>` — compiler linkage appearing in no `.d.ts`, as occasion 4's internal edition (Part 5 §7).

**No module publishes a JavaScript export named `then`** *(#1014)*. JavaScript's promise resolution reads any object's `then` property and, when it is callable, calls it in place of resolving to the object; a module namespace is such an object, so a module exporting a callable `then` never arrives from `import()` or `await import(…)` — the promise machinery calls `then` instead. Lexer §4.4 lets an exported term be named `then` (a Hexagon combinator, or an extern member row binding a promise-like's `then`), so the emitter withholds that one spelling, callable or not, since one rule is simpler to read than a type test: the term exports under `__then` only, exactly that spelling (the export namespace holds no other `__then`), and the `.d.ts` declares no `then`. `__then` is also the internal edition's spelling where a two-edition split already exists (occasion 4), and Hexagon importers reach it as they reach any internal edition; a receiver call needs neither, emitted inline as `task.then(f)` (Part 5 §2.2). A JavaScript consumer calls `.then` on the object itself. Nothing is renamed: the Hexagon name is `then` everywhere a Hexagon reader sees it, and the published surface lacks exactly the binding JavaScript would misread (§1's second stated exception). A Part 8 specialization edition carries its own name (`thenInt`) and publishes normally. Other keyword-named terms publish under their own spelling (`export { match }`, `export { __catch as catch }` under §1.2 rule 4). A *value* with a callable `then` field — a record, a union payload — is a thenable in JavaScript exactly as the same object would be, and this rule does not reach it.
...
This part introduces **two hard errors of its own** — #478's `isHexError` collision (§6), the Part 8 §6.2 family applied to a fixed generated face, and #876's refusal of an exported value binding that would share captured storage (§7). Every other boundary-shape and collision error it relies on lands elsewhere:
...
**Confirmed.** Opaque records, opaque unions, extern types, and extern class types all use the single non-exported-`unique symbol` form. The symbol remains private to the declaration file; the runtime representation remains untouched. *(Amended, #622: the §2.3-pinned types are excluded — their declaration seats are the pin's alias, never a brand (§14.5). The confirmation stands for every unpinned opaque export.)*
...
| Direct-vs-wrapper rule: direct wherever representation and public ABI match; one stable module-level wrapper for adapted top-level positions, exported receiver members, constrained generic editions only when ABI plumbing requires it, **and functions whose signatures name a captured collection** *(#876; occasion 4 — public name binds the wrapper, Hexagon importers bind the internal edition, no Hexagon-to-Hexagon copy)*; wrapper identity stable (once per ESM binding); per-value adapters and Part 6 §5.5's callback wrappers remain distinct; `Array` left the direct-export list | §7 |
| *(#876)* Exported non-function binding whose type names a captured collection is a hard error (rewrites: export a function, or a `Vector`); the internal/foreign value-binding split is the deferred upgrade | §7, §11 |
...
| *(2026-08-04, #227)* A generated `.d.ts` carries a type-only named import for every other-module Hexagon type its faces mention: a source-written import owns every name it binds (a term+type name keeps its type row) *(the named import, and with it this ownership, retired — Modules §3, #762; a module import's line is rung 3's)*, prelude-supplied types ride a resolver-published inventory filtered to what the faces reference, and generated locals are probed like `Hex`. The preview keeps bare names | §2.4, §14.2 |
| *(#574, #268, #617, #618)* A `.d.ts` names a nominal through one sink, answering in order: the module's own declaration, a named import's local *(rung 2, since retired with the form — Modules §3, #762 — its number kept)*, the alias a **qualified occurrence** was written through, the prelude inventory, then an import the file mints itself. The file's imports are exactly what those answers owe, so an import no face reached through emits no line; where a declaration contests a qualified alias, the alias yields the bare spelling and steps aside to `_1` as a source name, deciding independently of the emitted JavaScript | §2.4, §14.3 |
...
| *(#619)* A compiler-chosen `.d.ts` spelling suffixes with an underscore — `Hex_1`, `Option_1`, `<name>Brand_1` — collision-only, unsuffixed spellings unchanged, the doubled form refused: a face is not a hygiene name | §2.1 |
| *(#662)* A face survives its own file: the contested vocabulary (the TS-lib spellings the face rules emit — `Iterable`, `ReadonlyArray`, `ReadonlyMap`, `ReadonlySet`, `Error`; `IterableIterator` with #659) joins every compiler probe universe, and where a user binding takes a contested spelling anyway, that file's faces in it qualify through `globalThis` — collision-only, byte-identical elsewhere; no user spelling moves, Part 1 §10 absolute, emitted JS untouched by *this* repair (its own is the row below) | §1.1, §14.6 |
| *(#666)* The emitted JavaScript survives its own module: the runtime vocabulary (the global spellings emitted runtime text references — feeder-defined, tripwire-enforced) is guarded in value space by §1.1's same move — a contested module imports the globals it contests as reserved captures from the program's runtime module at Part 1 §8.3's reserved `hex.js` seat, and the compiler's references qualify (`new __global_Error(…)`, Lexer §3.2's `global` category); Unit spells `void 0` where `undefined` is bound; unsafe value locals and import locals take §1.2 rule 4's descriptive `__<sourceName>` rename (including `eval`/`arguments` and the import-local leg), with collision-only numeric suffixing; on-spelling per module, byte-identical elsewhere; no published name moves, in either file, and no user spelling moves at all for a vocabulary capture | §1.2, §14.7 |
| *(#1014)* No module publishes a JavaScript export named `then` — promise resolution would call it in place of yielding the namespace; a `then`-named term exports only under the reserved `__then`, which Hexagon importers reach, and the `.d.ts` declares no `then` | §7 |
| *(#622)* Pins get a criterion and reach the declaration seat: a type is pinned when its whole boundary meaning is a JS-native notion JavaScript's own syntax consumes (`Bool` → `boolean` for `if`, `Seq(a)` → `Iterable<a>` for `for...of`; `Stream(a)` the standing candidate #659 settles); a pinned type's home `.d.ts` exports its name as a transparent alias of the pinned face, never §5's brand — the brand asserts the identity the boundary deliberately declines to demand | §2.3, §2.4, §5, §14.5 |
...
- **A type-only namespace import per owning module** (`import type * as Option_ns from "./Option.js"` — `Seq.d.ts` and `Option.d.ts` share `Hex/` — faces `Option_ns.Option<a>`). Uniform, but it renames *every* cross-module face to dodge a collision that is nearly unreachable (§2.4's probe note), and the faces stop matching what a Hexagon-side reader sees — the cost §2.2 was written to avoid.
...
- The suffix idiom the probes use is #619's, not this record's; the spellings here carry it.
...
### 14.5 A pinned type's declaration seat is the pin's alias, not a brand (#622)

**Origin.** §5 gave every opaque export the uniform brand; §2.3 pinned `Seq(a)`'s every face to `Iterable<a>`; and nothing connected the two, so `Seq.d.ts` declared a brand type its own faces contradicted. Measured over the real emitted set: the exported `Seq<a>` was uninhabited from outside (its `unique symbol` unexported) and assignable with the published face in **neither** direction — `TS2741` both ways — while the name was the one a consumer following the corpus would pick first. #620's settling of pins ahead of the naming sink made the mismatch provable and permanent (no generated face will ever spell the declared name), and fenced it here. The prior §2.4 sentence — "a pin governs faces and not declaration seats" — was that fence's wording, and this record retires it.

**The principle** (the ruling recorded on #622). A type is pinned when its entire boundary meaning is a JS-native notion that JavaScript's own *syntax* consumes — the primitive `if` tests (`Bool` → `boolean`), the protocol `for...of` drives (`Seq(a)` → `Iterable<a>`; `Stream(a)` the standing candidate #659 settles) — with nothing beyond the notion promised outbound or demanded inbound. Nominal law inside Hexagon, the native notion at the border. Where that is the whole contract, the exported TypeScript name must *mean* the notion: a pin governs the type everywhere, faces and declaration seat alike. The home `.d.ts` exports the type's own name as a transparent alias of the pinned face at the type's own parameters (`export type Seq<a> = Iterable<a>;`), the doc comment riding it; the brand is not emitted. Export correspondence (§1) keeps its seat, and the name and every face denote one type.

**Consequences, with owners:**

- The declaration emitter's brand path skips pinned types and emits the alias row instead; the `declare const <name>Brand` line disappears from `Seq.d.ts`. No face changes anywhere — the pin was already settled ahead of the sink — so the change is confined to the home module's declaration seat.
- The issue's measured consumer program compiles clean: `import type { Seq }` then `Seq<number>` receives exported values and passes to exported functions, both directions, zero errors.
- `Bool`'s declaration seat **already emits the alias form**: when prelude reachability includes `Bool.hex`, its `.d.ts` reads `export type Bool = boolean;` with the constructors as §4's nullary constants — the rule is satisfied there today with no edit, and an implementer must not add a pinned-alias path that duplicates or fights the union arm already producing it.
- `Stream` is unaffected today — its Part 3 §14.2 face is unimplemented (#659) and its `.d.ts` still renders brand-and-sink coherently — and this rule governs its declaration seat the day #659 settles its membership and lands the face.
- The consolidation layer is edited with this record, not left to drift (the Part 1 §8.3 precedent): `ffi.md` invariant 9, the §6 master-table opaque-families row, and the §10 summary row each carry the pinned-type exclusion; Part 3 §9.5's residue note and its §13 row carry their discharge pointers here.

**Rejected alternatives (do not re-litigate):**

- **Suppressing the declaration seat entirely.** Breaks §1's export correspondence for a genuinely exported type and deletes the type's doc comment — the prose a TypeScript consumer most needs.
- **Exporting the brand in a usable form** (exported symbol, or a brand over `Iterable<a>`). A brand a consumer must possess refuses values the boundary lawfully adapts, unless every face also spelled it — which contradicts the pin. Incoherent with Part 3's inbound door by construction.
- **Unpinning the type so faces name the brand.** Internally coherent (today's `Stream.d.ts` demonstrates it) but re-litigates Part 3's settled boundary doctrine: replayable out, adapt-anything in, `Iterable<a>` as the honest spelling of both, is the design.
- **Spelling faces through the alias name now that it works.** The alias is transparent, so this buys TypeScript nothing, while re-entering the naming machinery — minted imports, occlusion, probes — that pins exist to bypass; the native spelling owes no import in any file. (Its occlusion exposure was #662's, shared by every TS-lib-spelled face and unrepaired by this alternative, which cannot reach the structural rows at all; §1.1 repairs it for all of them.)

### 14.6 The contested vocabulary: the face qualifies through `globalThis`, and nothing moves (#662)

**Origin.** Faces written in TypeScript's standard-library vocabulary were assumed uncontestable — the probes of §2.1 and §2.4 protect compiler-chosen spellings, and Part 1 §10 forbids moving a user export — so nothing protected a lib reference from the module's own type-space bindings. TypeScript resolves names file-locally before its library, so `export record Iterable` beside an exported `Seq` face captured the face. Four severity classes, all measured: a non-generic capture ships a `.d.ts` that fails the consumer's compile (`TS2315`); a *same-arity* capture (`export union Iterable(a)`) compiles clean everywhere with every `Seq` face silently meaning the user's union — the error then lands on an innocent consumer, blaming their correct call; an `Error` capture leaves every exception face an intersection with the user's type, silently stripping the real `Error` members and degrading `isHexError`'s predicate; an exception *itself named* `Error` captures its own face into a circular alias (`export type Error = Error & {…}`, `TS2456`) — the shipped file compiles under no TypeScript at all. Found by the #622 review while testing the pin rule's "no occlusion" claim, which was retracted and fenced here.

**The ruling was made twice, and the record keeps both.** The first ruling — a contested export publishes under the `_1` suffix as a unit in both emitted files — was reached on the stated ground that the lib spelling is unrecoverable in the capturing file, "TypeScript has no qualifier for a shadowed global type." That ground was **false**: review measurement showed `globalThis.Iterable<a>`, `globalThis.Error`, and the rest of the vocabulary resolving to the genuine library types in type position inside the capturing file, full member access, on every measured toolchain (tsc 4.7, 5.0, 5.6, current). What is true is narrower: an unqualified alias written in the capturing file resolves to the shadower, and TypeScript refuses every *half*-rename of a type-and-value export (`TS2323`/`TS2300` across the renamed-type-only, double-rename, and cross-file re-export forms). With the qualifier in hand, a repair exists that renames nothing, and the ruling was remade on the measurement.

**The rule** is §1.1's, in place: the contested vocabulary joins every compiler probe universe, and where a user-written type-space binding takes a contested spelling anyway, that file's faces in that spelling qualify through `globalThis` — collision-only, so an uncontested file emits the bare vocabulary byte-identically. No user spelling moves in either emitted file; Part 1 §10 stands absolute with no exception; the emitted JavaScript is untouched *by this repair* — its own capture, discovered by this ruling's implementation and repaired by §1.1's same move in value space, is §1.2's rule and §14.7's record (#666).

**Consequences, with owners:**

- The declarations emitter gains the vocabulary in its probe universes and a per-file qualification decision at the face-rendering seats: a contested spelling qualifies exactly where the file's §2.4 universe — the flat one, per §1.1's stated over-claim license — contains it. The decision is per (file, spelling), computed from the module's universe before rendering — §2.4's discipline.
- **The inspection preview qualifies identically.** The #622 precedent transfers — the preview shows what would ship — and §14.3's bare-names scope note does not reach this rule: its ground is that the one-pane preview has nothing to import from, and qualification imports nothing.
- Conformance owes: the four measured capture classes emitting qualified faces that pass real `tsc` from the consumer side, both directions; the two `Error` classes recovering genuine `Error` members on exception faces and `isHexError`, the self-named exception compiling at all; a module import aliased `Iterable` (the alias binds the consumer's type space — its file qualifies, and the import keeps working); a minted local never taking a vocabulary spelling; a value-space-only universe member (a constructor sharing the spelling) triggering the licensed harmless qualification; a source-written `import Iterable` whose line no face owes triggering nothing (the alias is outside the universe by the gated-alias rule), and one whose line is carried triggering only the licensed qualification — never because a namespace binding captures, which it does not; the uncontested negative — a corpus binding no contested spelling emits byte-identically to before the change; the emitted-JavaScript negative — no `.js` output changes anywhere; and the preview's inline runtime namespace, whose `extends Iterable<a>` clauses share the pane's one scope with the user's bindings and were a live capture before qualification (`TS2315` in exactly the contested files) — a qualified entity name in an `extends` clause is measured legal, and the pane compiles.

**Rejected alternatives (do not re-litigate without new information):**

- **The unit rename** (the superseded first ruling): a contested export publishes as `Name_1` in both files, on spelling, probed once against the union of both universes. It bought bare spellings absolutely everywhere and one-name-in-both-files aesthetics; it cost the first-ever exception to Part 1 §10, renamed published JavaScript bindings, joint-probe machinery, and a rename event at every vocabulary growth. Superseded the moment a repair existed that renames nothing.
- **Refusing the export**, in either shape: the unconditional list bans legal Hexagon names on the emission target's grounds; the collision-conditioned form is §14.3's already-rejected genre — a legal program failing to emit its declaration file is a worse answer than the error it replaces.
- **Routing contested faces through a compiler-owned home** (`Hex.Iterable`, runtime-`.d.ts` lib re-exports): verified expressible, and strictly dominated by `globalThis` — same qualified shape, but with invented machinery and an import where TypeScript's own global reference needs neither.
- **The invisible half-rename** (`export type { Iterable_1 as Iterable }`): measured to work only for type-only exports and to be inexpressible wherever a value shares the name — every record — with a mixed regime rejected for unpredictability.
- **Constant qualification** (`globalThis.` on every vocabulary face unconditionally): spends every shipped file's readability on a hazard only self-colliding files have; the readable-emission commitment decides it.
- **Documenting the capture as contract-violation territory**: the same-arity class publishes a silently wrong contract for an honest program with no foreign code involved; Part 1 §3.1 covers fabricated values, not this.
- **A yielding leg for namespace aliases**: proposed in drafting on the belief that a namespace import binding captures bare type references; measured false (with a control), so no such leg exists — recorded so it is not re-invented.

### 14.7 The runtime vocabulary: the emitted JavaScript qualifies through reserved imports, and nothing moves (#666)

**Origin.** §14.6's repair was scoped to the declaration file on the recorded sentence "the emitted JavaScript is untouched" — and its implementation found the other file's capture: a module declaring an exception inlines a construction helper spelling `new Error(…)`, and `export record Error` in the same module makes every raise throw `TypeError: Error is not a constructor` at runtime, with no diagnostic and no type-level symptom. Filed as #666 with three claimed live spellings and a claimed not-capturable list; this ruling's review re-measured the lists item by item and corrected the scope in both directions. Over-claimed: `Map` — the emitted `"Map.empty"`/`"Map.fromVector"` texts are string literals inside derived `show` bodies, and no user-module runtime text references the native constructor. Under-claimed, each falling to the same inlining the issue itself measured: `Symbol` (the range and `Seq` machinery's `[Symbol.iterator]`, live in any module with a `for` loop — measured broken), `Boolean`/`TypeError`/`String` (the `Seq` inbound adapter — `export exception Boolean` beside a `Seq` boundary makes `Seq.length` of a three-element sequence answer `0`, executed: the adapter's `Boolean(__next.done)` reads the exception constructor, a second measured instance of the silent-wrong-value class), `RangeError` (the match lowering's unreachable-default arm, in any module with a `match`), `WeakMap` (the memoizer), `Math`/`Number`/`Array` (the derived-instance helpers — where `String` appears a second time, in derived `show`/`hash` and interpolation, beside its adapter seat above), `BigInt` (the widening seats — `export let w(x: Int): BigInt = x` emits `BigInt(x)`, and a union constructor under the spelling captures it exactly as `Error`'s classes do: `TypeError: BigInt is not a function`, executed; the `BigInt` companion's conversion doors write it too, stdlib-side; found by the tripwire rather than by the enumeration — the enforcement §1.2 promises, demonstrated), and `undefined` — Unit's own spelling (Products §2.6), capturable at function scope because it is lowercase: every Unit in scope silently becomes the shadowing value. `console` is neither list's member: its one reference is the debug probe's module-evaluation capture inside a stdlib module no user binding can reach — in the vocabulary by the feeder rule, not user-capturable today, recorded so the distinction is not re-blurred. Two pre-existing holes in the reserved-word rename are the same family and fold in: `eval`/`arguments` missing from the reserved set (strict mode refuses the emitted binding — the module never parses), and the import-local leg missing entirely (a minted `import { await }` emits verbatim — likewise; an alias `Error` poisons the importing module's helper). Uppercase spellings are module-level exposures only — `let` requires a non-uppercase-start name.

**The ruling was reached through four measured candidates, each killed on its own evidence:**

1. **Value-position `globalThis` qualification** (the issue's candidate, §14.6's move verbatim): TypeScript resolves type-position `globalThis` immune to the file's bindings; JavaScript does not — `export let globalThis: Int = 1` emits `const globalThis = 1;` verbatim, a function local may bind it too, and no spelling of the global scope survives shadowing in value position. The qualifier itself is capturable — `globalThis.undefined` fails on the same binding that fails `globalThis.Error` — while a *reserved import* does rescue `undefined`; the rejected-alternatives entry below carries that measurement and why `void 0` wins anyway.
2. **The internal rename of published declarations** (`const Error_1 = …; export { Error_1 as Error };` — the published name pinned by the export alias): mechanically sound in both files, measured — and killed by a measured leak: TypeScript prints the *internal* alias in consumer diagnostics (`Type 'number' is not assignable to type 'Error_1'`) for a consumer who only ever wrote `Error`. An internal spelling that reaches a consumer's tooling is not internal.
3. **Sibling-file relocation** (`export { Error } from "./main.__names.js"` — a re-export binds nothing locally, so the contested declaration keeps its true bare name one file over): measured working end-to-end in both files, including the ESM import cycle the exception constructor needs (function declarations hoist across cycles) and bare-`Error` consumer diagnostics. Accepted, then superseded: four emitted files per contested module, a new artefact kind through every host, and definition jumps landing in a file the reader must first comprehend — all bought back by candidate 4's shape at no cost.
4. **Full-identity import qualification in both files** (`import { Error as __Error }` in the `.js` *and* type-only in the `.d.ts`, one spelling everywhere): measured leak-free — tsc resolves the import alias to the lib symbol and prints bare `Error` — but the inspection preview is one importless pane, so the shipped file and the preview would diverge in spelling. **A preview divergence is not a payable cost** (ruled here as a standing constraint; churn is), and the design also re-rules §1.1 for a difference no consumer can observe.

**The rule** is §1.2's: the runtime vocabulary — feeder-defined by the emitter's own references, tripwire-enforced — is guarded by §1.1's same move in value space, with the protected spelling manufactured rather than found: the vocabulary and JavaScript's reserved words join the probe of every minted `.js` import local (§1.2 rule 1); a contested module — its trigger reading declarations, import locals, *and namespace-import aliases*, the one binding form on which the two sections' triggers deliberately diverge — imports the globals it contests as `__global_<Name>` captures from the program's runtime module, which takes the `hex.js` seat Part 1 §8.3 reserved (amended there with this record); Unit takes `void 0` exactly where `undefined` is bound, and the unpublishable seats extend the reserved-local rename (with `eval`/`arguments` and the import-local leg closed; the original `__binding` spelling is superseded by §1.2 rule 4's descriptive spelling). On the spelling alone, never on the emission's reference set — the reference set is the quantity that re-draws silently, and a trigger reading it fails exactly when the hazard moves. The final spelling was itself a measured choice: a `__globalThis.X` mirror of §1.1's qualifier was drafted and superseded on the readable-emission commitment — `__global_Object.assign(new __global_Error(…))` keeps the original text visible behind the prefix, the `__` prefix is already the emitted file's own idiom, and the import line is a manifest of the module's contested spellings. The category is not cosmetic: a bare `__Number` capture sits on the spelling other generated names mirror — the hoisted `JsKind` constants (Unions §6.2) would probe to `__Number_1` around it — and the category separates the captures from rule 1's and rule 4's `__<name>` locals by construction. Families that can still spell a capture — an exported constrained term named `global_Error` publishes its internal edition as `__global_Error` (§7) — take the ordinary collision probe in an importer, which seeds the captures.

**Consequences, with owners:**

- The emitter single-sources the runtime vocabulary beside §1.1's, adds it (with the reserved words) to the probe of every minted `.js` import local, reads one flat trigger quantity (top-level declarations and module-import aliases) before rendering, and qualifies at every runtime-reference seat — helper bodies and inline expression seats alike; the `undefined` check alone also reads function-scope bindings. The runtime module joins the emission-decided reachability channels under §8.3's settled stem, and the compiled project gains its seat beside the runtime declarations' — with the one obligation §8.3's type-only artifact never carried: `hex.js` is executable, so **every host's execution set must load it** like a prelude module, or every contested program dies at its first import (the Playground's execution set is built from the compiled modules and is the live instance of this obligation).
- Conformance owes: the four capture classes measured from the runtime side — executed, not merely emitted (a raise under `record Error`+`record Object`, a loop under `record Symbol`, a Unit under a bound `undefined`, **a `Seq.length` under `exception Boolean` — the second silent-wrong-value instance**, and the two `SyntaxError` classes parsing at all); the cross-module import-capture leg; the namespace-alias leg executed (`import Error` beside a declared exception, the `.js`-only capture route); the minted-local negative — a minted `.js` import local never takes a runtime-vocabulary spelling or a reserved word; the uncontested corpus byte-identical; the runtime module byte-stable across programs; and the **tripwire** — every helper rendered and every reference seat scanned, referenced globals asserted a subset of the vocabulary, so the capturable set moves only by conscious edit. The #665 conformance file pinning the broken `.js` verbatim demanded a conscious edit as its repair condition; this is that edit.
- The `.d.ts` emitter, §1.1, §14.6, and the shipped #665 qualification are untouched — the record above is scoped by this one.

**Rejected alternatives (do not re-litigate without new information):**

- **Reserving `undefined` (or `eval`, `arguments`, `globalThis`) in Hexagon**: fixes a handful of spellings out of a vocabulary that must be guarded anyway; a reservation list grows only by breaking source programs, where the emitter's list grows invisibly; and Modules §11.2 already assigns emitted-name collisions to the emitter. Available separately as a pure readability ruling, never as this repair.
- **An on-collision trigger** (qualify only where the module's emission actually references the contested global): requires the whole-emission reference analysis whose misses are precisely the silent failures being repaired, and makes emitted spellings depend on distant edits. The four candidates above all share the on-spelling trigger for this reason.
- **Relocating helpers to a runtime module** (so no helper sits in user scope): leaves the inline seats — Unit's `undefined` in user function bodies, and `String(…)` in interpolation and derived-`show` constants, are written outside every helper (measured; every `[Symbol.iterator]` occurrence, by contrast, sits inside one) — and un-bakes the per-module exception brand (#488).
- **Qualifying `undefined` through the runtime module** (`const __undefined = globalThis.undefined; export { __undefined };`): measured working — `undefined` is a property of the global object, and the reserved import rescues it exactly as it rescues `Error` — and rejected because `void 0` owes no import, is immune at every scope with no per-module machinery, and is JavaScript's own idiom for the value. Recorded so the import is not re-invented as a "simplification", and so the false ground "no qualifier can rescue `undefined`" — briefly drafted here, the same unmeasured-claim shape §14.6 records — is not re-asserted.
- **Duplicating the exception helper into a relocation sibling** (from candidate 3's exploration): unsound, recorded so it is not re-invented — the sibling binds the contested name, so a duplicated helper's bare `new Error` is captured by the very binding the sibling exists to hold; the helper must stay in a scope where the global is unshadowed.
