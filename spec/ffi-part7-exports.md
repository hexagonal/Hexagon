...
- `Bool` is unaffected in emitted text (no shipped `.d.ts` declares a `Bool` type face today); the rule covers it by criterion, not by edit.
- `Stream` is unaffected today — its §14.2 face is unimplemented (#659) and its `.d.ts` still renders brand-and-sink coherently — and joins this rule the day that face lands.
...
- **Spelling faces through the alias name now that it works.** The alias is transparent, so this buys TypeScript nothing, while re-entering the naming machinery — minted imports, occlusion, probes — that pins exist to bypass; the native spelling is self-contained in every file.
