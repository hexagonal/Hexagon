...
A **namespace alias** is the one member of the universe that is not over-claimed: it counts only where some occurrence is **answered at rung 3** through it, because that is what puts its line in the file, and an alias absent from the file contests nothing. A written dot is not enough. Anything that settles an occurrence before rung 3 also keeps its alias out of the universe — §2.3's pins, which are applied ahead of the sink, and rung 2, whose local outranks rung 3 at a qualified seat as much as at a bare one — and the rule is stated on the answer rather than on a list of what pre-empts it, so a rung added later cannot leave the list stale. None of this is a rendering question: which occurrences rung 3 answers is settled in the typed tree, before a single spelling is chosen, so nothing here waits on a second pass. Counting an alias whose line the file does not carry would move a minted local aside for a name the reader cannot find, which is the failure this rung order exists to avoid.
