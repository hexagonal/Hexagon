...
4. **Unpublishable seats extend the emitter's existing rename.** JavaScript's reserved words already emit as `__binding`-prefixed locals with the source name restored at the export seat (`export { __binding0 as await }`, mirrored in the `.d.ts`) — the emitter's ordinary renaming, the liberty Modules §11.2 assigns it; the `__binding` scheme itself enters the corpus here. Two holes close: `eval` and `arguments` join the reserved set (strict mode refuses them as binding names — today they emit verbatim and the module fails to parse), and **import locals gain the missing leg** — a minted `import { await } from …` emits verbatim today, a `SyntaxError`; the local aliases (`import { await as __binding3 }`) and every in-module reference follows. The rename is lawful at these seats and nowhere else, on a stated ground rather than luck: an internal alias leaks into a consumer's diagnostics exactly when it names a *type* (measured — §14.7), every JavaScript reserved word is lowercase, and a Hexagon type name is parser-gated uppercase — so a `__binding` alias can carry a value's export seat but never a type, and never reaches a consumer diagnostic. A future addition to the reserved set must preserve that: it is the lowercase gate, not the seat, doing the protecting.
...
**Export directly when the runtime value already has its declared JS representation**: primitives, `Nullable`, genuine `Hex.Vector`/`Hex.Map`/`Hex.Set` values whose element types name no captured collection, records and unions naming none, opaque erased values, exceptions, and representation-direct functions and callbacks. Direct export means the ESM binding *is* the runtime value or emitted function — raw identity, no indirection (Part 6 §1). `Array(a)` left this list at #876, and `JsMap`/`JsSet` at #875: it is a captured foreign collection (Part 1 §2.2), and what JavaScript receives at any `Array` position is a copy.
...
**The wrapper is allocated once with the ESM binding, not per reference or call, so its JS identity is stable** — a JS consumer storing, comparing, or deregistering the export observes one function forever. No wrapper validates its inputs (§1). (Occasion 4's stable export wrapper is stable in exactly this sense; Part 6 §5.5's conversion wrappers are per-crossing values and carry a different name for that reason.)
...
| *(#666)* The emitted JavaScript survives its own module: the runtime vocabulary (the global spellings emitted runtime text references — feeder-defined, tripwire-enforced) is guarded in value space by §1.1's same move — a contested module imports the globals it contests as reserved captures from the program's runtime module at Part 1 §8.3's reserved `hex.js` seat, and the compiler's references qualify (`new __Error(…)`); Unit spells `void 0` where `undefined` is bound; locals, import locals, and reserved words (now with `eval`/`arguments` and the import-local leg) take the unpublishable `__binding` rename; on-spelling per module, byte-identical elsewhere; no published name moves, in either file, and no user spelling moves at all for a vocabulary capture | §1.2, §14.7 |
