...
Two of those rows **pin** a nominal type: prelude `Bool` faces as `boolean`, prelude `Seq(a)` as `Iterable<a>`. A pin is not just another face-table row — `Nullable` and `Array` have no declarations to bypass, while a pinned type is a module-owned nominal whose faces deliberately do not name it. The criterion *(#622)*: **a type is pinned when its entire boundary meaning is a JS-native notion that JavaScript's own syntax consumes** — the primitive `if` tests, the protocol `for...of` drives — nothing beyond the notion promised outbound or demanded inbound. Nominal law inside Hexagon, the native notion at the border; Part 1 §8.2's `Seq` carve-out states the same criterion from the face side ("the one type whose face is the protocol rather than a brand over it"). `Vector`, `Map`, and `Set` carry the iterable protocol too and are not pinned: satisfying a protocol as a courtesy is not being exhausted by it — their boundary contract is their identity, and their faces name it (`Hex.Vector<a>`, §2.1). `Range` is the sharp case: its outbound face exposes iteration and nothing else (Part 1 §8.1), and the criterion's *inbound* leg alone excludes it — a `Range` position demands a genuine `Range`, where a pinned type accepts anything satisfying the notion. Part 3 §14.2's `Stream(a)` (outbound face `IterableIterator<a>`) is the standing candidate: its membership — including the one-spelling question its inbound door raises — is settled with its implementation (#659).
