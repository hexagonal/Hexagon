...
What a face carries is an **identity, never a spelling** — the discipline of the `Bool`/`Seq` pins. How that identity is spelled is a property of neither the type nor, quite, the module: one record is `Point` where it is declared, `LibPoint` where an import renamed it, and `Lib.Point` at a seat the source qualified. §2.3's **pinned faces are settled before the sink is asked**: in a rendered face, prelude `Bool` is `boolean` and prelude `Seq(a)` is `Iterable<a>` however the occurrence spelled them, qualified or bare, and neither imports anything. A pin governs faces and not declaration seats — `Seq.hex`'s own `.d.ts` still declares its §5 brand type under its own name, which is what a hand-written TypeScript consumer names the opaque type by, even though no generated face will ever spell it. What is left the declaration file resolves through one sink, which answers in this order — by identity at every rung but the third, which reads the occurrence, for the reason given there:
