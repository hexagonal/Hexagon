...
JavaScript resolves names file-locally before the global scope, exactly as TypeScript does (§1.1) — and the emitted `.js` writes runtime text in the global vocabulary: the exception helper's `new Error(…)` and `Object.assign` (Exceptions §7.2), the range and `Seq` machinery's `Symbol.iterator`, the memoizer's `WeakMap`, the derived-instance helpers' `Math`, `Number`, `String`, `Array`, the probe's `console`, and Unit's own spelling `undefined` (Products §2.6). A module-scope binding under one of those spellings captures every such reference in its module. Four severity classes, all measured *(#666)*: `export record Error` (or `Object`) beside a declared exception makes **every raise throw `TypeError: Error is not a constructor`**; `export record Symbol` beside any `for` loop makes the first iteration throw — the range's `[Symbol.iterator]` reads the record constructor; `let undefined = 1` — lowercase, so legal at module *or function* scope — makes every Unit in scope **silently become `1`**, the worst class, wrong values with no crash; and a binding JavaScript itself refuses (`export let eval`, or `import { await }` reaching the importer's `import` line verbatim) is a **load-time `SyntaxError`** — the module never runs at all. Import locals capture like declarations: `import { Error } from "./lib"` poisons the *importing* module's helper. Uppercase spellings are module-level-only exposures — `let` requires a non-uppercase-start name, so no function local can bind them.
...
1. **Contested references qualify through reserved imports of the globals themselves.** Where a module's top-level bindings — declarations and import locals, one flat quantity read before rendering, §2.4's discipline — contain a runtime-vocabulary spelling, the module imports that global's reserved capture from the program's globals module, and every compiler-written reference to the contested spelling uses it: `new __Error(__message)`, `__Object.assign(…)`, `[__Symbol.iterator]`. This is §1.1's qualification in value space, with the substitution the language forces: TypeScript resolves type-position `globalThis` specially, immune to the file's bindings (measured, §1.1), but JavaScript protects *no* spelling of the global scope — value-position `globalThis` is an ordinary identifier (`export let globalThis: Int = 1` emits `const globalThis = 1;` verbatim, and a function local may bind it too — measured) — so the protected spelling is manufactured from the one namespace no user binding or import local can enter, Lexer §3.2's reserved prefix, and arrives as an import binding. The import line doubles as a manifest: `import { __Error, __Object } from "./hex.globals.js"` names exactly the spellings this module contests. Per module and per spelling, on the spelling alone: the trigger reads what the module *binds*, never what its emission happens to reference, because the reference set is exactly the quantity that re-draws silently — a trigger that read it would fail precisely when the hazard moves. A module binding no runtime-vocabulary spelling emits the bare text it always did, byte-identical.

2. **Unit takes its immune spelling where contested.** A module that binds `undefined` anywhere — module scope or function scope — spells Unit and the arity-0 tuple `void 0` throughout (the one global reference no qualifier can rescue: it is captured *as a value spelling*, not resolved through a scope the handle can reach around). Everywhere else Unit stays `undefined`, the readable spelling.

3. **Unpublishable seats extend the emitter's existing rename.** JavaScript's reserved words already emit as `__binding`-prefixed locals with the source name restored at the export seat (`export { __binding0 as await }`) — Modules §11.2's ordinary renaming. Two holes close: `eval` and `arguments` join the reserved set (strict mode refuses them as binding names — today they emit verbatim and the module fails to parse), and **import locals gain the missing leg** — `import { await } from …` emits verbatim today, a `SyntaxError`; the local aliases (`import { await as __binding3 }`) and every in-module reference follows. These seats publish nothing — a local and an import local are visible in no consumer's tooling, no diagnostic, no `.d.ts` — which is why the rename is lawful here and nowhere else: an internal alias on a *published* declaration leaks into consumer diagnostics (measured — the record record is §14.7), and no design below tolerates that.

**The globals module.** One per program — `hex.globals.js`, beside Part 1 §8.3's `hex.d.ts` and under its path discipline — emitted exactly when some module of the program is contested, carried on its own reachability channel like every emission-decided import. Its text is the capture no contested module can perform for itself, one binding per vocabulary member, the full vocabulary always — its bytes depend on the vocabulary alone, never on which program or module asked:
...
**Origin.** §14.6's repair was scoped to the declaration file on the recorded sentence "the emitted JavaScript is untouched" — and its implementation found the other file's capture: a module declaring an exception inlines a construction helper spelling `new Error(…)`, and `export record Error` in the same module makes every raise throw `TypeError: Error is not a constructor` at runtime, with no diagnostic and no type-level symptom. Filed as #666 with three claimed live spellings; this ruling's measurement corrected the scope in both directions. Over-claimed: `Map` — the emitted `"Map.empty"`/`"Map.fromVector"` texts are string literals inside derived `show` bodies, and no user-module runtime text references the native constructor. Under-claimed: `Symbol` (the range and `Seq` machinery's `[Symbol.iterator]`, live in any module with a `for` loop — measured broken), `WeakMap` (the memoizer), `Math`/`Number`/`String`/`Array` (the derived-instance helpers), `console` (the debug probe, captured at module evaluation — a later shadow is a *load-time* `ReferenceError`), and `undefined` — Unit's own spelling (Products §2.6), capturable at function scope because it is lowercase, and the worst class: every Unit in scope silently becomes the shadowing value. Two pre-existing holes in the reserved-word rename are the same family and fold in: `eval`/`arguments` missing from the reserved set (strict mode refuses the emitted binding — the module never parses), and the import-local leg missing entirely (`import { await }` emits verbatim — likewise; `import { Error }` poisons the importing module's helper). Uppercase spellings are module-level exposures only — `let` requires a non-uppercase-start name.
...
- **Relocating helpers to a runtime module** (so no helper sits in user scope): leaves the inline seats — `[Symbol.iterator]` and Unit's `undefined` are written into user function bodies, not helpers — and un-bakes the per-module exception brand (#488).
