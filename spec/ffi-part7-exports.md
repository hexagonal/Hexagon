...
- **No published name moves, in either emitted file, for any reason.** The emitted JavaScript and the `.d.ts` therefore agree on every export name by construction — the agreement is absolute, not maintained by machinery. §2.4's rule that the two files plan their *internal* aliases independently stands on its unobservability license, unchanged.
...
**No module publishes a JavaScript export named `then`** *(#1014)*. JavaScript's promise resolution reads any object's `then` property and, when it is callable, calls it in place of resolving to the object; a module namespace is such an object, so a module exporting a callable `then` never arrives from `import()` or `await import(…)` — the host or consumer loading it calls `then` instead. Lexer §4.4 lets a module-level term be named `then` (a Hexagon combinator, or an extern member row binding a promise-like's `then`), so the emitter withholds that one spelling: the term exports under the reserved `__then` only — its internal edition's spelling where a two-edition split already exists (occasion 4), otherwise the same fixed spelling — which Hexagon importers reach it by (`Task.__then` in the importer's emitted text for a first-class reference; a receiver call is emitted inline as `task.then(f)`, Part 5 §2.2), and the `.d.ts` declares no `then`. A JavaScript consumer calls `.then` on the object itself. Nothing is renamed: the Hexagon name is `then` everywhere a Hexagon reader sees it (Part 1 §10), and the published JavaScript surface lacks exactly the binding JavaScript would misread. Other keyword-named terms publish under their own spelling (`export { match }`, `export { __catch as catch }` under §1.2 rule 4).
