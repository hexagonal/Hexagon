...
An exported **face** cannot mention a type its owner keeps private — the checker refuses it at every carrier a face has: a binding's signature, a type alias's target, a record's fields, a union constructor's payload, an exception's payload (Modules §4.3, `` exported binding `probe` exposes private type `Hidden`; export the type, perhaps opaquely, or keep the binding private ``, the carrier's own noun in each) — so no rung is ever asked for an identity its home module withholds: every nominal a face reaches is exported, perhaps opaquely, somewhere, and rung 1 in particular is never asked to answer with a name the file does not bind *(#621; correction record §14.4)*.
...
- The alias set the probe's universe counts is the aliases whose lines the file carries — read from the faces it will render, not from every qualifier the module wrote — and it is settled before any spelling is chosen, so the universe stays a pre-rendering quantity and an alias whose line is absent moves nothing. A written dot is not the test; §2.4 says why, and the rejected alternatives below record what counting one costs. Part 1 §8.3 obligation 2 is amended to match: the `Hex`-spelled alias forces `Hex1` where its line is carried.
...
**The rule** is Modules §4.3's, in place: an exported face whose type mentions a private nominal is a hard error at every carrier, one message family with the carrier's own noun, reported at the offending mention's seat, once per type per carrier. Transparency cuts both ways — a private alias expanding to a private nominal launders nothing — and the walk that answers is the boundary check's existing one, whose locality signal (`representationVisible` as stamped on imported copies) already keeps elsewhere-public types out of the refusal.

**Consequences, with owners:**

- The checker's boundary check visits the four carriers beside the bindings it already reads, reusing the walk and its locality signal; the diagnostic anchors at the field annotation, the slot annotation, or the alias's right-hand side, and dedupes per (carrier, type) at the first offending seat.
- The declaration emitter's union arm joins every other arm: a private union contributes **no** `.d.ts` row. Safe exactly because the checker now refuses every face that could reach one; landing both together is what leaves no state in which a clean compile ships a broken or leaking declaration file. The runtime modules' declaration files lose their never-referenced private-union rows (`Tree`, `Root`, `Frames`) under the same move.
