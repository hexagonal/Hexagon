...
What a face carries is an **identity, never a spelling** — the discipline of the `Bool`/`Seq` pins. How that identity is spelled is a property of neither the type nor, quite, the module: one record is `Point` where it is declared, `LibPoint` where an import renamed it, and `Lib.Point` at a seat the source qualified. §2.3's **pinned faces are settled before the sink is asked**: prelude `Bool` faces as `boolean` and prelude `Seq(a)` as `Iterable<a>` however the occurrence spelled them, qualified or bare, and neither imports anything. What is left the declaration file resolves through one sink, which answers in this order — by identity at every rung but the third, which reads the occurrence, for the reason given there:
...
3. **The occurrence is qualified through a *namespace* import's alias** (Modules §3.3) — **`Alias.Name`**, the spelling the source wrote at that seat:
   ...
   **A qualifier is readable only in the module that wrote it.** It is a spelling, and this section's first sentence is that a face carries an identity and never a spelling; a spelling that travelled would be meaningless at best. It is worse than meaningless where a type crosses a module boundary carrying one — a type alias's expansion travels with the qualifiers its *writer* used (§1) — because the importer may bind that same alias to a different module, and the face would then name a real type that is the wrong one, with nothing to report it. Rung 3 therefore answers only where the alias is one **this** module binds; a qualifier from elsewhere is not an occurrence here and falls to rung 5. No derived declaration loses anything to this: each is emitted by the module that wrote its signature.
