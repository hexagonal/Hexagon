...
**The runtime module takes the reserved seat.** Part 1 §8.3 reserves the runtime's code home in so many words — *if a later pass ever materializes shared runtime JavaScript, `hex.js` is its designated home, so the type home and the code home converge on one module identity instead of inventing a second* — and this is that materialization: the globals capture is program-scoped runtime JavaScript, and it takes `hex.js`. One per program, emitted exactly when some module of the program is contested, carried on its own reachability channel like every emission-decided import, and named by the **same stem §8.3's probe settles**: whatever filename the runtime declaration seat won (`hex`, `hex1`, …), the code home follows it — one probe, one identity, a user module never renamed on its account. When both artifacts are emitted, the declaration file additionally declares the runtime module's exports (`declare const __Error: ErrorConstructor; …`), so the pair remains a truthful declaration of one module; when only `hex.js` is owed, it is emitted alone. Its text is the capture no contested module can perform for itself, one binding per vocabulary member, the full vocabulary always — its bytes depend on the vocabulary alone, never on which program or module asked:
