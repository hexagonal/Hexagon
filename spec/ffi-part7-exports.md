...
**The wrapper is allocated once with the ESM binding, not per reference or call, so its JS identity is stable** — a JS consumer storing, comparing, or deregistering the export observes one function forever — and it faces as the export it stands in for: its `name` is the public name and its `length` the declared arity (Part 6 §1), never a compiler spelling (Lexer §3.2). No wrapper validates its inputs (§1). (Occasion 4's stable export wrapper is stable in exactly this sense; Part 6 §5.5's conversion wrappers are per-crossing values and carry a different name for that reason.)
