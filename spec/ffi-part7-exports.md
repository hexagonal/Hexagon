...
What a face carries is an **identity, never a spelling** — the discipline of the `Bool`/`Seq` pins. How that identity is spelled is a property of neither the type nor, quite, the module: one record is `Point` where it is declared, `LibPoint` where an import renamed it, and `Lib.Point` at a seat the source qualified. The declaration file therefore resolves every rendered nominal through one sink, which answers in this order — by identity at every rung but the third, which reads the occurrence, for the reason given there:
...
5. **No rung above answers for this occurrence** — the file **mints its own import**, `import type { Name as Local }` from the identity's home module. The local is the type's own name, so a bare occurrence stays bare; it moves only under the probe below, and against a namespace alias only where the file actually carries that alias's line. This rung is not a fallback kept for tidiness. It carries the bare occurrences rung 3 declines, and it carries a nominal this module names under no spelling whatever: faces hold a type alias's *expansion* rather than its name (§1, Modules §11.4), and an importer that binds only the alias binds nothing the expansion mentions.
   ...
   One identity can reach both rungs in one module — qualified at one seat, bare at another — and then the file carries **two** lines for it, the alias's and the minted one, and spells it two ways. That is the shape the source wrote, and TypeScript binds both to the same declaration.

An exported **binding** cannot mention a type its owner keeps private — the checker refuses it (`` exported binding `probe` exposes private type `Hidden`; export the type, perhaps opaquely, or keep the binding private ``), per §1's boundary rule — so along that route no rung is asked for an identity its home module withholds. The guard is the binding's alone. A type alias, a record field, a union constructor's payload and an exception's payload each carry a module-private nominal into an exported face with no diagnostic today, and rung 1 then answers with a name the file does not bind; that is **fenced to #621**, against Modules §11.4 and §1 both, and this paragraph loses its qualification once it is repaired.
...
- **An occurrence an earlier rung answered is never minted.** That rung took it over, the same take-over the term side already performs.
...
A **namespace alias** is the one member of the universe that is not over-claimed: it counts only where the module qualifies some occurrence through it, because only then does the file carry its line, and an alias absent from the file contests nothing. That is not a rendering question — whether any occurrence is qualified is settled in the typed tree, before a single spelling is chosen — so nothing here waits on a second pass. Counting a gated alias would move a minted local aside for a name the reader cannot find, which is the failure this rung order exists to avoid.

**Placement.** Compiler-written imports precede the module's own items: the `Hex` runtime import first (§2.1), then rungs 4 and 5's imports in inventory order. A line rung 2 or rung 3 owes keeps its source position — it is the module's own import, not the compiler's — and is simply absent where no answer owed it.
...
**What never imports:** the pinned faces (prelude `Bool` as `boolean`, prelude `Seq(a)` as `Iterable<a>` — §2.3), structural faces, a **type** alias's own name, and the `Hex.*` runtime types (§2.1's namespace owns those). A type alias's *expansion* is a different question, and it is rung 5's.
...
- The alias set the probe's universe counts is settled from the typed tree — the aliases some occurrence qualifies through — before any spelling is chosen, so the universe stays a pre-rendering quantity and a gated alias moves nothing. Part 1 §8.3 obligation 2 is amended to match: the `Hex`-spelled alias forces `Hex1` where its line is carried.
...
- An uncontested, unqualified module must emit its `.d.ts` byte-identically, except where a line no face reached is now absent. Two shapes make that visible and are expected rather than findings: a type-only row for a name no face mentions disappears, and a module that exports nothing and namespace-imports for terms — whose whole `.d.ts` was that one line — becomes `export {};`.
