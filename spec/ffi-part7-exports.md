...
- **Spelling faces through the alias name now that it works.** The alias is transparent, so this buys TypeScript nothing, while re-entering the naming machinery — minted imports, occlusion, probes — that pins exist to bypass; the native spelling owes no import in any file. (Its occlusion exposure is #662's, shared by every TS-lib-spelled face and unrepaired by this alternative, which cannot reach the structural rows at all.)
