...
**The rule** is §1.2's: the runtime vocabulary — feeder-defined by the emitter's own references, tripwire-enforced — is guarded by §1.1's same move in value space, with the protected spelling manufactured rather than found: the vocabulary and JavaScript's reserved words join the probe of every minted `.js` import local (§1.2 rule 1); a contested module — its trigger reading declarations, import locals, *and namespace-import aliases*, the one binding form on which the two sections' triggers deliberately diverge — imports the globals it contests as `__`-reserved captures from the program's runtime module, which takes the `hex.js` seat Part 1 §8.3 reserved (amended there with this record); Unit takes `void 0` exactly where `undefined` is bound, and the unpublishable seats extend the reserved-local rename (with `eval`/`arguments` and the import-local leg closed; the original `__binding` spelling is superseded by §1.2 rule 4's descriptive spelling). On the spelling alone, never on the emission's reference set — the reference set is the quantity that re-draws silently, and a trigger reading it fails exactly when the hazard moves. The final spelling was itself a measured choice: a `__globalThis.X` mirror of §1.1's qualifier was drafted and superseded on the readable-emission commitment — `__Object.assign(new __Error(…))` is nearly the original text, the `__` prefix is already the emitted file's own idiom, and the import line is a manifest of the module's contested spellings.
