...
4. *(#876)* **an exported function whose signature names a captured foreign collection** anywhere Part 1 §5.4's walk reaches — a parameter or result of type `Array(a)`, a record parameter with an `Array` field, a callback parameter over arrays. The wrapper runs the walk over each such parameter on entry and over the result on exit (Part 1 §5.4), so a JavaScript caller's array becomes Hexagon's copy and Hexagon's array leaves as a copy, and calls the internal function. **The public name binds the wrapper; the internal edition is exported under an internal name that appears in no `.d.ts`, and Hexagon importers bind that name** — the convention the trailing-evidence generic edition already uses (the edit note below) — so a cross-module Hexagon call is an ordinary Hexagon-to-Hexagon call and never copies (Part 1 §2.2). The internal name is a reserved `__` spelling (Lexer §3.2), absent from the `.d.ts`; a JavaScript module that imports it anyway reaches the unwalked function, and that act stands with fabricated brands and dictionaries in Part 1 §3.1 — recorded at Part 1 §5.4, not hidden behind "appears in no `.d.ts`". Exported **constructors** are functions for this occasion (Part 1 §5.4 item 4): a union or record constructor whose payload names a captured collection takes the wrapper. Where occasion 4 meets a constrained export it follows the published face exactly as occasion 1 does: each fundamental specialization with a captured position takes the wrapper, the internal edition takes none. A polymorphic export with a captured position — `first(xs: Array(a)): a` — walks the array and carries `a` by identity (Part 1 §5.4's type-variable clause).
