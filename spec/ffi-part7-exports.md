...
- **A namespace import binding is not a capture route.** A TS namespace import does not occupy the plain type-name space — measured with a control *(#662)* — so a source-written `import module Iterable …` contests nothing and no yielding is triggered on its account; it stays in the universe on the flat-check convention (Part 12 §11.1), whose only cost is the harmless over-claim above. The claim is this declaration file's alone: the `.js` rules the opposite way, because an ESM namespace import occupies the value-name space like any binding (§1.2 rule 2, measured).
