...
2. **User-written spellings never move — Part 1 §10 stands absolute — and the face steps around them.** Where a file's type space binds a contested spelling anyway — an exported type under that name, or a carried named import's local — every face in that file written in that spelling qualifies through TypeScript's own global-scope reference: `globalThis.Iterable<a>`, `globalThis.Error & {…}`. The qualified spelling **is** the library type — assignability, inference, and member access are byte-for-byte those of the bare spelling, consumers never write the qualifier themselves, and the reference works on every toolchain that can consume these declarations at all (verified through real `tsc` at 4.7, 5.0, 5.6, and current). **Collision-only:** a file whose type space binds no contested spelling emits the bare vocabulary it always did, byte-identical — the house rule that a probe which has not had to move emits the text it always emitted, read at the face.
...
- The declarations emitter gains the vocabulary in its probe universes and a per-file qualification decision at the face-rendering seats: a contested spelling qualifies exactly where the file's type-space universe binds it. The decision is per (file, spelling), computed from the module's universe before rendering — §2.4's discipline.
- Conformance owes: the three measured capture classes emitting qualified faces that pass real `tsc` from the consumer side, both directions; the `Error` class recovering genuine `Error` members on exception faces and `isHexError`; the source-written named import of a type actually named `Iterable` (the local binds the consumer's type space — its file qualifies); a minted local never taking a vocabulary spelling; the on-collision negative — an uncontested corpus emits byte-identically to before the change; and the emitted-JavaScript negative — no `.js` output changes anywhere.
- The inspection preview is out of scope (§14.3's scope note): one pane, bare names by design.
