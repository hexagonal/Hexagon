...
- **Not that this is special.** Nothing here is peculiar to `Array`. The same holds of `Iterable<JsMap(k, v)>` and `Iterable<JsSet(a)>` — FFI Part 10 §6 binds their members, and the build realizes those rows only when the types themselves gain representations. A Decided row outrunning the build is the corpus's normal condition, not an exception needing a doctrine.
