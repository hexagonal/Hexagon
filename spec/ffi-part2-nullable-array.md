...
**Conformance obligations on the implementation** (fixed here so shipping required no re-deciding; the shipped `Vector.toArray` discharges all four; obligation 4 and the boxed rule above reach the rest of the quartet, whose mechanism is theirs to settle — `Array.toVector` took the same door from `stdlib/Array.hex`):
