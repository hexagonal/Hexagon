...
**Conformance obligations on the implementation** (fixed here so shipping required no re-deciding; the shipped `Vector.toArray` discharges all four, and they stand as the bar the remaining three conversions meet at their own companion):
...
> **Edit note (for `ffi.md` §9.1 — DISCHARGED when `Vector.toArray` shipped; retained as the record, nothing to re-apply):** add the §9 conversion quartet's implementation to the obligations list as a mirror of the `stdlib-roadmap.md` §2 row, carrying this section's order — outbound `Vector.toArray` first (issue #238); inbound later, separately motivated — and the absence-not-stubs rule (§9.1 here). Flagged, not directed: §9.1 there heads itself "global ledger", which reads against `stdlib-roadmap.md` rule 1's "no other ledger exists". This ruling has no authority over `ffi.md`'s heading and does not decide it — the roadmap's consolidation pass does. Note for whoever does: the index reading is an inference from rule 1 plus `stdlib-roadmap.md` §6's silence on FFI's *stdlib* half, not a statement §6 makes, so cite rule 1.
...
| Calling an unshipped §9 conversion (`Array.toSeq`, `Array.fromSeq`, `Array.toVector` today; `Vector.toArray` has shipped) | not a new diagnostic — the ordinary resolver error, ``module `Array` does not export `toVector` ``, deliberately (§9.1: absence, never a stub; Rewrite Rule not engaged). `Array` and `Vector` are ordinary prelude modules and an unknown operation is an unknown export, everywhere |
