...
- **Where the capture happens** is Part 1 §5.4's position table, not restated: every declared boundary position whose type names `Array` — an extern result or argument, an `extern let` once at initialization, a receiver member's slots, an exported function's parameters and result through its conversion wrapper, a callback's slots at each invocation, and `JsValue.from`'s argument. The copy runs in **both directions**: an `Array` handed *to* foreign code is a fresh array too, so the value Hexagon retains cannot be reached by the code that received it. A Hexagon-to-Hexagon call never copies — two Hexagon functions passing an `Array(Int)` share one value, as they share a `Vector`.
- **What the copy is.** A fresh JavaScript array with the source's `length`, each index read exactly once in index order through native array access, each element carried through the walk at `a` — by identity for every type but a captured collection or a function that names one (Part 1 §5.4). Holes are §6.4's business. The emitter's spelling of the copy is its own (`slice`, `Array.from`, an indexed loop); the access pattern above is the contract, because an exotic array object can observe it.
...
- **Effects.** The copy performs none of its own. The acquisition is coloured by the row that performs it (Part 4 §4.5): an unannotated extern row is `->!`, and a `->` row returning an `Array(Int)` is the author's claim that the foreign function is pure — the row's result is then a value by capture, the reads of it are Effects §6.2 species (c), and nothing about the value's later use is coloured by how it was acquired.
...
**The prelude consequence of #876**, stated so the implementation is not the first place it is met: every exported function whose signature names `Array(a)` takes Part 7 §7 occasion 4's conversion wrapper under its public name, with Hexagon importers binding the internal edition — `stdlib/Array.hex`'s `length`, `get`, and `toVector`, and `stdlib/Vector.hex`'s `toArray`, among them. A JavaScript caller of `Vector.toArray` therefore receives a copy of the fresh array the conversion built (Part 2 §13.2 item 2's price, met here), and a JavaScript caller of `Array.length` pays a copy to read a length it could read natively; Hexagon callers pay nothing.
