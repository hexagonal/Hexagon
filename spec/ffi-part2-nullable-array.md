...
- **Fresh arrays Hexagon makes** — a slice (§6.3), `Array.fromSeq`, `Vector.toArray` (§9) — are captured values from birth: made by Hexagon, held by Hexagon, unreachable from foreign code until they cross, at which point they are copied like any other (save the next bullet's exception). There is no "stable while exclusively held" clause any more, because every captured `Array(a)` value is exclusively held — save where the next bullet's temporary exception places one inside a borrowed map handed out, and Part 10 §2's contract holds it still instead.
- **The temporary development-stage exception (until #875).** A borrowed `JsMap`/`JsSet` is a container the walk does not enter (Part 1 §5.4), so an `Array(a)` inside one is neither captured on the way in nor copied on the way out. Inbound, an array extracted from a borrowed `JsMap(k, Array(a))` or `JsSet(Array(a))` may remain *borrowed* rather than captured; outbound, an `Array(a)` Hexagon retains and places in a borrowed map it hands to foreign code (`JsMap.fromSeq`, `Map.toJsMap`) stays Hexagon's — the consumer receives the outer map, not ownership of the arrays inside. Both are **supported use under one temporary borrow contract** — Part 10 §2's, which binds the foreign side to keep every piece of storage observable through such a value stable wherever Hexagon retains the ability to observe it: references retained independently of the map, escaped closures, and deferred traversals included — and it is an exception to enforced snapshot *isolation* only: the value's contents may not change between observations, and no no-hoist rule substitutes for that stability, so every operation on such an array is as pure and as freely shared as on a captured one. It is documented apart from the reserved-internal-name route (Part 1 §5.4), which is a contract violation, not a use. Every other `Array(a)` value is a capture; #875 replaces the exception with snapshot protection in both directions.
...
**Every observation of an `Array(a)` — indexing, `length`, slicing, iteration, any `Seq` derived from it — observes the captured array, which is stable** (the arrays §6.2's temporary exception governs until #875 — one extracted from a borrowed `JsMap`/`JsSet`, or a captured one Hexagon retains and hands out inside a borrowed map — are stable under Part 10 §2's borrow contract instead, and equally stable: the exception is to ownership, not to what an observation may see). This is the resolution of the observation-semantics question Collections Part 5 §6.1 deliberately left to this spec, and it is now the plain one: there is nothing live to observe. Iteration copies nothing *at iteration time* because the copy was made at the crossing; a `Seq` derived from the array may be forced a year later and sees the same elements.

Consequently, **native `for...of` emission is permitted and preferred** (§8.2), because a native array iterator over an array nobody else can reach is a traversal of a value (and where §6.2's temporary exception lets foreign code reach one, Part 10 §2's contract supplies the same stability). `Array.toVector` remains the conversion into persistent storage for callers who want `Vector`'s operations (§9); it is no longer an escape from anything.
...
- **`Array.toSeq` is lazy and allocates no copy**: it traverses the captured array, which is stable, so the sequence may be forced at any time and observes the same elements (§6.5). Everything about the resulting `Seq`'s persistence and memoization is Part 3's; nothing here adds to it, and no borrow is extended by holding it, because a captured array has no borrow — a `Seq` over an array extracted from a borrowed `JsMap`/`JsSet` (§6.2's temporary exception) extends that map's borrow obligation through its consumption lifetime, as Part 10 §2 says, until #875.
...
| `Array` beneath one of the five Hexagon runtime containers the walk cannot enter, inside an opaque representation, in an exception payload, or as an exported value binding; `JsValue.from` at an unresolved variable | Part 1 §5.4's refusals (single owner; this part inherits) |
...
| *(#876)* Temporary development-stage exception until #875: an `Array` extracted from a borrowed `JsMap`/`JsSet` may remain borrowed, and an `Array` Hexagon places in a borrowed map it hands out stays Hexagon's, both under Part 10 §2's contract (full observation lifetime, independently retained references, escaped closures and deferred traversals included) — supported use, one exception to enforced snapshot isolation and not to purity, documented apart from the reserved-internal-name violation; removed when #875 captures the pair | §6.2, §6.5 |
