...
| Calling an unshipped §9 conversion (`Vector.toArray` today) | not a new diagnostic — the ordinary checker error, ``the companion of `Vector` has no core operation `toArray` ``, deliberately (§9.1: absence, never a stub; Rewrite Rule not engaged). *Not* an unknown-export error: that message exists but fires only where a user's own `import * as Vector` shadows the prelude companion |
