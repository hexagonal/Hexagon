...
- **Fresh arrays Hexagon makes** — a slice (§6.3), `Array.fromSeq`, `Vector.toArray` (§9) — are captured values from birth: made by Hexagon, held by Hexagon, unreachable from foreign code until they cross, at which point they are copied like any other. There is no "stable while exclusively held" clause any more, because every captured `Array(a)` value is exclusively held, always.
