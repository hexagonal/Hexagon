...
2. The mechanism is the implementer's choice within existing doctrine, but the choice is gated on scheduling rather than preference. The intrinsic door (`spec/intrinsics.md`; a new flat-inventory key on the `vectorToArray` pattern) is the designed route for a stdlib-owned compiler-provided operation — with the gate that a `"hex:intrinsic"` block is legal only in modules compiled as standard-library source (`intrinsics.md` §5.2), which in v1 means the prelude set, and `stdlib/Vector.hex` is not in it. The resolver refuses the block there today (``the `hex:` specifier scheme is reserved to standard-library source``). That gate opens at the `Vector` arc's own milestone, where `Vector.hex` converts its public-name-door wrappers to declarations (`intrinsics.md` §3.2, §9.2), or earlier if `stdlib-roadmap.md` §5.2 stage 1's loader boundary lands first; an implementer must not assume the intrinsic door is open — taking that route means sequencing behind that milestone. Extending `CollectionOperation` is disfavored for a different reason: `Vector`'s rows in that family are themselves scheduled for deletion at the same milestone (`intrinsics.md` §9.2), so a new row joins a list already being emptied. But disfavored is not foreclosed, and the distinction matters to scheduling: the public-name door "keeps working unchanged" until each companion's milestone (`intrinsics.md` §9.1), and `stdlib/Vector.hex`'s shipped `toSeq` is that wrapper shape already, so a `vectorToArray` row plus the matching wrapper discharges obligations 1, 3 and 4 today with no privilege change. **#238 itself is therefore not gated; only the preferred route is.** The *family* is not scheduled for retirement — `Node` outlives the four companions in it, and whether a one-member family is the intended terminus is #223.
...
| Calling an unshipped §9 conversion (`Vector.toArray` today) | not a new diagnostic — the ordinary checker error, ``the companion of `Vector` has no core operation `toArray` ``, deliberately (§9.1: absence, never a stub; Rewrite Rule not engaged). *Not* an unknown-export error: that message exists but fires only where a user's own `import * as Vector` shadows the compiler-provided `Vector` companion |
...
| *(2026-08-02, #237 ruling)* Shipped Hexagon-owned surface carries no declared-but-throwing stubs: an operation that cannot yet be implemented is absent — no declaration, no export, no `.d.ts` entry — and the spec is the forward contract; §9's quartet is currently unshipped (verified); interim JS surface of a crossed `Vector` = iteration/spread/`Array.from` + author exports, confirming Part 1 §8.3 (in flight on the #128 branch, as are all §8.3/§8.4 citations in this part); no new diagnostic, Rewrite Rule not engaged; outbound `Vector.toArray` ships first (#238) — a one-item priority, not a partition, since `Array.fromSeq` is outbound too — the other three later and separately motivated, Part 10's doors gated on `Map.hex`/`Set.hex`; implementation obligations fixed (contract per §9, companion home, intrinsic-door route designed but gated on standard-library-source privilege (§9.1 obligation 2), no partial shipping); **no carve-out**: the "callable surface, not semantic content" exemption an earlier draft added is withdrawn unreplaced, as unnecessary and unsound, leaving the doctrine without an exception clause; the debt's ledger row is `stdlib-roadmap.md` §2 under that file's rule 1, with `ffi.md` §9.1 a mirror index and the two files' "global ledger" claims recorded under rule 5, not chosen | §9.1 |
