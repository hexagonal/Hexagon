...
**Status:** Decided (July 2026), revised in place after external review (Sol) before landing; §6.3 amended 2026-08-02 — `Array.size` is renamed `Array.length` under Collections Part 1 §10.1, and the `.length` diagnostic is re-charactered, see correction record §13.1. Amended 2026-08-02 (#237 ruling): §9 gains its shipping doctrine — an unshipped conversion is absent, never a declared-but-throwing stub; outbound `Vector.toArray` ships first — §9.1; rejected alternatives, including James's retracted `NotImplemented` interim, §9.2. The same amendment reconciles §8 with §9.1's finding of fact: §8.1's `Iterable<Array(a)>` row is shipped and true without the `Array.toSeq` symbol (no v1 program can call a provided row's `iterate`), so Collections Part 5 §6's discharge is whole, while §8.3's suite membership is marked decided-but-unshipped; the debt's ledger row is `stdlib-roadmap.md` §2. Normative promotion of `spec/notes/ffi-proto-spec-questions.md` §2 and the §4 `Nullable` package, drafted per `spec/notes/ffi-roadmap.md` Part 2. The three questions the draft recorded as promotion blockers (`Nullable` idempotence, §2.1; the `Array` accessor surface, §6.3; sparse arrays, §6.4) were resolved by James and Sol before promotion. Reads Part 1 (`ffi-part1-boundary.md`) for the category vocabulary and failure doctrine; restates neither.
...
**The row holds without the symbol** *(added 2026-08-02, #237 ruling)*. §9.1 finds that `Array.toSeq` does not exist in the shipped toolchain, and that finding does not falsify this row, because **a provided row's `iterate` is not a callable in any v1 program.** Three v1 rules close every path to it: the projection-bearing binder ban makes each `for..in` site monomorphic in the iterable's outer constructor, so instance resolution is static and dictionary-free loop emission is total — *"there is no program in which a runtime instance lookup could occur at a loop"* (Collections Part 5 §9.1); `Iterable` cannot constrain a type variable in v1, so no signature can carry the row as evidence to call through (Part 5 §3.2); and no `Iterable`, `Item`, or instance machinery reaches `.d.ts` (Part 5 §9.3). For `Array(a)` in particular, §8.2's native `for...of` consults no member at all. `iterate = Array.toSeq` accordingly states what iterating an `Array` **means** — the element type and the traversal — and the compiler realizes that meaning directly; it does not oblige a symbol. **The row is shipped, and Collections Part 5 §6's discharge stands.** What §9.1 finds unshipped is the *callable* `Array.toSeq`, which §8.3 and §9 name, and which §9.1's doctrine governs.
...
**Membership is decided; the names are not yet shipped** *(added 2026-08-02, #237 ruling)*. Unlike §8.1's row, this sentence is a claim about callable surface, and neither `Array.toSeq` nor `Array.fromSeq` exists today (§9.1's finding of fact). Suite membership is nonetheless settled as design — no later document may reopen whether `Array(a)` belongs — and the names ship under §9.1's doctrine and order, never as stubs in the interim. Read "joins the suite, under exactly those names" as fixing *which* names discharge the membership, not as asserting that they are callable now.
...
- **The type system is the contract of record, and "typechecks" must mean "can run."** The defect #128 removes from the `.d.ts` faces is precisely a promise the runtime denies — `map.get(k)` typechecking and throwing (Part 1 §8.4 item 1). A stub reproduces that defect deliberately, relocating it from the faces into the operations, and worsens its position: the lie is discovered at first execution instead of at the typecheck. Absence fails at the earliest, cheapest site — the resolver's existing diagnostic, ``module `Vector` does not export `toArray` ``, is truthful today and stays truthful. **No new diagnostic is introduced; the Rewrite Rule is not engaged.**
...
**The doctrine governs callable surface, not semantic content.** A name is subject to it when a program can write that name — an export, a companion operation, a `.d.ts` entry, a completion item. It is not engaged by a specification's statement of what an operation or instance *means*. §8.1 is the live case: the row `Iterable<Array(a)>` binds `iterate = Array.toSeq` while `Array.toSeq` is unshipped, and the row is nonetheless shipped and true, because v1 admits no program that can call a provided row's `iterate` (§8.1's three closures, from Collections Part 5 §9.1/§3.2/§9.3) and `Array` iteration emits native `for...of` through no member at all (§8.2). Collections Part 5 §6's discharge is therefore whole, not nominal. The distinction is narrow by construction: it covers only members of compiler-provided rows that no v1 program can name. §8.3's suite membership, which *does* name callables, is decided-but-unshipped and says so.
...
**Order of shipping, per James (2026-08-02): outbound before inbound.** `Vector.toArray` — the direction that *produces values for JavaScript*, and the direction #128's narrowing exposes — ships first, tracked as issue #238. `Array.toVector` (with `Array.toSeq`/`Array.fromSeq`) is the inbound half: later, separately motivated by pulling foreign data in, and deliberately not gated on #128 or filed alongside #238. Part 10's `Map.toJsMap`/`Set.toJsSet` are outbound but additionally gated on `Map.hex`/`Set.hex` existing at all (no such modules yet); their internal order is Part 10's business. Ordering is scheduling, not design: every §9 contract, and every corpus reference to these names while unshipped (e.g. Pattern Matching §11.1's "convert with `Array.toVector`"), remains the normative design.
...
2. The mechanism is the implementer's choice within existing doctrine: the intrinsic door (`spec/intrinsics.md`; a new flat-inventory key on the `vectorToArray` pattern) is the designed route for a stdlib-owned compiler-provided operation; extending `CollectionOperation` is disfavored, that family being scheduled for retirement (intrinsics.md §9; #223).
3. Its `.d.ts` face follows Part 1 §4.1's `Array(a)` row as it stands at ship time (currently `ReadonlyArray<a>`; #228 is the open conformance defect on that row). This section adds no face rule.
...
**Ledger.** This section discovers stdlib debt — the §9 quartet is specified and unimplemented — and `stdlib-roadmap.md` rule 1 makes that ledger's row mandatory and exclusive ("no other ledger exists"). The row is added there by this ruling, in §2 (v1 obligations), carrying the order and the absence rule. `ffi.md` §9.1 is the FFI spec's local index into that ledger, not a competing one, and gets a mirror entry; the two files' conflicting self-descriptions are recorded, not chosen, under `stdlib-roadmap.md` rule 5.

> **Edit note (for FFI Part 1 §8.3/§8.4, applied on next touch of that document, after the #128 branch lands):** no text change is required — this note records verification. §8.3's interim-surface bullet ("v1 interim surface … iteration plus author exports, named conversions unshipped, spread as the universal rewrite") and §8.4 item 1's price were tested 2026-08-02 against a live proposal to ship `Vector.toArray` declared-but-throwing, and they stand as written; the proposal is rejected at §9.2 item 1 here. On touch, add one cross-reference to §8.3's interim-surface bullet: "(shipping doctrine and order: Part 2 §9.1)".

> **Edit note (for `ffi.md` §9.1, applied on next touch):** add the §9 conversion quartet's implementation to the obligations list as a mirror of the `stdlib-roadmap.md` §2 row, carrying this section's order — outbound `Vector.toArray` first (issue #238); inbound later, separately motivated — and the absence-not-stubs rule (§9.1 here). §9.1 there heads itself "global ledger"; on the same touch, conform that heading to `stdlib-roadmap.md` rule 1 (it is the FFI index into the global ledger, per that file's §6).
...
3. **A recognized-but-unshipped diagnostic** (the compiler knows the name `Vector.toArray` and answers "specified but not yet available"). Rejected: it builds a shadow inventory of unshipped names, maintained by hand, that will drift stale — and it makes the compiler assert scheduling facts the spec owns. **Price of keeping it out:** the truthful unknown-export error teaches nothing about the plan; a user who found `toArray` in this spec learns from the diagnostic only that the module does not export it, and must return here to learn why and until when.
...
| Calling an unshipped §9 conversion (`Vector.toArray` today) | not a new diagnostic — the ordinary unknown-export error, deliberately (§9.1: absence, never a stub; Rewrite Rule not engaged) |
...
| *(2026-08-02, #237 ruling)* The row above is **shipped and true while `Array.toSeq` is unshipped**: a provided row's `iterate` is not callable in any v1 program (binder ban / no `Iterable` on a type variable / nothing in `.d.ts` — Collections Part 5 §9.1, §3.2, §9.3), and `Array` iteration emits native `for...of` through no member; the binding names semantic content, not a symbol, so Part 5 §6's discharge is whole rather than nominal. §8.3's suite membership, which *does* name callables, is **decided but unshipped** and now says so | §8.1, §8.3 |
...
| *(2026-08-02, #237 ruling)* Shipped Hexagon-owned surface carries no declared-but-throwing stubs: an operation that cannot yet be implemented is absent — no declaration, no export, no `.d.ts` entry — and the spec is the forward contract; §9's quartet is currently unshipped (verified); interim JS surface of a crossed `Vector` = iteration/spread/`Array.from` + author exports, confirming Part 1 §8.3; no new diagnostic, Rewrite Rule not engaged; outbound `Vector.toArray` ships first (#238), inbound later and separately motivated, Part 10's doors gated on `Map.hex`/`Set.hex`; implementation obligations fixed (contract per §9, companion home, intrinsic-door route favored, no partial shipping); the doctrine governs **callable** surface only, never a specification's statement of meaning (§8.1's row is the live carve-out, narrow by construction); the debt's ledger row is `stdlib-roadmap.md` §2 under that file's rule 1, with `ffi.md` §9.1 a mirror index and the two files' "global ledger" claims recorded under rule 5, not chosen | §9.1 |
