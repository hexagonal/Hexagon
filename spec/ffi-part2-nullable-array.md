...
*(Recorded in passing, owned by `stdlib-roadmap.md` §2's `iterate` qualified-home row and not by this ruling: the bare prelude name `iterate` is unbound today and reserved for the member — Part 5 §2.3's sole-exporter bullet keeps it so, and `Seq`'s seed/step producer is `Seq.successors` for that reason. The bare spelling resolves once the `Iterable` declaring module lands.)*
