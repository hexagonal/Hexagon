...
**Conformance obligations on the implementation** (fixed here so shipping required no re-deciding; the shipped `Vector.toArray` discharges all four; obligation 4 and the boxed rule above reach the remaining three, whose mechanism is theirs to settle):
