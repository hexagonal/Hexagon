...
| `end` | the module closer `end module Name` at the head of a top-level item — the two words together, the second the contextual header word above (Modules §2.2, #829); recognition includes a closing name with a segment that is not uppercase-start (`end module geometry`), whose report is §2.2's closer-naming rule, never §2.1's casing refusal; `end` alone is an ordinary name everywhere, and stays one: `SliceError(start: Int, end: Int)` in `stdlib/Vector.hex` is live use |
