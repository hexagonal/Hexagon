...
Nothing qualifies the example *(#868)*: `compose(save2, audit2)` is bare in every body, inlet-bearing ones included, because constructing a closure is not invoking it and a colour nothing in `compose`'s body constrains is pure (§3.4's defaulting). The same holds of every closure builder — `defer(action)`, `Stream.map(randoms, double)` (`stream.md` §4.2) — and of the function that returns one:
...
The predecessor rule read an undetermined call inside an inlet-bearing body as a conduit, so that `defer`'s callers inside `fold`-shaped bodies had to write `defer?(action)` and `defer` itself displayed a second variable the grammar cannot spell (`(() ->?¹ Unit) ->?² () ->?¹ Unit`). It is withdrawn (§11): `?` names a preserved dependency and nothing else.
...
- **Settled at body close.** A declaration's own colour and its calls' mark obligations are resolved when its body closes, not at module end — a `fun` knot's members excepted, whose sibling-call obligations wait for the knot's close (the knot bullet below, #868). This is required, not latitude: under end-of-module settling, a module-internal call to a not-yet-generalized neighbour would see a still-linked variable, and the pure corpus would read as conduits — every bare call in `Seq.hex` would demand `?`.
- **A body's own colour is solved by three arms, in order.** A body that absorbs an impure-constant call is a **source**: its own colour is the impure constant (and if its written face is `->`, that is §4.2's pure-face error, at the offending call). A body whose `?` call absorbs the signature's variable is a **conduit**: its own colour *unifies with* that variable — this is how one-variable-per-signature emerges rather than being imposed, and why `fold`'s outer arrow is linked (§2.2). A body that is neither has an **unconstrained** colour, and it **defaults to pure before generalization, whatever the signature's inlets** *(#868)* — after the body's dependencies are resolved (its callees settled, its `?` calls joined) and before the binding's scheme is built: at body close for a lone binding, at the knot's close for a `fun` block member. Nothing observed the colour, so pure is what it is: constructing a closure that will run a callback is not running it. `store : (() ->? String) -> Int`; `defer : (() ->? Unit) -> (() ->? Unit)`, the returned closure's arrow linked to the callback's through the body's `?` call and the outer arrow pure. What the defaulting never touches is a **dependency**: a signature parameter's variable (next bullet), and a body's own colour once a `?` call has joined it — those are the linked faces the language exists to keep. *(The predecessor arm withheld the default under an inlet, "rather than pure-pinning by omission"; the omission it feared was a caller-supplied colour, which the parameter's own variable already carries. Withdrawn — §11.)*
- **The defaulting clause, at calls.** A call's colour still undetermined when marks are checked defaults to **pure** (bare), in every body *(#868)*. A callee generalized at an unconstrained colour instantiates a fresh variable that nothing binds, and pinning it pure weakens no face, because no face depended on it — the enclosing body's own colour is defaulted by the arm above on the same ground. The one undetermined colour a call can meet that *will* be determined later is a knot sibling's, and its obligation waits (next bullet). A **signature parameter's** effect variable is never defaulted by either clause: it generalizes with the binding like any type variable, which is exactly what keeps `store(callback: () ->? String)` polymorphic (§2.2).
- **Knot obligations settle at the knot's close** *(#868)*. Within a `fun` block's strongly-connected component a sibling's colour is a not-yet-generalized monotype (the first bullet), so a call to a sibling may meet a colour that is undetermined now and solved by a body not yet checked. Its mark obligation is **recorded, and settled when the knot closes** — after every member's body has been solved and the unconstrained colours defaulted pure — never at the caller's body close, and never by conducting because the colour was unsolved at the time. `a(cb) = b(cb)` beside `b(cb) = 1` is two bare calls and two pure faces, `(() ->? Unit) -> Int` each; `even`/`odd` passing a callback down and calling it at the base are conduits, `?` on the sibling calls as on the callback, because the knot's colour joined the signature's. Generalization is still per member at its SCC (Functions §7.4, §8); only the *timing* of the sibling obligations moves, and only to where the knot's colours exist to read.
...
| Effect variables are tyvars: monomorphic in the SCC knot, settled at body close; source/conduit/unconstrained arms; defaulting is pure only where no inlet exists — inlet-bearing bodies stay polymorphic and their undetermined calls conduct *(the inlet clause and the conduct clause withdrawn by #868, next row)* | §3.4 |
| **An unconstrained own colour defaults to pure before generalization, whatever the signature's inlets** — after the body's dependencies are resolved, at body close for a lone binding and at the knot's close for a `fun` member; parameter variables and joined dependencies are never defaulted; an undetermined call is bare in every body; knot sibling obligations settle at the knot's close; `?` has one meaning (#868) | §3.3, §3.4, §4.1, §11 |
