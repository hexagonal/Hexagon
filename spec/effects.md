...
`->?` is legal **only within a function signature, and only where a variable can be owned — by that signature's own inlet, or by an enclosing signature that lends it one (§2.2.2's lexical ownership rule).** Anywhere else it is an error (§4.4); it is never given a second reading.
...
A signature with no inlet has a variable no caller can pin — where no enclosing signature can lend it one (§2.2.2's lexical ownership rule; a local header or annotation under an inlet-bearing signature borrows instead of quantifying): **the caller pins exactly what it supplies, and an occurrence it only receives pins nothing.** Two face shapes take the error on that ground, and two positions take it for having no signature at all:
...
The impure-direction report stands **at the signature's written `->?`**, not presumptively at the outer arrow: the constantified variable may be spelled only on a nested arrow — `(h: ((Int) ->? Int) -> Int)` over a body that pins `h`'s callback impure — while the outer arrow is honestly `->` or `->!`. The advice never tells the writer to change an arrow that is not there. The two directions place their primary differently for a reason *(#873)*: an impure-direction body performs its effect in a call of its own, and the one signature that lies is that body's, so the arrow is the report's seat; a pure-direction pin is an act of unification that can contradict several signatures' arrows at once, so the pin is the seat and the arrows are its labels. In both directions the arrows named are exactly the ones that spell the variable.
...
- **Distinguish, and only where the grammar cannot spell it.** A face carrying **exactly one** distinct effect variable displays it undecorated, `->?`, wherever that variable stands. A face carrying **more than one** — inference produces these; the written grammar, which links every `->?` in a signature into one colour, cannot — numbers them in order of first appearance, `->?¹`, `->?²`. Constants are never numbered: `->` and `->!` mean what they say in every position. The numbering's unit is the one displayed type expression, nested function types included. The decoration is display-only, not grammar: pasted into source it fails at the lexer.
  ...
  A local face carrying an enclosing signature's variable (§2.2.1's local-face paragraph) displays it undecorated as any single variable *(#873)*. The display is right about the colour, but the undecorated spelling is not a guaranteed paste: a written inlet-less `->?` names the **nearest** enclosing signature that can own a variable (§2.2.2), which is the displayed variable's owner only where no inlet-bearing signature stands between the binding and it. Pasted under a nearer one it joins two variables inference kept apart — a narrowing with no report of its own, and §4.2's pure-direction report is what surfaces it once something pins. Numbering applies exactly as elsewhere when such a face also carries a variable of its own — `(() ->?¹ Int) ->?² Int` for a nested helper polymorphic in its own callback and conducting a captured one (§3.4).
