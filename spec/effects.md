...
- **What the defaulting never touches is a dependency**, and four things are dependencies: a signature parameter's variable (next bullet); a body's own colour once a `?` call has joined it; a body's own colour once a **written `->?`** has joined it — a `->?` on the outer arrow of a binding annotation that is itself a function type — its own signature where it carries an inlet, the enclosing signature's fragment where it does not (§2.2.2) — links the colour to that signature's variable exactly as a `?` call does, so `let f: (() ->? Unit) ->? Int = (action) => 1` keeps the face it writes rather than defaulting under it (a written `->` or `->!` joins nothing: it is checked against the solved colour, §4.2); and a knot member's own colour once a sibling call has joined it (the knot bullet). The linked faces are what the language exists to keep; the defaulting reaches only what nothing has claimed.
