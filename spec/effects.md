...
- **Knot colours and obligations settle at the knot's close** *(#868)*. Within a `fun` block's strongly-connected component a sibling's colour is a not-yet-generalized monotype (the first bullet), so a call to a sibling may meet a colour that is undetermined now and solved by a body not yet checked. The colour is left as it is, and the call's mark obligation is **recorded and settled when the knot closes** — after every member's body has been solved and the unconstrained colours defaulted pure — never at the caller's body close, and never by conducting or by pinning because the colour was unsolved at the time. `a(cb: () ->? Unit) = b(cb)` beside `b(cb: () ->? Unit): Int = if True then 1 else a(cb)` — one component, each member calling the other — is two bare calls and two pure faces, `(() ->? Unit) -> Int` each; `even`/`odd` passing a callback down and calling it at the base are conduits, `?` on the sibling calls as on the callback, because the knot's colour joined the signature's; and a sibling that turns out to be a source makes every call to it `!`, however early that call's body closed. Generalization is still per member at its SCC (Functions §7.3, §7.4, §8); only the *timing* of the sibling colours and obligations moves, and only to where the knot's colours exist to read.
...
- `->!` over a body that performs no unconditional effect: *"this face is the impure constant `->!`, but the body performs no unconditional effect — it is effect-polymorphic, and its face is `->?`"* — fixit `->?` — when the body conducts; *"…but the body performs no effect — its face is `->`"* — fixit `->` — when the body's colour is pure *(#868: a pure body is pure, not polymorphic; the advice names the face the body has)*.
...
| **An unconstrained own colour defaults to pure before generalization, whatever the signature's inlets** — after the body's dependencies are resolved, at body close for a lone binding and at the knot's close for a `fun` member; a parameter's variable and a colour joined by a `?` call or a written arrow are dependencies and are never defaulted; an undetermined call is bare in every body, a knot sibling's colour never pinned; knot colours and sibling obligations settle at the knot's close; `?` has one meaning (#868) | §3.3, §3.4, §4.1, §4.2, §11 |
