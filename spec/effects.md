...
- **What the defaulting never touches is a dependency**, and four things are dependencies: a signature parameter's variable (next bullet); a body's own colour once a `?` call has joined it; a body's own colour once a **written `->?`** has joined it — a `->?` on the outer arrow of a binding annotation that is itself a function type (§2.2.2's second boundary bullet) links the colour to the signature's variable exactly as a `?` call does, so `let f: (() ->? Unit) ->? Int = (action) => 1` keeps the face it writes rather than defaulting under it (a written `->` or `->!` joins nothing: it is checked against the solved colour, §4.2); and a knot member's own colour once a sibling call has joined it (the knot bullet). The linked faces are what the language exists to keep; the defaulting reaches only what nothing has claimed.
...
- **Knot colours and obligations settle at the knot's close** *(#868)*. Within a `fun` block's strongly-connected component a sibling's colour is a not-yet-generalized monotype (the first bullet), so a call to a sibling may meet a colour that is undetermined now and solved by a body not yet checked. The colour is left as it is, and the call's mark obligation is **recorded and settled when the knot closes**, never at the caller's body close, and never by conducting or by pinning because the colour was unsolved at the time. The knot's close runs the three arms to a **fixpoint over the whole component** before anything defaults: a member whose sibling call absorbs a source is a source in turn, and so on until no member changes; a member whose sibling call absorbs a colour the knot has joined to a signature variable is a conduit of it; only the colours that remain unconstrained after that default pure, and only then are the recorded obligations read. `a(cb: () ->? Unit) = b(cb)` beside `b(cb: () ->? Unit): Int = if True then 1 else a(cb)` — one component, each member calling the other — is two bare calls and two pure faces, `(() ->? Unit) -> Int` each; `even`/`odd` passing a callback down and calling it at the base are conduits, `?` on the sibling calls as on the callback, because the knot's colour joined the signature's — and so is a member that never calls the callback itself but calls the sibling that does; a sibling that turns out to be a source makes every member that calls it a source in turn, its calls `!` and its own face `->!`, however early that member's body closed. Generalization is still per member at its SCC (Functions §7.3, §7.4, §8); only the *timing* of the sibling colours and obligations moves, and only to where the knot's colours exist to read.
...
| **An unconstrained own colour defaults to pure before generalization, whatever the signature's inlets** — after the body's dependencies are resolved, at body close for a lone binding and at the knot's close for a `fun` member; a parameter's variable and a colour joined by a `?` call, a written `->?`, or a sibling call are dependencies and are never defaulted; an undetermined call is bare in every body, a knot sibling's colour never pinned; the knot's close runs the arms to a fixpoint over the component, then defaults, then reads the recorded obligations; `?` has one meaning (#868) | §3.3, §3.4, §4.1, §4.2, §11 |
