...
Inside a body, a `->?` written in a **local type position** — a binding annotation, an ascription, or a local declaration's header — with no inlet of its own denotes **the nearest enclosing signature's variable**: `let h: () ->? String = g` and `let h = (g : () ->? String)` are the same claim, and both link. This is the **lexical ownership rule**, and it is one rule for every written `->?`: a function type that carries an inlet of its own owns its variable; an inlet-less `->?` borrows the nearest enclosing signature that can own one; where there is none, it is §2.2.1's error *(#873)*. The legality condition is the enclosing signature's own (§2.2.1) — a local `->?` is legal exactly where the enclosing signature admits one, and it never *supplies* the inlet it needs: inlets are the signature's, not the body's.

Two boundaries keep this exact:

- **A local function type that carries its own inlet is its own signature.** `let f: (Tx ->? a) ->! a = …` quantifies its own colour, exactly as before; only an inlet-less local `->?` reaches out to the enclosing signature. The variable a fragment names is the nearest enclosing signature that can own one. A **local declaration's header** — a `let h(…)` or `fun h(…)` nested in a body — is the same fragment under the same rule *(#873)*: with an inlet among its parameters it is its own signature, `fun h(cb: () ->? Int): Int` quantifying `cb`'s colour as a module-level header would; without one, a `->?` in its return annotation borrows — the `->?` in `fun h(): () ->? Unit = () => action?()` inside `outer(action: () ->? Unit)` is `outer`'s variable, exactly as it is in `let h: () ->? Unit = () => action?()`. A header is refused for want of an inlet only where no enclosing signature can lend one: at module level, and in a body that no inlet-bearing signature encloses.
...
**The recovery binds nothing it meets** *(#873)*. Unified with a variable, the recovered constant absorbs as the error type does and leaves the variable as it was — an enclosing signature's colour, a captured callback's, a knot sibling's — so no face, colour, or mark outside the refused position changes on the recovery's account, and only what traces to the recovered colour itself is suppressed. Under the previous reading the constant flowed: a refused `->?` in a nested header met `outer`'s callback variable, bound it to the constant, and rewrote `outer`'s own face to `(() ->! Unit) ->? …` with no report at `outer` — the silent poisoning this sentence forbids. Scaffolding holds up the refused position and nothing beyond it.
...
| `->?` face, body pins the variable pure | "…solves it to the pure constant — the honest face is `->`" — primary at the pin, a label at each written `->?` the variable spells, fixit `->` at each; one report however many signatures share the variable (§4.2, #873) |
...
| `->?` in a signature with no inlet (an outer-only or received-only face — §2.2.1), where no enclosing signature can lend the variable — a module-level face, or a local header or annotation in a body no inlet-bearing signature encloses; under one, an inlet-less local header or annotation borrows instead (§2.2.2, #873) | same frame, "nothing a caller of this signature supplies carries `->?`, so nothing instantiates it" + fixit `->!` (§4.4) |
...
- **Distinguish, and only where the grammar cannot spell it.** A face carrying **exactly one** distinct effect variable displays it undecorated, `->?`, wherever that variable stands. A face carrying **more than one** — inference produces these; the written grammar, which links every `->?` in a signature into one colour, cannot — numbers them in order of first appearance, `->?¹`, `->?²`. Constants are never numbered: `->` and `->!` mean what they say in every position. The numbering's unit is the one displayed type expression, nested function types included. The decoration is display-only, not grammar: pasted into source it fails at the lexer.
  ...
  A local face carrying an enclosing signature's variable (§2.2.1's local-face paragraph) displays it undecorated as any single variable *(#873)*: pasted as a local annotation it is legal and means the same thing (§2.2.2). Numbering applies exactly as elsewhere when such a face also carries a variable of its own — `(() ->?¹ Int) ->?² Int` for a nested helper polymorphic in its own callback and conducting a captured one (§3.4).
