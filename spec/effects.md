...
A `->` on a boundary row — user-written or intrinsic — whose foreign code does touch the world is a lie, except for three shapes where the claim is *sound*, not merely customary. The ruling names them so every future claim can be tested against one of them:
