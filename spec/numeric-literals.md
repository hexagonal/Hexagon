...
- **The home.** Where the seat's expected type is **concrete** — it contains no type
  ...
  reports, with one order removed *(#1107)*: under a concrete expected type, each value path
  of its forms meets that type at its own turn (Functions §4.3's forwarding forms), and so
  does each value path of a faced tree holding a value outside the tower — a numeric value
  entering the face as its home, every other value meeting it by unification — so no path
  is measured against another, and each that cannot meet the type is refused at itself;
  without one, the paths unify with one another in source order, as ever.
...
the tower owes (#1107): `let r: Dec = if c then "x" else f` and its mirror refused at `"x"`
with the ordinary mismatch and at `f` with §6's entry report, `let r: Dec = if c then "x"
