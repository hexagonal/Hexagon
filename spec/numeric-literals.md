...
- **The tree.** An expression's **tower member calls** (below; in every spelling, a dot
  ...
  `Ord` members sharing their subject) — with the values on any argument's spine read
  against a seat the callee's parameter types write as that variable, a literal's
  components and a constructor's arguments among them (Functions §4.3's argument spine,
  #1096), so `both(price, [n])` at `both<a>(x: a, y: Vector(a))` is one tree — and the
  elements of one vector literal. Any
...
- **The home.** Where the seat's expected type is **concrete** — it contains no type
  ...
  (§4). A tree none of whose values is numeric keeps ordinary unification and ordinary
  reports, with one order removed *(#1107)*: under an expected type that leaves its paths
  nothing to fill in — a declared variable counting as written — each value path of its
  forms meets that type at its own turn (Functions §4.3's forwarding forms), and so
  does each value path of a faced tree holding a value outside the tower — a numeric value
  entering a numeric face as its home, every other value meeting it by unification — so no path
  is measured against another, and each that cannot meet the type is refused where it
  stands, several in one report (§6);
  otherwise, the paths unify with one another in source order, as ever.
...
- **The tree's reach, and no further.** Among a call's arguments at one type
  variable — inside a literal or constructor application on an argument's spine too
  (Functions §4.3, #1096) — the literal is a sibling of the others, so `Num.add(0.5,
  price)`, `price < n * 1.5`, `h(n * 1.5, price)` at `h<t: Num>(x: t, y: t): t`, and
  `bothR([0.5], price)` at `bothR<a>(y: Vector(a), x: a)` meet at `Dec`, and the
  siblings' home is chosen before any lambda literal on an argument's spine is
...
§6's settled-callback report, in either argument order (ruling A2), and so, nested on an
argument's spine, `g6(m, Some((v) => v + n))` at `g6<t>(x: t, p: Option((t) -> t))` and
`apply2(m, if c then (v) => v + n else ident)` (#1096); the siblings on a spine, `both(price,
[n])` at `both<a>(x: a, y: Vector(a))` and `bothR([0.5], price)`, at `Dec`; the literal
...
refused, and a decimal literal pattern at `Dec` refused (#1054). The order removed outside
the tower owes (#1107): `let r: Dec = if c then "x" else f` and its mirror refused once, at
the first path in its own words — the ordinary mismatch at `"x"`, §6's entry report at `f` —
the other a label on it, and `if c then f else if c then g else "x"` once, at `f`, labelled at
`g` and `"x"`, `let r: Dec = if c then "x"
else n` refused at `"x"` alone, `n` entering the face, and `let r: Dec = if c then f else g`
(`g : Float`) refused once, at `f`, as before; Functions §4.3 owes the non-numeric forms'.
...
- Both rules above are about a literal's `α` at the literal. A binding whose body holds a literal gives its users a demand of their own, standing at each use (Functions §10, "Where a report stands"): `let g(x) = x + 1` then `g(True)` is reported at `g` as `Bool`'s missing `Num` instance, and names no literal the caller did not write. Blocked defaulting there stands where an annotation can pin the stuck type. Where the use's value carries it — a call's result, or the binding itself where it is not called — the report names the binding, since the caret is on it and its own type may be a function: `let k(u: Unit) = 1 / 2` then `k(())` gives "the type this use of `k` gives cannot default to `Int`: `Frac` is not a defaultable constraint; add a type annotation to pin it". Where a call's result does not carry it, an annotation there pins nothing, so the report stands at the first value the call supplies that does, in source order (a dot call's receiver first; a literal argument is one value, as Functions §4.3 has it), as "this expression's type …": `let label<a: Tag>(x: Option(a)): String` then `label(None)` is reported at `None`, whose repair is `label((None : Option(String)))`. The repair sits on a value the call supplies, as the function-result report's below sits on an argument.
...
- *(#1062.)* **When a value cannot enter its expression's home** (§5.1), the report names the value, its type, the home, and what gave the home — the seat's type ("the home `: Dec` writes") or the value that established it ("the home `price` gives this expression") — at the declining value. **When two values establish different homes** (a conflicting tree), the report names both — "`x` is a `Float` and `price` a `Dec`; an expression's arithmetic runs at one type, and neither enters the other". Each names the named door where either type has one into the other (`Dec.fromFloat(x, places)`, `price.toFloat()`; friendly-numerics tenet 7), and otherwise no conversion, there being none to name. Precedence: at a dot call's receiver, Method Syntax §9 row 16 and Operators §11's form report; under a written face, the #808 stand-down wording above wherever a tower member call stood down; otherwise these — among them a value that meets a written face with no operation between it and the seat (`let x: Dec = if c then f else price`: "`f` is a `Float` and cannot enter `Dec`, the home `: Dec` writes"), which is reported once, at the value — a gated call's result being such a value (`let x: Nat = if c then n - k else m` names `n - k`). **Outside the tower** *(#1107)*, a form's value path that cannot meet an expected type leaving it nothing to fill in (§5.1) — or a value of a faced tree that holds one outside the tower — is refused where it stands, in the ordinary words; a numeric value in this entry report, a path whose operation stood down in the #808 report above. **One expression, one report**: where several of its paths are refused, the first checked carries its report, in its own words, and each other is a label on it, at the path, saying what the path is where its type was known before the check — "`g` is a `Float`", "`y` is an `a`" — and repeating the path's own report otherwise ("integer literal cannot have type `String`"), so every place is shown and the explanation given once (`let r: Dec = if c then f else if c then g else "x"` is one report at `f`, labelled at `g` and at `"x"`; `if c then o1 else o2` under `Option(Dec)` one report at `o1`, labelled at `o2`; a vector literal's elements alike). A path's refusal is what its check reports at the path itself: a demand made elsewhere, validated as the check solves a variable, keeps its own report where it was made. A path refused within itself, as a conflicting operation is, is one of the expression's refused paths, its report the one the others fold into when it is first, and says nothing more at the form or the seat. A dot call's receiver is included where neither of its own reports above is owed; and a declared variable is refused once, at the first path it declines on, being repaired at its binder (Functions §10). A numeric tree's declining values keep their one report, unlabelled (`if c then f else g`, `g : Float`, is refused at `f`). **When a callback's body finds its call's home settled** (Functions §4.3) at a type that would have widened into the body's — "`m` settled this call's `Nat` before the callback was checked, and the callback's body returns `Int` — a callback's body chooses no type for the values beside it; write `(m: Int)`, or annotate the callback: `(v: Int) => …`" — the report is given once, at the body, naming the value that established the home, and only where the siblings' own values chose that home as their group closed — where a value checked at its turn had already solved the variable, the ordinary mismatch stands (`twice2(m, natF, (v) => v + n)` at `twice2<a>(x: a, f: (a) -> a, g: (a) -> a)`, and `apply2(m, if c then (v) => v + n else natF)`, `natF : (Nat) -> Nat`); the annotation is offered only for a parameter written as the settled variable itself, and neither repair where this site cannot spell the body's type. A callback on an argument's spine (Functions §4.3, #1096) is reported as any callback is, naming the call and the value that settled its home, a literal's component among them: `g(((v) => v * price, n))` at `g<t>(p: ((t) -> t, t))` draws "`n` settled this call's `Int` before the callback was checked, and the callback's body returns `Dec` — …; write `(n: Dec)`, or annotate the callback: `(v: Dec) => …`". At the components of a literal on no spine (Functions §4.3's component schedule, #1066) the report names the literal where it names the call — "`n` settled this tuple's `Int` before the callback was checked, and the callback's body returns `Dec` — a callback's body chooses no type for the values beside it; write `(n: Dec)`, or annotate the callback: `(v: Dec) => …`" — and, at a record literal, says "record". One sentence of reason serves every seat — "the values beside it" — whether what the callback stands beside is an argument, a component of its own argument, or a component of a literal. **When a dot call's receiver closed before the dot** (Method Syntax §2.2) and a sibling then refuses it, where every value of the receiver's own tree would have entered that sibling's type and that type honors the member's constraint, the report says so and names the repairs that compile: "`n * 1.5` settled at `Float` before `.multiply` saw `price` — a dot call's receiver is settled on its own; write `n * 1.5 * price`, or name the home: `let a: Dec = …`". The operator rewrite is offered at the four arithmetic members; the home is named by the binding's annotation where the call is an unannotated binding's whole value, by ascribing the receiver anywhere else (`(n * 1.5: Dec).compare(price)`), never where the binding already writes one, and not at all where this site cannot spell the home. Each is decided from recorded types, for the report alone.
...
**A literal as one argument** (#1096). Elaborate a literal argument whole at its turn: its callbacks checked then, and its components closing as trees of their own before the call's sibling groups. Rejected: a callback's body would choose its call's home whenever it is wrapped — `g6(m, Some((v) => v + n))` compiling beside `apply2(m, (v) => v + n)` refused — and a literal's components would meet the call's siblings in source order, so `both3([n], prices)` would be refused beside `both3(prices, [n])`. Functions §4.3's argument spine reads the literal into its call's argument list.

**Components grouped by the solved expectation** (#1096). Group a literal's components by the variables their expectation holds when the literal's turn comes. Rejected: it reads the schedule's solutions to choose whether parts join (friendly-numerics §4) — `k(ident, (n, price))` at `k<a, b>(f: (a) -> b, p: (a, b))` would group `n` with `price` after `ident` had made `a` and `b` one, where `k2(ident, n, price)` refuses. The spine is read against the callee's parameter types before any argument is checked.

**A forwarding form checked whole** (#1096). Check an `if`, `match`, `try`, or block holding a callback entire, either at its turn in the first pass or, waiting, in the second. Rejected: the first lets a callback's body choose its call's home from inside a branch (`apply2(m, if c then (v) => v + n else ident)`); the second hides a visible value beside a callback, refusing `apply2(m, if c then (v) => v + n else incInt)` though `apply2(m, incInt)` compiles. On the spine, a callback on a value path waits and the rest of the form is checked at its turn.
