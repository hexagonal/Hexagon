...
multiplication ran at `Float`" (§6) — the operation's kept type, below. For the report alone, a stood-down call's
...
Dec = () => n` and `t := n * 1.5` at `var t: Dec`; the seats a face does not reach, `let a: Dec = id(n * 1.5)` refused; the
...
- *(#808.)* **When the expected-type lift stands down** (§5.1) because an operand cannot reach the face, the refusal that then fires at the consuming seat names that operand and the algebra the operation ran at instead, its kept type — "`price` is a `Float` and cannot enter `Rat`, so the multiplication ran at `Float`" — so the report keeps what a refusal at the operand would have carried; at a dot call's receiver, Method Syntax §9 row 16 says instead that the operation could not run at the face. The checker therefore records, at a stand-down, which operand declined and why, and carries it to the seat that refuses — a binding, an argument seat, or a dot call's receiver seat *(#821)*, where the report adds the two facts that seat alone knows: why the face reached the receiver (the spelling's rung) and the written boundary that keeps the receiver at its own type, an ascription or a separate binding (Method Syntax §9 row 16). Every stand-down ends at such a seat (§5.1), so the note is always spent.
