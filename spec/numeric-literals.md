...
- Both rules above are about a literal's `α` at the literal. A binding whose body holds a literal gives its users a demand of their own, standing at each use (Functions §10, "Where a report stands"): `let g(x) = x + 1` then `g(True)` is reported at `g` as `Bool`'s missing `Num` instance, and names no literal the caller did not write. Blocked defaulting there names the binding, since the caret is on it and its own type may be a function: `let k(u: Unit) = 1 / 2` then `k(())` gives "the type this use of `k` gives cannot default to `Int`: `Frac` is not a defaultable constraint; add a type annotation to pin it".
