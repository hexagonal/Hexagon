...
**Not in scope:** `Seq(a)`'s intrinsic semantics (Loops §6, consumed); single-pass `Iterator<a>` acceptance, async iteration, callback-position adapters, and any separate resource-managed stream type (**deferred**, §10); `Array.toSeq` over the captured array (Part 2 §6.2/§9); callback signatures generally (Part 6).
...
1. **The representation rule is inherited as an observable contract, not as an attachment mechanism.** An exported collection value satisfies its declared face's traversal protocol *as part of its representation*, never via a boundary artifact. This is stated over observations precisely so each representation pays its own way: `Map` and `Set` HAMT values already carry `[Symbol.iterator]` (paid); `Vector`'s trie values carry `[Symbol.iterator]` (paid — the persistent-vector core landed keeping the protocol on the value, exactly as this clause required of it; the emitter splices the iterator at construction, `Hex.Runtime.VectorTrie`'s wiring). Iteration yields what each collection's own spec gives its traversal: elements in index order for `Vector`, entries as `[k, v]` pairs for `Map`, members for `Set`, in the traversal order Collections Parts 3–4 define (unspecified-but-per-value-stable where those specs say so).
...
| *(#488)* The brand carries the declaring module: the refusal brands `$hex: "Hex.Seq"` (the full name since #829), and exception identity is the (module, name) pair — supersedes the `$hex: true` spelling in the row above; that row stays as the historical record | §7.4 |
...
| *(#622)* The residue above is discharged: `Seq.d.ts` declares no brand — the declaration seat is Part 7 §2.3's transparent alias of the pinned face (`export type Seq<a> = Iterable<a>;`), so the exported name and the §9.1 face denote one type; the face itself unchanged (Part 7 §14.5; that row stays as the historical record) | §9.1, §9.5 |
