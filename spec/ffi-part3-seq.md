...
**The ruling: the spine refuses it.** A pull that would begin forcing while a forcing of that spine is in flight raises a JavaScript `TypeError`, observed in Hexagon through `JsError`. The refusal itself touches nothing — not the source, not that spine's memo, not the iterator.
...
- **A refusal is an ordinary failure to every spine it reaches, and that ends the value's foreign traversability.** This is the clause to read twice. JavaScript's only route back into a `Seq` is the §9.4 face, so a reentrant traversal almost always arrives through a **second** spine — the boundary view — and the refusal surfaces there as a throw out of *its* source, which §7.1 memoizes at that position permanently. The Hexagon-side traversal completes correctly; the value is finished as an `Iterable` to JavaScript, and says so on every later attempt.

  **That is ruled intended, not tolerated.** The apparent repair — decline to memoize a refusal a spine did not itself raise — is worse, and measurably so. The refusal travels out through `seqToIterable`'s generator, and a generator that has thrown is completed; the next forcing of that position calls `next()` on it, reads `done`, and memoizes **end**. The `Seq` then reads as *empty* to JavaScript rather than raising. Trading a persistent error for silent truncation is the exact exchange this ruling exists to refuse, so the memoization stands.
...
**Where the check lawfully lives** (Part 1 §3.2). §3.2 permits validation *only* where a named operation establishes an invariant or where protocol participation inherently requires a check, and its Part 3 entry is the minimum native iteration performs. A reentrancy guard is not one of those: native iteration performs no such check, and this one inspects no foreign value. It is lawful for a different reason, stated here so §3.2's "only" is not stepped past silently — **§3.2 governs validation of foreign values, and this validates the adapter's own state.** The adapter is Hexagon's; the invariant it protects is §4's one-outcome-per-position, which the adapter exists to establish. No inbound value is examined, narrowed, or distrusted.

**One case is made uniform, and the error kind is chosen separately.** Every spine whose source is a `seqToIterable` generator — every `Seq.memoize` (Loops §6.4) and every §9.4 boundary view — already had the reentrant advance refused by JavaScript itself, with `TypeError: Generator is already running`. Only a foreign iterator whose `next()` is an ordinary function reached the incoherent path, and that is the case the ruling conforms. But the check now precedes the platform's, so it **replaces** that error rather than deferring to it, and platform uniformity is not what selects the kind. `TypeError` is chosen for §7.2's reason alone: **no separate Hexagon error type is introduced.** A branded Hexagon exception was available — the emitter raises `IndexError` and `SliceError` that way — and is declined at a stated cost, since a `JsError` payload is interrogable only by message (Exceptions §6.1 leaves classification of foreign errors to userland). The message is the emitter's, as §2.2 leaves the door's exact test to the emitter.
...
| *(2026-08-02, issue #123 ruling)* Forcing is not reentrant: a pull that would begin forcing while a forcing of that spine is in flight raises a JS `TypeError` via `JsError`. Replay of an already-memoized position is untouched (the refusal is scoped to *beginning* a forcing); a refusal reaching a second spine is memoized there and ends that value's foreign traversability, ruled intended because declining to memoize it yields silent truncation instead; lossless-and-order-preserving is impossible, not merely unchosen; the check validates the adapter's own state, not a foreign value, which is why Part 1 §3.2's "only" does not reach it; `TypeError` is §7.2's no-new-error-type rule, with a branded exception declined at a stated cost; re-derivation is out of scope and fails fast by stack overflow; not a Part 1 §3.1 contract violation. Narrows §9.4 property 2. Inherited by `Vector`/`Set`/`Map` as vacuous — §9.5 item 4 gives them no spine | §7.3 |
