...
Anything with an `Iterable` instance (§7). The definitive v1 instance table — element types, `toSeq` strategies, and the capture notes — is owned by **Collections Part 5 §4**. By ownership, the provided families are:
...
- **FFI-owned captured foreign collections:** `Array(a)` (#876), `JsMap(k, v)`, `JsSet(a)` (#875) — FFI Part 1 §2.2; rows recorded in Collections Part 5 §4; semantics in their owning FFI parts.
...
- **Re-derivation is the internal default.** A pure `Seq` is persistent because re-driving it recomputes it; there is no hidden buffer under `map`/`filter`/`iterate` pipelines. *(Amended for #355 — this bullet's consequence clause read "re-driving a `Seq` replays any effects its steps perform", with explicit memoization as the caller's cue. The effects ruling closes that door: **`Seq` is pure by construction** — `pull` and every combinator callback demand `->`, so an effectful step is a type error, not a hazard to manage (Effects §7, §4.3). Re-driving recomputes, and recomputation is unobservable. The effectful sequence is the nominal sibling `Stream` (`stream.md`); the one place effect-flavoured foreignness still stands behind a `Seq` is the boundary launder, sound as a species (b) at-most-once read — FFI Part 3 §4, §14.)*
- **`memoize : Seq(a) -> Seq(a)` is the explicit opt-in** — it wraps any `Seq` in the runtime's memoizing spine (the same mechanism as FFI Part 3's inbound adapter), making replay O(1) per cached element at the cost of retention. Ship-list entry owed to `stdlib-roadmap.md`; the spine is runtime-provided, not `.hex` (a mutable buffer cannot be written under Statements §6.2). *(#125 ruling, 2026-07-28: the spine stays runtime-provided, but its declaration is now canonical `.hex` — `Seq.hex` names it through the intrinsic door, `spec/intrinsics.md` §3.2.)* *(Implemented 2026-07-28. The lowering builds no new spine: it is FFI Part 3's inbound adapter driven over the value's own traversal, which is what "the same mechanism" above always meant. Every property in this bullet — laziness preserved, each position derived at most once across all traversals, the argument left unconsumed, and §7.1 failure memoization inherited — is pinned behaviourally in `compiler/src/conformance/seq-memoize.test.ts`.)*
- **The export boundary memoizes unconditionally.** FFI Part 3 §9.1 already fixes this: repeated JavaScript traversals of an exported `Seq` observe the same memoized sequence and never re-run its computation or effects. The internal default therefore never leaks re-derivation across the boundary; nothing in Part 3 changes. *(2026-07-28, defect 12 ruling: the mechanism is now specified — a lazily created per-value memoized boundary view behind the value's own `[Symbol.iterator]`, FFI Part 3 §9.4. Hexagon-internal traversal never uses that channel, so this bullet and the re-derivation default coexist by construction.)*

### 6.5 Emission

`Seq(a)` is emitted onto **JS's native iterable/iterator protocol** — `for (const x of s)` is directly valid on the JS side, `Seq` pipelines read like the generator-library code a JS developer already knows, and the `.d.ts` face is `Iterable<a>`. *(Decided 2026-07-26:)* the representation is **the wrapper preserving persistence of `next`** — the §6.6 record — with the boundary bridges (FFI Part 3) supplying the iterable face; the previously recorded alternative (plain iterable-of-`[Symbol.iterator]`) is closed. *(Amended 2026-07-28, defect 12 ruling: the iterable face now rides the record itself — every `Seq` value carries the boundary traversal method as part of its emitted representation, FFI Part 3 §9.4. This does not reopen the closed alternative: the record remains the representation, `next`/`pull` remain the protocol, and Hexagon-internal traversal never drives the face; the amendment relocates only where the face lives, from a bridge at the crossing to a member of the value.)* §6.2's semantics were always the constraint: `Seq.next(s)` must not consume `s` from the caller's perspective; persistence is by re-derivation with explicit memoization (§6.4). This is the one place the pure protocol costs something; monomorphic loops avoid the machinery entirely (§8). `for..in`, `while`-based cursor consumption, and `Seq`-consuming combinators must use constant-stack iteration in emitted JS — Hexagon does not promise tail-call optimization (FFI Part 3 §6).
...
- **Opacity is load-bearing:** `pull` is private to `stdlib/Seq.hex`. Outside it, the §6.2 protocol — `next` plus `Option` destructuring — is the *only* access. Opacity hides structure, not capabilities (Modules §4.2): the `Iterable` instance and `for x in` work unchanged, and the covariance claim `+a` crosses the boundary because it is written (Modules §4.2.1; `decisions-ml-dialect-generalization-2026-08.md` §11.4 wrote it into `Seq.hex`, retiring the transitional claim-table row) *(#932)*.
