...
| A `dependencies` entry of the project's manifest, or of a package's in the package set, is uppercase-start and otherwise not a package name (`"Acme.Tools"`) | §2.1's shape refusal, "a package name is one uppercase-start identifier: write `\"Acme\"`" — a package name written wrong, never npm's (§4.4) |
...
| The project's manifest, or a package's in the package set, lists `"Hex"` under `dependencies` | "`Hex` is every package's dependency; remove the entry" (§2.4) |
...
-- {"dependencies": ["Acme.Tools"]}          -- ERROR: a package name is one uppercase-start identifier:
                                             --   write "Acme" (a name written wrong, not npm's — §4.4)
