...
| A `dependencies` entry of the project's manifest, or of a package's in the package set, is uppercase-start and otherwise not a package name (`"Acme.Tools"`) | §2.1's shape refusal, "a package name is one uppercase-start identifier: write `\"Acme\"`" — a dotted spelling names the module-name form as the place for dots; a package name written wrong, never npm's (§4.4) |
...
| The project is the root package, unnamed unless published, and never qualifies itself; a host may compile a directory holding no manifest as the project, the one package without one | §2.5 |
...
| One copy per package name per program, the project counted, read over the package set alone: two entries reaching two directories, or, on a closure entry's walk, two roots at one level declaring the name sought at two directories, refused; an entry reaching the project's own directory adds an edge and no package, one reaching a package of the project's name elsewhere refused; identity is the canonical directory; a copy nothing reaches is refused by nothing; revisable at stage two | §4.3 |
