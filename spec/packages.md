...
| The standard library is the package `Hex`; the prelude is in scope without import; `import Hex.Option as Opt` is a second alias; its sources sit in `stdlib/`, the one deliberate mismatch of directory and name; the runtime modules are `Hex.Runtime.*` | §2.4 |
