...
Modules §11 owns emission; packages add the layout. The program's output root holds the **project's** modules by their declared names, dotted segments as directories (`Geometry.js`, `Render/Geometry.js`), and every other package's modules under a directory named by the package (`Acme/Geometry.js`, `Acme/Render/Geometry.js`) — the standard library included, so the prelude's `Option` emits `Hex/Option.js` and its emitted import is `"./Hex/Option.js"` (FFI Part 1 §8.3 as amended by #829; before #829 the prelude's files were root siblings, and every pinned emitted specifier of theirs moves) — the full name (§2.3) as a path, with the project's package segment elided because a project may have none (§2.5). A dependency's source directory is never written into. Program-scoped artifacts — `hex.d.ts`, `hex.js` (FFI Part 1 §8.3, Part 7 §1.2) — sit at the root, their filename probe reading the emitted filenames (Part 1 §8.3, as respelled by #829). Two packages' same-named modules therefore never collide on disk — and a project module could collide with a package's directory only by beginning with that package's name, which Modules §2.2 refuses for every package in the program — at the header where the project sees the package, at the whole-program check where the package is reached only transitively (a dependency's own `module Acme.Tools` emits under its package, `Bolt/Acme/Tools.js`, and is refused by the same rule whenever `Acme` is in the program) — and the emitted import specifiers — computed from the two modules' full names (Modules §11.2) — are relative paths within the root, so the output runs from any location and imports nothing from `node_modules` on Hexagon's account; a package's JavaScript dependencies keep their bare specifiers (FFI Part 4 §2.3) and resolve as npm resolves them.
