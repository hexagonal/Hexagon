...
- **`name`** — the package's name: one uppercase-start identifier (Lexer §3.1's `UpperName`), not dotted. It is the namespace segment of every module the package holds (§2.3). `Hex` is reserved for the standard library (§2.4) and refused in the project's manifest and in any package the lookup answers with (§4.1); a manifest declaring it anywhere else is a candidate for no name, and answers nothing.
...
| The project's manifest, or a package's in the package set, declares `"name": "Hex"` | "`Hex` is the standard library's package name" — a manifest the walk merely scans draws nothing (§2.1, §2.4, §4.1) |
| A manifest lists `"Hex"` under `dependencies` | "`Hex` is every package's dependency; remove the entry" (§2.4) |
| The project's manifest, or a package's in the package set, carries a `name` that is not one uppercase-start identifier (`"acme"`, `"Acme.Tools"`) | "a package name is one uppercase-start identifier: write `\"Acme\"`" — a dotted spelling names the module-name form as the place for dots; a manifest the walk merely scans draws nothing (§2.1, §4.1) |
