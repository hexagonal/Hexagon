...
| A `dependencies` entry of the project's manifest, or of a package's in the package set, begins as no package name can — an npm name | "`tiny-json` is not a Hexagon package: bind it with `extern from \"tiny-json\"`" — by the spelling, nothing on disk read (§4.4) |
...
| JavaScript-only packages are the FFI's; never listed as Hexagon dependencies; the refusal drawn by the entry's spelling alone, an uppercase-start misspelling taking the shape refusal instead | §4.4 |
