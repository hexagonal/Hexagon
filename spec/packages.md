...
- **`name`** — the package's name: one uppercase-start identifier (Lexer §3.1's `UpperName`), not dotted. It is the namespace segment of every module the package holds (§2.3). `Hex` is reserved for the standard library (§2.4) and refused in the project's manifest; a manifest declaring it anywhere else is a candidate for no name (§4.1) and answers nothing, so no program can hold it, and the lookup carries no mechanism for catching a published package that declares it — none is needed, since it can never be reached.
