...
- **`name`** — the package's name: one uppercase-start identifier (Lexer §3.1's `UpperName`), not dotted. It is the namespace segment of every module the package holds (§2.3). `Hex` is reserved for the standard library (§2.4) and refused in the project's manifest; a manifest other than the project's declaring it is a candidate for no name (§4.1) and answers nothing, so no program can hold a second `Hex`, and the lookup carries no mechanism for catching a published package that declares it — none is needed, since it can never be reached.
- **`dependencies`** — the names of the Hexagon packages whose modules this package's modules may import (§3.1). Each is judged by its spelling first (§4.4) and, where lawful, resolved by §4.1's lookup, from this package's own directory, by the `name` each candidate's manifest declares; a listed name that lookup does not answer with, or that it finds at two canonical directories with none nearer (§4.1), is refused (§7).
...
The standard library's modules form the package `Hex`. Its **prelude** — the fixed, ordered list Modules §5.5 governs — is in scope in every module of every package without an import, under its unqualified names; the rest of `Hex`, when there is a rest, is imported by name like any package's module. A prelude module's full name is `Hex.Option`, `Hex.Vector`: an import `import Hex.Option as Opt` binds a second alias onto it (Modules §3.1), and where a project declares its own `module Option`, the unqualified name is the project's for its importers and `Hex.Option` stays reachable (§3.2). `Hex` needs no `dependencies` entry: every package depends on it, and an entry naming it is refused with the reason — "`Hex` is every package's dependency; remove the entry" (§7). Its source is kept in the repository's `stdlib/` directory, and the directory is not called `Hex`: the directory says what it holds, and `Hex` is what a program writes, a package's name coming from its manifest and never from a directory (§2.1). The whole library is `Hex`'s: the prelude, the rest, and the **runtime modules** the emitted program runs on — `Hex.Runtime.VectorTrie`, `Hex.Runtime.HashTrie`, and `Hex.Runtime.Regex` when #927 lands it, each declared `module Runtime.<Name>` and kept under `stdlib/Runtime/` by §2.2's convention. They export nothing at the Hexagon level, so they are modules like any other with nothing to reach: `import Hex.Runtime.VectorTrie` binds an alias and any qualified use draws the ordinary does-not-export report, and no rule carves them out of §3.1's visible set. Their `runtime` flag — until its scheduled deletion at `Node`'s migration into the door's `type` form (`intrinsics.md` §9.2, #927) — attaches to their membership in `Hex`'s runtime list, never to a path, and the intrinsic door is every `Hex` module's (`intrinsics.md` §5.2, #927) — the prelude's, the rest's, and theirs; a supplied source stands in for a member only through an explicit compiler-host grant identifying that registered member and its source (`intrinsics.md` §5.2, #843). A user module with the same name merely occludes the standard-library module; neither its filename, its declared name, nor its manifest grants trust.

**Ordering within `Hex`.** The prelude's fixed order and the compiler's required
runtime placements remain the leading sequence. Ordinary library modules follow
that sequence in a deterministic dependency order: an imported library module is
checked before its importer. The fixed prelude visibility rule does not turn the
rest of `Hex` into a fixed declaration-order list. An ordinary library module sees
the whole prelude and its explicit imports; neither a directory's nesting depth
nor a module name establishes a dependency. The same rule applies to sources
supplied through an explicit standard-library replacement grant. Import cycles
remain errors under Modules §8.1, and checking order does not change the runtime
load order of Modules §8.2.

### 2.5 The project

The root package — the one the host compiles — is the **project**. Its manifest may carry no `name`: a project that is never published needs none, since its own modules are addressed by their declared names alone (§3.2), and a project never qualifies its own module by its own package name — `import MyApp.Geometry` inside `MyApp` is the unknown-module error, not a synonym, the project's instance of §3.3's rule for every package (and no module in the program can be *declared* `MyApp.Geometry` to answer it: Modules §2.2 forbids a dotted module beginning with any package in the program). A project that is published as a dependency of others gives its manifest a `name`, and that name is what its consumers write; no entry of the package closure may resolve it to a directory other than the project's own (§4.3). A host may also compile a directory holding no manifest as the project, under an **implicit empty manifest**: §1's "a project's own source", read by every rule here exactly as a project whose `hexagon.json` is `{}` — no `name`, no `dependencies`, `Hex` implicit as everywhere (§2.4) — its files those beneath the directory the host compiles, with §2.2's ordinary bounds read from that directory: nested packages, `node_modules`, and the host's output excluded. The implicit manifest is read only after project ownership is settled: a directory inside a package belongs to that package's project (`environment.md` §4), and the default never creates a second project there or discards the owning manifest. Nor does it make a directory an installed package: a level's package roots are the entries that hold a `hexagon.json` (§4.1), and an implicit manifest is no file, so no lookup answers with the directory. Library and application are therefore not distinct kinds of package, exactly as they are not distinct kinds of module (Modules §8.3): the same directory may be compiled as a root and installed as a dependency.
...
This spec designs no version syntax, selection policy, or lockfile: `package.json` declares npm dependencies at whatever versions, npm resolves and locks them, and the Hexagon manifest names which Hexagon packages its lookup is to answer for. What this spec adds is one constraint on the result:

### 4.3 One copy of a package per program

A program holds **exactly one** package per Hexagon package name — the project counted among them — and the rule is read over the **package set**, §4.1's closure, and over nothing wider. Where two entries of the closure resolve one name to two directories — over records, two packages of one name at two directories in the closure — the program is refused before checking, naming both directories and the npm version each carries where its npm manifest declares one: "package `Acme` is installed twice: `node_modules/@acme/geometry` (2.1.0) and `node_modules/@bolt/tools/node_modules/@acme/geometry` (1.4.0); a program holds one copy of each Hexagon package". That is the nested-duplicate case in full: npm nests a second copy exactly when a dependency wants another version, `Bolt` finds it first from its own directory (§4.1), the project finds the root's, and the program would hold both. Two package roots at one level of a walk resolving an entry of the closure, declaring the name it seeks at two canonical directories, draw the same report (§4.1); a lookup run only to decide a diagnostic draws none. **Directory identity is the canonical path**, links followed: two links reaching one package are one copy, never two, and the report prints each directory relative to the project — climbing where the walk climbed, `../node_modules/@acme/geometry`. A copy nothing in the closure reaches is no package in the program and is refused by nothing — no lookup resolving an entry of the closure answers with it, and none, seeking its name, answers from its level (§4.1) — so a stray copy in an ancestor's `node_modules` cannot fail a program that never resolves to it. Where an entry of the closure resolves the **project's** own `name` (§2.5) to an installed package at a directory other than the project's own — a dependency lists a package named as the project is, or the project lists its own name and a package of that name is installed (§4.1) — the program is refused the same way, at the manifest: "this project declares `"name": "Acme"`, and `Acme` is also installed at `node_modules/@acme/geometry`; a program holds one package of each name" — without it two modules would share one full name, §2.3's spelling would stop being an address, §6 would emit one at the root and one under `Acme/`, and Exceptions §7.1's brand would stop being unique. An entry that resolves the project's name to the project's **own** canonical directory adds an edge to the package already in the set, and no package — canonical identity applied, the rule by which two links are one copy — and the ordinary cycle rule reports the cycle the edges close (§4.1): a workspace links the project into a sibling's `node_modules`, the project lists the sibling, and the sibling lists the project. Only edges reachable from the project enter its graph, so a link alone, into any `node_modules`, refuses nothing: a manifest nobody lists answers no entry of the closure (§4.1). The installed-twice refusal above is reserved for a different canonical directory declaring the same name; no case here has a rule of its own. The refusal keeps two copies out rather than telling them apart, and says so: two copies would be two packages of one name, whose same-named nominal types are distinct declarations and whose instances may both be lawful, and a diagnostic that keeps them apart must expose the version split at every mention — a design this stage does not undertake. A later stage may admit them as distinct identities; nothing here forbids it, and nothing here relies on their absence except the spelling of full names (§2.3), which would gain a disambiguator then.

### 4.4 JavaScript-only packages are the FFI's

An npm package with no `hexagon.json` is a JavaScript package. Its modules are reached by `extern from "pkg"` (FFI Part 4 §2.1) and `extern import "pkg"` (Part 4 §8), never by `import`: the string specifier appears in the one and never in the other (Modules §1), and a JavaScript package declares no Hexagon module, type, constraint, or instance. A Hexagon package's manifest never lists its JavaScript dependencies, which are `package.json`'s, and `dependencies` never holds a specifier: an entry is a package name (§2.1), as the dependency's own `hexagon.json` declares it, and nothing else. **An entry's spelling establishes whether it is a package name, and nothing about what any package holds.** `"@acme/geometry"` names an npm distribution that may well carry a `hexagon.json` declaring `"name": "Acme"`, and `"acme"` may be `Acme` miscased; so no spelling is read as evidence that the package wanted is JavaScript, and the lookup reads no directory that holds no `hexagon.json` (§4.1) and needs to read none. An entry is judged by its spelling, `"Hex"` first — which §2.4 refuses by its spelling, read ahead of the rest, and which no manifest can supply (§2.1). A lawful package name (§2.1) is sent to §4.1's lookup, drawing §7's reports over what that package's own lookup answers with: one candidate, a package; none, the unresolvable-name report, whatever npm holds under a directory of that spelling; two at one level, the installed-twice report (§4.3). Any other spelling is refused as no package name, with guidance the spelling tailors and never a verdict on the package: one that begins uppercase but is not one identifier (`"Acme.Tools"`, §8.8) draws §2.1's shape refusal; one that does not begin uppercase — an npm name's shape, or a name miscased — draws "`"@acme/geometry"` is not a package name; `dependencies` expects a Hexagon package name, as the dependency's `hexagon.json` declares it — for a JavaScript dependency, declare it in `package.json` and bind it with `extern from "@acme/geometry"`", and where upper-casing its first letter yields a lawful name other than `Hex` — a spelling the entry could take (§2.4) — the message names that spelling too — "as the dependency's `hexagon.json` declares it, perhaps `"Acme"`" — since a miscased name is the likelier mistake there (§7).
...
A bare data selection (`import bare Shape from Geometry`) resolves Geometry
through exactly the same visible-module and package-identity rules as a full
import. It binds only the selected view, changes no producer declaration, and
preserves the declaration's original nominal identity. Data support may need
additional internal output files as specified in [Bare Imports](bare-imports.md),
while full public module paths remain the paths below. Those support files are
compiler output, not additional package modules or author-written interfaces.
...
| A dotted import resolving to nothing whose first segment names an installed Hexagon package the resolving package does not list — installed in §4.1's sense, one the resolving package's own lookup answers with | "`Bolt` is not a dependency of this package; add `"Bolt"` to `dependencies` in `hexagon.json`" — applied edit where the host can write the manifest; where a module of any package in the program, imported or not, is declared under that first segment, the unknown-module report fires instead and no edit is offered (§3.3, §3.4) |
...
| `dependencies` names a Hexagon package the resolving package's lookup does not answer with | "no installed package declares `\"name\": \"Bolt\"`; install it, or check the name in its `hexagon.json`" — where the lookup scanned a manifest it could not read, the report carries it: "(`node_modules/@bolt/tools/hexagon.json` could not be read: …)"; *installed* is §4.1's sense, what this package's lookup answers with, so a copy under a sibling's nested `node_modules` draws it (§2.1, §4.1) |
| A `dependencies` entry of the project's manifest, or of a package's in the package set, is uppercase-start and otherwise not a package name (`"Acme.Tools"`) | §2.1's shape refusal, "a package name is one uppercase-start identifier: write `\"Acme\"` — a module's name is where dots belong" at a dotted spelling; a package name written wrong (§4.4) |
| A `dependencies` entry of the project's manifest, or of a package's in the package set, does not begin uppercase (`"tiny-json"`, `"@acme/geometry"`, `"acme"`) | "`\"@acme/geometry\"` is not a package name; `dependencies` expects a Hexagon package name, as the dependency's `hexagon.json` declares it — for a JavaScript dependency, declare it in `package.json` and bind it with `extern from \"@acme/geometry\"`" — where upper-casing the first letter yields a lawful name other than `Hex`, "…as the dependency's `hexagon.json` declares it, perhaps `\"Acme\"` — for a JavaScript dependency, …"; by the spelling, nothing on disk read, and no verdict on what the package holds (§4.4) |
...
| The project's manifest, or a package's in the package set, lists `"Hex"` under `dependencies` | "`Hex` is every package's dependency; remove the entry" — read ahead of §4.4's spelling rule (§2.4, §4.4) |
...
3. **Ranked resolution** (nearest package wins; the first-listed dependency wins; the project's dependency order decides): refused (§3.3) — an unqualified spelling that means one module today and another after `npm install` is the silent re-meaning Modules §5.5 refused for names.
...
-- {"dependencies": ["@acme/geometry"]}      -- ERROR: "@acme/geometry" is not a package name; dependencies
                                             --   expects a Hexagon package name, as the dependency's
                                             --   hexagon.json declares it — for a JavaScript dependency,
                                             --   declare it in package.json and bind it with
                                             --   extern from "@acme/geometry" (by the spelling; the
                                             --   distribution may well declare "name": "Acme" — §4.4)
-- {"dependencies": ["acme"]}                -- ERROR: "acme" is not a package name; dependencies expects a
                                             --   Hexagon package name, as the dependency's hexagon.json
                                             --   declares it, perhaps "Acme" — for a JavaScript
                                             --   dependency, declare it in package.json and bind it with
                                             --   extern from "acme" (§4.4)
-- {"dependencies": ["Hex"]}                 -- ERROR: Hex is every package's dependency; remove the entry
                                             --   (read ahead of the rest of §4.4's spelling rule — §2.4, §4.4)
-- {"dependencies": ["Acme.Tools"]}          -- ERROR: a package name is one uppercase-start identifier:
                                             --   write "Acme" — a module's name is where dots belong
                                             --   (a name written wrong — §4.4)
-- {"dependencies": ["Lodash"]}, node_modules/lodash holding no hexagon.json
                                             -- ERROR: no installed package declares "name": "Lodash";
                                             --   install it, or check the name in its hexagon.json
-- the project lists Acme and Bolt; Bolt at node_modules/@bolt/tools lists Acme; Acme is at node_modules/@acme/geometry (2.1.0)
...
| The project is the root package, unnamed unless published, and never qualifies itself; a host may compile a directory holding no manifest as the project under an implicit empty manifest — no name, no dependencies, ordinary bounds, read after ownership is settled, never discoverable as installed | §2.5 |
...
| Distribution rides npm; versions and lockfile are npm's; the npm name is not the Hexagon name; what a package is (a record the compiler validates) is kept apart from how its directory is found (a host's mechanism, npm's layout the first); each package's `dependencies` are looked up from its own canonical directory, nearest `node_modules` first, walking up, a farther copy shadowed for that walk, a package nobody lists entering no program, the lookup for an unlisted first segment deciding one report and refusing nothing; a manifest the walk cannot use is no candidate and stops nothing, §2.1's `name` refusals the project's manifest's alone, a package that enters the set checked for its own `dependencies` by its own lookup; the package set is the acyclic closure so assembled, plus the project and `Hex`; a cycle refused by name | §4.1–§4.2 |
| One copy per package name per program, the project counted, read over the package set alone: two entries reaching two canonical directories, or, on a closure entry's walk, two roots at one level declaring the name sought at two canonical directories, refused; an entry reaching the project's own directory adds an edge and no package, one reaching a package of the project's name elsewhere refused; identity is the canonical directory; a copy nothing reaches is refused by nothing; revisable at stage two | §4.3 |
| JavaScript-only packages are the FFI's; never listed as Hexagon dependencies; an entry's spelling establishes whether it is a package name and nothing about what a package holds — `"Hex"` read first, a lawful name handed to the lookup, any other spelling refused as no package name with guidance the spelling tailors — the shape refusal where it begins uppercase, the field's expectation and the conditional JavaScript route where it does not — and no verdict on the package | §4.4 |
