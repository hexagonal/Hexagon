...
| A `dependencies` entry of the project's manifest, or of a package's in the package set, is spelled as no package name can be — an npm name | "`tiny-json` is not a Hexagon package: bind it with `extern from \"tiny-json\"`" — by the spelling, nothing on disk read (§4.4) |
