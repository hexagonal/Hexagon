...
An npm package with no `hexagon.json` is a JavaScript package. Its modules are reached by `extern from "pkg"` (FFI Part 4 §2.1) and `extern import "pkg"` (Part 4 §8), never by `import`: the string specifier appears in the one and never in the other (Modules §1), and a JavaScript package declares no Hexagon module, type, constraint, or instance. Listing such a package under `dependencies` is refused — "`tiny-json` is not a Hexagon package: bind it with `extern from "tiny-json"`" — and a Hexagon package's manifest never lists its JavaScript dependencies, which are `package.json`'s. The refusal is drawn by the entry's **spelling**, and by nothing on disk: a `dependencies` entry is a package name (§2.1), an npm name is never one — lowercase, scoped, punctuated (§8.6) — so, `"Hex"` apart — which §2.4 refuses by its spelling too, read ahead of this rule, and which no manifest can supply (§2.1) — a spelling that does not begin uppercase is read as npm's and sent to the door; one that begins uppercase but is not one identifier (`"Acme.Tools"`, §8.8) is a package name written wrong and draws §2.1's shape refusal, since the door it would be sent to names a JavaScript module that cannot exist; and a lawful package name is sent to §4.1's lookup, drawing §7's reports over what that package's own lookup answers with — one candidate, a package; none, the unresolvable-name report, whatever npm holds under a directory of that spelling; two at one level, the installed-twice report (§4.3). The lookup reads no directory that holds no `hexagon.json` (§4.1) and needs to read none.
...
| A `dependencies` entry of the project's manifest, or of a package's in the package set, does not begin uppercase — an npm name | "`tiny-json` is not a Hexagon package: bind it with `extern from \"tiny-json\"`" — by the spelling, nothing on disk read (§4.4) |
...
-- {"dependencies": ["tiny-json"]}           -- ERROR: tiny-json is not a Hexagon package; bind it
                                             --   with extern from "tiny-json" (by the spelling — §4.4)
...
| JavaScript-only packages are the FFI's; never listed as Hexagon dependencies; the refusal drawn by the entry's spelling alone, `"Hex"` read ahead of it, an uppercase-start misspelling taking the shape refusal instead, a lawful name handed to the lookup | §4.4 |
