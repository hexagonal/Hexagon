...
| Two entries of the closure resolve one name to two directories, or a walk's requested name is declared by two package roots at one level, at two directories | "package `Acme` is installed twice: `<dir>` (`<version>`) and `<dir>` (`<version>`); a program holds one copy of each Hexagon package" — directories canonical, printed relative to the project; the version where the npm manifest declares one (§4.1, §4.3) |
...
-- node_modules/acme-a and node_modules/acme-b both declare "name": "Acme"; nobody lists Acme
                                             ...
                                             --   it declares and nothing more (§4.1)
-- an ancestor's node_modules/x/hexagon.json declares "name": "Hex"; nobody lists it
                                             -- OK, silent: §2.1's refusals read the package set alone (§4.1)
...
-- a workspace links the project MyApp into packages/bolt/node_modules; Bolt lists MyApp
