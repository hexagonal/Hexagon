...
- **Distribution rides npm.** A Hexagon package is an npm package that carries `hexagon.json`; installation, versions, and the lockfile are npm's, and this spec designs none of them (§4). What a package *is* and how its directory is *found* are kept apart: the compiler reads a package as a record and validates it by the rules here, and npm's layout is the first mechanism a host supplies for finding one (§4.1) — each package's dependencies looked up from that package's own place, as Node itself looks, so that every refusal names something the program holds.
...
A Hexagon package is published to and installed from **npm**, as an npm package whose root holds `hexagon.json` and the package's `.hex` source. Two things are kept apart here, deliberately. What a package **is** — a directory holding a manifest, identified by the manifest's `name` and by the directory's canonical path, validated by the rules of this spec — is the language's. How a package directory is **found** is a host's mechanism (compiler architecture, `environment.md` §4): the compiler reads packages as records — a name, a `dependencies` list, a directory, a version where one is known, the files beneath — and never reads a `node_modules` directory itself. npm's layout is the first mechanism a host supplies; another may supply directories later, and no rule here changes when it does. This spec designs no mechanism beyond npm's and adds no manifest field for one.

**Lookup, under npm's layout.** A package's `dependencies` are resolved **from that package's own directory** — its canonical one, links followed (§4.3) — by the walk Node itself makes for a requested name: the package's `node_modules`, then each ancestor directory's, outward to the filesystem root. npm names a directory and Hexagon names a manifest, so the walk learns the names at a level by reading the `hexagon.json` of every package root at that level, scoped directories included, and the **nearest** level holding a manifest that declares the requested name answers. A copy at a farther level is shadowed: never read, never in the program, never refused. A package no entry of the closure names is never sought, whatever its manifest says. The one place this differs from Node's rule is the one Node never meets, since npm forbids two directories of one npm name: two package roots at **one level** declaring the same `name` are refused — §4.3's installed-twice report, both directories named — because no nearest-first rule orders them. This is what makes a workspace's hoisted layout work: `packages/app` lists `Acme`, has no `node_modules` of its own, and the workspace root's answers; a workspace package installed as a link resolves from its real directory, which is where its own hoisted dependencies are found.

**A manifest the walk cannot read supplies no candidate and stops nothing.** The walk meets packages nobody asked for. One whose `hexagon.json` does not parse, or declares no lawful `name` (§2.1), is a candidate for no name and refuses no lookup; it is named only inside the unresolvable-name report of a lookup that scanned it — "no installed package declares `"name": "Acme"`; install it, or check the name in its `hexagon.json` (`node_modules/@acme/geometry/hexagon.json` could not be read: …)" (§7) — since that is the one moment it could have been the package wanted, and where every lookup succeeds it is silent. An npm package with no `hexagon.json` at all is a JavaScript package (§4.4), read for nothing.

The project's `dependencies` are resolved so, and each package reached has its own resolved in turn, from its own directory, outward from the project, until the list closes. A package is **installed**, for the package resolving a name, exactly when that package's lookup finds it — the sense in which Modules §2.3's not-a-dependency report reads the word. The closure is **acyclic**, as Modules §8.1 makes the import graph, on grounds of its own kind: a `dependencies` cycle is refused naming it — "dependency cycle: `Acme` → `Bolt` → `Acme`" (§7) — so that packages have one deterministic order, the closure is a set assembled outward from the project and closed, and no manifest's meaning depends on a cycle. One consequence the rule also secures, at the mutual case: two packages that listed each other would each see the other, and a cross-package instance would have two lawful homes (§5.1) — the acyclic closure leaves it one. A package listing its own `name` is the one-package cycle, found as every cycle is, by directory identity — the lookup from its own place reaches its own directory — and renders "dependency cycle: `Acme` → `Acme`"; a project is under no `node_modules`, so a project listing its own `name` reaches itself never: where its lookup finds nothing it draws §2.1's unresolvable-name refusal, and where it finds an installed package of that name it draws §4.3's project-name refusal, the program holding two packages of one name. The packages so reached, with the project and `Hex`, are the program's **package set** — what §3.1 calls the packages *in the program* — assembled from manifests alone before any module is parsed. The npm package's own name (`@acme/geometry`) is not read by the language and need not resemble the Hexagon name: what npm names is a distribution, and what Hexagon names is a namespace.
...
A program holds **exactly one** package per Hexagon package name — the project counted among them — and the rule is read over the **package set**, §4.1's closure, and over nothing wider. Where two entries of the closure resolve one name to two directories, the program is refused before checking, naming both directories and the npm versions each carries: "package `Acme` is installed twice: `node_modules/@acme/geometry` (2.1.0) and `node_modules/@bolt/tools/node_modules/@acme/geometry` (1.4.0); a program holds one copy of each Hexagon package". That is the nested-duplicate case in full: npm nests a second copy exactly when a dependency wants another version, `Bolt` finds it first from its own directory (§4.1), the project finds the root's, and the program would hold both. Two package roots at one level of one walk declaring the same name draw the same report (§4.1). **Directory identity is the canonical path**, links followed: two links reaching one package are one copy, never two, and the report prints each directory relative to the project. A copy nothing in the closure reaches is no package in the program and is refused by nothing — the walk never reads it (§4.1) — so a stray copy in an ancestor's `node_modules` cannot fail a program that never resolves to it. Where an entry of the closure resolves the **project's** own `name` (§2.5) to an installed package — a dependency lists a package named as the project is, or the project lists its own name and a package of that name is installed (§4.1) — the program is refused the same way, at the manifest: "this project declares `"name": "Acme"`, and `Acme` is also installed at `node_modules/@acme/geometry`; a program holds one package of each name" — without it two modules would share one full name, §2.3's spelling would stop being an address, §6 would emit one at the root and one under `Acme/`, and Exceptions §7.1's brand would stop being unique. The refusal keeps two copies out rather than telling them apart, and says so: two copies would be two packages of one name, whose same-named nominal types are distinct declarations and whose instances may both be lawful, and a diagnostic that keeps them apart must expose the version split at every mention — a design this stage does not undertake. A later stage may admit them as distinct identities; nothing here forbids it, and nothing here relies on their absence except the spelling of full names (§2.3), which would gain a disambiguator then.
...
| `dependencies` names a Hexagon package the resolving package's lookup does not find | "no installed package declares `\"name\": \"Bolt\"`; install it, or check the name in its `hexagon.json`" — where the lookup scanned a manifest it could not read, the report carries it: "(`node_modules/@bolt/tools/hexagon.json` could not be read: …)" (§2.1, §4.1) |
...
| Two entries of the closure resolve one name to two directories, or two package roots at one level of a walk declare one name | "package `Acme` is installed twice: `<dir>` (`<version>`) and `<dir>` (`<version>`); a program holds one copy of each Hexagon package" — directories canonical, printed relative to the project (§4.1, §4.3) |
| An entry of the closure resolves the project's `name` to an installed package | "this project declares `\"name\": \"Acme\"`, and `Acme` is also installed at `<dir>`; a program holds one package of each name" (§4.3) |
...
-- node_modules/acme-a and node_modules/acme-b both declare "name": "Acme"; the project lists Acme
                                             ...
                                             --   and node_modules/acme-b; … (one level, two roots — §4.1)
...
-- Acme at node_modules/@acme/geometry, and again in an ancestor's node_modules
                                             -- OK: the nearest answers; the farther is never read (§4.1)
...
-- project {"name": "MyApp", "dependencies": ["MyApp"]}, nothing installed under that name
                                             ...
                                             --   install it, or check the name in its hexagon.json
                                             --   (a project is under no node_modules — §4.1)
...
| Distribution rides npm; versions and lockfile are npm's; the npm name is not the Hexagon name; what a package is (a record the compiler validates) is kept apart from how its directory is found (a host's mechanism, npm's layout the first); each package's `dependencies` are looked up from its own canonical directory, nearest `node_modules` first, walking up, a farther copy shadowed and a package nobody lists never sought; a manifest the walk cannot read is no candidate and stops nothing; the package set is the acyclic closure so assembled, plus the project and `Hex`; a cycle refused by name | §4.1–§4.2 |
| One copy per package name per program, read over the package set alone: two entries reaching two directories, or two roots at one level, refused; identity is the canonical directory; a copy nothing reaches is refused by nothing; revisable at stage two | §4.3 |
