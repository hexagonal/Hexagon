...
| `Rat` rational type: focused specification plus stdlib module | `rat.md`; `integral-constraint.md` §1/§9; `division-remainder.md`; `numeric-literals.md` §5 | v1 requirement; numerator and denominator are represented with `BigInt`, with Euclidean normalization, fixed integer division conventions, and exact integral `fromInt`; `rat.md` fixes encapsulation, runtime shape, API, operations, and instances | — | `rat.md` + `stdlib/Rat.hex` landed; listing records numeric inventory |
