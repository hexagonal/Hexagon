...
**Recorded under rule 5 (2026-08-02, #237 ruling).** `ffi.md` §9.1 heads itself *"V1 stdlib work (global ledger per `v1-spec-consolidation-plan.md`)"*, which reads against rule 1's "no other ledger exists". Both are live and neither is chosen here. The reconciling reading, and the one this file works to: **`ffi.md` §9.1 is the FFI spec's local index into this ledger, not a second ledger** — every one of its five entries already has a row here, and its own items describe themselves as issued to a ledger elsewhere ("ledger entry issued by Part 11 §10"). Two consequences are recorded rather than legislated: §6's "no row is duplicated across ledgers" is, read strictly, already false of those five FFI-originated stdlib rows (it holds exactly for `ffi.md` §9.2, the non-stdlib half); and a document that discovers stdlib debt owes a row **here** first, whatever it also mirrors. `ffi-part2-nullable-array.md` §9.1 carries the edit note that conforms `ffi.md` §9.1's heading on next touch. The consolidation pass owns the final wording.
...
| **No declared-but-throwing stubs.** A shipped Hexagon-owned operation exists when it works: one the listing cannot yet implement is *absent* — no declaration, no export, no `.d.ts` entry — and the owning spec is the forward contract that reserves its name. Governs callable surface only, and does not reach user code or development-time scaffolding *(added 2026-08-02, #237 ruling; routed here for discoverability — the doctrine reaches shipped surface generally but is written in a document titled for `Nullable`/`Array`)* | `ffi-part2-nullable-array.md` §9.1 (rejected alternatives, with prices, §9.2) |
