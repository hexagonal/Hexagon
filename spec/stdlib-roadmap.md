...
| `Int.hex` | **landed** (#344): Euclidean `div`/`mod`, iterative `gcd`, the zero-divisor and negative-exponent guards, the checked family (`checkedAdd`/`checkedSub`/`checkedMul` as exact pre-checks — Primitive Types §2.1), and the public instances (`Num`, `Signed`, `Eq`, `Ord`, `Show`, `Pow`, `Hash`, `Integral`) as source `honor` blocks | the thirteen-key primop inventory of intrinsics §3.2 (`intAdd` … `intRem`) — every own-operation member body, per Constraints §6.1's door-backed law |
| `Nat.hex` | **landed** (#344): the checked boundary conversion `fromInt`'s sign check (Primitive Types §1), the zero-divisor guards, and the public instances (`Num`, `Eq`, `Ord`, `Show`, `Pow`, `Hash`, `Integral`) as source `honor` blocks | the eleven-key primop inventory of intrinsics §3.2 (`natAdd` … `natFromIntUnchecked`) — every own-operation member body plus the one conversion core |
