...
| `Regex.hex`, `Regex/Syntax.hex`, `Regex/Engine.hex` *(#927, scheduled)* | the whole library in three ordinary `Hex` modules outside the prelude, the first non-prelude source through the door under the widened gate (intrinsics §5.2): the dialect's parser and public syntax tree, class sets and simple case folding, the NFA compiler, the Pike VM, the lazy DFA, and the `Regex` surface | the storage type `Buffer(a)` and its four keys (`bufferNew` … `bufferLength`), declared through the door's `type` form (intrinsics §3.3) under Effects §6.2's species (d); and the Unicode-property range tables of UTS #18 RL1.2 as named keys — the tables kept in one place and referenced by name, never inlined into emitted programs |
