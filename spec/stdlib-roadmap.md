...
| `Seq.hex` | **the `Seq(a)` declaration itself** (`export opaque record`, Loops §6.6) plus `next` and the combinator core (`successors`, `map`, `filter`, `take`, `fold`, and the ship-list; the seed/step producer is `successors`, never `iterate` — bare `iterate` is the `Iterable` member's, single-exporter by Collections Part 5 §2.3) | memoizing spine (`memoize`'s buffer + the FFI Part 3 inbound adapter) and the boundary traversal face (FFI Part 3 §9.4 — formerly listed as "the `toJsIterable` bridge"; see the 2026-07-28 edit note below) |
