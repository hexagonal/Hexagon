...
| `Float.hex` | **landed** (#344): `mod` over `rem` (the Euclidean adjustment, Division & Remainder §5 — the only Hexagon-above-the-door logic; no guards exist at `Float`) and the public instances (`Num`, `Signed`, `Frac`, `Eq`, `Ord`, `Show`, `Pow`, `Hash`) as source `honor` blocks; the wider wrapper surface (`Math` selections, IEEE constants) stays a listing item | the thirteen-key primop inventory of intrinsics §3.2 (`floatAdd` … `floatRem`) — every own-operation member body plus the two crossing conversions and the one plain-export core |
