...
| `Regex.hex`, `Regex/Syntax.hex`, `Runtime/Regex.hex` *(#927, scheduled; contract `regex.md`)* | two ordinary `Hex` modules outside the prelude — of which `Regex.hex` is the first non-prelude source through the door under the widened gate (intrinsics §5.2) — and one runtime module: `Regex.Syntax` owns the dialect's parser and public syntax tree, the explicit-range class algebra, and the placement of folding and flags in the tree, all pure and **door-free** (`regex.md` §5); `Hex.Runtime.Regex` owns class resolution against the Unicode tables, the NFA compiler, the Pike VM, and the lazy DFA, written `->!` throughout over `Buffer` (`regex.md` §6); `Regex.hex` owns the surface as pure Hexagon over two sealed rows (`regex.md` §4) | in the runtime module: the storage type `Buffer(a)` and its four keys (`bufferCreate` … `bufferLength`, three of them `->!`), declared through the door's `type` form (intrinsics §3.3), and the two Unicode-table keys `regexPropertyRanges` (UTS #18 RL1.2's properties as ranges) and `regexFoldOrbit` (simple case folding), the tables kept in one place and referenced by name, never inlined into emitted programs; in `Regex.hex`: the confined type key `regexProgram` (`Program`, declarer `Hex.Regex`) and the two `->` sealed rows `regexCompile`/`regexSearch` lowering to the runtime module's compiled bodies under Effects §6.2's species (d) (`regex.md` §7) |
