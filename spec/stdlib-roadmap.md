...
| `toSeq` qualified home (`Iterable.toSeq` or equivalent) | `collections-part5-iterable.md` §2.3/§4 (via `modules.md` §6.4 invariant) | `toSeq` is an ordinary prelude term; the `Iterable` member (the per-collection `toSeq` spellings are its honored-member reads, Modules §5.3) | — | listing: prelude inventory |
...
1. primitive constraint declarations and their canonical instances — the declarations half is discharged (#335): every pre-registered constraint has canonical prelude source (`Show.hex`, `Num.hex`, `Signed.hex`, `Frac.hex`, `Pow.hex`, `Concat.hex`, `Eq.hex`, `Hash.hex`, `Ord.hex`, `Integral.hex`, `Iterable.hex`); the instances half is the companion arc (#344), landed per companion in the fixed order `BigInt`, `Int`+`Nat`, `Float`+`String` — **complete**; Constraints §5.3's wired-row-retirement law governed each landing and no wired instance remains;
