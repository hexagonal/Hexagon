...
| Numeric narrowing set (`Int.fromFloat`, kin) | `ffi-part1-boundary.md` §6 ("if included") | checked, `Option`-returning, `Number.isSafeInteger` discipline; `BigInt.toInt` already core. Primitive Types §3's #919 `Float.floor`/`ceil`/`trunc`/`round`/`roundEven` do not discharge this candidate: they explicitly change fractional values under a named rule and throw on an unrepresentable result, while `Int.fromFloat` would accept only an already-safe integer | — | listing: numeric |
...
| ↳ #919 rounding extension | `floor`, `ceil`, `trunc`, school `round`, and ties-to-even `roundEven`, all `Float -> Int`; target-owned `IntRangeError`, safe-integer validation, and canonical zero; discharges these five members of the wider Math-wrapper listing without adding `Int.fromFloat` | three private cores join the inventory: `floatTrunc`, `floatIsSafeInteger`, `floatToIntUnchecked`; direction and tie logic remains in `.hex` |
