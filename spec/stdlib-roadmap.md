...
| `Regex.hex`, `Regex/Syntax.hex`, `Runtime/Regex.hex` *(#927, scheduled)* | two ordinary `Hex` modules outside the prelude — the first non-prelude source through the door under the widened gate (intrinsics §5.2) — and one runtime module: `Regex.Syntax` owns the dialect's parser and public syntax tree, class sets and simple case folding, all pure; `Hex.Runtime.Regex` owns the NFA compiler, the Pike VM, and the lazy DFA, written `->!` throughout over `Buffer`; `Regex.hex` owns the surface as pure Hexagon over two sealed rows | the storage type `Buffer(a)` and its four keys (`bufferCreate` … `bufferLength`, three of them `->!`), declared through the door's `type` form (intrinsics §3.3) in the runtime module; the two `->` launder rows `regexCompile`/`regexSearch` lowering to the runtime module's compiled bodies under Effects §6.2's species (d); and the Unicode-property range tables of UTS #18 RL1.2 as named keys — the tables kept in one place and referenced by name, never inlined into emitted programs |
