...
| Subject-first now also determines **dot-callability** (exported ∧ first-parameter-`T`-headed, unioned with honored subject-first members since #304/#335, and at `Nat`/`Int` with the tower rungs they do not honor since #808) | `method-syntax.md` §4.2/§15 |
...
| **The bare set is closed at twenty names** — the prelude's twelve exception constructors (including #919's `IntRangeError` and Dec's `DecimalPlacesOverflowError` and `NegativeDecimalPlacesError`); `True`, `False`, `Some`, `None`, `Ok`, `Err`; `ignore`; `show`. Every other prelude export is reached by the dot or its qualified home: no other function, no other union constructor (`Ordering.Less`, `JsKind.Null`), no other constraint member. A listing row spends no bare vocabulary unless it declares an exception; an addition to the set is a design ruling, never a listing entry. `Ordering` is homed at `stdlib/Ordering.hex`; `Prelude.hex` homes `ignore` alone | `modules.md` §5.5 |
...
|---|---|---|---|---|
| Exact BigInt-source widening into opted-in numeric destinations | `integer-widening.md` (decided, 2026-09-20) | Approved route: Nat/Int through BigInt into Rat/Dec; detailed `FromBigInt` design passed Sol spec and compiler-feasibility review | Preserve exactness and existing bounded target selection during implementation | Implemented locally with BigInt, Rat, and Dec; combined local validation status in `dec.md` |
| `Math` module | `math.md` (decided 2026-09-19; prelude seat clarified 2026-09-19) | Prelude-seated module with qualified-only terms; SML surface minus `pow`; concrete `Float`; ECMAScript numeric behaviour with the explicit mathematical-versus-rounded `atan` range; no throwing domain guards | — | compiler/stdlib implementation and Playground conformance landed; CLI exercise awaits the repository's future command-line compiler |
...
| `Rat` rational type: focused specification plus stdlib module | `rat.md`; `integral-constraint.md` §1/§9; `division-remainder.md`; `numeric-literals.md` §5 | v1 requirement; top and bottom are represented with `BigInt`, with Euclidean normalization, fixed integer division conventions, and exact `Num.fromNat` / `Signed.fromInt`; `rat.md` fixes encapsulation, runtime shape, API, operations, and instances | — | `rat.md` + `stdlib/Rat.hex` landed; `Rat` is `Hex.Rat`, embedded with the rest of the library and reached by `import Rat` in every host; listing records numeric inventory |
| `Dec` finite decimal type: focused specification, prelude module, and Primitive Types teaching | `dec.md` (API revision decided, 2026-09-21) | Shorter operation names, Int places, and separate exact multiplication/rounding agreed; runtime text parsing deferred until other numeric types establish conventions (§12) | — | First release deployed; revised specification/book/code approved by Sol Medium, validation recorded in `dec.md` |
...
| Companion inventories for `Range`, `Seq`, the primitives, and the prelude nominals `Option`/`Result`/`Ordering` (what exists to dot-call; membership of each inventory remains a listing decision) | `method-syntax.md` §12.3 (+§14(k) `Option.defaultValue` as the worked companion example); `collections-part5-iterable.md` §4; `unions.md` §8 | dot-callability mechanism fixed (§1); `Range` iteration semantics per `loops-ranges-iteration.md` §3/§5; `Option`/`Result` declarations fixed by `unions.md` §8 | — | listing: per-type companions |
...
| **`stdlib/Debug.hex`** — the debug probe (#407; surface widened at #419; influence: Elm's `Debug` module) | `effects.md` §6.2 species (a), which names the member; `intrinsics.md` §3.2 | `log<a: Show>(value: a): Unit` — Hexagon's version of `console.log`, not `console.log` precisely: any operand honoring `Show`, rendered through `show` (identity at `String`, Primitive Types §7) — ordinary Hexagon above the unexported `debugLog(message: String) -> Unit` intrinsic key, pure-faced, sink captured at module initialization (§6.2's caveat is part of the row's contract); the boundary face is a constrained export's (FFI Part 7 §7: `.d.ts` = Part 8's fundamental specializations, generic edition internal-named with its Part 9 evidence suffix — `trace`'s existing shape), accepted; `trace<a: Show>(label: String, value: a): a` — Elm's `Debug.log` shape under Haskell's name — renders `label: value` through `log` and returns `value` unchanged, ordinary Hexagon above it; both reached as `Debug.log`/`Debug.trace` (`modules.md` §5.5 — neither is in the bare set; the qualifier says what the row's own contract says, that this is a probe and not logging). **Consequences:** the natural logarithm is named `ln` (decided when bare `log` was spent, and it stands on its own name); its module home is now `Math.ln` (`math.md`, 2026-09-19); a value without `Show` (a function, a foreign brand) is refused at compile time, unlike `console.log`; and this is not real logging — species (a) forfeits multiplicity and ordering, so a counted or ordered log is a banged extern, permanently. `Debug.hex` is also the seat the v2 `Debug` constraint takes if it comes (Primitive Types §5.4) | — | landed with #407; widened with #419; listing: prelude inventory |

## 3. Ship/defer decisions (12; the listing session decides, constraints recorded)

| Surface / question | Origin | Fixed semantics | Revisit bar | Discharge |
|---|---|---|---|---|
| Program I/O: stdout/stderr, arguments, exit status, stdin, and paths | [Program IO Plan](notes/program-io-plan.md) (planning note, 2026-09-22) | Whole-file text I/O landed in #1006; `experimental-file.md` owns its contract. [Stdio](experimental-stdio.md) implements effectful output with LF everywhere; local Sol review and runtime validation complete. Later APIs and release scope remain undecided | Settle substantial later decisions with James | Incremental experimental contracts and runtime validation; no blanket v1 obligation |
...
| `JsMap.keys`/`JsMap.values` projections; `JsSet` algebra reads | `ffi.md` §9.1.5; `ffi-part10-js-map-set.md` §9 items 1–2 | derivable via `toSeq` combinators or conversion; captured-collection semantics fixed by `ffi-part10-js-map-set.md` (#875) | field demand | listing: FFI collections |
...
| ~~`String.join` and text-processing conveniences~~ **implemented (#924)** | `string-text-processing.md`; `collections-part5-iterable.md` §5.3/§14.2 | subject-first `String.join(parts, separator)` supplements, never replaces, `String.fromSeq`; the complete promoted surface and Unicode contract are owned by String Text Processing | — | landed: #924 |
...
| Numeric narrowing set (`Int.fromFloat`, kin) | `ffi-part1-boundary.md` §6 ("if included") | checked, `Option`-returning, `Number.isSafeInteger` discipline; `BigInt.toInt` already core. Primitive Types §3's #919 `Float.floor`/`ceil`/`trunc`/`round`/`roundAway` do not discharge this candidate: they explicitly change fractional values under a named rule and throw on an unrepresentable result, while `Int.fromFloat` would accept only an already-safe integer | — | listing: numeric |
...
| :--- | :--- | :--- |
| `BigInt.hex` | **landed** (#344): Euclidean `div`/`mod`, iterative `gcd`, divide-first `lcm`, the zero-divisor and negative-exponent guards, `toInt`'s range check, `toFloat`'s overflow guard (#533 — `FloatRangeError`, shared from `Float.hex` per Primitive Types §3; the edge that seats this companion after `Float.hex` in the prelude order), and the public instances (`Num`, `Signed`, `FromBigInt`, `Eq`, `Ord`, `Show`, `Pow`, `Hash`, `Integral`, `Real`) as source `honor` blocks. The **`BigInt.pow` door** joined at #541 and took the declaration form at #546 — `widens Pow.pow(value: BigInt, exponent: BigInt): BigInt` (Constraints §4.7), the exact power at exponents past `Int`'s range, one written body carrying the negative-exponent guard, whose restriction over the exact `Int -> BigInt` conversion **is** the `Pow<BigInt>` member, derived and accounted for in the block as `pow = widened` | the fifteen-key primop inventory of intrinsics §3.2 — primitive operation bodies plus the conversions' unchecked cores (`bigIntAdd` … `bigIntToFloatUnchecked`), per Constraints §6.1's door-backed law |
| `Int.hex` | **landed** (#344): Euclidean `div`/`mod`, iterative `gcd`, the zero-divisor and negative-exponent guards, the checked family (`checkedAdd`/`checkedSub`/`checkedMul` as exact pre-checks — Primitive Types §2.1), and the public instances (`Num`, `Signed`, `Eq`, `Ord`, `Show`, `Pow`, `Hash`, `Integral`, `Real`) as source `honor` blocks | the twelve-key primop inventory of intrinsics §3.2 (`intAdd` … `intRem`) — primitive operation bodies except the keyless self-identity `fromInt`, per Constraints §6.1's door-backed law and its strictly-simpler counterpart |
| ↳ #919 target-range exception | `IntRangeError`, shared by the five throwing `Float` rounding exits and any future throwing door whose promised target is `Int`; `Option`-returning checked conversions do not throw it | no new `Int` intrinsic |
| `Nat.hex` | **landed** (#344): the Euclidean `div`/`mod` and iterative `gcd` member bodies (coinciding value-wise with the truncated pair on the non-negative domain), the zero-divisor guards, the checked boundary conversion `fromInt`'s sign check (Primitive Types §1), and the public instances (`Num`, `Eq`, `Ord`, `Show`, `Pow`, `Hash`, `Integral`, `Real`) as source `honor` blocks. `Pow<Nat>`'s member gained the family's **negative-exponent guard** at #541: the `Int` exponent seat makes a negative exponent spellable here, where the old homogeneous member's `Nat` seat refused it by type | the ten-key primop inventory of intrinsics §3.2 (`natAdd` … `natFromIntUnchecked`) — primitive operation bodies plus the one conversion core |
| `Float.hex` | **landed** (#344): `mod` over `rem` (the Euclidean adjustment, Division & Remainder §5) and the public instances (`Num`, `Signed`, `Frac`, `Eq`, `Ord`, `Show`, `Pow`, `Hash`, `Real`) as source `honor` blocks. The IEEE special-value surface joined at #358: `infinity` and `nan` as exact float divisions and `isNan`/`isFinite` over `Eq<Float>`, all four door-free plain exports — the detectors are load-bearing rather than convenience, since SameValueZero makes `x != x` uniformly `False` and Primitive Types §3 owns the reading. The range exception joined at #526: `FloatRangeError`, declared here for conversions in other companions, because the range that fails is `Float`'s and the brand follows the declaring module — `Rat.toFloat` is its first thrower and the reason it exists, `BigInt.toFloat` its second (#533), and every door from an exact type into `Float` shares it rather than minting its own. The **`Float.pow` door** joined at #541 and took the declaration form at #546 — `widens Pow.pow(value: Float, exponent: Float): Float` (Constraints §4.7), the analytic power, total and honestly IEEE, one written body whose restriction to integer exponents **is** the `Pow<Float>` member, derived and accounted for as `pow = widened`. `Real<Float>` adds source `abs` with positive-zero normalization and `sign` with a NaN guard throwing `UndefinedSignError` (Constraints §7). The wider wrapper surface (`Math` selections) stays a listing item | the twelve-key primop inventory of intrinsics §3.2 (`floatAdd` … `floatRem`) — primitive operation bodies except the keyless composed `fromNat`, plus the one crossing conversion and the one plain-export core |
| ↳ #919 rounding extension, #974 naming revision | `floor`, `ceil`, `trunc`, ties-to-even `round`, and ties-away `roundAway`, all `Float -> Int`; target-owned `IntRangeError`, safe-integer validation, and canonical zero; discharges these five members of the wider Math-wrapper listing without adding `Int.fromFloat` | three private cores join the inventory: `floatTrunc`, `floatIsSafeInteger`, `floatToIntUnchecked`; direction and tie logic remains in `.hex` |
| `String.hex` | **landed** (#344; text surface #924): the public instances (`Eq`, `Ord`, `Show`, `Concat`, `Hash`) as source `honor` blocks, `Show`'s identity body keyless in ordinary Hexagon; `fromSeq` landed at #353; the complete `string-text-processing.md` surface — including `length`, splitting, dropping, trimming, exact/full-fold matching and replacement, joining, positions, scalar conversion, and Unicode 17 casing — is ordinary source over the narrow capabilities in the next column | thirteen keys: the original four primops (`stringConcat`, `stringEquals`, `stringCompare`, `stringHash`), `stringFromSeq`, and #924's eight private Unicode-property/mapping/scalar capabilities (`stringIsWhitespace` … `stringFromCodepointUnchecked`); matching and result policy do not cross the door |
...
| `Vector.hex` | **landed** (#218): companion API in ordinary Hexagon above the eight-key boundary inventory of intrinsics §3.2 (`vectorLength` … `vectorFromSeq`, and `vectorToArray` for FFI Part 2 §9's outbound conversion); combinators stay a listing item | the persistent-vector representation core — landed as `stdlib/Runtime/VectorTrie.hex` (#299/#303) and wired at the emitter milestone (#306) |
| `Map.hex` | **landed** (#370): the Collections Part 4 §6.1 core in ordinary Hexagon above the seven-key inventory of intrinsics §3.2 (`empty`'s generalizing wrapper, `isEmpty`/`containsKey` over `size`/`get`, `keys`/`values` as projections of one `entries` traversal, the `toSeq`/`fromSeq` synonyms, `fromEntries`'s fold, `fromVector`), plus the `KeyError` prelude declaration; combinators stay a listing item | the seven-key inventory (`mapEmpty` … `mapEntries`) onto `Hex.Runtime.HashTrie`'s emitted module — six direct aliases of its operations, `mapEmpty` answering its one shared empty constant — the HAMT representation core the previous form of this row named |
| `Set.hex` | **landed** (#373): the Collections Part 4 §6.2 core in ordinary Hexagon above the eight-key inventory of intrinsics §3.2 (`empty`'s generalizing wrapper, `isEmpty` over `size`, the whole algebra — `union`/`intersect`/`difference`/`isSubsetOf` as §5.3's operations, specified iterate-one-side-and-modify (§2.2) and implemented to §2.2's min-/left-side bounds — and the `fromSeq`/`fromVector` pair); combinators stay a listing item | the eight-key inventory (`setEmpty` … `setLookup`; `setElements` surfacing as the §6.2 `toSeq`, `setLookup` unexported beneath `intersect`) onto `Hex.Runtime.HashTrie`'s emitted module through the wrapper record #373 ruled (`HashSet` over `HashTrie(a, Unit)`, carrying element-only iteration) — the HAMT membership/insertion/removal the previous form of this row named |
| `Regex.hex`, `Regex/Syntax.hex`, `Runtime/Regex.hex` *(#927, scheduled; contract `regex.md`)* | two ordinary `Hex` modules outside the prelude — of which `Regex.hex` is the first non-prelude source through the door under the widened gate (intrinsics §5.2) — and one runtime module: `Regex.Syntax` owns the dialect's parser and public syntax tree, the explicit-range class algebra, and the placement of folding and flags in the tree, all pure and **door-free** (`regex.md` §5); `Hex.Runtime.Regex` owns class resolution against the Unicode tables, the NFA compiler, the Pike VM, and the lazy DFA, written `->!` wherever it touches a buffer's contents (`regex.md` §6); `Regex.hex` owns the surface as pure Hexagon over two sealed rows (`regex.md` §4) | in the runtime module: the storage type `Buffer(a)` and its four keys (`bufferCreate` … `bufferLength`, three of them `->!`), declared through the door's `type` form (intrinsics §3.3), and the two Unicode-table keys `regexPropertyRanges` (UTS #18 RL1.2's properties as ranges) and `regexFoldOrbit` (simple case folding), the tables kept in one place and referenced by name, never inlined into emitted programs; in `Regex.hex`: the confined type key `regexProgram` (`Program`, declarer `Hex.Regex`) and the two `->` sealed rows `regexCompile`/`regexSearch` lowering to the runtime module's compiled bodies under Effects §6.2's species (d) (`regex.md` §7) |
...
emitter. *(Discharged in full, #373: the structural core is `stdlib/Runtime/HashTrie.hex`
...
2. Reduce BigInt's compiler-owned boundary to the genuinely irreducible native
   ...
   **Implemented 2026-07-28**: the form, the gate, and inventory verification are
   ...
   privilege covers it and none happened then; the gate widened at #927 for the
   Regex arc, the case §5.2 anticipated — intrinsics §5.2 records both.)* **Discharged (#344)** — the keys landed with `BigInt.hex`
...
1. primitive constraint declarations and their canonical instances — the declarations half is discharged for the first thirteen (#335): each has canonical prelude source (`Show.hex`, `Num.hex`, `Signed.hex`, `Frac.hex`, `Pow.hex`, `Concat.hex`, `Eq.hex`, `Hash.hex`, `Ord.hex`, `Integral.hex`; the eleventh, `Iterable.hex`, joined at #353; the twelfth, `Real.hex`, now supplies `abs` and `sign`; `FromBigInt.hex` is the thirteenth; `Bitwise.hex`, the fourteenth, is owed by #1029's implementation, `bitwise.md`); the instances half is the companion arc (#344), landed per companion in the fixed order `BigInt`, `Int`+`Nat`, `Float`+`String` — **complete**; Constraints §5.3's wired-row-retirement law governed each landing and no wired instance remains;
...
3. Map/Set algebra over a retained tuned HAMT core — *the core landed as `stdlib/Runtime/HashTrie.hex` (#365), the Map half as `stdlib/Map.hex` (#370, per the sequencing note below), and the Set half as `stdlib/Set.hex` (#373) — **complete***;
...
## Real and Sign addition (2026-09-14)

`Real<a: (Num, Ord)>` and the qualified-only `Sign` union are standard-library
surface. Constraints §7 owns their members and semantics; Primitive Types and
Rat §5 record the five instances. `Float.UndefinedSignError` joins bare scope
through the existing prelude-exception category. The implementation supplies
ordinary source instances and generated library sources; no generic implied-type
extension is required. Canonical conditional chains use multiline `else if`
(Operators §11.3).
