...
| **The bare set is closed at eighteen names** — the prelude's ten exception constructors (including #919's `IntRangeError`); `True`, `False`, `Some`, `None`, `Ok`, `Err`; `ignore`; `show`. Every other prelude export is reached by the dot or its qualified home: no other function, no other union constructor (`Ordering.Less`, `JsKind.Null`), no other constraint member. A listing row spends no bare vocabulary unless it declares an exception; an addition to the set is a design ruling, never a listing entry. `Ordering` is homed at `stdlib/Ordering.hex`; `Prelude.hex` homes `ignore` alone | `modules.md` §5.5 |
...
| Numeric narrowing set (`Int.fromFloat`, kin) | `ffi-part1-boundary.md` §6 ("if included") | checked, `Option`-returning, `Number.isSafeInteger` discipline; `BigInt.toInt` already core. Primitive Types §3's #919 `Float.floor`/`ceil`/`trunc`/`round`/`bankRound` do not discharge this candidate: they explicitly change fractional values under a named rule and throw on an unrepresentable result, while `Int.fromFloat` would accept only an already-safe integer | — | listing: numeric |
...
| ↳ #919 rounding extension | `floor`, `ceil`, `trunc`, school `round`, and ties-to-even `bankRound`, all `Float -> Int`; target-owned `IntRangeError`, safe-integer validation, and canonical zero; discharges these five members of the wider Math-wrapper listing without adding `Int.fromFloat` | three private cores join the inventory: `floatTrunc`, `floatIsSafeInteger`, `floatToIntUnchecked`; direction and tie logic remains in `.hex` |
