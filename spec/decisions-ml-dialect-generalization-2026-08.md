...
| parameterized `export opaque` with a declaration site (`Seq`, and every user type) | the **declared claim** (§6) — used uniformly by every module, the home module included |
...
| **compiler-known parameterized types the compiler implements and owns outright** (`Vector` — no declaration site exists; `Vector.hex` is a companion over the compiler's representation core) | a row in the **compiler-side claim table** (below) — each row carrying §7's parametricity obligation |
| **compiler-known parameterized types that are borrowed foreign views** (`Array`, `Nullable`, `NullableCase`, `JsMap`, `JsSet`, `Node` — FFI Parts 2, 10; `intrinsics.md` §3.3's non-declared boundary types) | **invariant in v1.** Their stability is a boundary contract (FFI Part 1 §3.1, Part 2 §6.2, Part 10 §2), not a language guarantee, and a variance claim may not rest on a contract. A claim for any of them needs its own ruling |

**The compiler-side claim table** *(this ruling establishes it)*: compiler-known parameterized types with no Hexagon declaration site take their claims from a compiler-owned table, each row under the same regime as a written sigil — verified against the representation where one is Hexagon-visible, carried by §7's parametricity obligation where it is not (the implementer is the compiler; the trust argument is `intrinsics.md` §3.4's). Initial rows: **`Vector(+a)`** (the persistent immutable trie — element-transporting throughout), and **transitionally `Seq(+a)`** until §11.4's sweep writes the sigil into `Seq.hex` itself, at which point the row is deleted (a written claim always supersedes a table row for the same constructor; carrying both is a compiler defect). Each self-declaration milestone of `intrinsics.md` §9 retires table rows the same way — when a companion gains a real declaration, its claim moves into the source.
...
| Compiler-owned parameterized types without declaration sites: compiler-side claim table under the §7 obligation (initial rows `Vector(+a)`, transitional `Seq(+a)`); borrowed foreign views invariant in v1; written claims supersede table rows | §5.3 |
