...
*(#927.)* An intrinsic `type` row (`intrinsics.md` §3.3) is an opaque declaration for this rule: its bare parameters are invariant everywhere, its home module included, since no representation exists to infer from; a written sigil is a claim held under §7's obligation, written in source where a trusted claim-table row would otherwise hold it. `Buffer(a)`, mutable, writes none and is invariant; `Node(+a)` at its migration writes the `+` its trusted row carries today.
