...
| **compiler-known parameterized types that are borrowed foreign views** (`Array`, `Nullable` and `NullableCase` — FFI Part 2; `JsMap`, `JsSet` — FFI Part 10) | **invariant in v1.** Their stability is a boundary contract (FFI Part 1 §3.1, Part 2 §6.2, Part 10 §2), not a language guarantee, and a variance claim may not rest on a contract. A claim for any of them needs its own ruling |
...
- **Verified rows** — the representation *is* Hexagon-visible, in a privileged runtime module, and §6.3's machinery checks the row against it exactly as it would a written sigil, re-verifying on every representation edit. **`Vector(+a)` is a verified row**, computed from `runtime/VectorTrie.hex`: given `Node(+a)`, `a` under `Node(a)` is `+`; `Tree(a) = Leaf(values: Node(a)) | Branch(children: Node(Tree(a)))` reaches `+` at the fixpoint; every occurrence in `TrieVector`'s five fields sits at `+` with no arrow-argument position anywhere — so the row is a computation, not an assertion, and a future `a`-in-argument-position field in `VectorTrie.hex` breaks at the row, not silently.
- **Trusted rows** — no Hexagon-visible representation exists, and §7's parametricity obligation is the row's entire warrant (the implementer is the compiler; the trust argument is `intrinsics.md` §3.4's). **`Node(+a)` is a trusted row**: a fixed-32 immutable slot type, read-only from Hexagon, its disposition owned by #126 (the reopener for this row).
...
| Compiler-owned parameterized types without declaration sites: compiler-side claim table in two grades — verified rows checked against visible representations (`Vector(+a)`, computed from `runtime/VectorTrie.hex`), trusted rows on §7's obligation alone (`Node(+a)`, reopener #126) — plus the dated transitional `Seq(+a)` exception; borrowed foreign views invariant in v1; a written sigil supersedes a row; post-sweep, one claim source per constructor is a build-time invariant | §5.3 |
