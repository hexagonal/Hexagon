...
The only source of `−` is function-argument position, because Statements §6.4 leaves the language **no Hexagon-owned mutable type constructor** — the classic source of forced invariance (a readable-writable cell) does not exist in the language. The foreign collections (`Array` — FFI Part 2; `JsMap`, `JsSet` — FFI Part 10) were the exception while they were borrowed views of genuinely mutable foreign objects whose stability was a boundary *contract*, not a language guarantee; since #876 and #875 they are captured and that ground is gone, and §5.3 keeps them invariant for want of a ruling, not for want of stability. The only Hexagon-side source of `±` is a parameter genuinely occurring on both sides of an arrow (`type Endo(a) = a -> a`). The analysis is accordingly small: signs, multiplication, one fixpoint.
...
| **compiler-known parameterized types the compiler implements and owns outright** (`Vector` — no declaration of the *name* exists until `Vector.hex`'s public intrinsic `type` row declares it, #1071, after which its claim is that row's written sigil, checked there against the representation record, `intrinsics.md` §3.3; `Vector.hex` is a companion over the compiler's representation core — and `Node`, the hidden fixed-32 slot intrinsic under it, `stdlib/Runtime/VectorTrie.hex`; `intrinsics.md` §3.3, #223; scheduled to leave the table at `Node`'s migration into the door's `type` form, #927) | a row in the **compiler-side claim table** (below) |
| **compiler-known foreign collections** (`Array` — FFI Part 2; `JsMap`, `JsSet` — FFI Part 10; captured since #876/#875) | **invariant in v1.** Their original ground — stability supplied by a boundary contract rather than a language guarantee — is gone since capture, but its removal supplies no variance claim. A claim for these collections needs its own ruling. At their public intrinsic `type` rows (#1076) the invariance becomes the rows' bare parameters and these table rows are deleted — one claim source per constructor. |
...
This is not a reversal of `intrinsics.md` §4.2's "types are normative in the declaration, not in any compiler-side table": that clause governs constructors that *have* a declaration to be normative in. The table holds claims with no declaration site today. When a companion gains a real declaration, its claim moves into source and its row is deleted. For `Vector`, `Map`, and `Set` that declaration is a public intrinsic `type` row in the companion's door block (`intrinsics.md` §3.3, #1071): the sigil is written there and checked at the row against the computed variance of the runtime record the key's entry names — a declaration-site hard error by §6.3's rule, where the table row was held by a conformance test. `Node` remains until its scheduled migration into an intrinsic `type` row (#927); `Nullable` remains a deliberately compiler-owned boundary type (FFI Part 2 §2.1, #786), so its trusted covariance row survives that migration. The `CollectionOperation` family's retirement at `Node`'s migration is a separate matter and is unchanged.
...
Variance is a property of the **constructor**, fixed once at its declaration — it is not part of any type expression. Use sites, annotations included, always write `Seq(a)`, never `Seq(+a)` (parse error, Preamble §2.1): there is no "dropping" a sigil at an annotation, because no annotation ever carries one. `let x: Seq(a) = y` is therefore an ordinary annotated binding — `a` rigid while the RHS is checked, generalized at the binding (Functions §4.2.1's composition) — and `Seq`'s covariance participates in it not at all. The analysis answers **one question — may this variable be generalized (Functions §8.7's clause (b), and §6.3's verification)** — and lends its sign product to one other client, the constraint seat's comparison, which reads the product as the direction of an ordering over effect colours at each arrow and unifies nothing by it (Effects §13.2); it introduces **no subtyping**: unification remains equality-based, `Seq(Nat)` and `Seq(Int)` remain unrelated types, and no value is ever implicitly coerced between instantiations. (Garrigue's ⊥-subsumption is the *soundness argument* for quantifying, not a coercion the checker performs.)
...
*(#927.)* An intrinsic `type` row (`intrinsics.md` §3.3) is an opaque declaration for this rule: its bare parameters are invariant everywhere, its home module included, since no representation is declared to infer from; a written sigil is a claim held under §7's obligation, written in source where a trusted claim-table row would otherwise hold it — or, where the key's inventory entry names a representation record (`Vector`, `Map`, `Set`; #1071), checked against that record's computed variance at the row by §6.3's rule. `Buffer(a)`, mutable, writes none and is invariant; `Node(+a)` at its migration writes the `+` its trusted row carries today.
...
Declare claims on the parameterized opaque exports that **have declaration sites**: today that is `Seq(+a)` (retiring its transitional table row, §5.3). `Vector` has no declaration to annotate — its claim lives in the compiler-side table until its `intrinsics.md` §9 self-declaration milestone gives it one, a public `type` row (`intrinsics.md` §3.3, #1071), at which point the sigil moves into source and the row dies. The Map/Set family follows the same path at the same milestone (claims computed from their representations, then declared, then reviewed). Sequenced after Step 2 lands in the checker; the `Seq.hex` edit is one sigil and a natural first exercise of §8.2's code action.
