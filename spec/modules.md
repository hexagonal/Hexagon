...
**The alias.** Without `as`, the alias is the module's declared name's **last segment**: `import Geometry` binds `Geometry`, and `import Render.Geometry` binds `Geometry` too — the segments before it are the name's, not the importer's, and never bind (§2.3). With `as`, the alias is the written name: `import Geometry as Geo`, `import Render.Geometry as RenderGeo`. An alias is uppercase-start, mandatorily — a module's name is uppercase-start by the header rule (§2.1), so the default always qualifies, and `import Geometry as geo` is refused with the alias's rule named. The module name in the head is uppercase-start by the same rule, and `import geometry` is a parse error at the head, before any resolution: "a module name is uppercase-start; write `import Geometry`" — the rewrite upper-casing each dot-separated segment at its start and keeping the dots (`import render.geometry` → `import Render.Geometry`), keeping the alias the author wrote (`import geometry as Geo` → `import Geometry as Geo`), and naming the slot, `import <Name>`, where upper-casing yields the spelling back (`import 用户`, `import _x`); a head that trips both rules, `import geometry as geo`, draws the name's report alone, the first seat read. Two imports landing on one alias spelling — `import Render.Geometry` beside `import Physics.Geometry` — are §5.2's one collision, and `as` is its fixit.
...
| Non-uppercase-start module name in an import head (`import geometry`, `import render.geometry`, `import geometry as Geo`) | parse error at the head, before resolution: "a module name is uppercase-start; write `import Geometry`" — each dot-separated segment upper-cased at its start, the dots kept (`import Render.Geometry`), the written alias kept (`import Geometry as Geo`), the slot `import <Name>` where upper-casing yields the spelling back; `import geometry as geo` draws this report alone (§2.1, §3.1) |
...
| Rename (LSP) of a name declared by a module the project does not hold — a prelude member, a dependency's export | refused, the **declaring** module named by its full name and never its file: "`compare` is declared in module `Ord`, which this project does not own"; a module that merely mentions the name is never the one named (§1; Packages §2.5) |
...
import geometry as geo                       -- ERROR (parse): the name's report alone
...
| Module names uppercase-start as types and constraints are; a lowercase import head refused at parse with the segment-wise rewrite, the alias kept, the slot where no uppercase-start form exists; an LSP rename of a name a module outside the project declares refused naming the declaring module | §2.1, §3.1, §1, §10 |
