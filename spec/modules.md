...
| Use of a prelude name above the binder, declaration, or import that shadows or occludes it — bare constructor patterns included | whatever the same shape draws at a user-written name (§5.4 reservation): the Functions §7.2 declared-later error above a binding or declaration (pattern position keeps its own wording, "move the union's declaration above this use"), the failed-reference error above an import — never the prelude's meaning |
