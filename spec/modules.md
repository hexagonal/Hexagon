...
**The rule is local.** The private nominals it reads are the module's **own** — a type declared in another module contributes nothing to this check, whatever its privacy at home *(#629)*. The restraint leaves nothing unguarded: a private type reaches a consumer's face only through some exported carrier or binding of its home module, and that carrier is refused **there**, in the one module that holds the declaration the label points at and can perform the remedy the message names. One defect, one report, at its home.
