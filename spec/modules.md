...
- **Two module aliases with the same name** in one module: hard error at the second import line — the one collision rule an import can trip. (Two aliases onto one module under different spellings are legal, §3.1.) A refused import binds no alias, whatever refused it — nothing resolved, or a contest (§2.3) — so a later import of the same default spelling collides with nothing.
...
| A dotted module whose first segment names a package the declaring package does not see, met at the program (`Bolt` declares `module Acme.Tools`; the program holds `Acme` too) | at the whole-program check: "module `Acme.Tools` of package `Bolt` begins with the name of the package `Acme`, also in this program; drop one of the two, or combine them once `Acme` is renamed or `Bolt` renames its module" (§2.2; Packages §7) |
| The same, the named package the project itself (`Bolt` declares `module MyApp.Tools` under a project named `MyApp`) | "module `MyApp.Tools` of package `Bolt` begins with the name of this project, `MyApp`; rename the project or drop its manifest `name`, drop `Bolt`, or combine them once `Bolt` renames its module" (§2.2) |
...
-- project depending on Acme (module Tools) and Bolt (module Acme.Tools; Bolt lists no Acme):
                                             ...
                                             --   in this program; drop one of the two, or combine
                                             --   them once Acme is renamed or Bolt renames its module (§2.2)
