...
5. **A name at the suffix seat** — against the closing parenthesis of a parenthesised list, `(…)name` — resolves in the **pattern namespace**: the module's own patterns and aliases, and the exported patterns of the modules it imports, own occluding and contests refused at the use; in pattern position, beside it, the expected type's home (Pattern Declarations §3.3). No other namespace is consulted at that seat, and the pattern namespace answers at no other.
