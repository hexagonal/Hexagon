...
- **Two module aliases with the same name** in one module: hard error at the second import line — the one collision rule an import can trip. (Two aliases onto one module under different spellings are legal, §3.1.) A refused import — one that resolves to no module — binds no alias, so a later import of the same default spelling collides with nothing.
