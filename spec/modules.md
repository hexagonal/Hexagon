...
- **The derivation fixit's composition** *(#644)*: where the required constraint is derivable (Constraints §4.5 — the four prelude identities, never a same-spelled user constraint) and the subject is a union or nominal record whose declaration sits in project source with a path to name, the report also carries Constraints §8's derivation fixit in its list-aware, base-complete form, **appended** after the two-home clause. At a **derivable-only** constraint — `Hash`, whose hand-written form every user module refuses (Constraints §4.5) — the fixit does not join the clause but **replaces** it: neither home may lawfully hold the hand-written honor, so the two-home template would offer seats the checker itself refuses. The report says so and names the one seat that works — "`Hash` instances cannot be hand-written, so the only repair is `derives Hash` on the declaration of `Weird` in `./main.hex`" — the offering discipline above generalised: a home is offered only where what the reader would write there is accepted. The same discipline reads one step further at `Hash`: where the subject's `Eq` instance is hand-written, derivation is itself barred (Collections Part 2 §4.3 — a derived hash is only consistent with a derived equality), no repair through the type's own instances exists, and the report says so and names the sanctioned route in the fixit's place — "`Weird`'s hand-written `Eq` bars `derives Hash`; key on a wrapper type with derived instances (Collections Part 2 §4.5)". Subjects with no `derives` seat to edit — primitives, prelude nominals, extern types, structural subjects — draw no fixit and keep their branches unchanged, and the unnameable branch composes with none of this, the derivable constraints being prelude-declared and always nameable.
...
| Missing `C<T>` instance | names the legal homes (§7.6; two, or one if they coincide): "…could only be declared in `./config.hex` (declares `Config`) or the module declaring `Ord`" — prelude- and compiler-supplied homes stated, never offered; structural subjects keep their own messages; unnameable constraint → the declaring module alone (§7.6's unnameable branch); derivable constraint at a project-source nominal → Constraints §8's derivation fixit appended, or replacing the clause at derivable-only `Hash` (§7.6) |
...
| Discoverability: ordinary `C<T>` use brings both legal homes into the graph; residue = inferred-never-named types and isolated-file checking; missing-instance diagnostics name the legal homes, offering only the writable ones, and name the declaring module alone for an unnameable constraint; pre-1.0 LSP names the activating import; effect-import nearly vestigial; packages/re-exports widen the residue | §7.6 |
