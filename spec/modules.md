...
- **Bare seeding is by channel, and the bare set is closed** *(#742)*. Injection into the outermost layer seeds a prelude member's **module alias** (§6.4's qualified home) and its **type names** — its constraint names resolve by pre-registration, not seeding (Constraints §5.1.1) — and, in the term namespace, **nothing by default**. A prelude export reaches a consumer's bare term scope only through one of four channel rules:
  ...
  **A bare reference to a prelude name outside the set is refused with its routes named** (§10). One message shape serves every channel, differing only in the routes it lists: a function names the dot form and every exporter's qualified spelling; a member names the dot form and the declaring module; a constructor names its qualified spelling, in a pattern as in an expression. The message names no import route. It subsumes the collided-name refusal above for every name outside the set — that rule keeps governing the set itself, where nothing collides today. The Rewrite Rule (Declarations Preamble §1.1) is discharged by the message.
...
| Bare reference to a prelude function outside the bare set (§5.5, #742) | ``no bare `empty`; write `Seq.empty`, `Vector.empty`, `Map.empty`, or `Set.empty` `` — every visible exporter's qualified spelling enumerated, in prelude order, and the dot form where the function is dot-callable (``write `xs.map(f)` or `Seq.map(xs, f)`; also exported by `Stream` ``); never an import route |
...
| Bare reference to a collided name in the bare set (§5.5) | ``the prelude name `X` is ambiguous: exported by `A` and `B`; write `A.X` or `B.X` `` — vacuous in the shipped inventory (exception names unique; a member has one exporter, Constraints §4.6), kept for the set |
...
| Repair-family refusal, exactly one workspace exporter of the spelling (LSP) | the compiler-tier message + applied fixit inserting `import module Scale from "./scale"` above the refused use; several exporters → no insert, the candidates named (§5.1) |
