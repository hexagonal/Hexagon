...
| Bare reference to a prelude function outside the bare set (§5.5, #742) | ``no bare `map`; write `xs.map(f)`, `Seq.map(xs, f)`, or `Stream.map(xs, f)` `` — the dot form first where the function is dot-callable (its first parameter is its module's own type, Method Syntax §4.2; the receiver is spelled by the declared parameter name), then **every** visible exporter's qualified spelling, in prelude order, single-homed (``no bare `isNan`; write `x.isNan()` or `Float.isNan(x)` ``) and multi-homed alike; a function that is not dot-callable names the qualified spellings alone (``no bare `fromSeq`; write `String.fromSeq(values)`, `Vector.fromSeq(values)`, …``); never an import route |
