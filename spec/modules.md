...
**The pre-1.0 LSP obligation for this repair family** *(#577; the §7.6 pattern, applied to names rather than instances — the compiler signposts from the graph it sees, the workspace tier applies the edit)*: where a refusal in this family — rule 1's type-not-module, rule 2's nothing-answers arm, and the constraint seats whose rows Constraints §8 sends here — names `import module` as a repair, and the workspace holds **exactly one** module exporting a type or constraint of the refused spelling, the tooling must offer the applied edit inserting `import module Pow from "./pow"` at the import block. More than one exporter: no insert — the message names the candidates instead, the exported inventory driving which repairs are named, as everywhere in this family.
...
| Repair-family refusal, exactly one workspace exporter of the spelling (LSP) | the compiler-tier message + applied fixit inserting `import module Pow from "./pow"` at the import block; several exporters → no insert, the candidates named (§5.1) |
