...
An `import` binds a **module**, and nothing smaller *(#762)*. The first two lines are the one binding form: a **module import**, whose alias gives qualified access to every export (§3.1). The third is the effect import, which binds nothing (§3.3). There is no named import: no `import { area } from "./geometry"`, no `as` alias on an item, no form that lifts one export into the importer's bare scope. A module's exports are reached through the alias, and a name that is wanted bare in the importer is bound by an ordinary declaration (§3.2). Every name's origin is therefore visible at its use site, and the module system carries no import-item machinery — no per-name collisions, no alias start classes, no question of which namespaces an item spans (§9.11 records the rejection and what it buys).
...
`import "./telemetry"` imports nothing and loads the module for its top-level effects (§8.2). It also activates any instances declared in that module (§7), a use §7.6 explains is nearly vestigial in v1. **The form is kept and discouraged.** A pure Hexagon module cannot hold mutable state (§1), so it cannot be a registration point; what a top-level effect can do is print, or call foreign code that mutates — and the idiom for that is an exported function the importer calls, `Telemetry.init()`, which puts the effect where the reader can see it ordered. A module that exists to be loaded is a root module (§8.3), not an import. Workspace-aware tooling may still suggest this import when it finds a lawful instance outside the current graph (§7.6, §10).

The foreign counterpart is `extern import "telemetry/register"` (FFI Part 4 §8): it loads a JavaScript module for effects and introduces no Hexagon bindings. That form is the one genuinely needed — polyfills and registrations live on the JavaScript side — and the `extern` keyword keeps the foreign and Hexagon module graphs visibly distinct.
---
...
Instances are visible program-wide once their module is in the import graph — not imported, not exported, not hidden (Constraints §5.3). `export honor` is the §4.1 error. The effect-import form (§3.3) can pull an otherwise-unreferenced instance home into the graph; §7.6 explains why that use is nearly vestigial in v1.
...
| JavaScript's namespace head `import * as`, the named list `import { … }`, or the former head `import module` | parse error, Rewrite Rule: "Hexagon imports bind modules: write `import Geo from "./geometry"` and reach `area` as `Geo.area`" — the named list's rewrite naming its first listed item and an alias derived from the specifier's basename; the `module` head's rewrite dropping the word (§3.1, #762) |
...
import { area } from "./geometry"            -- ERROR (parse): Hexagon imports bind modules;
                                             --   write import Geo from "./geometry" and
                                             --   reach area as Geo.area
...
let c = Circle(1.0)                          -- ERROR: no bare Circle; write Shape.Circle(1.0)
...
| **An `import` binds a module and nothing smaller** (#762, superseding #565's `import module` head): `import Geo from "./geometry"` — the alias uppercase-start and no value, JavaScript's default-import shape knowingly worn; the effect import kept and discouraged; no named or aliased items; the old heads refused with a rewrite; a bare name in the importer is a declaration (`let`, `type`) or a companion fallback | §3, §9.11–§9.13 |
