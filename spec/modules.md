...
| Missing `C<T>` instance | names the legal homes (§7.6; two, or one if they coincide): "…could only be declared in `./config.hex` (declares `Config`) or the module declaring `Ord`" — prelude- and compiler-supplied homes stated, never offered; structural subjects keep their own messages; unnameable constraint → the declaring module alone (§7.6's unnameable branch) |
