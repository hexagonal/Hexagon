...
| `export record Point = {...}` | `Point` the type **and** `Point` the constructor (the name-carried surface: construction, constructor patterns). Field access, destructuring, and update are type-directed — they travel with the type, not the import (§4.2) |
...
| Transparent representation visibility travels with the type (sole-authority rule): field access, destructuring, and update are import-insensitive; imports carry names (constructor included); no intermediary or per-signature re-abstraction | §4.2 |
