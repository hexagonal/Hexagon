...
- **The name is the module's identity.** `Geometry` above is what every importer writes (§3), what every diagnostic names, and what the orphan rule's "home module" means (§7.2). It is uppercase-start, as module aliases are (§3.1) — the type namespace's own convention, a module name standing beside `Point` and `Ord` in the same case, so a lowercase spelling in an import head is refused with the rule named (§10) — and it may be **dotted** — `module Render.Geometry` — each segment uppercase-start; the dotted name is one name, and its segments are not modules (§2.3).
...
| Non-uppercase-start module name in an import head (`import geometry`) | "a module name is uppercase-start; write `import Geometry`" — the same rule at the name's seat; no module can be declared so (§2.1, §3.1) |
| Rename of a name declared by a module the project does not own — a prelude member, a dependency's export (LSP) | "`Show` is declared in module `Ordering`, which this project does not own" — the declaring module named, never its file (§1) |
