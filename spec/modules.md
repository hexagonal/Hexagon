...
| Private-in-public: hard error for nominal types at every exported face — `export`ed binding signatures, alias targets, record fields, union and exception payloads, never an `opaque` declaration's interior — reported once per type per carrier; a type carrier at the mention's seat, a binding at the binding; transparent aliases exempt (expansion used, and a private expansion refused the same) | §4.3 |
