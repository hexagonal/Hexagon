...
- An exported **constraint** is the family's sixth member, and its only non-carrier *(#626)*: member signatures have no `.d.ts` face (FFI Parts 8–9), so nothing crosses the emitter — but the rationale above never was the emitter's, and it applies to a member signature verbatim: an honor abroad must produce what the signature names, a member call hands it back, and neither party can name or use the type. A private nominal in an exported constraint's member signature is refused at the member, once per (constraint, type) at the first offending member, the label at the private type's declaration as everywhere. **Default bodies are bodies, not faces** — they stay free (a default may use its module's private bindings; Constraints §6.5), as instances are (§7.4). Without the refusal the private type would function abroad as *undeclared* opacity — nameable never, usable only through the members — which is the pattern `opaque` exists to spell honestly (§4.2); the message's "perhaps opaquely" is that exact recovery, and it preserves every lawful use of the shape. (F#'s accessibility-consistency error and OCaml's signature escape check refuse exactly this at exactly this seat; Rust diagnoses it at the same seat.)
- **A private constraint gating an export is lawful — the sealing idiom, stated deliberately** *(#626)*. A private constraint in an exported binding's signature, or as a base constraint of an exported one, crosses nothing: the constraint is the gate, not the cargo. Nothing unnameable lands in a consumer's hands — a call passes with a type the constraint's home honors, or fails resolution by the constraint's own name — and no consumer can honor the constraint at a new type, which is the point: the author keeps the honored set closed, and the internal-organizing pattern — constraint private, public face of plain exported functions — is the same idiom's other face. The family reads private nominal **types** in signatures; a constraint in a binder's constraint list is not a type mention.
