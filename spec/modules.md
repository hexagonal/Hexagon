...
| `Point.` inside `module Point` — the reporting module's own default alias (its declared name's last segment), at any seat, a same-spelled type in scope or not | "a module does not qualify through itself; write `make(1.0, 0.0)`" — the qualifier dropped as the applied edit at each use where the unqualified spelling, in the namespace the seat resolves in, would resolve to the module's own binding, read as that use reads it (no head binder eclipsing it; in the term namespace, Functions §7.2's top-down law with §7.3's `fun`-block visibility included); the sentence alone where it would not; and where the bare spelling is a hard keyword (Lexer §4.4) no rewrite, the sentence naming the dotted route instead — "`Task` is a type, not a module; `then` is reserved, and this module reaches its `then` through a dot — `x.then(…)` on its home type" (§5.1 rule 1; §3.1) |
