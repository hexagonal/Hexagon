...
| Missing `C<T>` instance | names the legal homes (§7.6; two, or one if they coincide): "…could only be declared in `./config` (declares `Config`) or the module declaring `Ord`" — prelude homes stated, never offered; unnameable constraint → the declaring module alone (§7.6's unnameable branch) |
