...
| a declared pattern bare | `import Rat` | the pattern namespace is filled by imports (Pattern Declarations §3.3): `(n, d)rat` in a pattern and `(6, 10)rat` in an expression, no second line. `pattern rgb = Color.rgb` renames — a contest's repair (§3.4). Never `let rat = Rat.rat`: a pattern is not a term |
...
4. **A name at the suffix seat** — after a parenthesised list, `(…)name` — resolves in the **pattern namespace**: the module's own patterns and aliases, and the exported patterns of the modules it imports, own occluding and contests refused at the use; in pattern position, where the namespace has nothing, the expected type's home (Pattern Declarations §3.3). No other namespace is consulted at that seat, and the pattern namespace answers at no other.
