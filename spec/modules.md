...
- **A written dotted name has two readings, and both are matched.** `import Acme.Tools` matches a visible module whose *declared* name is `Acme.Tools`, and a visible package `Acme`'s module `Tools` — its full name; the second reading is taken over the visible packages **other than the resolving package's own**, which never qualifies its own modules (below). Inside one package §2.2's first-segment rule keeps the readings apart, so occlusion and this contest never both apply; across packages they can meet, since a dependency `Bolt` that does not see `Acme` may declare `module Acme.Tools`. Where both readings answer in the resolving package, the import is refused naming both full spellings, the contest rule again — the written spelling is genuinely ambiguous, and neither reading is ranked ahead: "`Acme.Tools` names `Acme`'s module `Tools` and `Bolt`'s module `Acme.Tools`; write `import Tools` for the first and `import Bolt.Acme.Tools` for the second" (Packages §3.3). The first repair is the bare spelling, which the first bullet above resolves to `Acme`'s module alone (Bolt's declared name is longer); where that bare spelling is itself occluded or contested — the resolving package declares its own `Tools`, or a third visible package provides one — no shorter spelling of the first reading exists, and the message's first clause states the block instead ("the first has no shorter spelling here: `Tools` is also provided by `Chroma`"), offering the second repair alone.
- **A package never qualifies its own modules by its own name.** `import MyApp.Geometry` inside the package `MyApp` is the unknown-module error, not a synonym for `import Geometry`: the second reading above excludes the resolving package, and §2.2 refuses a module *declared* `MyApp.Geometry` there, so nothing answers (Packages §2.5). The hint names the plain form: "a package's own modules are imported by their declared names: `import Geometry`".
...
| A dotted import matched under both readings (`import Acme.Tools` where `Acme` has `Tools` and a visible `Bolt` declares `module Acme.Tools`) | "`Acme.Tools` names `Acme`'s module `Tools` and `Bolt`'s module `Acme.Tools`; write `import Tools` for the first and `import Bolt.Acme.Tools` for the second" — where bare `Tools` is itself occluded or contested, the first clause states that block ("the first has no shorter spelling here: `Tools` is also provided by `Chroma`") and only the second repair is offered (§2.3; Packages §3.3) |
| A package qualifying its own module (`import MyApp.Geometry` inside `MyApp`) | unknown module — hint: "a package's own modules are imported by their declared names: `import Geometry`" (§2.3; Packages §2.5) |
...
-- project depending on Acme (module Tools) and Bolt (module Acme.Tools; Bolt does not see Acme):
...
import Tools                                 -- OK: Acme.Tools — Bolt's declared name is longer (§2.3)
import Bolt.Acme.Tools                       -- OK: the full name
