...
- **A miscased header is refused, and the reading recovers as the rewrite spells it.** At the head of a top-level item, `module geometry` is no header by the contextual-word rule above, and no lawful item either, since Hexagon has no juxtaposition; the seat is claimed for the refusal. The line is a parse error under the Rewrite Rule: "a module name is uppercase-start; write `module Geometry`" — each dot-separated segment upper-cased at its start, the dots kept, the slot `module <Name>` where upper-casing yields the spelling back. The rest of the file is then read under the header the rewrite spells, `Geometry`, and §2.2's rules apply to it as to any header, each with its own report and fixit at its own seat: no headerless report follows, since the file has its header; a closer is judged against `Geometry`, so `end module geometry` draws §2.2's closer-naming error, whose rewrite reaches legal code by itself; a second header met while `Geometry` is open draws §2.2's second-header report beside its own; and in a package that also declares `module Geometry`, the duplicate-name report names both. The closer is never a casing seat — its name takes §2.2's rule alone — and below the top level the seat is not claimed either: no header was ever possible there, so there is nothing to redirect to, and `module geometry` in a block is the ordinary parse error.
...
| `module geometry` at the head of a top-level item — no header by the contextual-word rule, no lawful item either | parse error, Rewrite Rule: "a module name is uppercase-start; write `module Geometry`" — segments and slot as the import row; the reading recovers as the rewrite spells it, so no headerless report follows and §2.2's reports (closer naming, second header, duplicate name) fire against `Geometry` as at any header, each with its own fixit; the closer is never a casing seat; below the top level the line is the ordinary parse error (§2.1) |
