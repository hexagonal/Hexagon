...
| Qualified access reaches honored members (#304/#335, uniform access principle): resolution order exports → declared-constraint members → members honored at the module's own type; several-own-types → §5.5-family refusal naming the three routes; member binding qualifiable but never a bare export (one-exporter law); wired-primitive companions transitional until the companion arc *(narrowing per companion since #344 — see the row below)* | §5.3, §10; Constraints §4.6 |
| **A fixed prelude companion's "own type" is its primitive** (#344): qualified access reaches instances honored at the companioned primitive (`BigInt.gcd` = `Integral<BigInt>`'s member); the transitional wired route serves only the not-yet-migrated companions, dying per companion in the fixed order (`BigInt`, `Int`, `Nat` landed; `Float`, `String` remain) | §5.3; Constraints §5.3 |
