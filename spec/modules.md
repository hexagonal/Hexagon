...
- **Instances are global and unfenced.** `honor` ignores the export system entirely: an instance is visible program-wide when its full implementation module is activated in the import graph — never imported, never exported, never hidden (Constraints §5.3). Coherence is checked over the whole graph (§7).
- **Required parts are acyclic, hard error.** Full-module dependencies and selected data dependencies form one acyclic part graph (§8.1; Bare Imports §7). Selecting data can avoid an implementation dependency; it does not legalize recursive modules or ESM initialization cycles.
- **Imports bind module views by declared name.** Ordinary full imports are
  `import Geometry` and `import Geometry as Geo`; the selected data form is
  `import bare Shape from Geometry` (§3.1.1). Both resolve source module identity
  by name, never filesystem path. The latter supplies only the selected data
  view, not a general named-value import or export list. ESM emission retains
  one public entry point per module and may add internal data support (§11).
  The FFI's `extern from` block still declares foreign names from a JavaScript
  specifier; its spelling and semantics are unchanged.
...
An ordinary full import names a **module** and binds its alias, giving qualified
access to its public exports (§3.1). Its head contains the module name and an
optional `as` alias, with no path or `from`. The selected bare-data form in
§3.1.1 uses `from` only to separate a selected declaration from its module.
Neither form provides a named function import, JavaScript-style export list,
or constructor names injected unqualified into term scope.

A full import also supplies the module's exported patterns to the pattern
namespace under the existing suffix rules (Pattern Declarations §3.3); a bare
data selection imports no such patterns. Constructor matching against a known
scrutinee remains separate and unchanged. A name wanted unqualified in an
ordinary consumer can still be declared locally (§3.2), and there remains no
import solely for effects (§3.3).
...
**A module does not qualify through itself.** No alias binds a module's own name inside it — not its declared name, not its default alias — so `Point.make(…)` inside `module Point` is the §5.1 rule 1 report, whose repair is the qualifier dropped where the unqualified spelling resolves to the module's own binding at that use, and a module's own names are spelled bare there (§5.4) — a keyword-named one excepted, whose bare spelling is the keyword, and which the module reaches only by a dot call on its home type, where one exists (Lexer §4.4). An import may bind the spelling to another module (`import Render.Point` inside `module Point` collides with nothing, §5.2), and `Point.` then means what it imports.
...
### 3.1.1 Selected bare data imports

`import bare Shape from Geometry` selects an exported record, union, or type
alias through a restricted view named `Shape`. Its public type and constructor
operations follow the existing fallback, expression, and pattern rules. It
imports no functions, constraints, user-defined patterns, or instances, including
derived instances. The producer's declaration is unchanged. The complete
contract is [Bare Imports](bare-imports.md).

A consumer may select several data declarations from Geometry, but cannot also
write a full import of that resolved module, even under another alias. Full
provider lookup must not bypass this per-consumer restriction. This form is the
specific data-selection exception to the ordinary module import; it provides
no named function import or arbitrary export list. It binds a restricted module
view rather than a freestanding type-left-of-dot meaning.

### 3.2 Names wanted without qualification
...
| a union's constructors bare in a `match` | write them bare | Pattern Matching §2.2: an unqualified constructor pattern resolves in the scrutinee's type where scope has nothing (#763); `Direction.North` fixes the type where the door needs it |
...
| `export pattern rat(top: BigInt, bottom: BigInt): Rat` … | the pattern — its matching direction and, where declared, its building direction (Pattern Declarations §2.3); the head is written in full (§4.1.1); its stable JavaScript/`.d.ts` export is `__patt_rat`, so a term `rat` may be exported alongside it (Pattern Declarations §6) |
...
"write `<a: (Lib.Heft, Ord)>` — `Heft` is declared in module `Lib`, and this
...
1. **`Name.` — uppercase immediately followed by `.`** resolves in the module-alias namespace **first**. Types and constraints never take `.`, so no genuine ambiguity exists; the ordering is stated so the implementation is deterministic. If no module alias `Name` exists, the first question is whether `Name` is the reporting module's own default alias — its declared name's last segment (§3.1), `Point` inside `module Point` and `Geometry` inside `module Render.Geometry`. A module does not qualify through itself (§3.1), a same-spelled type in scope being the companion idiom's ordinary case (§5.3) and `import Point` inside `module Point` being §8.1's cycle, so that seat draws its own sentence ahead of every branch below: "a module does not qualify through itself; write `make(1.0, 0.0)`" — the qualifier dropped, carried as the applied edit at each use where the unqualified spelling, read at that use in the namespace the seat resolves in (a constructor pattern's being the term namespace, §5.4), would resolve to the module's own binding: §5.4's module layer, read as that use reads it (no head binder eclipsing it, Statements §5; in the term namespace, under Functions §7.2's top-down law with §7.3's forward visibility inside a `fun` block included), and the sentence without its repair clause where it would not, since the unqualified spelling would then mean a binder's, the prelude's, a declaration the use does not yet see, or nothing. The type branch below takes the same repair under the same condition, and keeps its own fact: where the type's home is the reporting module, the report is "`Point` is a type, not a module; write `make(1.0, 0.0)`" — never an import of the module into itself. Otherwise the error says so — "no module alias `Rat`; `import Rat`", the same report at every seat the alias reaches (§3.1: term, type, constraint, pattern). Its repair clause is settled in this order, by the tests §2.3 states: where `import Rat` would resolve — the resolving package's own module first, then the one visible package bearing the name (§2.3; Packages §3.2) — the report names the import as its repair and carries the applied edit (below); where that import would be contested (Packages §3.3), the report names every full spelling as its repair and carries no edit; where it would resolve to nothing, the same report without its repair clause, no module invented. In term position an alias the module binds lower down is not this case but §3's declared-later report; at the other seats the alias binds and nothing is refused (§3). The `widens` head's member qualifier is not one of these seats: it keeps Constraints §8's own row (Constraints §4.7). A type in scope of the spelling takes the type branch that follows instead (its home being another module — the reporting module's own types keep the type's fact and take the drop above), mentioning the type: "`Shape` is a type, not a module; `import Geometry` and qualify through it" — the module import being the one route (§3), and the compiler naming it whole: the type's home module is known by name to the checker that reached the type, and a module import carries no path (#829; the applied edit, below). The `.` token remains the one from Operators §10 — field access and module path, resolved by what the left side names; a module alias on the left makes it a module path.
2. **`Name` in type position** resolves in the type namespace **first**. Where the type namespace has nothing for the spelling, and a module alias `Name` is visible whose module exports a type `Name`, the reference resolves to that exported type — **the companion fallback** *(#531; §5.3's idiom is what it exists for)*. The fallback answers only where resolution had already failed, and the alias still binds nothing in the type namespace: a same-spelled type declaration — or a `type` alias of the importer's own — wins outright everywhere, with no collision and no refusal. The compiler-owned boundary types (`Array`, `Nullable`, the hidden `Node` — §5.5's parenthetical) stay **last**: they answer only after declarations *and* after the companion fallback, which is §5.5's own sentence applied — no resolution claim of the compiler's outranks a user's declaration, and the companion fallback resolves to one, reached through the user's own import. Conservativity is exact everywhere but at the boundary spellings: every program that resolved without the fallback resolves identically under it, **except** where a module imported under a boundary type's name and exporting a same-spelled type now means the user's type — the reading whose absence the old resolution's own error marked as confusion (the same-name-both-sides tell, `expected Array(Int), found Array(Int)`). At `Array` and `Nullable` the exception is reachable in any module; at `Node`, only in a runtime-privileged one — the one place the spelling answers at all (intrinsics §5.2's `runtime` flag, gating the §3.3 fallback family) — because shipped runtime source holds no import line binding a boundary type's name (`Hex.Runtime.Regex`, when the arc lands it, will import `Regex.Syntax` — `regex.md` §6.1 — an import, but not one under `Node`'s spelling), but a source explicitly supplied by the compiler host as a trusted replacement for a registered runtime module (intrinsics §5.2) may, and the same exception applies there. (`JsMap` and `JsSet` are compiler-owned too but stand outside the carve: they answer ahead of the companion fallback, exactly as `Vector`, `Map`, and `Set` do. Through the prelude route the ordering is unobservable at all three companions, since `stdlib/JsMap.hex`/`stdlib/JsSet.hex` — like `stdlib/Array.hex` — export no type of the alias's spelling.) A visible alias whose module exports no type of the alias's own spelling adds nothing: resolution proceeds to whatever answered before it (at the boundary spellings, the boundary fallback — where it reaches), and where nothing answers, bare `Name` in type position is an error naming the working repairs (`P.Point`; realias as `import Point`; or `type Point = P.Point`). **`Alias.Name` in type position** resolves `Name` in the *exported type namespace* of the aliased module. **Constraint position carries the same fallback** *(extended in the #531 ruling)* — everywhere an unqualified name resolves in the constraint namespace: a binder's constraint list and an `honor` head alike. **`Name`** resolves in the constraint namespace first; where it has nothing and a module alias `Name` is visible whose module exports a constraint `Name`, the reference resolves to that exported constraint — answering, never binding, every property above holding one namespace over, and with conservativity exact and uncarved: the thirteen pre-registered constraint names are always present (Constraints §5.1.1), so the fallback never fires at them, and no compiler-owned constraint fallback exists. The fallback carries **no members** — a constraint's members are reached through the alias (`D.describe(x)`) or by the dot (Constraints §2.2), so member spellings stay free everywhere, and the generalisation law's route (§5.3: to widen a member, import the module) is the only route there is. **`Alias.Name`** resolves in the aliased module's *exported constraint namespace* (§3.1).
...
**The applied-edit obligation for this repair family** *(#577; respelled #829)*: where a refusal in this family — rule 1's plain unbound alias and its type-not-module branch, rule 5's suffix refusals that name an import (Pattern Declarations §3.3, §7), and the constraint seats whose rows Constraints §8 sends here — names a module import as a repair, the report carries the applied edit inserting `import Scale`, placed so the module stays well-formed — below the module's own header in a multi-module file — and any term-position use sits below it (§3: term references read top-down; type and constraint names are order-insensitive, and a doc comment keeps its declaration). One edit per module, however many seats draw the report: every seat carries the same insert, the same range and the same text, so a host applying any set of them at once applies one per module, and a host recomputing after one finds the line present and offers nothing. A refused import head that offers the same line by its own rewrite (§3.1, `import geometry`) is that line already offered, and the seats below it carry none. Before #829 this obligation sat with the LSP's workspace tier, because a module import carried a path the compiler tier could not know and the workspace had to find the one exporter; a module import now names a module (§3.1), the checker knows the declaring module of every type and constraint it has reached by name, and the edit is the compiler's own. Where the refused spelling is a name the module has *not* reached — an unknown type the workspace may export from several modules — the tooling names the candidates, the exported inventory driving which repairs are named, as everywhere in this family; several exporters draw no insert.
...
The prelude is a **fixed, ordered availability schedule over ordinary `.hex` source modules and their required data/full parts** (canonical source under `stdlib/`) — the modules of the standard-library package `Hex` that every program sees without an import (Packages §2.4), checked through the same data/full machinery as project modules and injected into the outermost scope layer of §5.4. Multiple availability seats for one source share its original declaration identities. Each has a header like any module (`module Option`, §2.1), and its full name is `Hex.Option` (§2.3): the import `import Hex.Option as Opt` binds a second alias onto it (§3.1), and a project's own `module Option` occludes the unqualified name for its importers while `Hex.Option` stays reachable (§2.3). Four rules make it a set rather than a heap:

- **Ordered intra-prelude visibility.** Each checked prelude part sees the availability entries **before its own seat, and only those**. A data entry grants definitions and public constructors; a full entry additionally grants the implementation. Option data precedes Seq and full Option follows Seq (Bare Imports §9); Seq precedes the collection modules that convert to it. The same declaration identities are shared across the seats. Provider edges remain subject to part-level cycle checking; ambient name availability alone is not a runtime load.
- **Ordinary prelude access is implicit; bare selection is explicit.** Prelude modules use earlier names implicitly. The new `import bare Type from Module` form is permitted to state a restricted data dependency; Seq writes `import bare Option from Option`. No general ordinary-import exemption is introduced. This is deliberate pedagogy: the stdlib is read as exemplary source, and an `import Option` line at the top of a prelude module would teach every reader a false lesson about the language. *(This bullet also prescribed a **header comment** naming the implicitly-scoped prelude names — ``// `Option`, `Some`, and `None` are implicitly in scope via the prelude.`` — as the house form. **Withdrawn 2026-08-01**, James's direction, and the comments deleted with it. It restated, once per file, what this very section guarantees for every prelude module, and it named three of the prelude's names as though the rest were different. A reader who does not know that prelude names are in scope does not learn it from that list; a reader who does, reads it as noise. That historical removal of header comments stands; the bare-import exception above is the later, explicit change to import syntax.)*
- **A collided unqualified name is refused.** Two visible prelude members may export the same term name — the collection vocabulary is shared by design (Collections Part 1 §3.1: `Seq.length` and `Vector.length` are both correct spellings) — and where both spellings are in the unqualified set (the next rule), neither owns the bare one: an unqualified reference to a name that two or more *visible* members export is an error naming every qualified home (``write `Seq.empty`, `Vector.empty`, `Map.empty`, or `Set.empty` `` — four homes since `Set.hex` joined, #373) — the ML answer, where `List.map` and `Seq.map` coexist and the use site qualifies. The collision set follows visibility, so a prelude member whose visible predecessors export a name once keeps the unqualified spelling even while consumers must qualify it. Everything §5.4 grants is untouched: a module's own declaration occludes the whole prelude layer, collided or not — `let map = Seq.map` is how a module takes a prelude spelling bare, and it is a declaration, not an import (§3.2). Dot calls are the other unaffected spelling — dispatch is type-directed (Method Syntax §4.2) and never reads the unqualified-name layer, so `values.length()` needs no qualifier however many members export `length`. The rejected alternative — the seeding order silently deciding a winner — would re-mean every bare `empty` in every existing program each time a member joins the prelude: a language-wide edit made by a list order.
- **Bare seeding is by channel, and the unqualified set is closed** *(#742)*. Injection into the outermost layer seeds a prelude member's **module alias** (§6.4's qualified home) and its **type names** — its constraint names resolve by pre-registration, not seeding (Constraints §5.1.1) — and, in the term namespace, **nothing by default** — and in the pattern namespace nothing at all: a prelude module's patterns enter it by an explicit import of that module, as any module's do (Pattern Declarations §3.3). A prelude export reaches a consumer's unqualified term scope only through one of four channel rules:
  - **Functions: none.** Every prelude function is reached qualified (`Seq.map(xs, f)`, `Float.isNan(x)`, `Debug.log(value)`) or, where its first parameter is the receiver's type, by the dot (`xs.map(f)` — Method Syntax §4.2 reads the export set and never this layer). One survivor, on its own ground: `ignore` — Statements §3.2's discard diagnostic names the unqualified spelling as its rewrite (§3.3 declares the function), and nobody declares the word.
...
Exported nominal names are reachable through a module alias — qualified, or bare by the companion fallback (§5.1) — and identity is declaration-site (§2). Exported aliases remain transparent everywhere. Type parameters and arities travel with the declaration. A `derives` clause remains on the authoritative source declaration, but its instances activate only with the full implementation; a bare data selection retains the type identity and imports no instances (Bare Imports §5).
...
Instances are visible program-wide once their full implementation module is active in the import graph — not individually imported, exported, or hidden (Constraints §5.3). Selecting a bare data view activates no instance. A consumer with bare access to a source module cannot use its full providers even when another consumer activates them; the candidate remains unique and the dependency is refused (Bare Imports §5/§7). `export honor` is the §4.1 error. No form loads a module for its instances alone (§3.3); §7.6 explains why none is needed.
...
A bare-mode refusal takes precedence over missing-instance repair advice. If
the unique provider exists but this consumer has bare access to its source,
name that provider and advise replacing the bare selections with a full import
(Bare Imports §7). Neither the ordinary missing-provider template nor the LSP's
activation hint may suggest adding a conflicting full import beside them.
...
- **The ordinary full-import case brings both legal homes along.** Source that names `C<T>` resolves `C` through the constraint's home and `T` through the type's home. With no v1 re-exports, both modules are therefore already in the import graph (or are the same module), and the one lawful instance arrives without a separate import.
...
A cycle among required parts is a **hard error** naming the participating
module parts and the source uses that require them. Ordinary full imports retain
their cycle refusal, including imports used only for types. The explicit bare
form can instead require selected data without the full implementation:
`Option full -> Seq full -> Option data` is acyclic. A later implicit function
or instance edge that requires the missing full part is not excused (Bare
Imports §7). Same-module recursive data declarations keep their existing rules;
a cross-module data cycle remains an error.

The emitted runtime graph must also be acyclic. This retains deterministic
initialization and avoids ESM's partially initialized bindings. Whole source
validation is distinct from runtime activation; unused explicit full-import
cycles are not optimized away (Bare Imports §10).


### 8.2 Top-level effects and load order

Non-binding `Unit`-typed expressions are **legal at module top level** (`print("loaded")` — JS-style, per the existing block rules: Statements §3.2 polices non-`Unit` discards there exactly as in any block). Load order is fixed: **a module's imports are loaded depth-first in source order, each required emitted part exactly once, before the full module's own top level runs** — a selected bare import initializes only its data support, a full import initializes its data support and full implementation, each emitted unit once (Bare Imports §8); ESM's order minus the cycle cases, well-defined because §8.1 bans those. Within a module, top-level items run in source order (type-namespace declarations are order-insensitive per their specs; term bindings — and value-position uses of constructors and constraint members — are read top-down, Functions §7.2; `let`s and effects run in order, with instance dictionaries emitted ahead of them all, Constraints §6.3).
...
4. **`Shape(..)` import sugar** (Haskell): the module import covers the want, and Pattern Matching §2.2's scrutinee-type door gives `match` arms their unqualified constructors with nothing entering scope (§9.11, #763).
...
| A dotted import resolving to nothing whose first segment names an installed Hexagon package the resolving package does not list (`import Acme.Tools` where `Acme` is `Bolt`'s dependency, or installed and listed by nobody — installed in Packages §4.1's sense, answering the resolving package's own lookup; a copy only `Bolt`'s nested `node_modules` holds draws the unknown-module report) | "`Acme` is not a dependency of this package; add `"Acme"` to `dependencies` in `hexagon.json`" — applied edit where the host can write the manifest; where a module of any package in the program, imported or not, is declared under that first segment, the unknown-module report fires instead and no edit is offered (§2.3; Packages §3.3, §3.4, §7) |
...
| `Point.` inside `module Point` — the reporting module's own default alias (its declared name's last segment), at any seat, a same-spelled type in scope or not | "a module does not qualify through itself; write `make(1.0, 0.0)`" — the qualifier dropped as the applied edit at each use where the unqualified spelling, in the namespace the seat resolves in, would resolve to the module's own binding, read as that use reads it (no head binder eclipsing it; in the term namespace, Functions §7.2's top-down law with §7.3's `fun`-block visibility included); the sentence alone where it would not; and where that spelling — the module's own binding — is a hard keyword (Lexer §4.4) no rewrite, the sentence naming the route instead in Lexer §10's words — "`Task` is a type, not a module; `then` is reserved; a declaration named `then` is reached only through a dot, from an importer or on its home type" (§5.1 rule 1; §3.1) |
| `Rat.` where no module alias `Rat` is bound, `Rat` is not the reporting module's own alias, and no type of the spelling is in scope — every position the alias reaches, term, type, constraint, and pattern (there the scrutinee-type door reads bare heads alone, Pattern Matching §2.2) | the same report at every seat: "no module alias `Rat`; `import Rat`" — the repair clause by §5.1 rule 1's tests in order: `import Rat` would resolve (the resolving package's own first) → the import named, the applied edit from the compiler tier, one edit per module; contested → every full spelling named, no edit; nothing → the report without its repair clause; in term position an alias bound lower down → §3's declared-later report instead; the `widens` head's member qualifier keeps Constraints §8's row (§5.1 rule 1) |
| `Point.` where `Point` is a type whose home is the reporting module and is not the module's own alias (`Point.make(…)` inside `module Geometry`; inside `module Point` the own-alias row takes it) | "`Point` is a type, not a module; write `make(1.0, 0.0)`" — the type branch's fact with the own-alias row's repair and condition; never an import of the module into itself (§5.1 rule 1) |
| `Name.` where `Name` is a type, not a module, whose home is another module and not spelled like the reporting module's own alias (the reporting module's own types take the row above; its own alias the own-alias row) | "`Shape` is a type, not a module; `import Geometry` and qualify through it" — the type's home named, the applied edit carried (§5.1) |
| Bare reference to a prelude function outside the unqualified set (§5.5, #742) | at a call, the routes are spelled with **the call's own arguments** — for `map(things, f)`: ``no bare `map`; write `things.map(f)`, `Seq.map(things, f)`, or `Stream.map(things, f)` `` — the dot form first where the function is dot-callable (its first parameter is its module's own type, Method Syntax §4.2), then **every** visible exporter's qualified spelling, in prelude order, single-homed (for `isNan(reading)`: ``no bare `isNan`; write `reading.isNan()` or `Float.isNan(reading)` ``) and multi-homed alike, with no elision; a function that is not dot-callable names the qualified spellings alone (for `fromSeq(pairs)` — first parameter `Seq`-headed, and `Seq.hex` exports no `fromSeq`: ``no bare `fromSeq`; write `Vector.fromSeq(pairs)`, `String.fromSeq(pairs)`, `Map.fromSeq(pairs)`, `Set.fromSeq(pairs)`, `Stream.fromSeq(pairs)`, `JsMap.fromSeq(pairs)`, or `JsSet.fromSeq(pairs)` ``); at a reference that is not a call, the qualified names alone (``no bare `empty`; write `Seq.empty`, `Vector.empty`, `Map.empty`, or `Set.empty` ``); the message invents no identifier the program does not contain, and never names an import route; a receiver the grammar would misread is parenthesised (for `div(-7, 2)`: ``no bare `div`; write `(-7).div(2)` or `Integral.div(-7, 2)` ``) |
| Bare reference to a prelude constraint member outside the unqualified set (§5.5) | ``no bare `compare`; write `a.compare(b)` or `Ord.compare(a, b)` `` — the dot form where subject-first, the declaring module always |
...
| Bare reference to a collided name in the unqualified set (§5.5) | ``the prelude name `X` is ambiguous: exported by `A` and `B`; write `A.X` or `B.X` `` — vacuous in the shipped inventory (exception names unique; the `show` seat is identity-keyed, so no second member ever enters unqualified scope), kept for the set |
...
let area = Point.area                        -- an unqualified name is a declaration (§3.2)
...
-- (o) Headers and closers (§2.1, §2.2; the strangers' unbound alias, self-qualification and the own-type carve, §5.1)
...
export fun unit(): Geometry.Point = ...      -- ERROR: no module alias Geometry; import Geometry
                                             ...
                                             --   the applied edit inserts import Geometry below
                                             --   the module Shapes header, §5.1)
...
| A qualified use through an alias nothing binds draws the same report in every position, "no module alias `X`; `import X`", with the compiler-tier applied edit where the import would resolve, one edit per module; a module's own default alias tested first and drawing its own sentence, the qualifier dropped as the applied edit at each use where the unqualified spelling, in the namespace the seat resolves in, would resolve to the module's own binding read as that use reads it (Statements §5; in the term namespace, Functions §7.2–§7.3) — its own sentence's home §3.1, §5.3 — the reporting module's own types taking the same repair and condition under the type branch's fact; the repair clause then settled in order by §2.3's tests (resolves; contested; nothing), the `widens` head's member qualifier excepted; the type branch ahead of the plain report where a type of the spelling is in scope; in term position the declared-later report for an alias bound lower down | §5.1 rule 1, §10, §13(o) |
...
| Prelude = ordered availability of ordinary `.hex` module parts; each part sees its earlier prefix; explicit bare data imports are permitted in prelude source (the header-comment convention that accompanied that rule is **withdrawn 2026-08-01**); type-declaring members ordinary; compiler resolution never outranks declarations (boundary intrinsics = fallback only) | §5.5 |
...
| Required module parts acyclic, hard error; ordinary full-import and cross-module data cycles refused; deterministic depth-first load order; top-level `Unit` effects legal; selected root module runs through ordinary ESM evaluation; no special `main` | §8 |
