...
**Companions:** Constraints spec (§5.1 duplicate reporting point; §5.3 orphan rule; §9.3 structural instances), Statements/Blocks/Mutability spec (§5.2/§10.2 prelude collisions; §6 `var` confinement), Declarations Preamble (§7 declaration inventory; Rewrite Rule §1.1), Unions §2 and Pattern Matching §2.2 (constructor qualification; the pattern-position constructor door, #763), Functions §8 (module-level generalisation), Operators §14 (`.` as module-path separator), Method Syntax §4/§8 (companion dispatch and emitted companion imports), Lexer & Layout (module top level as a block), and FFI Parts 4/7 (extern bindings and export surface).

---

## 1. Doctrine

- **A module is a file; a file is a module.** There is no `module` header declaration and no in-file submodule. A module's identity is its path; its name, where one is needed, is chosen by the *importer* (module alias, §3.1). One compilation unit per file, mirroring ESM exactly.
...
- **Imports bind modules; emission is ESM's own (#762).** The *export* surface is JavaScript's own, and one Hexagon module emits as one ESM module with `export` mapping to `export`. The *import* surface is not: an `import` binds a module and nothing smaller (§3) — no named items, no `as` aliases, no form that lifts one export into bare scope — so every name's origin is visible at its use site, and the module system carries no import-item machinery at all (§9.11 records the rejection and what the absence buys). The one binding form is spelled `import Geo from "./geometry"`, which is JavaScript's default-import shape and is not one: Hexagon has no default exports (§9.5), the alias is a module and no value (§3.1), and the emitted JavaScript never carries the source shape (§11.2). A JavaScript reader learns that Hexagon imports are different though spelt the same; the alias's mandatory uppercase start is the visible tell. The import section was once the language's most JS-identical surface, an on-ramp spent knowingly and in two steps — the namespace head first (#565), the rest here — while the on-ramp survives whole in emission, which remains JavaScript's own forms throughout (§11). The FFI's `extern from` block is untouched by any of this: it *declares* foreign names with their types (FFI Part 4) and is no import, which is why foreign names arrive bare and Hexagon names arrive qualified.
...
An `import` binds a **module**, and nothing smaller *(#762)*. Both lines are the one form: a **module import**, whose alias gives qualified access to every export (§3.1). There is no named import: no `import { area } from "./geometry"`, no `as` alias on an item, no form that lifts one export into the importer's bare scope. And there is no effect import: a module is imported for its names, never loaded for its effects (§3.3). A module's exports are reached through the alias, and a name that is wanted bare in the importer is bound by an ordinary declaration (§3.2). Every name's origin is therefore visible at its use site, and the module system carries no import-item machinery — no per-name collisions, no alias start classes, no question of which namespaces an item spans (§9.11 records the rejection and what it buys).

Imports are module-level declarations; an `import` inside a function body joins the Declarations Preamble §7.1 error family. Placement within the file is grammatically free, and the formatter will float imports to the top — but no import exemption from the reading laws exists: a module alias is a binding in the module-alias namespace (§5.1), **order-insensitive** in type and constraint position and in a pattern, and read **top-down** in term position (Functions §7.2) — `Geo.area(r)` above the import that binds `Geo` is the declared-later error with the import-shaped fixit, "move the import above this use", exactly as a use of any term binding above its declaration.

### 3.1 The module import

`import Geo from "./geometry"` binds the single name `Geo` as a **module alias** giving qualified access to every export: `Geo.area(...)`, and in type position `Geo.Point`, `xs: Vector(Geo.Shape)`. Constructors qualify the same way (`Geo.Circle(1.0)`), including in patterns (`match s` / `Geo.Circle(r) => ...`) (Unions §2). Constraints too: `Geo.Ord` in a binder (`<a: Geo.Ord>`) and in an `honor` head (`honor Geo.Describe<Box>` — Constraints §4.1), and a member through the alias as an ordinary term (`Geo.compare(a, b)`) — the left side is the module alias in every case, so §5.1's "types and constraints never take `.`" is untouched: it governs what may stand *left* of the dot. Module aliases are uppercase-start, mandatorily. Two reaches extend the alias without adding a binding, both §5.1's companion fallbacks: where the aliased module exports a type or a constraint spelled like the alias itself, the bare reference in that namespace's own position may resolve through it (rule 2), and where it exports a **constructor** spelled like the alias, the bare constructor in term position — an expression or a pattern — may resolve through it (rule 3, #763). Each answers only where the namespace has nothing.

**The head's grammar.** `import`, then the alias, then `from` and the specifier string (§2). The alias seat is uppercase-start, which settles every degenerate spelling by start class alone: `import geometry from "./geometry"` is refused as any non-uppercase-start alias would be, with the alias's rule named. Two heads a JavaScript author's muscle memory produces are **refused with a rewrite** under the Rewrite Rule (Declarations Preamble §1.1): the namespace glob `import * as Geo from "./geometry"` and the named list `import { area } from "./geometry"`, each a parse error naming the module form. For the named list: "Hexagon imports bind modules: write `import Geometry from "./geometry"` and reach `area` as `Geometry.area`" — its first listed item named, and the alias derived from the specifier's basename, each `-`, `_`, or `.`-separated segment upper-cased at its start and joined (`geometry` → `Geometry`, `search-params` → `SearchParams`); where the derivation yields no uppercase-start identifier, the message names the slot instead — `import <Alias> from "./2d-utils"`. For the glob head, the same sentence ending at the rewrite, which keeps the alias it wrote (`Geo`) — no item clause, since the program names no item. A targeted redirect at the exact place the habit fires, and the only positions the `*` and `{` glyphs ever stood outside an operand seat (Comments §3.1's adjacency argument simplifies accordingly). The head `import module Geo from "./geometry"`, this form's earlier spelling *(#565)*, is refused the same way, its rewrite dropping the word.

**Spelled like JavaScript's default import; not one.** `import Geo from "./geometry"` is byte-for-byte the shape JavaScript gives a default import. Hexagon has no default exports (§9.5) and the alias is no value (below): the spelling is the language's own, chosen because the package design's path-free form is `import Geometry` (§12.1) and the v1 form should be that form plus its path — a word between `import` and the alias would outlive its reason. A JavaScript reader learns that Hexagon imports are different though spelt the same; the alias's mandatory uppercase start — JavaScript's default imports are conventionally lowercase — is the visible tell, and the emitted JavaScript (§11.2) never carries the source shape.
...
**Two aliases onto one module** (`import Geo from "./geometry"` and `import Geometry from "./geometry"` in one file) are legal and name one module; nothing is duplicated, and §5.2's one collision rule concerns two aliases of one *spelling*.
...
| a type bare, under the alias's own name | `import Point from "./point"` | §5.1 rule 2's companion fallback — the companion idiom (§5.3), no second line |
| a type bare, under another name | `type Point = Geo.Point` | a transparent alias (§6.2): the same type, one spelling |
| a nominal record's constructor bare | `import Point from "./point"` | §5.1 rule 3's companion fallback: `Point({x = 1.0, y = 2.0})`, and `Point({x, y})` in a pattern, through the alias |
| a union constructor bare, under the alias's own name | `import Tag from "./tag"` | §5.1 rule 3's companion fallback: `Tag(7)`, expression and pattern alike |
...
**A Hexagon module is imported for its names, never loaded for its effects.** There is no `import "./telemetry"` *(#762)*. A pure Hexagon module cannot hold mutable state (§1), so it cannot be a registration point: what a top-level effect can do is print, or call foreign code that mutates, and the idiom for that is an exported function the importer calls — `Telemetry.init()` — which puts the effect where the reader can see it ordered. Top-level effects themselves stay legal (§8.2) and run when the module is loaded for its names, or when it is the root (§8.3): a module that exists to be *loaded* is a root module, not an import. Instances need no loading form either — naming `C<T>` brings both legal homes into the graph (§7.6). The refusal names the form the author was reaching for: "Hexagon has no effect import; a module is imported for its names — `import Telemetry from "./telemetry"` — or run as a root". An alias the file never uses is lawful, and the module loads as any import's does (§11.3): what is gone is the spelling that binds nothing, not the load.
...
module binds another `Heft`; `import Lib from "./lib"` and spell it
...
**The pre-1.0 LSP obligation for this repair family** *(#577; the §7.6 pattern, applied to names rather than instances — the compiler signposts from the graph it sees, the workspace tier applies the edit)*: where a refusal in this family — rule 1's type-not-module and the constraint seats whose rows Constraints §8 sends here — names a module import as a repair, and the workspace holds **exactly one** module exporting a type or constraint of the refused spelling, the tooling must offer the applied edit inserting `import Scale from "./scale"`, placed so the file stays well-formed and any term-position use sits below it (§3: term references read top-down; type and constraint names are order-insensitive, and a doc comment keeps its declaration). More than one exporter: no insert — the message names the candidates instead, the exported inventory driving which repairs are named, as everywhere in this family.

### 5.2 Collisions

- **Two module aliases with the same name** in one module: hard error at the second import line — the one collision rule an import can trip. (Two aliases onto one module under different spellings are legal, §3.1.)
...
The prelude is a **fixed, ordered list of ordinary `.hex` modules** (canonical source under `stdlib/`), compiled like any project module and injected into the outermost scope layer of §5.4. Four rules make it a set rather than a heap:
...
- **Bare seeding is by channel, and the bare set is closed** *(#742)*. Injection into the outermost layer seeds a prelude member's **module alias** (§6.4's qualified home) and its **type names** — its constraint names resolve by pre-registration, not seeding (Constraints §5.1.1) — and, in the term namespace, **nothing by default**. A prelude export reaches a consumer's bare term scope only through one of four channel rules:
...
- **Loading a module for its instances alone is unnecessary in v1** as a consequence of the first bullet, and no form exists for it (§3.3): the activating import the LSP names is the ordinary module import, whose alias the file may then never use. Future re-exports or packages could make a loading form wanted by separating names in scope from their instance homes; the package design inherits the question (§12.1).
...
| Opaque field access / construction outside home module | "`Point` is opaque outside `./point`; use its exported functions" — a constructor *pattern* abroad takes Pattern Matching §2.4's destructure sentence instead |
...
| Workspace instance outside the current graph (LSP) | existing Constraints §8 error + hint: "its instance is in `./x`; `import X from \"./x\"` brings it into the program" (§7.6, §3.3) |
| An effect import, `import "./x"` | parse error naming the route: "Hexagon has no effect import; a module is imported for its names — `import X from "./x"` — or run as a root" (§3.3) |
| Repair-family refusal, exactly one workspace exporter of the spelling (LSP) | the compiler-tier message + applied fixit inserting `import Scale from "./scale"` above the refused use; several exporters → no insert, the candidates named (§5.1) |
...
| Missing `C<T>` instance | names the legal homes (§7.6; two, or one if they coincide): "…could only be declared in `./config.hex` (declares `Config`) or the module declaring `Ord`" — prelude- and compiler-supplied homes stated, never offered; structural subjects keep their own messages; unnameable constraint → the declaring module alone (§7.6's unnameable branch); derivable constraint at a project-source nominal → Constraints §8's derivation fixit appended, or replacing the clause at derivable-only `Hash`; a hand-written `Eq` there → the wrapper-key report (§7.6) |
...
import { area } from "./geometry"            -- ERROR (parse): Hexagon imports bind modules;
                                             --   write import Geometry from "./geometry" and
                                             --   reach area as Geometry.area

-- (b) The companion fallbacks, both namespaces (§5.1 rules 2 and 3)
-- point.hex: export record Point = {x: Float, y: Float}
...
import Shape from "./rect"                   -- ERROR: module alias Shape already bound
import Circle from "./circle"
import Rect from "./rect"                    -- OK: the importer names
...
-- config.hex: export record Config = ...; honor Ord<Config> = ...
import Config from "./config"                -- the type's home is in the graph
...
import Mid from "./mid"                      -- Crate never imported here, in any form
...
-- shape.hex: export union Shape = Circle(radius: Float) | Rect(width: Float, height: Float)
...
-- (n) The old heads, refused with the rewrite
import module Geo from "./geometry"          -- ERROR (parse): … write import Geo from "./geometry"
import * as Geo from "./geometry"            -- ERROR (parse): same rewrite, the written alias
                                             --   kept; no item clause
import { area } from "./geometry"            -- ERROR (parse): … write import Geometry from
                                             --   "./geometry" and reach area as Geometry.area
import geometry from "./geometry"            -- ERROR: a module alias is uppercase-start
```
...
| **An `import` binds a module and nothing smaller** (#762, superseding #565's `import module` head): `import Geo from "./geometry"` — the alias uppercase-start and no value, JavaScript's default-import shape knowingly worn; no effect import — a module is imported for its names, never loaded for its effects; no named or aliased items; the old heads refused with a rewrite; a bare name in the importer is a declaration (`let`, `type`) or a companion fallback | §3, §9.11–§9.13 |
