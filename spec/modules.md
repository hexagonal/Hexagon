...
| Bare reference to a collided prelude name (§5.5) | ``the prelude name `empty` is ambiguous: exported by `Seq`, `Vector`, and `Map`; write `Seq.empty`, `Vector.empty`, or `Map.empty` `` — every visible home enumerated, in prelude order *(third home since `Map.hex`, #370)* |
