...
4. A module alias in **term position** other than the left of `.` is the "modules are not values" error (§3.3). Type and constraint positions are rule 2's to govern: the companion fallback where it answers, the boundary fallback after it (type position, at `Array` and `Nullable` — §5.5), and the refusal with its named repairs where nothing answers.
