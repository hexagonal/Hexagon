...
**A module does not qualify through itself.** No alias binds a module's own name inside it — not its declared name, not its default alias — so `Point.make(…)` inside `module Point` is the §5.1 rule 1 report, whose repair is the qualifier dropped where the unqualified spelling resolves to the module's own binding at that use, and a module's own names are spelled bare there (§5.4). An import may bind the spelling to another module (`import Render.Point` inside `module Point` collides with nothing, §5.2), and `Point.` then means what it imports.
...
| `Point.` inside `module Point` — the reporting module's own default alias (its declared name's last segment), at any seat, a same-spelled type in scope or not | "a module does not qualify through itself; write `make(1.0, 0.0)`" — the qualifier dropped as the applied edit at each use where the unqualified spelling, in the namespace the seat resolves in, would resolve to the module's own binding, read as that use reads it (no head binder eclipsing it; in the term namespace, Functions §7.2's top-down law with §7.3's `fun`-block visibility included); the sentence alone where it would not (§5.1 rule 1; §3.1) |
