...
| a declared pattern bare | `pattern rat = Rat.rat` | a pattern alias (Pattern Declarations §3.4): the same pattern — view and build — under this module's spelling; in a pattern the expected type's door (Pattern Declarations §3.3) usually makes the line unnecessary, and `Rat.rat(n, d)` is always the qualified spelling. Never `let rat = Rat.rat`: patterns are not values (Pattern Declarations §2.4) |
...
| `export pattern rat = Rat.rat` | the same pattern under this module's spelling (Pattern Declarations §3.4) |
...
match p                                      -- ERROR: cannot destructure opaque record Point;
    Point(r) => ...                          --   use an operation exported by its home module
                                             ...
                                             --   the sentence names it — Pattern Declarations §3.3)
