...
| `Dec` | exact for non-negative exponents, retaining the base's decimal places multiplied by the exponent; exponent zero returns `1d`. Every negative exponent throws `NegativeExponentError`; a retained-place product outside Nat throws `DecimalPlacesOverflowError` before coefficient exponentiation (`dec.md` §4) | ordinary source member in `stdlib/Dec.hex` (implemented locally; validation status in `dec.md`) |
