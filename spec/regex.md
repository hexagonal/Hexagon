...
- `findAll` is the lazy sequence of §3.5's enumeration: pure, re-derived on each traversal (Loops §6.4), `Seq.memoize`d by the caller who wants retention, and each element is produced by one search from §3.5's resumption position, with the cursor the previous search answered (§7) carried beside it in the sequence's state — the two differ by one after an empty match — so that no resumption re-walks the subject. Nothing is scanned until the first pull.
...
- **Explicit ranges are normalized**: within a `Class`, `Range` items are sorted, disjoint, and merged, and precede the property items, which keep written order. Property items are left by name; their sets, and the negation and folding of a class that holds one, are the engine's work (§6.1), because they need tables. A class with no property items is fully determined in the tree.
...
| `Regex` opaque over `Program`, no `Eq`/`Hash`, `Show` = source; `Match` opaque — `whole`, `groups`, `names` — holding its `Group`s with texts cut by the engine, eager cut accepted, constant-time accessors; `Group` transparent | §4.1, §4.5 |
