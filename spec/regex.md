...
| quantifier without an atom | the quantifier | nothing to repeat before `*` — and, after an assertion or a flag setting: `^` is an assertion, not an atom; a well-formed `{3}` with nothing before it takes this row, not the `{` row |
...
- `findAll` is the lazy sequence of §3.5's enumeration: pure, re-derived on each traversal (Loops §6.4), `Seq.memoize`d by the caller who wants retention, and each element is produced by one search from the previous match's resumption point, the search row's cursor (§7) carried in the sequence's state so that no resumption re-walks the subject. Nothing is scanned until the first pull.
...
- `groups` is the record's `groups` field: groups 1 through `groupCount` in order — `[]` for a pattern with no capturing group — `Vector`-indexed from 1 like every vector, so `groups(m)[i]` and `group(m, i)` agree. Every accessor is a constant-time read of the record (§6.4): `start`, `length`, `text`, and `group(m, 0)` read `whole`; `group(m, i)` and `named` read `groups`.
...
The six declarations and `parse` — a qualified-API signature in §4's sense, not declaration syntax — are the module's whole exported surface. `Property.value` is the canonical value spelling for `GeneralCategory`, `Script`, `ScriptExtensions`, and `Binary`, and `""` for `Any`, `Ascii`, and `Assigned`, which the engine's table answers by kind alone (§7).

- **Flags are resolved in the tree.** `x` is consumed by the scanner the parser drives (§2.2); `m` selects `LineStart`/`LineEnd` over `TextStart`/`TextEnd` for `^`/`$`; `s` sets `Any`'s `newline`; `i` sets `folded` on a `Class` and turns a literal under it into a one-range `Class` with `folded = True`. No flag survives into a node, so the engine reads no flags.
...
`Regex.hex`'s `searchProgram` row (§7) lowers to this module's `search(program: Compiled, text: String, start: Int, cursor: (Int, Int), spans: Bool) ->! …`, which answers the leftmost-first match at or after the start position — every group's span and text when `spans` is set, and with it clear only whether there is one — together with a resumption cursor:
...
- **The cursor.** Locating codepoint position `start` in a JavaScript string is a walk from the string's start, `O(start)`, which would make an enumeration of `k` matches quadratic. The row therefore takes and answers a **cursor** — a `(position, code-unit offset)` pair the engine has verified for this subject — and walks to `start` from the cursor where the cursor's position is at or before `start`, from the string's start otherwise. The cursor answered is for the whole match's end position; the surface carries it, unread, in the enumeration's state and hands it back with the next `start` (§4.3), so a traversal walks the subject once. `(1, 0)` is the cursor for every subject's start. The row is unexported and the surface its only caller, so a cursor is trusted as `Buffer` bounds are (§7): under runtime discipline, never checked.
...
| `Match` accessors | constant, every one: a `Match` holds its `Group`s with their texts already cut (§4.1) |
...
- **A spans-only search mode** for `replace` with a literal and for `split`, which read no group text: the growth path if §4.1's eager cut bites; one more value of the row's mode, no new key.
