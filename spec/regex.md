...
- `regexProgram`'s inventory entry names **`Hex.Regex`** as its one declarer: a `Program` is constructed and inspected only by the two rows, travels only inside the opaque `Regex` (§4.1), and is the carrier through which a value over `Buffer` storage — a compiled program with its cache — reaches a program without the storage being addressable anywhere but the runtime module. The type has arity 0. The foundations declared one confined type key, `buffer`; this is a second, and it meets Intrinsics §3.3's **confinement bar** (#930): never addressable outside its declarer, with no constructor, pattern, or derived instance — what `Map` and `Vector` cannot have and `Program` has. That the engine keeps its `Compiled` record (§6.1) behind it settles nothing, as `HashTrie` behind `Map` settles nothing: no module names it, no row exposes it.
- `compileProgram` and `searchProgram` write `->`, and their lowerings are the runtime module's compiled `->!` bodies of §6.1 and §6.2: the linkage `Map`'s keys take over `Hex.Runtime.HashTrie`, with the colour crossing species (d) licenses. Each is a value function of its arguments, and the two discharge it differently. `searchProgram` by construction for its own scratch (the VM's thread lists and capture vectors, §6.3) and by the transparency of the two memos §6.2 names — and, for its cursor, by the independence §6.2 states: over the row's domain the answer does not depend on which valid cursor is passed. `compileProgram` by the third shape, **frozen storage** (Effects §6.2, Intrinsics §4.2 — the shape this document added to the species): the buffers it allocates it fills before it returns, and no lowering writes them afterwards, its two memos excepted on the memo limb's terms, so every later read is a read of a value, and the fresh identity each call hands out is unobservable through any row — `Regex.hex`'s block declares no row that compares, hashes, or renders a `Program`, and a confined type has no pattern and derives nothing (Intrinsics §3.3), foreign code's view of an exported `Regex`'s representation is Effects §6.2's existing caveat, and `searchProgram`, the only row that inspects a program, answers from nothing but the `Pattern` the program was built from. That is why `Buffer.create` is `->!` while this row is `->`: a fresh buffer's identity is observable, a write to one and a read of the other telling them apart; a fresh program's is not, because nothing writes its frozen part. Confinement rules out interference from every module but the declarer and the lowering the door links: `Hex.Regex` is `regexProgram`'s one declarer, and `Hex.Runtime.Regex`, which handles the same values as `Compiled`, is the lowering the door names, its buffers confined in turn under `buffer`, whose declarer it is. The engine's obligation is the one sentence Effects §6.2 states, specialised: never write a frozen program again. Each takes no function-typed parameter, so nothing re-enters the engine mid-write. `searchProgram`'s answer, with `spans` set, is the spans — element 1 the whole match, always `Some`, and element `i + 1` group `i`, each `(start, length, text)` or `None`, from which the surface builds a `Match` once (§4.1) — paired with the cursor for the match's end (§6.2); with `spans` clear it is `Some` of the empty vector and the cursor it was given where a match exists, and `None` otherwise — unambiguous, since a spans answer always has its element 1.
...
| `regexProgram`, a second confined type key beyond the foundations' `buffer`, declarer `Hex.Regex`, under Intrinsics §3.3's confinement bar (#930): the engine's unexported `Compiled` record is the door's linkage, as `HashTrie` is `Map`'s, not a representation of the row's type | §7, Intrinsics §3.3 |
| Species (d) gains a third shape, frozen storage — filled before the row returns, never written again, its memos excepted — beside scratch and memo; `compileProgram` is its first member, its fresh identity unobservable through any row; Effects §6.2 and Intrinsics §4.2 carry the shape | §7, Effects §6.2, Intrinsics §4.2 |
