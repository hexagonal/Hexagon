...
- `compileProgram` and `searchProgram` write `->`, and their lowerings are the runtime module's compiled `->!` bodies of §6.1 and §6.2: the linkage `Map`'s keys take over `Hex.Runtime.HashTrie`, with the colour crossing species (d) licenses. Each is a value function of its arguments, and the two discharge it differently. `searchProgram` by the transparency of the two memos §6.2 names, the memo limb — and, for its cursor, by the independence §6.2 states: over the row's domain the answer does not depend on which valid cursor is passed. `compileProgram` by the third shape, **frozen storage** (Effects §6.2, Intrinsics §4.2 — the shape this document added to the species): the buffers it allocates it fills before it returns, and no lowering writes them afterwards, its two memos excepted on the memo limb's terms, so every later read is a read of a value, and the fresh identity each call hands out is unobservable through any row — a confined type has no `Eq`, `Ord`, `Hash`, or `Show`, no pattern, no derived instance (Intrinsics §3.3), foreign code's view of an exported `Regex`'s representation is Effects §6.2's existing caveat, and `searchProgram`, the only row that inspects a program, answers from nothing but the `Pattern` the program was built from. That is why `Buffer.create` is `->!` while this row is `->`: a fresh buffer's identity is observable, a write to one and a read of the other telling them apart; a fresh program's is not, because nothing writes its frozen part. Confinement rules out interference from every module but the two the door links — `Hex.Regex` names `Program`, `Hex.Runtime.Regex` handles the same values as `Compiled` — and the engine's obligation is the one sentence Effects §6.2 states: never write a frozen program again. Each takes no function-typed parameter, so nothing re-enters the engine mid-write. `searchProgram`'s answer, with `spans` set, is the spans — element 1 the whole match, always `Some`, and element `i + 1` group `i`, each `(start, length, text)` or `None`, from which the surface builds a `Match` once (§4.1) — paired with the cursor for the match's end (§6.2); with `spans` clear it is `Some` of the empty vector and the cursor it was given where a match exists, and `None` otherwise — unambiguous, since a spans answer always has its element 1.
