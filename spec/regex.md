...
`literal` is any codepoint that is not a metacharacter at its position; `punct` is any codepoint that is not an ASCII letter or digit (§2.3); `hex` is a hexadecimal digit, one to six of them (§2.3); `property` is §2.4's `\p`/`\P` forms. The grammar reads a stream from which `x`-mode scanning (§2.6) has already removed ignored whitespace and comments; that scanning is driven by the parser's current flag state — it takes effect immediately after the `)` of a `(?x)` setting or at the `:` of a `(?x:…)` group and ends where the flag's scope ends — so it is a mode of the parser, not a pass before it. A counted repetition and a `\u{…}` escape are read whole: `a{1, 3}` and `\u{6 1}` are refused under `x` as under any flag (§2.5, §2.9). An anchor and a flag setting are not atoms: a quantifier after one (`^*`, `\b+`, `(?i)*`) is refused as having nothing to repeat (§2.5).
...
The metacharacters — the codepoints that do not stand for themselves outside a class — are `\ . + * ? ( ) | [ { ^ $`. `]` and `}` outside their constructs are literals; `{` is not (§2.5). Every other codepoint is a literal matching itself. Under `x` (§2.6), `#` and every `White_Space` codepoint join them outside a class. Inside a class the metacharacters are `\ ] [ ^ -` (§2.4) — `^` only first, `-` except first, last, or escaped, so `[-a]`, `[a-]`, and `[a^b]` read the hyphen and the caret as literals.
...
- `[` inside a class is refused unless escaped — there are no nested classes and no POSIX classes, and `[[:alpha:]]` is refused at its inner `[` with `\p{Alphabetic}` named as the spelling (§2.9); this is one of the dialect's refusals of something RE2 accepts (§2.8 collects them), taken to keep `[[:alpha:]]` from reading silently as a class followed by a literal;
...
| `\p{Any}`, `\p{ASCII}`, `\p{Assigned}` | every codepoint; U+0000–U+007F; every codepoint whose General_Category is not `Cn`, surrogates excluded (§7) |

The names are Unicode's canonical long names and Unicode's own aliases: every alias PropertyAliases.txt gives an admitted *property* (`WSpace`, `White_Space`, and `space` all name White_Space), only the short and long value names — the second and third fields — of PropertyValueAliases.txt for a *value* (so `\p{digit}`, `\p{punct}`, and `\p{cntrl}`, its later fields, are out), and `Any`, `ASCII`, `Assigned` as UTS #18 spells them — matched **exactly** — case, underscores, and spacing as Unicode writes them. There is no loose matching (RL1.2a is not adopted): `\p{lu}`, `\p{uppercase letter}`, and `\p{Greek_}` are refused, each naming the canonical spelling. A bare name is looked up in one order — as a General_Category value, then a Script value, then one of the binary properties and `Any`/`ASCII`/`Assigned` of the table above; the three name spaces are disjoint over the admitted set, so the order settles determinism rather than a contest. `=` and `:` are both accepted between a property name and its value. `\P{…}` is the complement of `\p{…}`; the `\p{^Greek}` spelling is refused. The set is **closed**: no other property, no property of strings, no `=Yes`/`=No` form on the binary properties. A property joins by ruling and by table together — `\p{Emoji}` is the likely first request — never by a name the parser happens to accept.

A **lone surrogate** has no properties (§3.6): every `\p{…}` naming a property excludes it — `\p{Any}` alone includes it, being the whole codepoint space rather than a property — and every `\P{…}` naming a property, every negated class, and `.` include it (`\P{Any}` is empty). Since a well-formed pair is decoded to one scalar and a lone surrogate has no category, no codepoint the engine sees is a surrogate by category: `\p{Cs}` is admitted — the ruling admits General_Category verbatim — and matches nothing, `\P{Cs}` matches everything, and `\p{C}` reaches its other members.
...
**Flags are inline only.** `(?flags)` sets flags for the rest of the enclosing group (or the pattern) — to its closing `)`, across `|`, so that in `a(?i)b|c` the `c` is folded, as in RE2 and Rust; `(?flags:…)` sets them for the group's body alone. `flags` is a sequence of the letters below, optionally followed by `-` and letters to clear, each letter at most once across both sides (`(?i-i)` is refused) — `(?i)`, `(?-i)`, `(?i-s:…)`. `(?)` and `(?i-)` are refused. There is no flags argument to `compile` (§4.2) and no other flag: `U` (RE2's ungreedy) is refused by name.
...
| `s` | `.` matches `\n` too (§2.7) |
| `x` | extended: outside a class, White_Space is ignored and `#` begins a comment that runs to the next `\n` or the end of the pattern; inside a class both are literal (Perl's and Python's `/x`, not Rust's, which strips inside a class too — §11); the `x` scanner (§2.2) honours the backslash, so an escaped `White_Space` codepoint is kept and `\#` begins no comment, and `\ ` is a literal space anywhere |
...
- **Readings RE2 gives that this dialect refuses rather than misreads**, collected: a bare `{` and `a{,3}` (literal text in RE2); `[` inside a class; a range ending in a class escape or a range (`[\d-z]`, `[a-z-0]`); `\p{^Greek}`; `\x41` and `\x{…}` (write `\u{…}`); `\Q…\E`; `\C`; `\a`, `\f`, `\v`; the `U` flag; a quantifier on an assertion. Each is refused where it is met (§2.3–§2.6) with the spelling the dialect has.
...
| name without `>` | the `<` | group name is not closed; write `(?<year>…)` |
...
| reversed counted repetition | the `{` | `{3,1}` has a minimum above its maximum |
...
| bad flags | the offending letter, else the `)` | `U` is not a flag; the flags are `i`, `m`, `s`, `x`; `i` is set twice; nothing follows `-`; `(?)` sets no flag |
| bad name | the name | group name must match `[A-Za-z_][A-Za-z0-9_]*` |
| duplicate name | the second name | group `year` is already defined |

One report per pattern, at the first fault in reading order; the parser does not recover and go on. Two routings the table cannot carry: a well-formed `{3}` with nothing before it takes the nothing-to-repeat row, not the `{` row; `[[:` takes the `[`-inside-a-class row with its POSIX clause, never the missing-`]` row; and `(?)` takes the bad-flags row, not the bad-prefix row.

## 3. Matching semantics

### 3.1 Unit and positions

The subject is read as a sequence of codepoints — a well-formed surrogate pair is one codepoint, a lone surrogate is one codepoint — and every span is a **1-based codepoint start position** and a **codepoint count**. `"😀hello"` matched by `hello` has start 2 and length 5, exactly as `String.indexOf` answers `Some(2)` (`string-text-processing.md` §8). An empty match has length 0 at a boundary position: before the first codepoint at 1, after the last at `length + 1`. No code unit reaches the public surface: not a span, not a position argument, not a `RegexError`. The one code-unit offset outside the engine is the second component of the search row's resumption cursor (§6.2, §7), inside `Regex.hex`, written by the surface only as `(1, 0)` and otherwise carried back to the row uninterpreted.
...
Under `i` (§2.6) two codepoints match when their **simple case foldings** are equal — Unicode's `CaseFolding.txt` statuses C and S, UTS #18 RL1.5 — and every literal and class is closed under that relation once, before any codepoint is matched — at the program's first use, §6.1 — never per codepoint at match time: `(?i)k` matches `k`, `K`, and U+212A KELVIN SIGN; `(?i)[a-z]` matches `a`–`z`, `A`–`Z`, U+212A, and U+017F LATIN SMALL LETTER LONG S (whose simple fold is `s`), and nothing else; `(?i)ß` matches `ß` and U+1E9E and not `ss`. The unit stays one codepoint, which is what lets a class be a set of codepoints and the DFA's alphabet a partition of them.
...
Signatures below are the normative qualified API, not declaration syntax. Every function whose first parameter is a `Regex` or a `Match` is dot-callable on it by Method Syntax §4.2's home-module rule — `Regex`, `Match`, and `Group` all live in `Regex.hex`, so `regex.find(text)` and `match.group(1)` both resolve there; `compile` and `escape` take a `String` first and are written qualified, `Regex.compile(p)`. Inside `Regex.hex` the opaque records' fields are visible, so the fused `r.source()`, `r.groupCount()`, and `m.groups()` are Method Syntax §6's field/method collisions against the same-spelled exports — the module writes its bare calls internally; abroad no field is visible and no collision arises. `Regex.hex` imports `Regex.Syntax` under its default alias `Syntax`, and the blocks below write `Syntax.RegexError` as a consumer must (§4.1). Every function is `->` except the two whose replacement callback makes them conduits (§4.4).

### 4.1 Types

```
opaque record Regex = { source: String, groupCount: Int, names: Map(String, Int), program: Program }
opaque record Match = { whole: Group, groups: Vector(Option(Group)), names: Map(String, Int) }
export record Group derives (Eq, Show) = { start: Int, length: Int, text: String }
```

- **`Regex`** is a compiled pattern: a value, immutable, sharable, with no `Eq`, `Ord`, or `Hash` instance — two regexes are compared by `source`. It carries a **`Program`**, the confined type its door row declares (§7), and inside that the two memos the program owns, the lazy-DFA cache and the resolved classes (§6.2); the record is `opaque` because a confined type may travel only inside an opaque carrier (Intrinsics §3.3), and it derives nothing for the same reason. `Show<Regex>` is honored by hand and shows the source.
- **`Match`** is one match against one subject: group 0 as `whole`, every capturing group as a `Group` — its span and its text — or `None` where it did not participate, and the name table. The texts are cut by the engine, which holds the subject's code-unit offsets while it has them (§6.2, §7), so no accessor ever walks the subject, and the surface builds the record once from the row's answer (§7). The cost, accepted: every group's text is cut and retained for every match, read or not — `g` strings per `Match` for a pattern with `g` groups — and a `replace` with a literal replacement pays it too; §10 names the growth path if it bites. The record is opaque so that its face is the accessors of §4.5 and its layout is the library's to change. No instances in v1.
- **`Group`** is a captured span with its text, transparent, the value every accessor answers with. `RegexError` is §2.9's, home `Regex.Syntax`: a program that only matches on it needs no second import, and one that *names* it in a signature writes `import Regex.Syntax` and `Syntax.RegexError` — the cost of one parser owning its error and Hexagon having no re-exports (Modules §12, question 2), taken with eyes open (§4.6).

### 4.2 Compilation

```
Regex.compile(pattern: String): Result(Regex, Syntax.RegexError)
...
- `find` is the leftmost-first match (§3.2), or `None`. `isMatch` answers whether `find` would answer `Some`, asking the search row for no spans (§7) so that the engine runs the forward scan alone (§6.2).
...
- `findAll` is the lazy sequence of §3.5's enumeration: pure, re-derived on each traversal (Loops §6.4), `Seq.memoize`d by the caller who wants retention, and each element is produced by one search from §3.5's resumption position — two where §3.5 discards an abutting empty match — with the cursor the previous search answered (§7) carried beside it in the sequence's state — the two differ by one after an empty match — so that no resumption re-walks the subject. Nothing is scanned until the first pull.
...
- `start`, `length`, `text` are group 0's — the whole match. There is no `end`: the position after the match is `start + length`, and `findFrom(re, text, m.start() + m.length())` is not the enumeration (§3.5 skips an abutting empty match; `findAll` is the enumeration).
...
- `groups` is the record's `groups` field: groups 1 through `groupCount` in order — `[]` for a pattern with no capturing group — `Vector`-indexed from 1 like every vector, so for `1 ≤ i ≤ groupCount` `groups(m)[i]` and `group(m, i)` agree; outside that range they part on purpose — `group` is total, 0 answering the whole match and anything else `None`, while the bracket throws `IndexError` like any vector's and `Vector.get` is the total sibling. No accessor walks the subject (§6.4): `start`, `length`, `text`, and `group(m, 0)` are field reads of `whole`; `group(m, i)` and `groups` are one vector read; `named` is one name-table lookup and then that read.
...
- **No re-export of `RegexError`** — the type's home is the parser's module (§2.9, §4.1); a signature naming it imports `Regex.Syntax`. A copy in `Regex.hex` would be a second type, and re-exports are deferred corpus-wide (Modules §12, question 2).
...
export record Pattern = { root: Expr, groupCount: Int, names: Map(String, Int) }
...
The six declarations and `parse` — a qualified-API signature in §4's sense, not declaration syntax — are the module's whole exported surface. The tree is `Expr`, not `Node`: `Node` is the spelling of the compiler-owned boundary type that answers bare in a runtime-privileged module (Modules §5.5, Intrinsics §3.3), and `Hex.Runtime.Regex` is one and imports this module (§6.1) — the one module that would see both. `Dot` rather than `Any`, since `PropertyKind` has an `Any` and two unions of one module may not share a constructor (Unions §2). `Property.value` is the canonical value spelling for `GeneralCategory`, `Script`, `ScriptExtensions`, and `Binary`, and `""` for `Any`, `Ascii`, and `Assigned`, which the engine's table answers by kind alone (§7).

- **Flags are resolved in the tree.** `x` is consumed by the scanner the parser drives (§2.2); `m` selects `LineStart`/`LineEnd` over `TextStart`/`TextEnd` for `^`/`$`; `s` sets `Dot`'s `newline`; `i` sets `folded` on a `Class` and turns a literal under it into a one-range `Class` with `folded = True`. No flag survives into a node, so the engine reads no flags.
- **Perl classes desugar**: `\d` is `Class([Property(GeneralCategory, "Nd", negated = False)], negated = False, folded = …)` — `folded` per the `i` flag in scope at the construct —, `\s` likewise over `Property(Binary, "White_Space")`, `\w` over the five items of §2.4's table with `Range(0x200C, 0x200D)` standing for Join_Control; `\D`, `\S`, `\W` are the same items in a `Class` with `negated = True`; inside a written class each contributes its items. A non-capturing group `(?:…)` has no node of its own: it is its body. Property values are stored under **one canonical spelling per value** whatever alias was written — General_Category under its short name (`Property(GeneralCategory, "Lu")` for `\p{Uppercase_Letter}`), Script and Script_Extensions under their long names (`Property(Script, "Greek")` for `\p{Grek}`), binary properties under their long names — so the engine's tables are keyed once. A bare `\P{…}` is `Class([Property(…, negated = False)], negated = True, folded = …)`; `Property.negated` is set only for a complemented item written *inside* a class, `[\P{L}\d]`, so every construct has one tree.
- **Explicit ranges are normalized**: within a `Class`, `Range` items are sorted, disjoint, and merged, and precede the property items, which keep written order. Property items are left by name; their sets, and the negation and folding of a class that holds one, are the engine's work (§6.1), because they need tables. The tree fixes a class's written ranges and its `negated`/`folded` flags; item complement, union, folding, and class complement are the engine's for every class, because folding needs the fold key and the complement's range is the engine's (§6.1).
...
`Regex.hex`'s `searchProgram` row (§7) lowers to this module's `search(program: Compiled, text: String, start: Int, cursor: (Int, Int), spans: Bool) ->! Option((Vector(Option((Int, Int, String))), (Int, Int)))` — the row's result type of §7, with `Compiled` for `Program` —, which answers the leftmost-first match at or after the start position — every group's span and text when `spans` is set, and with it clear only whether there is one — together with a resumption cursor:

- **The lazy DFA** is built on demand from the NFA — a state is a *priority-ordered* set of NFA states, so that the forward scan finds the end of the leftmost-first match, not the longest (§3.2). The forward scan from the start position finds the match end; a reverse DFA over the reversed program, anchored at that end, finds the match start; where the program has no capturing groups the search ends there, no capture pass run; and with `spans` clear (`isMatch`, §4.3) the search ends at the forward scan's first match end, neither the reverse scan nor the VM run — RE2's existence test. The texts the row answers are cut here, from the code-unit offsets the scan has in hand, so a `Match` carries them and never walks the subject (§4.1).
...
- **The cache** is the program's owned memo (Effects §6.2 species (d)): every DFA state is a value function of the NFA state set it names, a hit and a miss are indistinguishable, and nothing in it records call history. It is bounded by a constant the engine fixes; exhaustion discards it and a later search rebuilds what it needs. One more transparent memo is admitted on the same terms, two in all: the resolved classes and alphabet of §6.1, a value function of the program. There is deliberately no memo of the subject: a memo keyed on the last subject would pin that string for the life of the `Regex`, and a literal pattern's `Regex` lives as long as its module (§8).
- **The cursor.** Locating codepoint position `start` in a JavaScript string is a walk from the string's start, `O(start)`, which would make an enumeration of `k` matches quadratic. The row therefore takes and answers a **cursor** — a `(position, code-unit offset)` pair — and walks to `start` from the cursor where the cursor's position is at or before `start`, from the string's start otherwise. With `spans` set the cursor answered is for the whole match's end position, and with it clear the row echoes the cursor it was given (§7); the surface carries it, unread, in the enumeration's state and hands it back with the next `start` (§4.3), so a traversal walks the subject once. **The row is specified only where `cursor` is `(1, 0)` or a cursor this row answered for this same `text`.** Over that domain the answer does not depend on which such cursor is passed — the cursor shortens the walk to `start` and decides nothing else — so the row is a value function of `(program, text, start, spans)` and the cursor is a hint; `(1, 0)` is valid for every subject, which is what lets `find`, `findFrom`, and `isMatch` pass it whatever `start` the caller chose. Passing any other cursor is a conformance violation of `Regex.hex`, as a buffer index out of range is one of the engine's (§7): never checked, and never reachable from outside `Regex.hex`, the row being unexported. No amendment rides on this: whatever cursor is passed the row is deterministic in its five arguments, so species (d)'s value-function obligation holds of the row as declared; the precondition governs only whether the surface's answer is the match §3.2 defines.
...
| `isMatch`, `find`, `findFrom` | `O(n)` on the DFA route, amortised over the cache, for `isMatch` or a program with no capturing groups; `O(n + span · m)` with capturing groups, the VM running over the span alone; `O(n · m)` on the VM route; never worse, whatever the pattern; plus the texts cut for `find` and `findFrom` (§4.1), and for `findFrom` a walk from the string's start to `start` |
| `findAll` over a whole subject | linear in the subject, plus the sum of the match lengths (each times `m` where the program has capturing groups), plus the texts cut — the cursor (§6.2) keeps each resumption from re-walking the subject, so the subject is walked once per traversal |
| `Match` accessors | field reads of `whole`, one vector read for a group, one name-table lookup for `named`; none walks the subject, the texts having been cut (§4.1) |
| `replace` family, `split` | one enumeration plus the output — the unmatched runs assembled in one pass over the subject's codepoints (`String.toSeq`, cut at the match boundaries, joined once), never by per-match slicing, which would re-walk the subject from its start at every match |
...
extern from "hex:intrinsic"
    ...
    fun regexPropertyRanges as propertyRanges(kind: Syntax.PropertyKind, value: String) -> Vector((Int, Int))   -- surrogates excluded
...
- `propertyRanges` answers a property's codepoints as sorted, disjoint, inclusive ranges, keyed by §5's `PropertyKind` and canonical value (the runtime module imports `Regex.Syntax` for the `Pattern` it compiles, so the kind is the tree's own); the tables are the compiler's Unicode 17.0.0 tables, the ones `String.hex`'s rows read, kept in one place and **referenced by name, never inlined** into an emitted program (§8). **Every property's ranges exclude U+D800–U+DFFF** — the table carves the surrogate block out of `C`, `Cs`, `Assigned`, and every other property, so that no `\p{…}` naming a property matches a lone surrogate (§2.4, §3.6) — and `Any` alone answers U+0000–U+10FFFF whole. An unknown name is unreachable — the parser closed the set — and answers the empty vector rather than a fault.
...
extern from "hex:intrinsic"
    ...
    fun regexSearch as searchProgram(program: Program, text: String, start: Int, cursor: (Int, Int), spans: Bool) -> Option((Vector(Option((Int, Int, String))), (Int, Int)))
```

- `regexProgram`'s inventory entry names **`Hex.Regex`** as its one declarer: a `Program` is constructed and inspected only by the two rows, travels only inside the opaque `Regex` (§4.1), and is the carrier through which a value over `Buffer` storage — a compiled program with its cache — reaches a program without the storage being addressable anywhere but the runtime module. The type has arity 0. *Amendment for ratification: the foundations declared one confined type key, `buffer`; this is a second, and it reads Intrinsics §3.3's bar — "no Hexagon representation at all" — as confinement: what a `type` row buys is the first two of the three properties §3.3 lists — never addressable outside its declarers; no constructor, pattern, or derived instance — which `Map` and `Vector` cannot have and `Program` has; the engine's `Compiled` record (§6.1) is the representation the door links to, exactly as `HashTrie` is `Map`'s, and is a representation of the row's type no more than `HashTrie` is a Hexagon representation of `Map` — no module names it, no row exposes it. Edit note: on ratification Intrinsics §3.3's bar sentence — "a `type` row is for a type that has no Hexagon representation at all, not for one that has a representation the door happens to implement" — is replaced by: "A `type` row is for a type the program cannot address — never addressable outside the modules its inventory entry names, with no constructor, pattern, or derived instance — whatever record a runtime module keeps behind it. The four collection companions' types are not that: they are addressed in exported signatures everywhere." On the same ratification §3.3's declarer list and §4.1's type-key list gain `regexProgram`, which this document alone names until then. Until then this document governs the point (README rule 4).*
- `compileProgram` and `searchProgram` write `->`, and their lowerings are the runtime module's compiled `->!` bodies of §6.1 and §6.2: the linkage `Map`'s keys take over `Hex.Runtime.HashTrie`, with the colour crossing species (d) licenses. Each is a value function of its arguments, and the two discharge it differently. `searchProgram` by the transparency of the two memos §6.2 names, the memo limb — and, for its cursor, by the independence §6.2 states: over the row's domain the answer does not depend on which valid cursor is passed. `compileProgram` by neither limb as Effects §6.2 and Intrinsics §4.2 word them: the buffers it allocates *are* read by every later `searchProgram`, so they are not scratch, and its answer is not a memo entry. What makes it a value function is that a `Program` has **no identity any Hexagon expression can observe** — a confined type has no `Eq`, `Ord`, `Hash`, or `Show`, no pattern, no derived instance (Intrinsics §3.3), and foreign code's view of an exported `Regex`'s representation is Effects §6.2's existing caveat — and is inspected by `searchProgram` alone, whose answer depends on nothing but the `Pattern` the program was built from; the buffers are frozen at return and read as a value thereafter, and the only storage written later is the memos. Confinement here rules out interference from every module but the two the door links — `Hex.Regex` names `Program`, `Hex.Runtime.Regex` handles the same values as `Compiled` — and the pair is what the discharge rests on. *Amendment for ratification: Effects §6.2's species (d) and Intrinsics §4.2 name two discharges; this is a third — a pair of rows over storage only the pair reads, jointly a value function, the constructing row's answer having no identity a Hexagon expression can observe (§13). Edit note: on ratification, Effects §6.2's obligation paragraph and Intrinsics §4.2's species-(d) sentence each gain the third shape; until then this document governs the point (README rule 4).* *Edit note: Effects §6.2's member sentence and Intrinsics §4.2's last sentence name the rows `compile` and `search`, which are this document's `compileProgram`/`searchProgram` (keys `regexCompile`/`regexSearch`), `Regex.compile` being the public function of §4.2; Effects §6.2 also names one memo, carried by the search row, where the program owns two — the lazy-DFA cache and the resolved classes and alphabet (§6.2) — and says the engine is `->!` "throughout", which §6 scopes to buffer contents; all applied on next touch.* Each takes no function-typed parameter, so nothing re-enters the engine mid-write. `searchProgram`'s answer, with `spans` set, is the spans — element 1 the whole match, always `Some`, and element `i + 1` group `i`, each `(start, length, text)` or `None`, from which the surface builds a `Match` once (§4.1) — paired with the cursor for the match's end (§6.2); with `spans` clear it is `Some` of the empty vector and the cursor it was given where a match exists, and `None` otherwise — unambiguous, since a spans answer always has its element 1.
- `compileProgram` and `searchProgram` are the only `->` rows over confined **state**: every row that reads or writes a buffer's contents is `->!`, `bufferLength` being `->` because a buffer's size is fixed at creation (Intrinsics §3.3), and the two table rows being `->` reads of fixed tables. No third sealed row is scheduled: `isMatch`, `findAll`, and the replacement family are ordinary Hexagon over `searchProgram`.

## 8. Literal patterns

A call `Regex.compile(p)` whose argument is a **string literal** is compiled as data: the compiler runs the emitted `Regex.Syntax` parser and the emitted `Hex.Runtime.Regex` constructor at compile time — the same code a dynamic `compile` runs, one door — and emits the resulting program as a constant in the module, its property classes referencing the runtime's tables by name (§7). The value the program observes is the same `Result(Regex, RegexError)` the dynamic call would produce: `Ok` of a `Regex` over the constant, or the same `Err`. No semantics ride on the emission; it is an evaluation the compiler is entitled to perform because `compile` is a value function of its argument (Effects §6.2, Intrinsics §4.2) and the argument is known, and it is why a module's patterns cost their parse once at build rather than once per load. A program emitted as a constant is no exception to Intrinsics §3.3's rule that a confined type's values arise only from the block's rows: it is `compileProgram`'s own lowering's value, evaluated at build rather than at the call, its memos (§6.1) unfilled in the constant and filled per load.
...
`Regex` and `Match` are opaque records and take FFI Part 7 §5's brand-only `.d.ts` face; `Group`, `RegexError`, and `Syntax`'s tree are transparent records and unions with the ordinary representation. A `Program` has no `.d.ts` face — it is inside an opaque record, and a confined type never has one (Intrinsics §3.3). A `Seq(Match)` crossing the boundary is FFI Part 3's. Marks and colours erase with the rest of the effects discipline (Effects §8); the runtime module's emitted JavaScript carries its operations out under an export list the emitter writes, as `HashTrie.js` does, and a Hexagon module that imports it reaches nothing (Packages §2.4).
...
- **A texts-free spans mode** for `replace` with a literal and for `split`, which read no group text: the growth path if §4.1's eager cut bites. It widens `regexSearch`'s mode parameter from `Bool` to three values — a change to an existing key's declared scheme (Intrinsics §4.2), not a new key.
...
- **A third sealed row (`findAll` in the engine)**: §7 — a `Seq` handed out by the engine would run `->!` code beneath a pure `pull` at every later step, a colour crossing at no sealed row; the enumeration is ordinary Hexagon over `searchProgram`, at the cost the cursor of §6.2 removes.
...
| Spans are 1-based codepoint positions and counts; empty matches at boundary positions; no code unit on the public surface, the search row's cursor the one internal exception | §3.1 |
...
| Engine: class resolution on first use, fold before complement; alphabet partition; ordered lazy DFA forward then reverse; Pike VM for captures and fallback; two transparent memos (the cache Ruling 3 named, and the resolved classes) and no memo of the subject; a resumption cursor instead; a runtime module with an import; bounds table | §6 |
| Door: `buffer` + four rows and the two table keys in the runtime, every property's ranges excluding surrogates; two sealed rows in `Regex.hex`, the search row taking a cursor and a spans mode and cutting the texts; no third sealed row | §7 |
| **Amendment for ratification** — `regexProgram`, a second confined type key beyond the foundations' `buffer`, declarer `Hex.Regex`; Intrinsics §3.3's "no representation" bar read as confinement (the engine's unexported `Compiled` record is the door's linkage, as `HashTrie` is `Map`'s, not a representation of the row's type); the replacement bar sentence filed as an edit note on Intrinsics §3.3 | §7, Intrinsics §3.3 |
| **Amendment for ratification** — `compileProgram`'s species-(d) discharge is a third shape beside scratch and memo: paired rows over storage only the pair reads, the constructing row's answer without identity a Hexagon expression can observe; Effects §6.2 and Intrinsics §4.2 owed the sentence on ratification | §7, Effects §6.2, Intrinsics §4.2 |
