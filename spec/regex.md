...
The metacharacters — the codepoints that do not stand for themselves outside a class — are `\ . + * ? ( ) | [ { ^ $`. `]` and `}` outside their constructs are literals; `{` is not (§2.5). Every other codepoint is a literal matching itself. Under `x` (§2.6), `#` and every `White_Space` codepoint join them outside a class. Inside a class the metacharacters are `\ ] [ ^ -` (§2.4) — `^` only first, `-` only between two singles, so `[-a]`, `[a-]`, and `[a^b]` read the hyphen and the caret as literals.
...
The names are Unicode's canonical long names and Unicode's own short aliases — the first two fields of PropertyAliases.txt and PropertyValueAliases.txt, and no later field, so `\p{digit}`, `\p{punct}`, and `\p{cntrl}` are out — matched **exactly** — case, underscores, and spacing as Unicode writes them. There is no loose matching (RL1.2a is not adopted): `\p{lu}`, `\p{uppercase letter}`, and `\p{Greek_}` are refused, each naming the canonical spelling. A bare name is looked up in one order — as a General_Category value, then a Script value, then one of the binary properties and `Any`/`ASCII`/`Assigned` of the table above; the three name spaces are disjoint over the admitted set, so the order settles determinism rather than a contest. `=` and `:` are both accepted between a property name and its value. `\P{…}` is the complement of `\p{…}`; the `\p{^Greek}` spelling is refused. The set is **closed**: no other property, no property of strings, no `=Yes`/`=No` form on the binary properties. A property joins by ruling and by table together — `\p{Emoji}` is the likely first request — never by a name the parser happens to accept.

A **lone surrogate** has no properties (§3.6): every `\p{…}` naming a property excludes it — `\p{Any}` alone includes it, being the whole codepoint space rather than a property — and every `\P{…}`, every negated class, and `.` include it. Since a well-formed pair is decoded to one scalar and a lone surrogate has no category, no codepoint the engine sees is a surrogate by category: `\p{Cs}` is admitted — the ruling admits General_Category verbatim — and matches nothing, `\P{Cs}` matches everything, and `\p{C}` reaches its other members.
...
- **`Match`** is one match against one subject: group 0 and every capturing group as a `Group` — its span and its text — or `None` where the group did not participate, element 1 being group 0, and the name table. The texts are cut by the engine, which holds the subject's code-unit offsets while it has them (§6.2, §7), so no accessor ever walks the subject; the record is opaque so that its face is the accessors of §4.5 and its layout is the library's to change. No instances in v1.
...
- `groups` is groups 1 through `groupCount` in order — `[]` for a pattern with no capturing group — `Vector`-indexed from 1 like every vector, so `groups(m)[i]` and `group(m, i)` agree. Every accessor is a constant-time read of the record (§6.4).
...
`Regex.hex`'s `searchProgram` row (§7) lowers to this module's `search(program: Compiled, text: String, start: Int, spans: Bool) ->! …`, which answers the leftmost-first match at or after the start position — every group's span and text when `spans` is set, and with it clear only whether there is one:
...
| `isMatch`, `find`, `findFrom` | `O(n)` on the DFA route, amortised over the cache, for `isMatch` or a program with no capturing groups; `O(n + span · m)` with capturing groups, the VM running over the span alone; `O(n · m)` on the VM route; never worse, whatever the pattern |
| `findAll` over a whole subject | linear in the subject, plus the sum of the match lengths (each times `m` where the program has capturing groups), plus the texts cut — the position memo (§6.2) keeps each resumption from re-walking the subject, per traversal |
...
extern from "hex:intrinsic"
    ...
    fun regexSearch as searchProgram(program: Program, text: String, start: Int, spans: Bool) -> Option(Vector(Option((Int, Int, String))))
```

- `regexProgram`'s inventory entry names **`Hex.Regex`** as its one declarer: a `Program` is constructed and inspected only by the two rows, travels only inside the opaque `Regex` (§4.1), and is the carrier through which a value over `Buffer` storage — a compiled program with its cache — reaches a program without the storage being addressable anywhere but the runtime module. The type has arity 0. *Amendment for ratification: the foundations declared one confined type key, `buffer`; this is a second, and it reads Intrinsics §3.3's bar — "no Hexagon representation at all" — at the door: the engine's `Compiled` record (§6.1) is a representation no module can name and no row exposes, which is `Map` over `HashTrie` one grade up, not the collection companions' declared-in-source case the bar contrasts. Intrinsics §3.3 carries the reading (§13).*
- `compileProgram` and `searchProgram` write `->`, and their lowerings are the runtime module's compiled `->!` bodies of §6.1 and §6.2: the linkage `Map`'s keys take over `Hex.Runtime.HashTrie`, with the colour crossing species (d) licenses. Each is a value function of its arguments, and the two discharge it differently. `searchProgram` by the transparency of the three memos §6.2 names, the memo limb. `compileProgram` by neither limb as Effects §6.2 words them: the buffers it allocates *are* read by every later `searchProgram`, so they are not scratch, and its answer is not a memo entry. What makes it a value function is that a `Program` has **no observable identity** — a confined type has no `Eq`, `Ord`, `Hash`, or `Show`, no pattern, no derived instance (Intrinsics §3.3) — and is inspected by `searchProgram` alone, whose answer depends on nothing but the `Pattern` the program was built from; the buffers are frozen at return and read as a value thereafter, and the only storage written later is the memos. *Amendment for ratification: Effects §6.2's species (d) names two discharges; this is a third — a pair of rows over storage only the pair reads, jointly a value function, the constructing row's answer having no identity the program can observe (§13).* Each takes no function-typed parameter, so nothing re-enters the engine mid-write. `searchProgram`'s answer, with `spans` set, is element 1 the whole match and element `i + 1` group `i`, each `(start, length, text)` or `None`; with `spans` clear it is `Some` of the empty vector where a match exists and `None` otherwise.
...
| `Regex` opaque over `Program`, no `Eq`/`Hash`, `Show` = source; `Match` opaque, holding its `Group`s with texts cut by the engine, constant-time accessors; `Group` transparent | §4.1, §4.5 |
...
| Door: `buffer` + four rows and the two table keys in the runtime, every property's ranges excluding surrogates; two sealed rows in `Regex.hex`, the search row taking a spans mode and cutting the texts; no third sealed row | §7 |
| **Amendment for ratification** — `regexProgram`, a second confined type key beyond the foundations' `buffer`, declarer `Hex.Regex`; Intrinsics §3.3's "no representation" bar read at the door (the engine's unexported `Compiled` record is not a representation any module can name) | §7, Intrinsics §3.3 |
| **Amendment for ratification** — `compileProgram`'s species-(d) discharge is a third shape beside scratch and memo: paired rows over storage only the pair reads, the constructing row's answer without observable identity | §7 |
