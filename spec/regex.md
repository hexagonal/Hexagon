...
**Status:** Decided (#927 — the eight rulings that document records; the foundations it stands on landed at #928). Scheduled, nothing shipped: `stdlib/Regex.hex`, `stdlib/Regex/Syntax.hex`, and `stdlib/Runtime/Regex.hex` implement this surface when the arc's implementation lands, and this document is their contract (`stdlib-roadmap.md` §5 records the row).
...
quantifier  ::= ('*' | '+' | '?' | '{' n '}' | '{' n ',' '}' | '{' n ',' m '}') '?'?
atom        ::= literal | '.' | class | escape | group | anchor
group       ::= '(' alternation ')'
              | '(?:' alternation ')'
              | '(?P<' name '>' alternation ')' | '(?<' name '>' alternation ')'
              | '(?' flags ')' | '(?' flags ':' alternation ')'
flags       ::= flag* ('-' flag+)?           -- each of i m s x at most once
anchor      ::= '^' | '$' | '\A' | '\z' | '\b' | '\B'
class       ::= '[' '^'? item+ ']'
item        ::= single | single '-' single | perlclass | property
```
...
The metacharacters — the codepoints that do not stand for themselves outside a class — are `\ . + * ? ( ) | [ { ^ $`. `]` and `}` outside their constructs are literals; `{` is not (§2.5). Every other codepoint is a literal matching itself. Inside a class the metacharacters are `\ ] [ ^ -` (§2.4).
...
| Escape | Meaning |
|---|---|
| `\n`, `\t`, `\r` | line feed, tab, carriage return — Lexer §6.2's table, so the two escape sets a Hexagon programmer meets are one |
...
- a single codepoint or an escape (§2.3) is a member; `a-z` is the inclusive range by codepoint value, and a range whose low end exceeds its high end is refused;
...
- `[` inside a class is refused unless escaped — there are no nested classes and no POSIX classes, and `[:alpha:]` is refused with `\p{Alphabetic}` named as the spelling (§2.9); this is the one place the dialect refuses what RE2 accepts, to keep `[[:alpha:]]` from reading silently as a class followed by a literal;
...
The names are Unicode's canonical long names and Unicode's own short aliases (PropertyAliases.txt, PropertyValueAliases.txt), matched **exactly** — case, underscores, and spacing as Unicode writes them. There is no loose matching (RL1.2a is not adopted): `\p{lu}`, `\p{uppercase letter}`, and `\p{Greek_}` are refused, each naming the canonical spelling. A bare name is looked up as a General_Category value first and a Script value second, which Unicode keeps disjoint. `=` and `:` are both accepted between a property name and its value. `\P{…}` is the complement of `\p{…}`; the `\p{^Greek}` spelling is refused. The set is **closed**: no other property, no property of strings, no `=Yes`/`=No` form on the binary properties. A property joins by ruling and by table together — `\p{Emoji}` is the likely first request — never by a name the parser happens to accept.

A **lone surrogate** has no properties (§3.6): every `\p{…}` excludes it, and every `\P{…}`, every negated class, `.`, and `\p{Any}` include it. Since a well-formed pair is decoded to one scalar and a lone surrogate has no category, `\p{Cs}` matches nothing.

### 2.5 Repetition

`x*`, `x+`, `x?`, `x{n}`, `x{n,}`, and `x{n,m}` are greedy; the same with a trailing `?` are lazy — preferring fewer iterations (§3.2). There are no possessive quantifiers and no atomic groups. Bounds: `0 ≤ n ≤ m ≤ 1000`, and across nested counted repetitions the product of the maxima (`n` standing for `{n,}`, unbounded operators counting 1) is at most 1000 — RE2's limit, which bounds the compiled program by the pattern's length. A count past the bound is refused.

A quantifier must follow an atom: `*a`, `|*`, `(*)`, and a quantifier on a quantifier (`a**`, `a+?*`) are refused — write `(a*)*` if that is meant. `{` that does not begin a well-formed counted repetition (`a{`, `a{,3}`, `a{x}`) is refused, never read as a literal brace; the literal is `\{`. That is the Scala-refusal rule of this corpus at the pattern's scale: one committed reading or a refusal that names the boundary.
...
**Flags are inline only.** `(?flags)` sets flags for the rest of the enclosing group (or the pattern); `(?flags:…)` sets them for the group's body alone. `flags` is a sequence of the letters below, each at most once, optionally followed by `-` and letters to clear — `(?i)`, `(?-i)`, `(?i-s:…)`. `(?)` and `(?i-)` are refused. There is no flags argument to `compile` (§4.2) and no other flag: `U` (RE2's ungreedy) is refused by name.
...
| quantifier without an atom | the quantifier | nothing to repeat before `*` |
...
| bad `\p` form | the `\` | `\p` takes one letter or a braced name |
...
A pattern may match the empty string, and `findAll` (§4.3) enumerates successive non-overlapping matches from left to right under one rule, RE2's and Go's: **after a match ending at `e` (its start plus its length) the search resumes at `e`; a match found there that is empty and follows a non-empty match is discarded and the search resumes at `e + 1`; after an empty match is yielded, the search resumes at `e + 1`**. So `a*` on `baaab` yields the empty match at 1, `aaa` at 2, and the empty match at 6 — not an empty match at 5 abutting `aaa` — and `""` on `abc` yields four empty matches, at 1, 2, 3, and 4. `Regex.split` (§4.4) is defined over that enumeration and therefore agrees with `String.split` on the empty pattern: both give `["", "a", "b", "c", ""]`. Python's rule, which admits the abutting empty match, is not adopted.
...
Signatures below are the normative qualified API, not declaration syntax. Every function is subject-first and dot-callable on its receiver by Method Syntax §4.2's home-module rule — `Regex`, `Match`, and `Group` all live in `Regex.hex`, so `regex.find(text)` and `match.group(1)` both resolve there. Every function is `->` except the two whose replacement callback makes them conduits (§4.4).
...
- **`Group`** is a captured span with its text, transparent, the value every accessor answers with. `RegexError` is §2.9's, home `Regex.Syntax`.
...
- `find` is the leftmost-first match (§3.2), or `None`. `isMatch` answers whether `find` would answer `Some`, with the engine free to stop at the first match end (§6.2).
...
- `split` answers the pieces of `text` between the matches of §3.5's enumeration, preserving empty pieces at both ends and between adjacent matches, so that `String.join(regex.split(text).toSeq(), sep)` reconstructs `text` when every match spelled `sep`. On the empty pattern it agrees with `String.split(text, "")`.
...
- `groups` is groups 1 through `groupCount` in order, `Vector`-indexed from 1 like every vector, so `groups(m)[i]` and `group(m, i)` agree.
...
export union Node =
    ...
    | Group(body: Node, index: Int, name: Option(String))
...
- **Groups carry their index** in §2.6's numbering; `Pattern.names` maps each name to its index. `Concat` and `Alternate` hold two or more parts; a one-part sequence is the part; an empty sequence is `Empty`.
...
`compileProgram` takes a `Syntax.Pattern` and answers a `Program`:

- **Class resolution.** Every `Class` becomes a set of codepoint ranges: property items are resolved through the property-table key (§7) by canonical name; the union is taken; a `folded` class is closed under simple case folding through the fold key; a `negated` class is complemented over U+0000–U+10FFFF. Resolution is per class, once, **on the program's first use**, memoised in the program (a transparent memo, §6.2) — so that the program construction itself reads no table, and a program emitted as a constant (§8) carries its properties by name and resolves them against the tables of the compiler that runs it.
...
- **The NFA.** One Thompson construction over the tree: literals and classes as consuming states, `Empty`, anchors and `\b`/`\B` as non-consuming states carrying their assertion, `Alternate` and `Repeat` as split states ordered by §3.2's priority, groups as capture-save states, counted repetitions expanded within §2.5's bounds. The program's size is linear in the pattern's size after expansion.
- **Totality.** The engine refuses nothing: a `Pattern` the parser answered always constructs (§5).

### 6.2 The search

`searchProgram` takes a program, a subject, and a start position and answers the leftmost-first match at or after that position, as spans:

- **The lazy DFA** is built on demand from the NFA — a state is a *priority-ordered* set of NFA states, so that the forward scan finds the end of the leftmost-first match, not the longest (§3.2). The forward scan from the start position finds the match end; a reverse DFA over the reversed program, anchored at that end, finds the match start; where the program has no capturing groups, or the caller wants only whether a match exists, the search ends there.
...
- **The cache** is the program's owned memo (Effects §6.2 species (d)): every DFA state is a value function of the NFA state set it names, a hit and a miss are indistinguishable, and nothing in it records call history. It is bounded by a constant the engine fixes; exhaustion discards it and a later search rebuilds what it needs. Two more transparent memos are admitted on the same terms: the resolved classes and alphabet of §6.1, a value function of the program; and the code-unit offset of the last codepoint position answered for the last subject, so that resuming a search from a position does not re-walk the subject from its start (§6.4).
...
| `isMatch`, `find`, `findFrom` | `O(n)` on the DFA route, amortised over the cache; `O(n · m)` on the VM route; never worse, whatever the pattern |
| `findAll` over a whole subject | linear in the subject plus the sum of match lengths — the position memo (§6.2) keeps each resumption from re-walking the subject |
| `Match` accessors | `start`, `length`, `group`, `named`: constant; `text` and a `Group`'s text: linear in the span, sliced on demand |
...
Three modules declare through the door; the inventory (Intrinsics §4.1) gains one type key beyond the foundations' `buffer` and six operation keys. `Regex.Syntax` declares nothing (§5).
...
extern from "hex:intrinsic"
    ...
    fun regexSearch as searchProgram(program: Program, text: String, start: Int) -> Option(Vector(Option((Int, Int))))
```
...
| Surface: `compile`, `escape`, `source`, `groupCount`, `groupIndex`, `isMatch`, `find`, `findFrom`, `findAll`, `replace`/`replaceFirst`/`replaceWith`/`replaceFirstWith`, `split`; replacement callbacks `->?` | §4.2–§4.4 |
| `Regex.Syntax` door-free and pure; flags resolved in the tree; property values canonical; explicit ranges normalized; no source spans in v1 | §5 |
| Engine: class resolution once; alphabet partition; ordered lazy DFA forward then reverse; Pike VM for captures and fallback; two transparent memos; bounds table | §6 |
| Door: `buffer` + four rows and the two table keys in the runtime; `regexProgram` (declarer `Hex.Regex`) + two sealed rows in `Regex.hex`; no third sealed row | §7 |
