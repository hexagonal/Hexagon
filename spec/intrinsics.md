...
*(#344.)* `stdlib/BigInt.hex` is the third customer and the primitive template's worked example, exercising both shapes at once. Its irreducible natives — the truncated pair the public language cannot express (`bigIntQuot`/`bigIntRem`), the representation crossings (`bigIntFromNat`, `bigIntFromInt`, `bigIntToFloat`, `bigIntHash`), and the guarded conversion's unchecked core (`bigIntToIntUnchecked`) — enter through the door; everything above them is ordinary Hexagon: the Euclidean `div`/`mod`, iterative `gcd`, divide-first `lcm`, the zero-divisor guards, and `toInt`'s range check over its unexported unchecked core. One delta from the first two customers: most of BigInt's operations are **constraint members**, not plain exports, so the door bindings sit unexported beneath `honor` blocks (Constraints §5.3's wired-row-retirement law) rather than beneath exported wrappers — the declaration form is indifferent to which.
