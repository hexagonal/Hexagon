...
| `Int`, `Float` | the same primitive-operation guard | §5.2 item 7's per-companion template application. The sequencing fact this row awaited is now given (#344): the companion arc runs `Int` with `Nat` next, then `Float` with `String` — each companion landing whole, instances included, per the template `BigInt.hex` recorded. (`Nat` and `String` have no guard rows of their own to remove; their milestones retire only their wired instances and Modules §5.3's transitional spellings) | their `PrimitiveOperation` guard rows, per companion as each arc lands; the family dies at the `Float` milestone, the last guard owner |

No calendar dates are fixable in this corpus — the project schedules by arcs, and every milestone above is bound to an arc. Where an arc's own position is already decided (`Seq`, `Vector`, Map/Set, and now the primitive companions in their fixed order — `BigInt` landed, then `Int`+`Nat`, then `Float`+`String`), the milestone is as fixed as the repo's scheduling admits. The residue that remains is one item: the family-terminal claim for `CollectionOperation` awaits **#223**'s disposition of the family once `Node` is its only member. Nothing else is handed back.
