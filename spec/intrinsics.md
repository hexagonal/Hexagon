...
| `Map`, `Set` | consumer-side guard rows (no `.hex` companions yet) | the Map/Set arc — after `Vector`, per James's sequencing (2026-07-28), which for the collection companions **supersedes the ledger's preferred-order list** (whose item 3 placed Map/Set before `Vector`, which that list reaches only at item 5); the supersession is recorded as an edit note at stdlib-roadmap §5.2 | their guard entries and the `Map`/`Set`/`Vector` collection rows. Whether the `CollectionOperation` *family* then dies entirely is **conditional on issue #126**: `Node` also resolves through it (the resolver's `runtime`-gated guard), is a deliberate non-declared fallback under Modules §5.5, and §3.3 keeps compiler-owned types out of the block — so `Node`'s disposition belongs to #126, not to this schedule. If #126 retires `Node`'s rows into the door (which would require moving §3.3 — a design change, not a wording one), the family closes fully; until then Modules §5.5's loop closes for the four companions and stays open exactly for `Node` |
...
| Intrinsic bindings emit as ordinary module bindings; call-site specialization stays latitude; defect 12 not presupposed | §8.3 |
| Public-name and primitive doors deprecated with per-companion termini; conversion is a prerequisite step of each self-declaration milestone; James's sequencing supersedes the ledger's collection order (edit note at stdlib-roadmap §5.2); `PrimitiveOperation` dies with the last primitive arc (`Int`/`Float` bound to §5.2 item 7, no independent milestone); `CollectionOperation`'s companion rows die at the Map/Set milestone, full family deletion conditional on #126 (`Node`) | §9 |
| Five alternatives rejected with prices recorded | §10 |
| Generic intrinsics carry a parametricity obligation (move/store/return only — never fabricate, coerce, or type-inspect); declared-scheme variance thereby semantically true; v1 inventory audited and passing (#205) | §4.2 |
