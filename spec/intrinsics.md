...
*(#344, the second landing.)* `stdlib/Int.hex` and `stdlib/Nat.hex` apply the template `BigInt.hex` recorded, and their inventories take the same primop shape. Thirteen keys at `Int`: `intAdd`, `intMultiply`, `intFromNat` (`Num`); `intSubtract`, `intNegate`, `intFromInt` (`Signed`); `intEquals` (`Eq`); `intCompare` (`Ord` — name-string lowering as at `BigInt`); `intShow` (`Show`); `intPow` (`Pow` — the raw native `**`; the negative-exponent guard is Hexagon above it); `intHash` (`Hash`); `intQuot`, `intRem` (`Integral`'s truncated pair — raw natives; the zero-divisor guards are Hexagon above them, as is the whole Euclidean family). Eleven at `Nat`: `natAdd`, `natMultiply`, `natFromNat` (`Num`); `natEquals` (`Eq`); `natCompare` (`Ord`); `natShow` (`Show`); `natPow` (`Pow`); `natHash` (`Hash`); `natQuot`, `natRem` (`Integral`); and one conversion core, `natFromIntUnchecked`, beneath `Nat.fromInt`'s sign check (Primitive Types §1). Three deltas from `BigInt`'s paragraph, each an absence. **The identity lowerings:** `intFromInt` and `natFromNat` are each type's literal anchor at itself, and `intFromNat` crosses `Nat`→`Int` over the one shared `number` representation — all three lower to the identity. The door still owns them, because a member body may not be written in the sugar that selects its own slot (Constraints §6.1), and an identity is as much a native as an operator is. **The guard that is not there:** `Pow<Nat>` is the raw native with nothing above it — a `Nat` exponent cannot be negative, so the guard `Int` and `BigInt` carry is dead by typing and is not written (Operators §6.3's amended row). **The family that never reaches the door:** `Int.checkedAdd`/`checkedSub`/`checkedMul` (Primitive Types §2.1) are ordinary Hexagon over the ordinary members — each overflow test is an exact pre-check phrased entirely inside the safe range, so no unchecked core and no host predicate exists to declare.
