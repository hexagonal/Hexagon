...
| Constructor pattern over an opaque type abroad — the door's refusal (§2.2), and the spelling §2.4's redirect would otherwise have suggested | the opaque family's sentence at the type's noun: "cannot destructure opaque record `Point`; use an operation exported by its home module", "cannot destructure opaque union `Handle`; use an operation exported by its home module"; where the home exports declared patterns over the type, "…; match it with `Rat.rat(top, bottom)`" naming them (§2.4; Modules §4.2; Pattern Declarations §3.3) |
