...
3. **Bare, with the route stated**, where the module has no pastable spelling at all — the constructor is neither in scope nor reachable through any alias, whether the union's type name was imported without it or the union reached this module only through an imported signature. The witness keeps the bare name — the spelling that becomes pastable the moment the import lands — and the message says where it lives and how to get it: ``match is missing cases: `Off` — `Off` is declared in `./flags`; `import { Off } from "./flags"` to spell it here``. One route clause per declaring module, covering every listed name it declares; the "…and N more" tail names no constructors and so routes none. The named import is always a working repair: a union whose values reach a foreign module crossed an exported face, and Modules §4.3 bars a private union from crossing one, so its constructors are importable wherever this tier can fire. The clause rides with the witness, not with one message: every diagnostic that renders a counterexample through this section — the §5.3 gate included — carries it under the same conditions.
4. **No hidden-name tier exists, on purpose.** An opaque union's constructors are unnameable abroad — and unreachable here: a witness deeper than `_` requires refutable arms, refutable arms require nameable constructors, so a match over an opaque union outside its home can only ever be missing `_` itself. No diagnostic under this section ever demands a name the reader cannot obtain — §3's opaque-destructure interception is the same law at the redirect seat: a diagnostic never signposts a spelling the reader cannot write.
