...
- Literals: an integer literal contributes `Num` and `Eq`, and a negative one `Signed` besides, and unifies with the scrutinee type, its variable resolving to a primitive or failing to type (§2.5); a `Float` or `String` literal is its own type, contributes `Eq`, and unifies with the scrutinee (§2.5).
