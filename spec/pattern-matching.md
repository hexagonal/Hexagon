...
0   1n   1.5   "yes"     -- literal patterns: bare integer, BigInt, Float, String (Bool's are constructors, #147)
p1 | p2                  -- or-pattern
...
### 2.5 Literal patterns — bare integer, `BigInt`, `Float`, and `String` forms, at the five primitives
...
- Patterns never invoke user code — a literal's `Eq` test is the compiler's, at the permitted primitive the literal stands at (`===` at `Int`, `Nat`, `BigInt`, and `String`, the SameValueZero shape at `Float`; §2.5) — except a declared pattern's pure `view`, applied at most once per pattern per position before the arms are tested (Pattern Declarations §5).
...
| Bare integer literal, its constraints all honored, resolved at a type outside the permitted primitives (`0 =>` at `Rat`, at a user type honoring `Num` and `Eq`; `Some(0)` at `Option(a)` under `<a: (Num, Eq)>`) | "`0` is not a pattern at `Rat`; bind a name and test it in a guard: `x when x == 0`" — beneath the top a fresh binder at the literal's position and the test in the arm's guard, `Some(y) when y == 0` — judged on the resolved type, after inference and defaulting (§2.5) |
...
| Literal forms: bare integer / `BigInt` / `Float` / `String` via `Eq`, standing at the five primitives (`Bool` removed 2026-07-29, #147 — constructor patterns now); ordinary inference (`Num` + `Eq` constraints for bare integer literals; a decimal literal is `Float`); a `Float` literal tests through SameValueZero and emits what `==` emits; named special values are guard terms; a literal's coverage identity is its value at its position's type under its `Eq`, never its spelling, at the primitives, whose `Eq` the compiler computes (`007` is `7`, `-0` is `0`, `-0.0` is `0.0`); a literal pattern is checked at the type of its position and never widens it — a bare integer literal contributes `Num` and `Eq` and unifies there, an undetermined nested position resolved by inference and defaulting, the permitted-primitive restriction checked on the resolved type (`0` at `Rat` is refused naming the guard); an `n`-suffixed literal requires `BigInt` and preserves arbitrary precision; a decimal literal requires `Float`; a negative bare literal demands `Signed`, refused at `Nat` with Modules §7.6's report — the former permanent `Float` ban is lifted (§10); no `Char` | §2.5, §7.2 |
