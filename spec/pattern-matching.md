...
- Literals: an integer literal contributes `Num` and `Eq` and unifies with the scrutinee type; a `Float` or `String` literal is its own type, contributes `Eq`, and unifies with the scrutinee (§2.5).
