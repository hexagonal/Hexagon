...
| Integer literal at a position whose concrete type honors no `Num` (`0 =>` at `String`, at a record) | what `x == 0` draws at that type, no voice of this section's: Numeric Literals §6's "integer literal cannot have type `String`" at `String` and a union; Modules §7.6's missing-instance report for `Num`, its home named, at a record; a structural subject's own message (Modules §7.6) — the constraint fails before the restriction is reached (§2.5) |
| Literal at a declared type variable whose constraint list does not entail the literal's (`0 =>` at `a` under `<a: Num>`, `-1 =>` under `<a: (Num, Eq)>`) | Functions §4.2's report, one per unmet constraint — "`a` is declared to honor `Num`, but the body requires `Eq`; write `<a: (Num, Eq)>`, or remove the constraint annotation to let it be inferred" — the negative literal's rewrite `<a: (Eq, Signed)>` (§2.5); under a list that entails them, `0 =>` at `a` is the permitted-primitive refusal below |
| Integer literal resolved at a `Num`-honoring type outside the permitted primitives (`0 =>` at `Rat`, at a user `Num` type) | "`0` is not a pattern at `Rat`; bind a name and test it in a guard: `x when x == 0`" — judged on the resolved type, after inference and defaulting (§2.5) |
| Negative integer literal at a type honoring `Num` but no `Signed` (`Nat`: `-1 =>`, `-0 =>`) | Modules §7.6's missing-instance report for `Signed` at `Nat`, as `x == -1` draws there: "type `Nat` has no `Signed` instance; its only legal homes are the module declaring `Signed` and `Nat`'s prelude companion module, both outside project source, so this pair's honored set is closed — change the type, or go through the operations those homes export; a written `Int` face runs the operation and admits the result (`let difference: Int = …`)" — at an arm, "change the type" is an ascription on the scrutinee, `match (size: Int)` (§2.5) |
