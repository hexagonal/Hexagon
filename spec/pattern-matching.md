...
| A literal's contributed constraint unmet at its position's type | the report a failed constraint of `x == lit` draws at that type, one per unmet constraint, Method Syntax §9 row 15's operation-seat rider omitted in pattern position and nothing offered in its place — a delegation of the diagnostic alone; the constraint fails before the restriction is reached (§2.5) |
