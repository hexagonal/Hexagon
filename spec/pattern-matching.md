...
### 2.5 Literal patterns — the `Int`, `Float`, and `String` forms, at the five primitives
...
- Literals: an integer literal contributes `Num` and `Eq` (a negative one `Signed` besides) and unifies with the type at its position, ordinary inference and defaulting resolving an undetermined nested one; the permitted-primitive restriction is checked on the resolved type; a decimal literal requires `Float`, a String literal `String`; nothing widens, and the arm's test is `Eq` at that type (§2.5).
...
| Literal forms: `Int`/`Float`/`String` via `Eq`, standing at the five primitives (`Bool` removed 2026-07-29, #147 — constructor patterns now); ordinary inference (`Num` + `Eq` constraints for integer literals; a decimal literal is `Float`); a `Float` literal tests through SameValueZero and emits what `==` emits; named special values are guard terms; a literal's coverage identity is its value at its position's type under its `Eq`, never its spelling, at the primitives, whose `Eq` the compiler computes (`007` is `7`, `-0` is `0`, `-0.0` is `0.0`); a literal pattern is checked at the type of its position and never widens it — an integer literal contributes `Num` and `Eq` and unifies there, an undetermined nested position resolved by inference and defaulting, the permitted-primitive restriction checked on the resolved type (`0` at `Rat` is refused naming the guard); a decimal literal requires `Float`; a negative literal demands `Signed`, refused at `Nat` with Modules §7.6's report — the former permanent `Float` ban is lifted (§10); no `Char` | §2.5, §7.2 |
