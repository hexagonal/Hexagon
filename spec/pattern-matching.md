...
- A literal pattern elaborates through **`Eq`** exactly as `==` does (Operators §5.1): the arm test is `equals(scrutinee, lit)`, and it emits what `scrutinee == lit` emits at the type — `===` at `Int`, `Nat`, `BigInt`, and `String`, and at `Float` the SameValueZero shape, the inline test or the on-demand `__floatEquals` helper (Decisions Batch §1.5; the choice between the two texts is the emitter's, as at every `Float` comparison, Constraints §6.1). A literal arm and a guard `when scrutinee == lit` test the same thing by the same code; at a primitive scrutinee, where an arm stands at all (the typing bullet below), the arm differs from the guard only in that it is a pattern — it counts for reachability (§7.2), and a guard establishes no coverage (§3, §7.1).
- **Typing joins ordinary inference.** An integer literal in a pattern contributes the same `Num` (via `fromNat`) and `Eq` constraints that `x == 0` would, and unifies with the scrutinee type; defaulting applies as usual. The literal's variable may thereby determine an undetermined scrutinee, as a scope-resolved constructor head may (§4): `match 0` with a `0` arm is a match at `Int`, by defaulting. The spec fixes the mechanism so that a polymorphic scrutinee never forces an improvised rule, and the rule below is judged where the literal's variable resolves — with the scrutinee, or by defaulting — and never earlier. A **decimal literal** is `Float`, monomorphic, in a pattern as in an expression (Numeric Literals §1), and unifies with the scrutinee type. No conversion runs at a pattern: the literal's own type unifies with the scrutinee's, so an integer literal reaches a `Float` scrutinee through its `Num` variable as ever, and `0.0` against an `Int` scrutinee is the ordinary mismatch, `expected Int, found Float` — not because a pattern is a special seat, but because `Float` is the source of neither of Numeric Literals §5.1's two conversions, which both run the other way (friendly-numerics' membrane). Its value is the lexer's (Lexer §5): the correctly rounded binary64 — `1.0e308` is a legal pattern, `1.0e-400` underflows to the pattern `0.0`, and `1.0e309` is the lexer's own overflow error (Lexer §10) and the one report: a construct the lexer has already diagnosed draws no further report from the pattern seat. **A literal pattern stands at a primitive scrutinee only.** The integer literal's `Num` variable, unified with the scrutinee — defaulting applying as usual — must resolve to one of the `Num`-honoring primitives — `Int`, `Nat`, `BigInt`, `Float`; its resolution to a `Num`-honoring type outside them (`Rat`, a user's `Money`) is a failure to type, at the literal, with the guard named as the spelling: "`0` is not a pattern at `Rat`; bind a name and test it in a guard: `x when x == 0`" — §7.3's fourth tier reading the arm as `_`. The ground is §2.8: at such a type the literal's value would be `fromNat` over the payload, and `negate` over that (Numeric Literals §3), run inside a pattern, for which there is no room; at a primitive the value is the compiler's (§7.2). A `Float` or `String` literal is its own type and needs no such rule: at any other scrutinee it is the ordinary mismatch.
...
- Negative literals: `-3` and `-2.5` are legal as literal patterns — the parser forms the signed literal pattern from the `-` and the token, which itself carries no sign (Lexer §5); patterns contain no operators, so there is no unary-minus expression to collide with. `-0.0` is a literal pattern, and it is the pattern `0.0` for every judgment (§7.2). A negative integer literal pattern contributes what `x == -3` would — `Signed`, which `-3` demands in any position (Numeric Literals §3: a negative source form is `negate` over the literal's payload), and `Eq`. The demand is the literal's meaning and not an elaboration: no `negate` runs at a pattern, and the key is a value the compiler computes (§7.2), a literal pattern standing at a primitive scrutinee only (the typing bullet above). At a primitive that honors `Num` but no `Signed` — `Nat` — the arm draws Modules §7.6's missing-instance report in its closed-pair form, its two legal homes named as that section requires, with an arm's tail in place of the operand's second clause ("go through the operations those homes export") and its illustration, which no arm can take: "type `Nat` has no `Signed` instance; its only legal homes are the module declaring `Signed` and `Nat`'s prelude companion module, both outside project source, so this pair's honored set is closed — ascribe the scrutinee a type that honors `Signed` (`match (size: Int)`), or remove the arm". The demand is judged whatever the payload, `-0` included; only where the sign is admitted is `-0` the literal `0` (§7.2).
...
- Literals: an integer literal contributes `Num` and `Eq`, and a negative one `Signed` besides, and unifies with the scrutinee type, its variable resolving to a `Num`-honoring primitive, or the pattern fails to type (§2.5); a `Float` or `String` literal is its own type, contributes `Eq`, and unifies with the scrutinee (§2.5).
...
- Patterns never invoke user code except the `Eq` test behind a literal (the type's own — `===` at `Int`, `Nat`, `BigInt`, and `String`, the SameValueZero shape at `Float`, which is every v1 case; §2.5) — and a declared pattern's pure `view`, applied at most once per pattern per position before the arms are tested (Pattern Declarations §5).
...
| An integer literal pattern at a non-primitive scrutinee that honors `Num` (`0 =>` at `Rat`, at a user `Num` type) | "`0` is not a pattern at `Rat`; bind a name and test it in a guard: `x when x == 0`" — the written spelling, at the literal (§2.5) |
| Negative integer literal at a primitive honoring `Num` but no `Signed` (`Nat`: `-1 =>`, `-0 =>`) | Modules §7.6's missing-instance report, homes named, with the arm's tail: "type `Nat` has no `Signed` instance; its only legal homes are the module declaring `Signed` and `Nat`'s prelude companion module, both outside project source, so this pair's honored set is closed — ascribe the scrutinee a type that honors `Signed` (`match (size: Int)`), or remove the arm" (§2.5) |
...
| Literal forms: `Int`/`Float`/`String` via `Eq` (`Bool` removed 2026-07-29, #147 — constructor patterns now); ordinary inference (`Num` + `Eq` constraints for integer literals; a decimal literal is `Float`); a `Float` literal tests through SameValueZero and emits what `==` emits; named special values are guard terms; a literal's coverage identity is its value at the scrutinee's type under its `Eq`, never its spelling, at the primitives, whose `Eq` the compiler computes (`007` is `7`, `-0` is `0`, `-0.0` is `0.0`); a literal pattern stands at a primitive scrutinee only — an integer literal at `Rat` or a user `Num` type is refused naming the guard; a negative literal demands `Signed`, refused at `Nat` with Modules §7.6's report — the former permanent `Float` ban is lifted (§10); no `Char` | §2.5, §7.2 |
...
match temp
    ...
    0 => "still never"                -- ERROR: the same — an integer literal at Float is Float's zero, per
                                      --   §2.5's Num reading (#519 may revisit)
...
match count                           -- count: Int — the identity law at Int (§7.2)
    ...
    -0 => "never"                     -- ERROR: this literal case is unreachable; it is already handled above
                                      --   (`-0` is the literal `0`)
    _ => "ok"
match ratio                           -- ratio: Rat — a literal pattern stands at a primitive only
...
match size                            -- size: Nat (a match at Nat: #519 lifts today's gate)
    -1 => "never"                     -- ERROR: type Nat has no Signed instance; its only legal homes are
                                      ...
                                      --   closed — ascribe the scrutinee a type that honors Signed
                                      --   (match (size: Int)), or remove the arm
    _ => "ok"
match temp
