...
- **Resolution of the constructor name** *(#763)*. A constructor pattern's head resolves **in scope first** — the module's own constructors, the prelude's bare set (Modules §5.5), a qualified spelling `Alias.Name` through a module alias (Modules §3.1), and the term-position companion fallback (Modules §5.1 rule 3), each exactly as in an expression — **and then in the expected type**: where scope has nothing for a bare uppercase-start head, and the pattern's expected type is a union or nominal record whose constructor set holds the spelling, the head resolves to that constructor. The expected type is the scrutinee's at the top of a pattern and the declared slot type inside one (§4's checking mode, read off declarations: `Pair(North, South)` looks `North` up in the declared component type, `Some(North)` under `Option(Direction)` in the instantiated payload), so nothing is inferred and nothing is searched — the door reads a type the pattern's position had already fixed. It answers only where scope has nothing, the Modules §5.1 companion-fallback shape — answering, never ranking — so every program that resolved without it resolves identically under it; a same-spelled constructor in scope wins outright, and where that constructor belongs to another union the arm is the ordinary type error, whose message names the qualified spelling of the constructor the expected type does hold (§12). The qualified pattern is unchanged and still **fixes** the scrutinee type where it was undetermined: `Direction.North` resolves in the module and unifies as §4 says. **Where the expected type is not determined when the pattern is checked, the door is closed**, and a bare head scope does not bind is refused naming the qualified spelling and the annotation that would open the door. The seats decide when that can happen: at `match`, `catch`, `let`, and `for..in` the subject is typed before its pattern — and at `match`, §6.1's abstract-type refusal already guarantees the scrutinee's head is known when its arms are checked, so the door there is never order-dependent; at a lambda parameter the type is known only where a supplying seat handed it in (Functions §4.3; §6.5), and a parameter pattern under no seat takes the refusal. The door reaches every union alike — the module's own (where scope answers first anyway), an imported one, and the prelude's qualified-only constructors (Modules §5.5): `match a.compare(b)` writes its arms `Less`, `Equal`, `Greater` bare, the lookup binding nothing in scope, which is exactly the pollution §5.5's qualified-only rule exists to prevent. In expression position there is no such door (Modules §9.13): `Direction.North`.
...
- **The constructor door at a parameter** (§2.2, #763): a bare constructor head that scope does not bind resolves in the parameter's type only where a supplying seat handed that type in before the pattern is checked (Functions §4.3) — `xs.map(Point({x, y}) => x)` under a known `xs`, or a parameter written with its type. Under no seat, the head is refused naming the qualified spelling and the annotation that would open the door (§12) — loud, and never silently different by seat. `UserId(n) => n` on the module's own type is untouched: scope answers first.
...
3. **Qualified, with the route stated**, where the module has no pastable spelling at all — the bare spelling is taken by a binding of this module's own (so §2.2's door, which reads scope first, does not open), and no in-scope alias reaches the constructor. The message says where it lives and names the one repair, the module import (Modules §3 — no per-name import exists, #762), the witness pasting qualified through the alias the edit binds: ``match is missing cases: `Flags.Off` — `Off` is declared in `./flags`, and this module binds another `Off`; `import Flags from "./flags"` and spell it `Flags.Off` ``. A repair to name always exists for a declared type that crossed a module boundary: visibility is never the obstacle — Modules §4.3 bars a private union, and its #629 extern arm a private foreign enum, from crossing an exported face — and the alias is the importer's to choose. The clause's module path renders **relative to the reporting module**: this is the one place the section promises a path string the reader pastes rather than looks up. One route clause per declaring module, covering the listed names that lack a pastable spelling — never the names a shallower tier already spelled; the "…and N more" tail names no constructors and so routes none. One corner has no import to name: a prelude constructor bare-occluded while a same-spelled module alias shadows the prelude module's own ambient name — prelude modules have no import path, so the clause states the shadowing and the repair is the alias's rename. The clause rides with the witness, not with one message: every diagnostic that renders a counterexample through this section — the §5.3 gate included — carries it under the same conditions; where a seat's message already carries its own trailing fixit (the lambda-parameter gate's §6.7 line), the route clause stands before that fixit.
...
| Bare constructor head scope does not bind; expected type known and lacking the spelling | "`Direction` has no constructor `Nort`" + near-miss; where scope binds the spelling to another union's constructor instead, the arm's ordinary type error carries the qualified spelling the expected type does hold: "`North` here is `Compass.North`; this arm matches a `Direction` — write `Direction.North`" (§2.2) |
| Bare constructor head scope does not bind; expected type undetermined (a lambda parameter under no supplying seat) | "no bare `North` here: the parameter's type is not determined — write `Direction.North`, or give the parameter a type" (§2.2, §6.5; the §6.1 rider's two rewrites apply) |
...
| **Constructor patterns resolve in scope first, then in the expected type** (#763): the scrutinee's type at the top, the declared slot type beneath; nothing inferred; every seat whose subject is typed before the pattern, a lambda parameter only under a supplying seat; the qualified pattern unchanged and still fixing the type; reaches the prelude's qualified-only constructors; no expression-side door (Modules §9.13) | §2.2, §4, §6.1, §6.5 |
...
let f = match                                -- the match function: parameter undetermined
    North => 1                               -- ERROR: no bare North here: the parameter's
    _ => 0                                   --   type is not determined — write Direction.North,
                                             --   or give the parameter a type
let g = match
    Direction.North => 1                     -- OK: the qualified pattern fixes the type
    _ => 0
union Compass = North | South                -- a local union takes the spelling
