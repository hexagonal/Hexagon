...
- **Once typed, a literal arm tests through `Eq` at the scrutinee's type.** The arm test is `equals(scrutinee, lit)` at that type — exactly the runtime comparison `scrutinee == lit` makes once both stand at one type — and it emits what that comparison emits (Operators §5.1): `===` at `Int`, `Nat`, `BigInt`, and `String`; at `Float` the SameValueZero shape, the inline test or the on-demand `__floatEquals` helper (Decisions Batch §1.5; the choice between the two texts is the emitter's, as at every `Float` comparison, Constraints §6.1); the type's own `equals` elsewhere. The analogy to `==` is the runtime comparison's and reaches no further: typing a literal pattern is the checking rule below, under which the scrutinee is never widened, where an operand of `==` may be (Numeric Literals §5.1). Typed, the arm differs from a guard `when scrutinee == lit` only in that it is a pattern — it counts for reachability (§7.2), and a guard establishes no coverage (§3, §7.1).
- **A literal pattern is checked at the scrutinee's type, and never widens it.** The type a literal is checked against is its position's (§4): the scrutinee's at the top of an arm, the slot's beneath — a constructor's payload, a tuple's component, a declared pattern's component (Pattern Declarations §3.1) — and the literal is checked against it as an expression is checked against an expected type (Functions §4.3): the literal adapts, the type it meets never widens. At the top that type is determined, or the match is §6.1's refusal, which this section leaves as it stands. Beneath the top a slot still undetermined when the literal is checked takes the integer literal's `Num` variable and resolves as `x == 0` resolves an undetermined `x` — by a sibling arm, or by defaulting (Numeric Literals §4): `Some(0)` under `match None` is a match at `Option(Int)` — under the order licence §2.2 already grants beneath the top. An **integer literal** resolves at that type through `Num`: it is `fromNat` of its payload at the scrutinee's type (Numeric Literals §1, §3), so `0` is a pattern at `Int`, `Nat`, `BigInt`, `Float`, `Rat`, and at any type honoring `Num`, its value the one `x == 0` compares against there; at a type honoring no `Num` — `String`, a record — the arm draws Modules §7.6's missing-instance report for `Num` at that type, as `x == 0` draws there. Nothing widens: an `Int` scrutinee is matched at `Int`, a `Nat` at `Nat`, and no seat of the numeric lift (Numeric Literals §5.1) exists in a pattern — the lift is an operation's, and a pattern is not one. A **decimal literal** requires `Float`: it is `Float`, monomorphic, in a pattern as in an expression (Numeric Literals §1), and against any other scrutinee type — `Int`, `Rat` — it is the ordinary mismatch, `expected Int, found Float`, `Float` being the source of neither of §5.1's two conversions. A **String literal** requires `String`. A decimal literal's value is the lexer's (Lexer §5): the correctly rounded binary64 — `1.0e308` is a legal pattern, `1.0e-400` underflows to the pattern `0.0`, and `1.0e309` is the lexer's own overflow error (Lexer §10) and the one report: a construct the lexer has already diagnosed draws no further report from the pattern seat. At a primitive the literal's value at the scrutinee's type is one the compiler computes (§7.2), and the emitted arm carries it in that type's spelling (Numeric Literals §5.2: `0.0` at `Float`, `0n` at `BigInt`); at any other `Num`-honoring type it is the construction `0` makes in an expression there (`Rat.fromNat(0)`, Numeric Literals §5.2), built as the arm is tested (§8).
...
Patterns contain **no operators, no calls, no expressions**. `C(...)` in pattern position is constructor syntax resolved by the case rule, never a call. There is no evaluation inside a pattern; the only runtime work a pattern performs is tag tests, field/slot reads, and, for a literal, its `Eq` test at the scrutinee's type and its construction there (§2.5, §8). Anything computational belongs in a guard. **One exception beyond a literal's own test and construction, and only one:** a declared pattern `(…)name` applies its `view` — a function the declaration requires to be pure, applied at most once per pattern per position (Pattern Declarations §2.1, §5) — which is why it is the exception this sentence can afford.
...
- Literals: checked at the scrutinee's type, which is never widened — an integer literal through `Num` of it (a negative one `Signed` besides), a decimal literal requiring `Float`, a String literal `String`; the arm's test is `Eq` at that type (§2.5).
...
- Patterns never invoke user code except the `Eq` test behind a literal at the scrutinee's type (`===` at `Int`, `Nat`, `BigInt`, and `String`; the SameValueZero shape at `Float`; the type's own `equals` at any other `Num`-honoring type — §2.5) and the literal's own construction at that type where it is a call (`Rat.fromNat(0)`; at a primitive it is a value the compiler computes) — and a declared pattern's pure `view`, applied at most once per pattern per position before the arms are tested (Pattern Declarations §5).
...
| Integer literal at a position whose type honors no `Num` (`0 =>` at `String`, at a record) | Modules §7.6's missing-instance report for `Num` at the type, as `x == 0` draws there (§2.5) |
...
| Literal forms: `Int`/`Float`/`String` via `Eq` (`Bool` removed 2026-07-29, #147 — constructor patterns now); ordinary inference (`Num` + `Eq` constraints for integer literals; a decimal literal is `Float`); a `Float` literal tests through SameValueZero and emits what `==` emits; named special values are guard terms; a literal's coverage identity is its value at the scrutinee's type under its `Eq`, never its spelling, at the primitives, whose `Eq` the compiler computes (`007` is `7`, `-0` is `0`, `-0.0` is `0.0`); a literal pattern is checked at the scrutinee's type and never widens it — an integer literal through `Num` (`0` is a pattern at `Rat`), a decimal literal requiring `Float`; a negative literal demands `Signed`, refused at `Nat` with Modules §7.6's report — the former permanent `Float` ban is lifted (§10); no `Char` | §2.5, §7.2 |
...
match count                           -- count: Int — the identity law at Int (§7.2)
    ...
    -0 => "never"                     -- ERROR: this literal case is unreachable; it is already handled above
                                      --   (`-0` is the literal `0`)
    _ => "ok"
match ratio                           -- ratio: Rat — an integer literal resolves at the scrutinee's type
    0 => "zero"                       -- OK: fromNat at Rat, tested through Eq<Rat> — the type's equals
