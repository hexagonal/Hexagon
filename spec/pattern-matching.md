...
| `Float` literal at a position whose type is not `Float` (`0.0 =>` at `Int`; `Some(0.0)` at `Option(Rat)`) | the ordinary type mismatch — `expected Int, found Float` (§2.5, §4); no widening, no fixit of the pattern's own |
| Integer literal at a position whose type honors no `Num` (`0 =>` at `String`, at a record) | what `x == 0` draws at that type: Numeric Literals §6's "integer literal cannot have type `String`" at `String` and a union; Modules §7.6's missing-instance report for `Num`, its home named, at a record — the constraint fails before the restriction is reached (§2.5) |
