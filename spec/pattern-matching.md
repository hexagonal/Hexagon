...
- Literals: an integer literal contributes `Num` and `Eq`, and a negative one `Signed` besides, and unifies with the scrutinee type, its variable resolving to a primitive, or the pattern fails to type (§2.5); a `Float` or `String` literal is its own type, contributes `Eq`, and unifies with the scrutinee (§2.5).
...
match ratio                           -- ratio: Rat — a literal pattern stands at a primitive only
    0 => "zero"                       -- ERROR: 0 is not a pattern at Rat; bind a name and test it in a
                                      --   guard: x when x == 0
    _ => "ok"
match big                             -- big: BigInt (a match at BigInt: #519 lifts today's gate)
