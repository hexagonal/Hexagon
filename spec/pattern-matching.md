...
match size                            -- size: Nat (a match at Nat: #519 lifts today's gate)
    -1 => "never"                     -- ERROR: type Nat has no Signed instance; its only legal homes are …,
                                      --   so this pair's honored set is closed — ascribe the scrutinee a
                                      --   type that honors Signed (match (size: Int)), or remove the arm
    _ => "ok"
match temp
