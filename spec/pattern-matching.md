...
- Patterns never invoke user code except the `Eq` test behind a literal (the type's own — `===` at `Int`, `Nat`, `BigInt`, and `String`, the SameValueZero shape at `Float`, which is every v1 case; §2.5) — with, at a type whose `Num` is its own honoring, the `fromNat` and `negate` that build the literal's constant before the test (§2.5) — and a declared pattern's pure `view`, applied at most once per pattern per position before the arms are tested (Pattern Declarations §5).
