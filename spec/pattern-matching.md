...
- **Once typed, a literal arm tests through `Eq` at the scrutinee's type.** The arm test is `equals(scrutinee, lit)` at that type — exactly the runtime comparison `scrutinee == lit` makes once both stand at one type — and it emits what that comparison emits (Operators §5.1): `===` at `Int`, `Nat`, `BigInt`, and `String`; at `Float` the SameValueZero shape, the inline test or the on-demand `__floatEquals` helper (Decisions Batch §1.5; the choice between the two texts is the emitter's, as at every `Float` comparison, Constraints §6.1) — every case, a literal standing at a permitted primitive only (the typing bullet below). The analogy to `==` is the runtime comparison's and reaches no further: typing a literal pattern is the checking rule below, under which the scrutinee is never widened, where an operand of `==` may be (Numeric Literals §5.1). Typed, the arm differs from a guard `when scrutinee == lit` only in that it is a pattern — it counts for reachability (§7.2), and a guard establishes no coverage (§3, §7.1).
- **A literal pattern is checked at the scrutinee's type, and never widens it.** The type a literal is checked against is its position's (§4): the scrutinee's at the top of an arm, the slot's beneath — a constructor's payload, a tuple's component, a declared pattern's component (Pattern Declarations §3.1) — and the literal is checked against it in §4's checking mode: the literal adapts, the type it meets never widens. An **integer literal** contributes its numeric and equality constraints — `Num` and `Eq`, and `Signed` besides for a negative one (below) — and unifies with the type at its position. A nested position may still have an undetermined type when the literal is checked; ordinary inference and defaulting resolve it — a sibling arm, or Numeric Literals §4's default: `Some(0)` under `match None` is a match at `Option(Int)` — under the order licence §2.2 already grants beneath the top. None of this alters §6.1's requirement at the top: a scrutinee still a variable at dispatch is refused there, before its arms are checked. **The permitted-primitive restriction is checked on the resolved type.** An integer literal's resolved type must be `Int`, `Nat`, `BigInt`, or `Float` — the literal is `fromNat` of its payload there, its value the one `x == 0` compares against — and a resolution to a `Num`-honoring type outside them (`Rat`, a user's `Money`), the constraints satisfied and the check after them, is refused at the literal with the guard named as the spelling: "`0` is not a pattern at `Rat`; bind a name and test it in a guard: `x when x == 0`" — a failure to type, so §7.3's fourth tier reads the arm as `_`. At a type honoring no `Num` (`String`, a record) the constraint fails first, and the report is the literal-type mismatch Numeric Literals §6 prescribes for a failure that traces to a literal — "integer literal cannot have type `String`" — never a constraint failure. Nothing widens: an `Int` scrutinee is matched at `Int`, a `Nat` at `Nat`, and no seat of the numeric lift (Numeric Literals §5.1) exists in a pattern — the lift is an operation's, and a pattern is not one. A **decimal literal** requires `Float`: it is `Float`, monomorphic, in a pattern as in an expression (Numeric Literals §1), and at a position of any other type — `Int`, `Rat` — it is the ordinary mismatch, `expected Int, found Float`, `Float` being the source of neither of §5.1's two conversions. A **String literal** requires `String`. A decimal literal's value is the lexer's (Lexer §5): the correctly rounded binary64 — `1.0e308` is a legal pattern, `1.0e-400` underflows to the pattern `0.0`, and `1.0e309` is the lexer's own overflow error (Lexer §10) and the one report: a construct the lexer has already diagnosed draws no further report from the pattern seat. At its resolved primitive the literal's value is one the compiler computes (§7.2), and the emitted arm carries it in that type's spelling (Numeric Literals §5.2: `0.0` at `Float`, `0n` at `BigInt`). A bare integer literal above `2^53 − 1` is the lexer's error in a pattern as anywhere (Lexer §5); the `n`-suffixed spelling is not a pattern form (§2).
...
- Patterns never invoke user code except the `Eq` test behind a literal (the type's own — `===` at `Int`, `Nat`, `BigInt`, and `String`, the SameValueZero shape at `Float`, which is every case, a literal standing at a permitted primitive only; §2.5) — and a declared pattern's pure `view`, applied at most once per pattern per position before the arms are tested (Pattern Declarations §5).
...
| Integer literal at a position whose type honors no `Num` (`0 =>` at `String`, at a record) | the literal-type mismatch Numeric Literals §6 prescribes, "integer literal cannot have type `String`" — the constraint fails before the restriction is reached, and it is never reported as a constraint failure (§2.5) |
