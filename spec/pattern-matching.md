...
0   1n   1.5   1.50d   "yes" -- literal patterns: bare integer, BigInt, Float, Dec, String (Bool's are constructors, #147)
p1 | p2                  -- or-pattern
...
### 2.5 Literal patterns — bare integer, `BigInt`, `Float`, `Dec`, and `String` forms
...
- **Once typed, a literal arm tests through `Eq` at the type of its position** — the scrutinee's at the top of an arm, the slot's beneath. The arm test is the position's value against the literal, through that type's `equals` — exactly the runtime comparison `scrutinee == lit` makes once both stand at one type — and it emits what that comparison emits (Operators §5.1): `===` at `Int`, `Nat`, `BigInt`, and `String`; at `Float` the SameValueZero shape, the inline test or the on-demand `__floatEquals` helper (Decisions Batch §1.5; the choice between the two texts is the emitter's, as at every `Float` comparison, Constraints §6.1) — at canonical prelude Dec the explicit numerical `Eq<Dec>` test (`dec.md` §8). Each case stands at a permitted literal type (the typing bullet below). The analogy to `==` is the runtime comparison's and reaches no further: typing a literal pattern is the checking rule below, under which the scrutinee is never widened, where an operand of `==` may be (Numeric Literals §5.1). Typed, the arm differs from a guard `when scrutinee == lit` only in that it is a pattern — it counts for reachability (§7.2), and a guard establishes no coverage (§3, §7.1).
- **A literal pattern is checked at the type of its position, and never widens it.** That type is the scrutinee's at the top of an arm and the slot's beneath (§4) — a constructor's payload, a tuple's component, a declared pattern's component (Pattern Declarations §3.1) — and the literal is checked against it in §4's checking mode: the literal adapts, the type it meets never widens. A **bare integer literal** contributes its numeric and equality constraints — `Num` and `Eq`, and `Signed` besides for a negative one (below) — and unifies with the type at its position. A nested position may still have an undetermined type when the literal is checked; ordinary inference and defaulting resolve it — a sibling arm, under the order licence §2.2 already grants beneath the top, or Numeric Literals §4's default, which no order changes: `Some(0)` under `match None` is a match at `Option(Int)`. None of this alters §6.1's requirement at the top: a scrutinee still a variable at dispatch is refused there, before its arms are checked. **The permitted-literal-type restriction is checked on the resolved type.** An integer literal's resolved type must be `Int`, `Nat`, `BigInt`, `Float`, or canonical prelude `Dec` — the literal is `fromNat` of its payload there, its value the one `x == 0` compares against — and a resolution to a type honoring `Num` and `Eq` outside them (`Rat`, a user's `Money` honoring both; a declared variable under `<a: (Num, Eq)>`, reachable beneath the top — `Some(0)` at `Option(a)` — where the guard the refusal names works), the constraints satisfied and the check after them, is refused at the literal with the guard named as the spelling: "`0` is not a pattern at `Rat`; bind a name and test it in a guard: `x when x == 0`" — beneath the top the rewrite replaces the literal with a fresh binder the arm does not already bind, the test going in the arm's guard (§3): `Some(y) when y == 0` — a failure to type, so §7.3's fourth tier reads the arm as `_`. At a type that does not honor what the literal demands the constraint fails first, and **the diagnostic is delegated**: the arm draws the report a failed constraint of `x == 0` draws at that type, one report per unmet constraint, with Method Syntax §9 row 15's written-face rider omitted in pattern position, and no replacement offered. The delegation is of the diagnostic alone — a pattern's typing is this bullet's, not `==`'s. Nothing widens: an `Int` scrutinee is matched at `Int`, a `Nat` at `Nat`, and no seat of the numeric lift (Numeric Literals §5.1) exists in a pattern — the lift is an operation's, and a pattern is not one. An **unsuffixed decimal literal** requires `Float`: it is `Float`, monomorphic, in a pattern (Numeric Literals §1), and at a position of any other type — `Int`, `Rat` — it is the ordinary mismatch, `expected Int, found Float`. The promotion an expression receives at a known exact target (Numeric Literals §5.1) does not reach patterns (below). A **String literal** requires `String`. A decimal literal's value is the lexer's (Lexer §5): the correctly rounded binary64 — `1.0e308` is a legal pattern, `1.0e-400` underflows to the pattern `0.0`, and `1.0e309` is the lexer's own overflow error (Lexer §10) and the one report: a construct the lexer has already diagnosed draws no further report from the pattern seat. At its resolved primitive the literal's value is one the compiler computes (§7.2), and the emitted arm carries it in that type's spelling (Numeric Literals §5.2: `0.0` at `Float`, `0n` at `BigInt`).
- **An unsuffixed decimal-point literal pattern is a `Float` pattern**, even at a
  `Dec` scrutinee: the promotion expressions receive (Numeric Literals §5.1, #525)
  does not reach patterns. At a `Dec` scrutinee the mismatch names the `d`
  spelling of the same value: "…; a `Dec` pattern is written with the `d` suffix:
  `0.5d`". Extending promotion to patterns is #1054.
- **A `d`-suffixed literal requires canonical prelude `Dec`**, with no widening
  of the scrutinee or lookup through a shadowing type/module name. Its exact
  coefficient and decimal places follow `dec.md` §3 and Lexer §5. This is a
  specific additional permitted nominal type, not permission for arbitrary
  `Num`/`Eq` types to supply literal patterns. Bare integer patterns at Dec use
  zero-place integer construction, then the same numerical equality. Tests and
  coverage ignore retained-place differences: `5d`, `5.00d`, and bare `5` at a
  Dec position are one case. Negative zero is the same case as zero. A finite
  list of Dec literals cannot exhaust Dec; its numerical domain is unbounded.
- **An `n`-suffixed literal requires `BigInt`, in a pattern as in an expression** (Numeric Literals §1, #898). `0n`, `9007199254740993n`, and `-9007199254740993n` are literal patterns. The payload retains arbitrary precision; it never passes through a binary64 number. This form introduces no polymorphic `Num` variable: it unifies its monomorphic `BigInt` with the type at its position, resolving an undetermined nested position to `BigInt`, and at another concrete type draws the ordinary mismatch (`expected Int, found BigInt`, for example). Nothing widens. A negative suffixed literal is the exact negation of its BigInt payload; `-0n` is `0n`. Matching uses `Eq<BigInt>` and emits `===` against the corresponding JavaScript bigint literal. These are full literal patterns wherever the grammar admits a pattern, including nested, or-, and as-patterns and exception payloads; ordinary refutability and coverage rules apply. A finite set of BigInt literals still needs a catch-all. Bare integer patterns retain their existing typing and safe-integer payload limit; the suffix is required to write an integer pattern outside that range.
- **An oversized bare integer in a pattern remains a range error, with a context-valid repair** (#898). The lexer detects the out-of-range payload and preserves an invalid recovery form so the enclosing pattern and following arms still parse. At a position resolved to `BigInt`, the diagnostic suggests appending `n`, preserving the sign and all digits. At any other position, or one whose type is unresolved, it reports the range error without that repair or an explicit-conversion suggestion: expressions are not patterns, and a BigInt literal cannot repair a different position type. The invalid literal supplies no numeric inference or literal coverage key. Recovery follows §7.3: read it as `_` for exhaustiveness and irrefutability, never as a shadower for reachability. It draws no additional pattern or layout error merely because its token was invalid.
- **A `Float` literal matches through `Eq<Float>`, which is SameValueZero** (Decisions Batch §1) — the language's one `Float` equality, and the one `==` uses. So the arm `0.0` matches `-0.0`, exactly as `x == 0.0` is true of it: a literal pattern reads as exactly as `==` reads, no more and no less, and a program that must tell the zeros apart does so in a guard, by an operation that can — `Float`'s division follows IEEE 754, so of the two values `x == 0.0` admits, `1.0 / x` sends negative zero alone to `-Float.infinity`, and `x when x == 0.0 and 1.0 / x == -Float.infinity` picks it out, both conjuncts load-bearing (a reciprocal overflows to `-Float.infinity` for every negative value below about `5.6e-309` too) — since neither `==` nor a pattern will. No tolerance is implied and none exists — a computed value matches `0.1` only where `==` says so — and the pattern adds no coercion and no rounding of its own beyond the lexer's. `Float.nan`, `Float.infinity`, and `-Float.infinity` are **terms, not literals** — a pattern names nothing but binders, constructors, and declared patterns (§2.8), and a negated name or a division is an expression — so the named special values are tested in **guards**, which the same equality makes work, `NaN` included:
  ...
  Three guards and a catch-all: the guards contribute nothing to coverage (§3, §7.1), the `_` supplies it by the ordinary rule, and nothing is special-cased for the special values. There is no constant pattern and no expression pattern; the guard is the spelling, and a spelling that reads like a pattern — `Float.nan` or `-Float.infinity` in pattern position — is refused at the name with the guard as its rewrite: "`Float.nan` is a value, not a pattern; bind a name and test it in a guard: `x when x == Float.nan`" — the Rewrite Rule's obligation (Declarations Preamble §1.1), and the former ban's own fixit, kept at the one seat where a reader still reaches for it. **The guard is offered only where it is valid at that position**: the term's type unifying with the position's, and the `Eq` the comparison needs — and, for a negated spelling, the `Signed` — in scope there; where it is not, the report stops at "`Float.nan` is a value, not a pattern" and offers nothing, a fixit that would not compile being worse than none. The same holds of every guard this section names as a rewrite. The refusal is the parser's, and it is general — the message carries the spelling written, `Float.nan` being its instance: a qualified name in pattern position is a constructor's spelling (`Direction.North`, §2.2), and one whose last segment is non-uppercase-start is no constructor's: the parser refuses the form, and the message is selected once names resolve — the sentence above where the segment names a term (`Helper.zero`); Pattern Declarations §7's own where the module exports a pattern of the spelling (`Rat.rat`: "there is no qualified suffix — `rat` is a pattern, written `(a, b)rat`"); and, where the module exports nothing of the spelling (`Rat.zilch`), the report the spelling draws in expression position, "module `Rat` does not export `zilch`" (Modules §3.1, §10), a rewrite never being offered for a value that does not exist. A qualifier that binds no module alias never reaches the selection: it is Modules §5.1 rule 1's report at this seat as at every other — "no module alias `Rat`; `import Rat`" (Modules §10's row names the pattern position). `-` in pattern position is followed by a numeric token, forming the negative literal, or by such a spelling, whose sign rides the value sentence — "`-Float.infinity` is a value, not a pattern; bind a name and test it in a guard: `x when x == -Float.infinity`" — and is dropped by the other two, which name no value. One report, at the name: a pattern that failed to parse is read as `_` by coverage and is never a shadower (§7.3's fourth tier), so no reachability report rides beside it.
- There is no `Char` type in Hexagon; single-character strings are `String` literals like any other.
- Negative literals: `-3`, `-3n`, `-2.5`, and `-2.5d` are legal as literal patterns — the parser forms the signed literal pattern from the `-` and the token, which itself carries no sign (Lexer §5); patterns contain no operators, so there is no unary-minus expression to collide with. `-0.0` is a literal pattern, and it is the pattern `0.0` for every judgment (§7.2). A negative bare integer literal is `negate` over its payload at the type of its position (Numeric Literals §3) and requires `Signed` of it: a pattern at `Int`, `BigInt`, `Float`, and canonical prelude `Dec` (the permitted literal types that honor `Signed`; a `Signed` type outside them is the typing bullet's refusal), and at a type honoring no `Signed` the demand is unmet and the diagnostic is delegated as above — at `Nat`, Modules §7.6's closed-pair sentence, Method Syntax §9 row 15's rider omitted, as in every pattern position: "type `Nat` has no `Signed` instance; its only legal homes are the module declaring `Signed` and `Nat`'s prelude companion module, both outside project source, so this pair's honored set is closed — change the type, or go through the operations those homes export" — at an arm, changing the type is an ascription on the scrutinee, `match (size: Int)`. The demand is judged whatever the payload, `-0` included; only where the sign is admitted is `-0` the literal `0` (§7.2).
...
Patterns contain **no operators, no calls, no expressions**. `C(...)` in pattern position is constructor syntax resolved by the case rule, never a call. There is no evaluation inside a pattern; the only runtime work a pattern performs is tag tests, field/slot reads, and `Eq` tests for literals. Anything computational belongs in a guard. **One exception, and only one:** a declared pattern `(…)name` applies its `view` — a function the declaration requires to be pure, applied at most once per pattern per position (Pattern Declarations §2.1, §5) — which is why it is the exception this sentence can afford.
...
- Literals: a bare integer literal contributes `Num` and `Eq` (a negative one `Signed` besides) and unifies with the type at its position, ordinary inference and defaulting resolving an undetermined nested one; the permitted-literal-type restriction is checked on the resolved type; an `n`-suffixed literal requires `BigInt`, an unsuffixed decimal literal `Float`, a `d`-suffixed literal canonical prelude `Dec`, a String literal `String`; nothing widens, and the arm's test is `Eq` at that type (§2.5).
...
| `(q1, …, qn)name` (declared pattern) | the pattern's subject | irrefutable iff every `qi` is | the view is total — the author's law, unchecked (Pattern Declarations §5) — so the outer form is always "sole constructor" (Pattern Declarations §3.1) |
...
- **Semantics by desugar, normatively.** `match` + arms is `$x => match $x` + arms, for a fresh, unnameable binder. Everything follows from the desugar: the type is `p -> r` (the patterns' type to the type the arm bodies meet at — Numeric Literals §5.1's expression home for numeric bodies — with the expected type flowing in from context as for any lambda — Functions §4.3's channel *(#513)*, whose supplying seats hand the parameter its type before the arms are checked); the full arm grammar with guards; colour — the arrow's colour joins from the arm bodies exactly as any lambda body's (Effects §3.1); single evaluation, trivially.
...
- Infinite domains (`Int`, `Nat`, `BigInt`, `Float`, `Dec`, `String`) are never covered by literals; exactness there means: **a catch-all (`_` or bare variable, possibly under `as`/or-composition per §5.1's coverage semantics) is required.**
...
- **A literal's identity, for both judgments, is its value at the type of its position under that type's `Eq` — never its spelling** — at a type whose `Eq` the compiler computes, the permitted types: `Int`, `Nat` (whose negative literals are refused before any key is read, §2.5), `BigInt`, `Float`, `String`, and canonical prelude `Dec`; at a type outside those permitted types no literal pattern stands (§2.5: the permitted-literal-type restriction, `BigInt` for an `n`-suffixed literal, `Float` for an unsuffixed decimal, canonical Dec for a `d` literal, `String` for a string), so the matrix never equates what it cannot evaluate — it never leans on an unchecked law. At `Int` this makes `7` and `007` one literal (Lexer §5 legalises the leading zeroes) and `-0` the literal `0`. At `BigInt`, bare `7`, `7n`, and `007n` are one literal, as are `0`, `0n`, and `-0n`; two distinct arbitrary-precision integers remain distinct even beyond the safe-integer range. At `Float` the value is the lexer's binary64 and the equality is SameValueZero (§2.5): `1.0`, `1.00`, `1.0e0`, and `10e-1` are one literal; `0.0` and `-0.0` are one; two spellings the lexer rounds to one double — underflow to zero included — are one. So an unguarded `0.0` arm makes a later `-0.0` arm unreachable, and in `1.0 | 1.00` the second alternative is dead, exactly as `0 | 0` is at `Int` — the same hard errors in the same words: "this literal case is unreachable; it is already handled above" at the top of an arm, once per dead alternative, and the general unreachable-arm report where the duplicate sits in a nested column, the matrix judging both. At Dec, the compiler uses the canonical numerical key from `dec.md` §8: trailing-zero variants and all representations of zero coincide, without discarding stored places from values. The canonical stdlib Eq contract governs this key; it is not inferred from arbitrary user code. At the permitted types the coverage key and the runtime test cannot disagree, because they are one function: what the type's `Eq` equates, the matrix equates.
...
4. **No hidden-name tier exists, on purpose.** An opaque union's constructors are unnameable abroad — and no witness may print them. In a well-typed match the tier is empty by construction: a witness that names a constructor can be demanded only by arms that name constructors of the same union (literal and vector-length refutations demand no constructor name), and an opaque union's are unnameable outside its home, so a match over one abroad can only ever be missing `_` itself — or a declared pattern's witness, `(_, _, _)rgb`, whose name is an *exported* one the arms themselves wrote (Pattern Declarations §3.3, §4). On an error program the emptiness is an obligation instead: **a pattern that failed to lex, to parse, or to type must not widen the witness's vocabulary** — at the `match` and `catch` seats and the three §5.3 gates alike, coverage reads the broken pattern as `_` while its row's remaining columns stand as written. The broken pattern is granted maximal cover because the deeper fault leads: a report fires only for cases that stay missing under every repair of the broken pattern, and its witness is built from unbroken patterns alone, so no unnameable constructor can enter it — a match whose one arm is broken reports that arm's own error and nothing else. The grant reaches exhaustiveness and irrefutability only; §7.2 takes the dual reading — an arm is dead only if it stays dead under every repair, so a broken pattern is never a shadower, while a genuine catch-all above still shadows a broken arm below it. No diagnostic under this section ever demands a name the reader cannot obtain — §3's opaque-destructure interception is the same law at the redirect seat: a diagnostic never signposts a spelling the reader cannot write.
...
- Patterns never invoke user code — a literal's `Eq` test is the compiler's, at the permitted type the literal stands at (`===` at `Int`, `Nat`, `BigInt`, and `String`, the SameValueZero shape at `Float`, and canonical stdlib numerical equality at `Dec`; §2.5) — except a declared pattern's pure `view`, applied at most once per pattern per position before the arms are tested (Pattern Declarations §5).
...
| **Refusing `Float` literal patterns** on the ground that `Eq<Float>` is SameValueZero | This spec once did, permanently, reasoning that `NaN` would never match its own literal and that `0.0`/`-0.0` would collapse. The first is false under SameValueZero — `NaN` equals `NaN` — and the second is what `==` does, which a literal pattern follows by §2.5's `Eq` bullet; a pattern is not more exact than the equality it elaborates through, and refusing it taught the wrong lesson about that equality. Lifted (§2.5); the named special values stay guard terms, and no tolerance or constant pattern rides in with the literal. Do not reinstate on the old ground. |
...
| `BigInt` literal at a position whose type is not `BigInt` (`0n =>` at `Int`; `Some(0n)` at `Option(Float)`) | the ordinary type mismatch — `expected Int, found BigInt`, `expected Float, found BigInt` (§2.5, §4); no widening, no replacement that changes the value |
| `Float` literal at a position whose type is not `Float` (`0.0 =>` at `Int`; `Some(0.0)` at `Option(Rat)`) | the ordinary type mismatch — `expected Int, found Float`, `expected Rat, found Float` (§2.5, §4); no widening, no fixit of the pattern's own |
| A literal's contributed constraint unmet at its position's type | the report a failed constraint of `x == lit` draws at that type, one per unmet constraint, Method Syntax §9 row 15's rider omitted in pattern position and nothing offered in its place — a delegation of the diagnostic alone; the constraint fails before the restriction is reached (§2.5) |
| Bare integer literal, its constraints all honored, resolved at a type outside the permitted literal types (`0 =>` at `Rat`, at a user type honoring `Num` and `Eq`; `Some(0)` at `Option(a)` under `<a: (Num, Eq)>`) | "`0` is not a pattern at `Rat`; bind a name and test it in a guard: `x when x == 0`" — beneath the top a fresh binder at the literal's position and the test in the arm's guard, `Some(y) when y == 0` — judged on the resolved type, after inference and defaulting (§2.5) |
| Negative bare integer literal at `Nat` (`-1 =>`, `-0 =>`) — the constraint-unmet row's `Signed` instance, quoted | Modules §7.6's closed-pair report for `Signed` at `Nat`, Method Syntax §9 row 15's rider omitted: "type `Nat` has no `Signed` instance; its only legal homes are the module declaring `Signed` and `Nat`'s prelude companion module, both outside project source, so this pair's honored set is closed — change the type, or go through the operations those homes export" — at an arm, "change the type" is an ascription on the scrutinee, `match (size: Int)` (§2.5) |
| A term's spelling in pattern position — a qualified non-uppercase-start name (`Float.nan`, `Helper.zero`), or `-` before such a name (`-Float.infinity`) — `-` before anything but a numeric token or such a name is the ordinary parse error | "`Float.nan` is a value, not a pattern; bind a name and test it in a guard: `x when x == Float.nan`" — the written spelling, sign included, where the segment names a term; the guard offered only where it is valid at the position — the term's type unifying with the position's, its `Eq` (and, negated, `Signed`) evidence in scope — the sentence otherwise stopping at "is a value, not a pattern"; at the name, one report, no cascade (§2.5, §7.3); a declared pattern's qualified spelling takes Pattern Declarations §7's sentence, and a spelling the module exports nothing for takes "module `Rat` does not export `zilch`" (Modules §3.1, §10), selected once names resolve; a qualifier bound to no module alias is Modules §5.1 rule 1's "no module alias `Rat`; `import Rat`" (Modules §10's row) before any of these (§2.5) |
| Bare integer outside the safe-integer range in a pattern | range error; append-`n` repair only at a position resolved to `BigInt`, no repair at other or unresolved positions; preserve the enclosing pattern and subsequent arms during recovery (§2.5) |
| `Float` literal that overflows binary64 (`1.0e309`) | the lexer's error, "Float literal is too large; use `Float.infinity`" (Lexer §5, §10), the one report — a construct the lexer has already diagnosed draws no further report from the pattern seat — and the named value it points to is a guard's term, not a pattern's (§2.5) |
...
| Literal forms: bare integer / `BigInt` / `Float` / `Dec` / `String`, at the permitted primitive types plus canonical prelude Dec. Literal patterns never widen the scrutinee; suffixed forms have exact types. Runtime tests use Eq; coverage uses the same numerical/text identity, never spelling. Dec trailing-zero variants and bare integers at Dec share keys; all zero representations coincide. Arbitrary user Num/Eq instances do not enable literal patterns; Rat uses its declared pattern or a guard. Negative bare patterns require Signed; Bool uses constructors; no Char | §2.5, §7.2; `dec.md` §3/§8 |
...
5. **Lexer** → `when` and pattern-position `as` need keyword-table entries — **discharged**: Lexer §4.2 carries both rows; no new layout rules (arms unchanged).
...
match temp
    0.0 => "zero"                     -- matches -0.0 too: SameValueZero; emits the SameValueZero test,
                                      --   __floatEquals(temp, 0.0) or the inline shape (Decisions Batch §1.5)
...
match temp
    ...
    0 => "still never"                -- ERROR: the same — an integer literal at Float is checked at Float
                                      --   through Num (§2.5): Float's zero
    1.0 | 1.00 | 1.0e0 => "one"       -- ERROR ×2: this literal case is unreachable; it is already handled above
    _ => "ok"
match count                           -- count: Int
    0.0 => "zero"                     -- ERROR: type mismatch: expected Int, found Float
    _ => "ok"
match count                           -- count: Int — the identity law at Int (§7.2)
    7 => "seven"
    007 => "never"                    -- ERROR: this literal case is unreachable; it is already handled above
    0 => "zero"
    -0 => "never"                     -- ERROR: this literal case is unreachable; it is already handled above
                                      --   (`-0` is the literal `0`)
    _ => "ok"
match ratio                           -- ratio: Rat — the restriction is checked on the resolved type
    0 => "zero"                       -- ERROR: 0 is not a pattern at Rat; bind a name and test it in a
                                      --   guard: x when x == 0
    1.5 => "never"                    -- ERROR: type mismatch: expected Rat, found Float — a decimal is Float
    _ => "ok"
...
match name                            -- name: String — the literal's Num is unmet: what name == 0 draws
    ...
    _ => "ok"
let g<a: (Num, Eq)>(o: Option(a)): String =
    match o                           -- constraints met; the restriction on the resolved type
        Some(0) => "zero"             -- ERROR: 0 is not a pattern at a; bind a name and test it in a
        _ => "other"                  --   guard: Some(y) when y == 0
match big                             -- big: BigInt (a match at BigInt: #519 lifts today's gate; §2.5 settles the literal)
    0 => "zero"                       -- OK: the literal's Num variable resolves to BigInt; emits === 0n
    _ => "big"
match big                             -- big: BigInt — explicit literal spellings (#898)
    9007199254740993n => "large"       -- exact, beyond the bare-literal payload range
    -9007199254740993n => "negative"
    0 => "zero"
    0n => "never"                     -- ERROR: this literal case is unreachable; it is already handled above
    _ => "other"
match count                           -- count: Int
    0n => "never"                     -- ERROR: expected Int, found BigInt — no widening
    _ => "other"
match None
    Some(0n) => "zero"                -- nested inference resolves this to Option(BigInt)
    _ => "other"
match size                            -- size: Nat (a match at Nat: #519 lifts today's gate)
    -1 => "never"                     -- ERROR: type Nat has no Signed instance; its only legal homes are
                                      --   the module declaring Signed and Nat's prelude companion module,
                                      --   both outside project source, so this pair's honored set is
                                      --   closed — change the type, or go through the operations those
                                      --   homes export   [Modules §7.6's report; Method Syntax §9 row 15's rider omitted]
    _ => "ok"
match temp
    ...
    Float.nan => "not a number"       -- ERROR: Float.nan is a value, not a pattern; bind a name and
                                      --   test it in a guard: x when x == Float.nan
    -Float.infinity => "cold"         -- ERROR: -Float.infinity is a value, not a pattern; … x when x == -Float.infinity
    _ => "ok"
