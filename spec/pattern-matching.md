...
| A term's spelling in pattern position — a qualified non-uppercase-start name (`Float.nan`, `Helper.zero`), or `-` before such a name (`-Float.infinity`) — any other `-` in pattern position is the ordinary parse error | "`Float.nan` is a value, not a pattern; bind a name and test it in a guard: `x when x == Float.nan`" — the written spelling, sign included; at the name, one report, no cascade (§2.5, §7.3); a declared pattern's qualified spelling takes Pattern Declarations §7's sentence instead, selected once names resolve |
...
match count                           -- count: Int — the identity law at Int (§7.2)
    ...
    0 => "zero"
    -0 => "never"                     -- ERROR: the same — `-0` is the literal `0`
    _ => "ok"
match temp
