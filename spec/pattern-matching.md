...
| A literal's contributed constraint unmet at its position's type (`0 =>` at `String`, at a record honoring no `Num` or no `Eq`; `-1 =>` at `Nat`; `Some(0)` at `Option(a)` under `<a: Num>`) | exactly what `x == lit` draws at that type, one report per unmet constraint as there — Numeric Literals §6's literal-type mismatch ("integer literal cannot have type `String`"), Modules §7.6's missing-instance report in the form the type decides (`Nat`'s closed-pair sentence is quoted in §2.5 and §15 (k)), Functions §4.2's constraint-list report at a declared variable — this section adding no voice of its own; the constraint fails before the restriction is reached (§2.5) |
| Integer literal resolved at a type honoring `Num` and `Eq` outside the permitted primitives (`0 =>` at `Rat`, at a user type honoring both; `Some(0)` at `Option(a)` under `<a: (Num, Eq)>`) | "`0` is not a pattern at `Rat`; bind a name and test it in a guard: `x when x == 0`" — beneath the top a fresh binder at the literal's position and the test in the arm's guard, `Some(y) when y == 0` — judged on the resolved type, after inference and defaulting (§2.5) |
| Negative integer literal at `Nat` (`-1 =>`, `-0 =>`) — the constraint-unmet row's `Signed` instance, quoted | Modules §7.6's closed-pair report for `Signed` at `Nat`, as `x == -1` draws there: "type `Nat` has no `Signed` instance; its only legal homes are the module declaring `Signed` and `Nat`'s prelude companion module, both outside project source, so this pair's honored set is closed — change the type, or go through the operations those homes export; a written `Int` face runs the operation and admits the result (`let difference: Int = …`)" — at an arm, "change the type" is an ascription on the scrutinee, `match (size: Int)` (§2.5) |
| A term's spelling in pattern position — a qualified non-uppercase-start name (`Float.nan`, `Helper.zero`), or `-` before such a name (`-Float.infinity`) — `-` before anything but a numeric token or such a name is the ordinary parse error | "`Float.nan` is a value, not a pattern; bind a name and test it in a guard: `x when x == Float.nan`" — the written spelling, sign included, where the segment names a term; at the name, one report, no cascade (§2.5, §7.3); a declared pattern's qualified spelling takes Pattern Declarations §7's sentence, and a spelling the module exports nothing for takes "module `Rat` does not export `zilch`" (Modules §3.1, §10), selected once names resolve; a qualifier bound to no module alias is Modules §5.1 rule 1's "no module alias `Rat`; `import Rat`" (Modules §10's row) before any of these (§2.5) |
...
match name                            -- name: String — the literal's Num is unmet: what name == 0 draws
    ...
    _ => "ok"
let f<a: Num>(o: Option(a)): String =
    match o                           -- beneath the top, at a declared variable: Functions §4.2's row
        Some(0) => "zero"             -- ERROR: a is declared to honor Num, but the body requires Eq;
        _ => "other"                  --   write <a: (Num, Eq)>, or remove the constraint annotation …
...
match size                            -- size: Nat (a match at Nat: #519 lifts today's gate)
    -1 => "never"                     -- ERROR: type Nat has no Signed instance; its only legal homes are
                                      ...
                                      --   homes export; a written Int face runs the operation and admits
                                      --   the result (let difference: Int = …)   [as size == -1 draws]
    _ => "ok"
match temp
