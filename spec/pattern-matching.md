...
match size                            -- size: Nat (a match at Nat: #519 lifts today's gate)
    -1 => "never"                     -- ERROR: -1 needs Signed, which Nat does not honor — no Nat is
    _ => "ok"                         --   negative; remove the arm, or match an Int
match temp
