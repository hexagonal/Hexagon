...
| A literal's contributed constraint unmet at its position's type (`0 =>` at `String`, at a record honoring no `Num` or no `Eq`; `-1 =>` at `Nat`; `Some(0)` at `Option(a)` under `<a: Num>`) | for each unmet constraint, exactly what `x == lit` draws at that type — Numeric Literals §6's literal-type mismatch ("integer literal cannot have type `String`"), Modules §7.6's missing-instance report in the form the type decides (`Nat`'s closed-pair sentence is quoted in §2.5 and §15 (k)), Functions §4.2's constraint-list report at a declared variable — this section adding no voice of its own; the constraint fails before the restriction is reached (§2.5) |
| Integer literal resolved at a type honoring `Num` and `Eq` outside the permitted primitives (`0 =>` at `Rat`, at a user type honoring both; `Some(0)` at `Option(a)` under `<a: (Num, Eq)>`) | "`0` is not a pattern at `Rat`; bind a name and test it in a guard: `x when x == 0`" — the guard spelled at the literal's position — judged on the resolved type, after inference and defaulting (§2.5) |
| Negative integer literal at `Nat` (`-1 =>`, `-0 =>`) — the row above's `Signed` instance | Modules §7.6's closed-pair report for `Signed` at `Nat`, as `x == -1` draws there: "type `Nat` has no `Signed` instance; its only legal homes are the module declaring `Signed` and `Nat`'s prelude companion module, both outside project source, so this pair's honored set is closed — change the type, or go through the operations those homes export; a written `Int` face runs the operation and admits the result (`let difference: Int = …`)" — at an arm, "change the type" is an ascription on the scrutinee, `match (size: Int)` (§2.5) |
...
| `match` on constraint-bounded abstract type | "cannot match on a value of abstract type `c`; use the operations its constraints provide" (§6.1) — the name printed where the variable has one, and omitted where an undetermined scrutinee outside the rider has none to print (§7.3's variable-display rule, #649); when the scrutinee is a lambda parameter whose type is an undetermined inference variable, the rider instead: "the parameter's type is not determined here; give the parameter a type — bind the function with its own annotated `let`, or use it where its parameter type is known" (§6.1, #513) |
...
match 0                               -- the scrutinee itself is still a variable at dispatch: §6.1 leads
    0 => "zero"                       -- ERROR: cannot match on a value of abstract type; use the
    _ => "other"                      --   operations its constraints provide — no arm is checked
