...
- Literals: an integer literal contributes `Num` and `Eq`, and a negative one `Signed` besides, and unifies with the scrutinee type; a `Float` or `String` literal is its own type, contributes `Eq`, and unifies with the scrutinee (§2.5).
...
| Literal pattern at a non-primitive scrutinee (`0 =>` at `Rat`, at a user `Num` type) | "`0` is not a pattern at `Rat`; bind a name and test it in a guard: `x when x == 0`" — the written spelling, at the literal (§2.5) |
...
| Literal patterns: `Int`/`Float`/`String` via `Eq` (`Bool` removed 2026-07-29, #147 — constructor patterns now); ordinary inference (`Num` + `Eq` constraints for integer literals; a decimal literal is `Float`); a `Float` literal tests through SameValueZero and emits what `==` emits; named special values are guard terms; a literal's coverage identity is its value at the scrutinee's type under its `Eq`, never its spelling, at the primitives, whose `Eq` the compiler computes (`007` is `7`, `-0` is `0`, `-0.0` is `0.0`) — the former permanent `Float` ban is lifted (§10); no `Char` | §2.5, §7.2 |
