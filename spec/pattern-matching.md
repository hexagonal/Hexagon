...
| Integer literal at a position whose type honors no `Num` (`0 =>` at `String`, at a record) | Modules §7.6's missing-instance report for `Num` at the type, as `x == 0` draws there — the constraint fails before the restriction is reached (§2.5) |
