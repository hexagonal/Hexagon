...
| Constructor pattern over an opaque type abroad — a written head or the door's (§2.2) | the opaque family's sentence at the type's noun: "cannot destructure opaque record `Point`; use an operation exported by its home module", "cannot destructure opaque union `Handle`; use an operation exported by its home module" (§2.4; Modules §4.2) |
