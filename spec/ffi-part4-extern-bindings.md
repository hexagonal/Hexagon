...
The foreign module specifier is a string literal and **may be a bare package specifier** (`"tiny-json"`, `"node:url"`). This construct is explicitly outside Hexagon-to-Hexagon import resolution: a Hexagon `import` names a module and carries no specifier at all (Modules §3, #829), so the string here is JavaScript's and only JavaScript's. Relative specifiers addressing foreign `.js`/`.ts` files are equally legal, in an `extern from` head and in `extern import` (§8) alike, and they too are JavaScript's own: a relative specifier is emitted **verbatim** and resolves from wherever the host writes the emitted module (Modules §11.1; Packages §6), never from the source file's directory. What the specifier must not name is another Hexagon module — Hexagon-to-Hexagon linkage is `import`'s job, and the diagnostic for an extern specifier resolving to a `.hex` source says so (§13). That refusal is the one reading the specifier ever gets from the compiler, taken once against the source tree; nothing else reads it, and emission copies it unchanged.

`extern from "./world.js"` in `module Deep.Nested` names `Deep/world.js` beside the emitted `Deep/Nested.js`, wherever the source file sat. The compiler rewrites nothing and places nothing: the file is the host's or the author's to put there, and a specifier naming a file nobody put there fails as any missing relative import fails. A JavaScript package is never a Hexagon dependency (Packages §4.4); this block is how it enters.
...
| Bare package specifiers legal (outside Hexagon-to-Hexagon resolution); a relative specifier emitted verbatim, resolving from the emitted file's place and never the source file's; a specifier must not name a Hexagon module — the one reading the compiler gives it | §2.1 |
