...
**Status:** Decided (July 2026; every callable row writes its effect arrow, and the impure default with the `pure`/`conduit` claims retire — #869, §4.5; a callable binding whose signature names a captured foreign collection denotes a stable copying wrapper and re-exports it; an exported `let` naming one is refused — #876, §4.3, §4.5, §7), revised in place after external review (Sol) before landing. Normative promotion of `spec/notes/ffi-proto-spec-questions.md` §5 — the core syntax, default-export binding, and effect-import packages — excluding the receiver-member and class material, which is Part 5's. The draft's four promotion questions were resolved in §12: unused binding imports may be elided; `extern let` asserts stability; exported extern types receive a generated opaque brand; generic extern declarations are deferred from v1.
...
`fun match(pattern: String) -> Matcher` is `PathToRegexp.match("/user/:id")` in an
importer. An `as` alias remains the author's way to choose a different name, or a
name the module can write bare.
...
A signature that names a **captured foreign collection** (Part 1 §2.2) — `fun rows() ->! Array(Int)`, a parameter of type `JsSet(String)` — takes the same shape on the same terms *(#876)*: the local extern binding denotes **one stable module-level copying wrapper**, allocated once, which runs Part 1 §5.4's walk over each captured argument on the way out and the captured result on the way in, then calls the raw import. Every reference observes that one wrapper; the raw import is never the Hexagon value, since detaching it would hand out a function that crosses arrays uncopied. Direct calls need not route through the wrapper — the emitter may inline the copy at the call — but the identity a first-class reference observes is the wrapper's. This is Part 1 §4.1's Functions row for extern bindings, and the extern-side twin of Part 7 §7 occasion 4.
...
- **Non-callable rows are unchanged.** `let version: String` keeps `:` — a value reference is colourless (Effects §2.6), so there is no colour to write; `type` declares no invocation, and an `enum` row (Foreign Enums §2.1) or a `class` header (Part 5 §6) introduces a type and its constructors or members, nothing invocable at the head. Foreign module initialization and `extern import` (§8) are unchanged.
- **The retired forms.** The impure default, the contextual `pure` and `conduit` modifiers, and `pure conduit`'s refusal are gone, with Effects §9's five rows and Lexer §4.2's two contextual words. A callable row written with `:` before its result takes §13's colon row. A retired word takes §13's redirect, whose whole rule is this bullet; §13 cites it.
  - *The seat.* Anywhere in an extern declaration head ahead of the keyword: immediately before `fun`, `let`, `type`, `enum`, `class`, or a Part 5 member keyword, before or after a leading modifier (`export`, `default`, `static`), or beside the other retired word. The parser selects the seat by position; the words are ordinary names everywhere else (Lexer §4.2).
  - *The partition.* A row **declares a callable** when it is a `fun` with a parameter list, a Part 5 `method`/`get`/`set`/`new` row, instance or static, standalone or in a class block, or a `let` that a parameter list or a function-typed annotation makes callable-intended (§4.1, §4.2). A row **declares nothing invocable** otherwise: a `let` with neither, `type`, `enum`, `class`, or a `fun` without a parameter list, whose rewrite is a `let` (§4.2).
  - *The report.* One per row, naming the words as written. On a row that declares a callable: "`pure` is retired — write the pure arrow on the row itself: `fun trim(document: String) -> String`"; "`conduit` is retired — write `->?` on the row's outer arrow: `fun runner(step: () ->? String) ->? Int`"; for both words, in either order, "`pure conduit` is retired — write `->?` on the row's outer arrow: `fun runner(step: () ->? String) ->? Int`" — the row the pair described is the conduit, `->?` being the weaker claim, so the colon never becomes the stronger arrow. Where the rewrite places no arrow because the row already writes one, the sentence's arrow clause gives way: "`pure` is retired, and this row's arrow is written — drop the word" (likewise for `conduit` and the pair); on a `set` row it reads "`pure` is retired, and a `set` row's arrow is `->!` — drop the word" (Part 5 §4.1). Where the row carries no `->?` parameter for a `->?` to link, the `conduit` and the pair report read instead "`conduit` is retired, and nothing this row is handed carries `->?` — write `->?` on the callback parameter this row runs, or write `->!`". Before a row that declares nothing invocable: "`pure` is retired, and a value reference is colourless — drop the word" (`let`, and `fun` without a parameter list) or "`pure` is retired, and a type declares nothing invocable — drop the word" (`type`, `enum`, `class`), the same sentences with `conduit`, or with both words in either order, in place of `pure`.
  - *The rewrite.* One per report, composed in this order: drop the words; before a row that declares nothing invocable, that is the whole rewrite; on a `set` row the arrow is `->!` whatever the words claimed and whatever the row writes (Part 5 §4.1); where the row already writes its arrow, the written arrow stands, and so does the arrow a function-typed `let`'s annotation writes (that §13 row spells the `fun` and its arrow both); where the `let`-with-parameters row fires, it spells the `fun` and the words supply its arrow by the clause that follows; otherwise the arrow the words said takes the colon's place — `->` for `pure`; `->?` for `conduit` and for the pair, or `->!` in its place where no `->?` parameter would link it. One migration, one report: the colon row, and Effects §4.4's inlet-less row at the row's outer arrow, are not also reported on a row that drew the redirect; a `->?` nested inside the signature with no inlet is that row's own defect and stays reported.
...
An extern `type` need not correspond to any exported TypeScript type in the foreign package; it names a Hexagon-side contract about values that foreign API produces and consumes. Two extern `type` declarations, even against the same foreign name in the same specifier, are distinct nominal Hexagon types — sharing a type across binding modules is done by exporting and importing the Hexagon type like any other (§7). *(#927.)* The intrinsic block is the one delta, under Intrinsics §3.1's licence for that block to state them: two `type` rows naming the same intrinsic key bind one compiler type (Intrinsics §3.3).
...
|---|---|---|
| extern callable declared with `let` (parameter list present) | "extern callable declarations use `fun` and write their effect arrow; write `fun parse(text: String) ->! JsonValue`" — this redirect fires first, and its rewrite already spells the arrow, so the colon row below is not also reported; a retired word on the row takes §4.5's redirect, whose composed rewrite is this row's `fun` with the words' arrow, one edit | §4.2, §4.5 |
| extern `let` annotated with a function type | "extern callable declarations use `fun`; a binding of type `Int -> Int` is callable — write `fun f(x: Int) -> Int`" — a retired word on the row takes §4.5's redirect, whose rewrite is this row's, the annotation's arrow standing, one edit | §4.1; Part 6 §2.4 |
| extern `fun` without a parameter list | "extern `fun` declares a callable and requires a parameter list; for a foreign value, write `let version: String`" — this row declares nothing invocable, so neither the colon row below nor the arrow redirects reach it; a retired word on it takes §4.5's drop-the-word report | §4.2 |
...
| the retired `pure`, `conduit`, or both, at the retired seat, on a row that declares a callable (§4.5) | §4.5's report for the words as written + §4.5's composed rewrite; the colon row and the inlet-less row are not also reported | §4.5 |
| the retired `pure`, `conduit`, or both, before a row that declares nothing invocable (§4.5) | §4.5's drop-the-word report for the row's kind + its fixit | §4.5 |
| `->?` on a callable row whose parameters carry no `->?` | Effects §4.4's inlet-less-signature row — "nothing a caller of this signature supplies carries `->?`, so nothing instantiates it" + fixit `->!` — carrying §4.5's advice in words at an extern callable row, foreign or intrinsic | §4.5; Effects §4.4, §9 |
...
| exported extern `let` whose type names a captured foreign collection | Part 1 §5.4 item 4's hard error at the export; rewrite: keep the row unexported and expose a Hexagon function whose result is walked | §7 |
...
| **Every callable row writes its effect arrow** (#869): `->` the trusted purity claim, `->!` the honest unknown and the join, `->?` the declared conduit under the inlet rule; no `:` on a callable row, omission a hard error with `->!` as fixit; intrinsic rows write theirs too, verified rather than trusted; `pure`/`conduit` retired; independently coloured slots still take `->!` | §4.5 |
