...
- **The retired forms.** The impure default, the contextual `pure` and `conduit` modifiers, and `pure conduit`'s refusal are gone, with Effects §9's five rows and Lexer §4.2's two contextual words. A row written with `:` before its result, or with `pure`/`conduit` before its keyword, takes §13's redirects: on a row that declares a callable, the arrow that says what the old form said; before a row that declares nothing invocable, the colourless refusal whose whole repair is dropping the word. A row written with both words is one report naming both, and the row it described is the conduit. The redirect's rewrite composes by one rule: drop the words; where the result is written after `:`, the arrow the words said takes the colon's place — `->!` where a `->?` would have no inlet, and on a `set` row; where the row already writes an arrow, that arrow stands. One migration, one report: a redirect whose rewrite spells the arrow suppresses the colon row, and a rewrite that would be refused as inlet-less names `->!` instead.
...
| extern `fun` without a parameter list | "extern `fun` declares a callable and requires a parameter list; for a foreign value, write `let version: String`" — this row declares nothing invocable, so neither the colon row below nor the arrow redirects reach it; a retired word on it takes the nothing-invocable row below | §4.2 |
...
| the retired `pure`, `conduit`, or both, at the retired seat — immediately before an extern declaration keyword (`fun`, `let`, `type`, `enum`, `class`, or a Part 5 member keyword), before a leading modifier (`default`, `static`), or before the other retired word; position-selected by the parser at an extern declaration head, the words having left the contextual table (Lexer §4.2) — on a row that declares a callable: `fun`, a Part 5 `method`/`get`/`set`/`new` row in either form, or the `let` a parameter list makes callable-intended (the row at the head of this table) | one report per row, naming the words written: "`pure` is retired — write the pure arrow on the row itself: `fun trim(document: String) -> String`"; "`conduit` is retired — write `->?` on the row's outer arrow: `fun runner(step: () ->? String) ->? Int`"; "`pure conduit` is retired — write `->?` on the row's outer arrow: …" + one fixit per §4.5's composition rule: drop the words; where the result is written after `:`, the arrow the words said takes the colon's place — `->` for `pure`, `->?` for `conduit` and for the pair, `->!` where the row has no `->?` parameter for a `->?` to link (the message adding "— nothing this row is handed carries `->?`") and on a `set` row whatever the words claimed (Part 5 §4.1); where the row already writes its arrow, or the `let`-with-parameters row's rewrite supplies one, the fixit drops the words only and the written arrow stands. The colon row and the inlet-less row below are not also reported | §4.5 |
| the retired `pure` or `conduit` before a row that declares nothing invocable — `let` without a parameter list, `type`, `enum`, `class`, or `fun` without a parameter list | the word has no arrow to redirect to, and the row says so instead: "`pure` is retired, and a value reference is colourless — drop the word" (`let`, and `fun` without a parameter list), "`pure` is retired, and a type declares nothing invocable — drop the word" (`type`, `enum`, `class`); the same two sentences with `conduit` in place of `pure` + fixit: drop the word | §4.5 |
| `->?` on a callable row whose parameters carry no `->?` | Effects §4.4's inlet-less-signature row — "nothing a caller of this signature supplies carries `->?`, so nothing instantiates it" + fixit `->!` — carrying §4.5's advice in words at an extern callable row, foreign or intrinsic: "write `->?` on the callback parameter this row runs, or write `->!`" | §4.5; Effects §4.4, §9 |
