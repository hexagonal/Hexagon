...
A signature that names a **captured foreign collection** (Part 1 §2.2) — `fun rows() ->! Array(Int)`, a parameter of type `JsSet(String)` once #875 lands — takes the same shape on the same terms *(#876)*: the local extern binding denotes **one stable module-level copying wrapper**, allocated once, which runs Part 1 §5.4's walk over each captured argument on the way out and the captured result on the way in, then calls the raw import. Every reference observes that one wrapper; the raw import is never the Hexagon value, since detaching it would hand out a function that crosses arrays uncopied. Direct calls need not route through the wrapper — the emitter may inline the copy at the call — but the identity a first-class reference observes is the wrapper's. This is Part 1 §4.1's Functions row for extern bindings, and the extern-side twin of Part 7 §7 occasion 4.
