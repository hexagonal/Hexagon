...
| `pure` before an extern declaration keyword | "`pure` is retired — write the pure arrow on the row itself: `fun trim(document: String) -> String`" + fixit: drop the word, `->` before the result. Its rewrite already spells the arrow, so — as with the `let`-with-parameters row — the colon row above is not also reported for a row written wholly in the retired form: one migration, one report. Before a non-callable keyword the word has no arrow to redirect to, and the row says so instead: "`pure` is retired, and a value reference carries no colour — drop the word" (`let`), "…and a type has no face — drop the word" (`type`, `enum`) + fixit: drop the word | §4.5 |
| `conduit` before an extern declaration keyword | "`conduit` is retired — write `->?` on the row's outer arrow: `fun runner(step: () ->? String) ->? Int`" + fixit: drop the word, `->?` before the result. The same two clauses as the `pure` row: the colon row is not also reported, and before a non-callable keyword the message is the colourless one with the word dropped | §4.5 |
| `->?` on a callable row whose parameters carry no `->?` | Effects §4.4's inlet-less-signature row — "nothing a caller of this signature supplies carries `->?`, so nothing instantiates it" + fixit `->!` — carrying §4.5's advice in words at an extern row, foreign or intrinsic: "write `->?` on the callback parameter this row runs, or write `->!`" | §4.5; Effects §9 |
