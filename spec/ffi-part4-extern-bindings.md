...
| `pure` immediately before `fun`, `let`, `type`, or `enum` in an extern block, on a callable row (`fun`, or `let` with a parameter list — the first row above) | "`pure` is retired — write the pure arrow on the row itself: `fun trim(document: String) -> String`" + fixit: drop the word, `->` before the result. Its rewrite already spells the arrow, so — as with the `let`-with-parameters row — the colon row above is not also reported for a row written wholly in the retired form: one migration, one report. Two retired words together (`pure conduit fun`) are two redirects, one per word — what the suppression removes is the colon row, never a second word's own report | §4.5 |
| `conduit` immediately before `fun`, `let`, `type`, or `enum` in an extern block, on a callable row | "`conduit` is retired — write `->?` on the row's outer arrow: `fun runner(step: () ->? String) ->? Int`" + fixit: drop the word, `->?` before the result. The `pure` row's two clauses apply: the colon row is not also reported, and a second retired word is its own redirect | §4.5 |
| the retired `pure` or `conduit` before a row that declares no callable (`let` without a parameter list, `type`, `enum`) | the word has no arrow to redirect to, and the row says so instead: "`pure` is retired, and a value reference is colourless — drop the word" (`let`; `conduit` likewise), "`pure` is retired, and a type declares no invocation — drop the word" (`type`, `enum`; `conduit` likewise) + fixit: drop the word. A `let` carrying a parameter list is a callable row and takes the first row above, its arrow named as on a `fun` row | §4.5 |
| `->?` on a callable row whose parameters carry no `->?` | Effects §4.4's inlet-less-signature row — "nothing a caller of this signature supplies carries `->?`, so nothing instantiates it" + fixit `->!` — carrying §4.5's advice in words at an extern row, foreign or intrinsic: "write `->?` on the callback parameter this row runs, or write `->!`" | §4.5; Effects §4.4, §9 |
