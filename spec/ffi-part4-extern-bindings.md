...
- **Non-callable rows are unchanged.** `let version: String` keeps `:` — a value reference is colourless (Effects §2.6), so there is no colour to write; `type` declares no invocation, and an `enum` row (Foreign Enums §2.1) introduces a type and nullary constructors, nothing invocable. Foreign module initialization and `extern import` (§8) are unchanged.
- **The retired forms.** The impure default, the contextual `pure` and `conduit` modifiers, and `pure conduit`'s refusal are gone, with Effects §9's five rows and Lexer §4.2's two contextual words. A row written with `:` before its result, or with `pure`/`conduit` before its keyword, takes §13's redirects: on a callable row, the arrow that says what the old form said; before a row that declares nothing invocable, the colourless refusal whose whole repair is dropping the word. Two retired words together are two redirects, one per word; what a redirect's rewrite suppresses is the colon row, never a second word's own report.
...
| extern `fun` without a parameter list | "extern `fun` declares a callable and requires a parameter list; for a foreign value, write `let version: String`" — the rewrite is a `let`, which keeps `:`, so the colon row below is not also reported on it, and a retired word on such a row takes the nothing-invocable row below, the rewrite's `let` having no arrow to write | §4.2 |
...
| the retired `pure` at its seat — immediately before an extern declaration keyword (`fun`, `let`, `type`, `enum`, or a Part 5 member keyword), before `default`, or before a second retired word — on a callable row (`fun`, or `let` with a parameter list) | "`pure` is retired — write the pure arrow on the row itself: `fun trim(document: String) -> String`" + fixit: drop the word, `->` before the result. Its rewrite already spells the arrow, so the colon row above is not also reported. On a `let` with a parameter list the `let`-with-parameters row (row 1) fires first and its rewrite already carries the arrow, so this row's fixit drops the word only | §4.5 |
| the retired `conduit` at the same seat, on a callable row | "`conduit` is retired — write `->?` on the row's outer arrow: `fun runner(step: () ->? String) ->? Int`" + fixit: drop the word, `->?` before the result; the `pure` row's two clauses apply | §4.5 |
| the retired `pure` or `conduit` before a row that declares nothing invocable — `let` without a parameter list, `type`, `enum`, or `fun` without a parameter list (whose rewrite is a `let`) | the word has no arrow to redirect to, and the row says so instead: "`pure` is retired, and a value reference is colourless — drop the word" (`let`, and `fun` without a parameter list), "`pure` is retired, and a type declares nothing invocable — drop the word" (`type`, `enum`); the same two sentences with `conduit` in place of `pure` + fixit: drop the word | §4.5 |
