...
- **The retired forms.** The impure default, the contextual `pure` and `conduit` modifiers, and `pure conduit`'s refusal are gone, with Effects §9's five rows and Lexer §4.2's two contextual words. A callable row written with `:` before its result takes §13's colon row. A retired word takes §13's redirect, whose whole rule is this bullet; §13 cites it.
  ...
  - *The partition.* A row **declares a callable** when it is a `fun` with a parameter list, a Part 5 `method`/`get`/`set`/`new` row in either form, or a `let` that a parameter list or a function-typed annotation makes callable-intended (§4.1, §4.2). A row **declares nothing invocable** otherwise: a `let` with neither, `type`, `enum`, `class`, or a `fun` without a parameter list, whose rewrite is a `let` (§4.2).
  - *The report*, one per row, naming the words as written. On a row that declares a callable: "`pure` is retired — write the pure arrow on the row itself: `fun trim(document: String) -> String`"; "`conduit` is retired — write `->?` on the row's outer arrow: `fun runner(step: () ->? String) ->? Int`"; for both words, in either order, "`pure conduit` is retired — write `->?` on the row's outer arrow: `fun runner(step: () ->? String) ->? Int`" — the row it described is the conduit, `->?` being the weaker claim, so the migration never strengthens what the row said. Where the row carries no `->?` parameter for a `->?` to link, the `conduit` and the pair report read instead "`conduit` is retired, and nothing this row is handed carries `->?` — write `->?` on the callback parameter this row runs, or write `->!`". Before a row that declares nothing invocable: "`pure` is retired, and a value reference is colourless — drop the word" (`let`, and `fun` without a parameter list) or "`pure` is retired, and a type declares nothing invocable — drop the word" (`type`, `enum`, `class`), the same sentences with `conduit` in place of `pure`.
  - *The rewrite*, one per report, composed in this order: drop the words; before a row that declares nothing invocable, that is the whole rewrite; on a `set` row the arrow is `->!` whatever the words claimed and whatever the row writes (Part 5 §4.1); where the row already writes its arrow, or another §13 row's rewrite supplies one (the `let`-with-parameters and function-typed-`let` rows spell the `fun` and its arrow), the written arrow stands; otherwise the arrow the words said takes the colon's place — `->` for `pure`; `->?` for `conduit` and for the pair, or `->!` in its place where no `->?` parameter would link it. One migration, one report: the colon row and Effects §4.4's inlet-less row are not also reported on a row that drew the redirect.
...
| the retired `pure` or `conduit` before a row that declares nothing invocable (§4.5) | §4.5's report for the row's kind — a value reference is colourless, or a type declares nothing invocable — + fixit: drop the word | §4.5 |
