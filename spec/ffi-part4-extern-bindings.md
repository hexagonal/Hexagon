...
| extern specifier naming a `.hex` source, extension written or not (read once from the importing file's own directory, §2.1) | "use `import` for Hexagon modules; `extern from` is for foreign JavaScript" | §2.1 |
