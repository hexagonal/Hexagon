...
| Bare package specifiers legal (outside Hexagon-to-Hexagon resolution); a relative specifier emitted verbatim, resolving from the emitted file's place and never the source file's; a specifier must not name a Hexagon module (that refusal is the compiler's one reading of the specifier) | §2.1 |
