...
*(Landed, #344 — the last companion milestone.)* Both live in `stdlib/Float.hex` as **plain exports** — `Float` is not `Integral` (Integral spec §1), so unlike every integer type's family these are not constraint members and never join a dispatch set. `rem` is the companion's intrinsic-door declaration (`floatRem`, lowering the bare `a % b` §6 tables); `mod` is ordinary Hexagon above it — the §5.1 adjustment written over `rem`, the ordinary comparison operators, `Num<Float>`'s add, and `Signed<Float>`'s negate, which is exactly the strictly-simpler layering `Int.hex` uses for its Euclidean family. One consequence rode in with the landing, accepted when it was ruled (Modules §5.5): bare `mod` and bare `rem` each gained a **second exporter** — `Integral.hex` exports them as members, `Float.hex` as plain functions — so the bare spellings are refused in favor of the qualified homes (`Integral.mod`/`Integral.rem` for the polymorphic members, `Float.mod`/`Float.rem` for the float family) and the dot call, which is type-directed and never reads the bare layer. The corpus had zero bare consumer uses and one generic conformance pin, rewritten to the dot call — the same accepted regression shape as `concat` (Concat + `Seq.hex`) and `fromInt` (Signed + `Nat.hex`, Primitive Types §1).
