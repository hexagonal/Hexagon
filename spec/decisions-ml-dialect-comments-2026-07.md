...
The second clause closes a **pre-existing gap**: a *nested* Hexagon block comment was never verbatim-emittable into JS — `/* outer /* inner */ outer */` is broken JavaScript, since JS does not nest — and the old §6 never said what the emitter should do about it. The re-spelling forced the translation question and the answer covers the old gap for free. Recorded in Comments' correction record (§11 there) as a gap closure.

*(Corrected 2026-07-31, at implementation: this section closed by calling the clause "not a behaviour change — no emitter currently preserves block comments". That was wrong. hexc preserves top-level comments in source order, block comments among them, so under the re-spelling it would have emitted `(* … *)` into JavaScript. The clause is a live obligation and §11's emitter bullet is superseded accordingly.)*
...
| Book: `chapters/05-layout.md`, `plans/05-layout.md`, `FEATURES.md`, `CONTINUITY.md` | The comments section re-taught with the new spelling and the JS redirect; the summary bullet, plan audit notes, features list, and continuity commitment re-spelled. |
...
**Status: applied 2026-07-31**, in one pass across hexc, the grammar, and the VS Code configuration. Each bullet below stands as written, with what landed noted at its end; the two places the notes turned out to be wrong — the emitter (§7's correction above) and the auto-closing pair — are marked in place.

- **hexc lexer** (`compiler/src/passes/lexer/lexer.ts`): re-spell the block-comment trivia scanner; add the `/*`/`*/` detection with the §4 messages and the recommended scan-to-`*/` recovery; the two existing test-pinned messages ("unmatched `` `*/` ``…", "unterminated block comment…") re-spell with their tests. Comments §5 messages remain the verbatim source of truth (Lexer §10's row). *(Applied. The `/*` redirect carries a two-edit fix — both delimiters at once — because rewriting the opener alone would leave the JavaScript closer inside an unterminated Hexagon comment. The stray-`*/` redirect carries no fix: rewriting it to `*)` only exchanges one error for the unmatched-closer one.)*
- **TextMate grammar** (single grammar since #163, shared by the VS Code extension and the Playground): comment scopes re-spelled; the `(*`-family begin/end rules are bracket-free, so this does not interact with #162's unterminated-bracket repaint beyond the repaint any unterminated block comment already causes. The grammar's inline notes citing Comments §3 (`editors/vscode/syntaxes/hexagon.tmLanguage.json`) go stale with the spelling and update in the same pass. **Correction (2026-07-30, once #162 was built):** the "does not interact" clause understates it. #162 split every type-context bracket group into a hanging and a bounded rule, so there are now seven `\(`-based `begin`s in that area, and each needs `(?!\*)` — include order will not do, because `#type-declaration-parameters` and the `derives` rules begin matching before their `(` and would otherwise claim the `(` of a `(*` opener. A trailing `(* … *)` after an opening bracket also reads as content to the hanging classifier, demoting the group to bounded; that is the spelling-independent behaviour the grammar's `//line-bail-guard` note records, not a new cost of the respell. *(Applied, all seven guards, pinned by `grammar.test.ts` like the bail-out guard's copies. Exactly two change any paint — the *bounded* `#type-declaration-parameters` rule and the *bounded* `derives` rule, both from the family this note predicted; the hanging variants cannot read a `(*` opener as end of line, and the remaining sites reach `#comments` first anyway. *(Count established by mutating each guard in turn; two earlier statements of it, "three redundant" and then "four load-bearing", were both wrong.)* **What the note missed:** guarding the `begin` is half the fix. A group's own `patterns` are the only rules tried inside it, so each of those bodies also had to include `#comments` — without it a `(*` inside the group fell through to `#punctuation` and the `*)` closed the group at its `)`, losing the colour the group exists to give. Found by the cold review, which reproduced it against `HEAD`. The JS spellings became a painted error too: `invalid.illegal.javascript-comment.hexagon`, one region per pasted `/* … */`, ended by `*/` or by #162's bail-out guard — a fourteenth copy of it. Ending the region at `$` instead was tried and rejected: it split a pasted JSDoc block into an error line plus `@param` painted as an invalid character, and let the tab rule fire on a tab in the comment body, which Comments §4 says is not indentation. It needed a colour in both themes and in Playground's map.)*
- **VS Code language configuration** (`editors/vscode/language-configuration.json`): `blockComment`, the `/*` auto-closing pair, and the `/**` `onEnterRules` all encode the JS spelling — until updated, Toggle Block Comment *inserts* the now-illegal form. Its spec-citing test (`editors/vscode/src/language-configuration.test.ts`, "matches the forms spec/comments.md §1 defines") currently passes while asserting the superseded forms and must flip with the config. Implementer caveat (cold review, finding 8): a `(*` auto-closing pair interacts with the existing `(` → `)` pair — not a pure find-and-replace. *(Applied, and the caveat decided the question: **there is no `(*` auto-closing pair.** It would fire only after `(` had already auto-closed, leaving `(* *))` with the first pair's `)` stranded; without it the same keystrokes work, since `(` gives `()`, `*` gives `(*)`, and the typed closing `)` types over the auto-inserted one. Toggle Block Comment inserts the pair from `comments.blockComment`, which is the affordance that mattered. `onEnterRules` re-spelled with `(**)` excluded from the doc-opener the way `/**/` was.)*
- **Emitter:** ~~nothing until block-comment preservation is implemented~~ — **superseded**: hexc already preserves top-level comments, so §7's translation clause was live from the moment the lexer re-spelled. *(Applied: a block comment emits as `/* body */`; a body containing `*/` emits as a run of whole-line `//` comments. Interior `(*`/`*)` pairs ride along as inert JavaScript text, which is what makes a nested comment emittable at all.)*
- **Formatter note** (`notes/canonical-formatting-and-naming.md`, non-normative): inherits the spelling on next touch. *(Still owed — verified at implementation: the note spells no block comment, so there was nothing to re-spell.)*
- **Conformance:** lex `(* (* *) *)` to trivia with correct depth; `(**)` empty; unterminated `(*` errors at the innermost opener; `*)` at depth zero errors; `/*`, `/**`, and stray `*/` produce the §4 redirects and the recommended single-diagnostic recovery; `x --1` still lexes `x - (-1)`; a `(* *)` inside `${...}` is trivia; strings still opaque to comments and vice versa. *(Applied as lexer tests — `lexer.test.ts` is where the §5 messages are pinned — plus the grammar's own scope assertions for the same forms. Comments §5's third row, the nesting **hint** on an unterminated comment whose body contains balanced pairs, is marked optional there and remains unimplemented; the depth suffix it accompanies is implemented.)*
