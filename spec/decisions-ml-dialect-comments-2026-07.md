...
- **No prefix `*` exists** — no dereference, no glob, no splat (Lexer §8.3's absent-token inventory; the only prefix operators in the language are unary `-` and `not`, Operators §3). So after `(`, a legal program can never continue with `*`: infix `*` needs a left operand and `(` provides none. **`(*` as a comment opener therefore steals no legal program.**
...
- The same argument, run for the JS spellings, licenses §4's detection: `/` and `*` are both infix-only, so `/*` and `*/` are never two adjacent tokens of a legal program either. Detection forfeits nothing.
...
> Emitted comments use **JavaScript's** comment syntax and the emitted file must remain syntactically valid JS. `//` comments emit verbatim. A `(* ... *)` comment whose body contains neither a comment delimiter nor the sequence `*/` emits as `/* body */`. Any other body — one containing nested comments or a literal `*/` — is re-presented as a run of `//` lines. Content is preserved; the presentation choice is quality-of-implementation.
...
What they must *not* be: history, doctrine, ruling numbers, spec cross-references, or change narration. That material has homes — the spec corpus, the decisions documents, issues, and git history — and a comment pointing *out* of the source into those homes is the exception that proves the rule, spent sparingly (a load-bearing normative fact with no code expression, e.g. "constructor order is normative," earns its line; its justification does not). The motivating specimen is `stdlib/Bool.hex` at ruling time: ~24 comment lines of doctrine and citations, all of it already recorded in the #147 closure document, in front of a three-line declaration. The doctrine is prospective for new code immediately; the sweep bringing existing files into compliance is **issue #172**, deliberately scheduled after this PR so it can re-spell any block comments in the same pass.
...
| Book: `chapters/05-layout.md` (+ `DRAFT-3.md` rebuild), `FEATURES.md`, `CONTINUITY.md` | The comments section re-taught with the new spelling and the JS redirect; the summary bullet, features list, and continuity commitment re-spelled. |

Owed (README rule 4 — applied on next touch): none. No other spec, shipped source, or conformance snippet contains a block comment (§3's verification); prose mentions of "doc comment" in Unions §7.2 / FFI Part 7 §4.1 concern JSDoc in emitted `.d.ts` — JavaScript's side of the boundary — and are untouched by a Hexagon source-spelling change.
...
- **TextMate grammar** (single grammar since #163, shared by the VS Code extension and the Playground): comment scopes re-spelled; the `(*`-family begin/end rules are bracket-free, so this does not interact with #162's unterminated-bracket repaint beyond the repaint any unterminated block comment already causes.
...
| Emitted comments use JS syntax; unsafe bodies (nested, or containing `*/`) re-present as `//` runs — closes the pre-existing nesting-emission gap | §7 |
