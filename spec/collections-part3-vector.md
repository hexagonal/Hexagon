...
- All elements meet at one type: by unification, and numeric elements at the home of the expression tree they form as siblings (Numeric Literals §5.1, #1062) — `[m, n]` is `Vector(Int)` for `m : Nat`, `n : Int`, in either order. As a call's argument at a parameter whose type, as the call finds it, is `Vector(a)`, the elements are siblings of the call's other values at `a` (Functions §4.3's argument spine, #1096): `both(price, [n])` at `both<a>(x: a, y: Vector(a))` meets at `Dec`. A heterogeneous literal is otherwise an ordinary type error, naming the offending element. A type written around the literal reaches its elements (Functions §4.3's literal forms, #1066): at `Vector(t)` every element is checked against `t`, so `let xs: Vector(Dec) = [n, 0.5]` and `[n]` are `Vector(Dec)` for `n : Int`.
