...
**Superseded forms in this record:** the import heads quoted below predate #762 and #829 (`import module X from "./x"` is now `import X`), and the diagnostic quoted under §B names a file where Modules §7.6 now names a module; §B.1's "effect-import-for-instances pattern" is no longer vestigial but unspellable — Modules §3.3 refuses the form, and §13 example (i) shows the refusal — and its forecast that a package story would widen the names-in-scope gap was answered the other way (Modules §7.6: packages do not widen the residue; re-exports still could); Modules §12.1, cited at the companion-edit line, is discharged to the Packages spec; the hosts govern.
