...
6. **Constrained-polymorphic exports — proto-decided:** direct named specializations for the closed fundamental set plus a base-name trailing-dictionary edition when public usable non-fundamental evidence exists. Public instances receive type-owned handles/factories (`Rat.signed`, `Vector.show(...)`); dictionary types are constraint-qualified with lowercase Hexagon binders (`Signed.Dictionary<a>`). See both companion notes; the normative FFI spec consolidates them and refines Constraints §6.1/§6.4.
