...
`fun` supports self- and mutual recursion — its body sees itself, and a `fun` block's members see each other (mutual recursion is spelled as one `fun` head over an indented block of members). That's its entire job — it's the special case, not the default. Unlike a JS `function` declaration it does not hoist: like everything else in a Hexagon file, it can only be used below its declaration (a block's members excepted — that's what makes mutual recursion work). Everyday Hexagon is `let` all the way down, and you'll notice every other function in this book is one; recursion is rarer than you'd think in a language with `for`, `while`, and a pipeline-friendly stdlib.
