...
6. **A captured foreign collection is copied at every declared crossing, in both directions, by the type-directed walk; through the published boundary, foreign mutation cannot change a collection value Hexagon retains; positions the walk cannot reach are refused** (the one route outside the published boundary — importing a reserved internal name — is a boundary-contract violation, Part 1 §5.4; until #875, an `Array` extracted from a borrowed `JsMap`/`JsSet` is the one dated exception, Part 10 §2). — Part 1 §2.2, §5.4; Part 2 §6 (#876). *For `JsMap`/`JsSet` until #875:* borrowed views expose no mutation and require foreign stability; under valid use, live and snapshot observation coincide, licensing native iteration and two-step lowerings — Part 10 §2.
...
**Unsupported nesting and callbacks (compile):** adapter-requiring type in a direct aggregate or foreign collection → Part 1 §5.3 (single owner; Parts 2/3/8/10 inherit); adapter-requiring callback signature → Part 6 §5.4 (three named rewrites); captured collection beneath an identity-crossing container, inside a Hexagon opaque representation, in an exception payload (Exceptions §2), as an exported value binding (Part 7 §7), or `JsValue.from` at an unresolved type variable (Part 11 §2) → Part 1 §5.4 (single owner of the six refusals; #876).
