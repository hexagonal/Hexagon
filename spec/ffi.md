...
|---|---|---|---|---|---|
| 1 | Boundary doctrine & type mapping | `ffi-part1-boundary.md` | trusted-declaration doctrine; four categories (the captured foreign collection replacing the borrowed view, #876); master table; nested-adapter hard error (§5.3); the capture walk, its positions and six refusals (§5.4); numeric trust; `JsError` door; `Hex` namespace, `Hex.Range` | Exceptions §6; Primitive Types; Loops §8 | `Hex`-alias scheme (**resolved**, §11.1 here); async/Promise semantics |
| 2 | `Nullable` & captured `Array` | `ffi-part2-nullable-array.md` | zero-wrapper `Nullable` + idempotence; `NullableCase`/`toCase`; captured `Array` — snapshot at every crossing, both directions (#876); accessor pair; `.length` rewrite error; conversion suite | Part 1; Collections Parts 1/3/5 | flow narrowing (language deep dive); default parameters |
| 3 | `Seq` interoperation; the raw `Stream` crossing (#355, §14 there) | `ffi-part3-seq.md` | one-iterator memoizing adapter; fresh adapter per crossing, no identity cache; replayable export; no deterministic disposal; `Stream(a)` crosses raw — per-pull shim, no memoization, no identity promise | Part 1 §2.3/§5.3; Loops §6; `stream.md` | async iteration; callback-position adapters; resource-managed disposal (the single-pass type itself now exists — `stream.md`) |
...
| 6 | Functions & callbacks | `ffi-part6-functions-callbacks.md` | identity calling convention; exact arity / no runtime validation; `Unit` discarding rule; exception round trips; representation-direct callbacks with same-object identity, plus conversion wrappers (no identity promise) for captured-collection signatures (#876); callback `this` ignored | Parts 1/3/4/5; Functions §5/§9; Exceptions §6–§7 | adapting callbacks; wrapper caches; async callbacks; callback-**visible** `this` and receiver-aware callback types (invocation-supplied `this` is already ignored and unobservable in v1) |
| 7 | Exports & `.d.ts` | `ffi-part7-exports.md` | export correspondence; named-ESM-only; lowercase binders; `Hex` import; records/unions/constructor materialization; **uniform opaque brand**; exceptions incl. nullary function shape; direct-vs-wrapper rule (occasion 4: captured-collection signatures; exported captured value bindings refused, #876) | Parts 1/3/6; Modules §11; Unions §6; Exceptions §7 | — (constrained exports routed to 8–9) |
...
| 10 | `JsMap` / `JsSet` | `ffi-part10-js-map-set.md` | borrowed views (**pending #875**, which moves both to the captured category), `ReadonlyMap`/`ReadonlySet` faces; no-`Hash` native equality (stated loudly); `jsMap[k]`+`KeyError`, `has`-before-`get`, no fusion; no set brackets; 2 `Iterable` rows; `fromSeq`; 4 snapshot conversions, inward cycle-checked `Result` | Parts 1/2/3/11; Collections Parts 1/4/5; Operators §10 | `keys`/`values` projections; set algebra; mutable door; `WeakMap`/`WeakSet` |
| 11 | `JsValue`, decoding, conversion failure | `ffi-part11-js-value-errors.md` | `JsValue` (`unknown` face, absorbs nullish); `JsValue.from` a release seat, refused at an unresolved type variable (#876); total property-free `kind`; strict scalar decoders + captured `toArray`; **`JsConversionError` = ordinary data** (`{reason, path}`); closed 5-segment path vocabulary; conservative `JsError.message`/`stack`; two-channel doctrine | Parts 1/2/10; Exceptions §6 | composable decoder family (stdlib ledger); `toJsMap`/`toJsSet` (cross-realm revisit bar) |
...
| **captured foreign collection** | a foreign collection copied at every declared crossing, both directions, type-directed and recursive; the copy is Hexagon's stable value and no foreign code can reach it — save the one temporary development-stage exception, storage reachable through a borrowed `JsMap`/`JsSet` until #875 (Part 1 §2.2) ("captured" in this sense only — an `extern enum`'s member values are read at initialization, not captured in this sense) | Part 1 §2.2, §5.4 (instance: Part 2 §6; `JsMap`/`JsSet` under #875) |
| **borrowed foreign view** *(retiring)* | zero-copy foreign-owned storage, observed under a stability contract — `JsMap`/`JsSet` only, until #875 | Part 1 §2.2 (instance: Part 10 §2) |
| **conversion wrapper** | the per-crossing function wrapper a captured-collection *callback value* receives; copies captured arguments and results at each invocation; no identity promise | Part 6 §5.5 |
| **stable export wrapper (occasion 4) / copying wrapper** | the stable module-level wrapper an exported Hexagon function (Part 7) or an extern binding (Part 4 §4.3) with a captured position receives; identity-stable, allocated once | Part 7 §7 occasion 4; Part 4 §4.3 |
...
| **Hexagon-owned value vs. foreign collection** | `Hex.*` persistent values cross by identity; `Array` is a captured copy of foreign storage; `JsMap`/`JsSet` are observed foreign storage until #875 | Part 1 §4.1 |
...
| `Nullable` companion surface, `toCase`, `Option` conversions; `Array` accessors, slices, sparse holes, the capture contract | Part 2 |
...
3. **Wrapper occasions are a closed set:** adapted top-level positions; receiver members; constrained generic editions *only when public ABI plumbing requires one* (a matching internal trailing-evidence function exports directly); exported Hexagon functions, published generic editions, and extern bindings whose signatures name a captured collection, and the captured-collection members of public dictionary handles and factory results (#876; Part 7 §7 occasion 4, Part 4 §4.3, Part 9 §3.4/§4/§9). — Part 6 §1 (the set); Part 7 §7 + Part 9 §9 (the direct-export correction).
4. **Every named boundary-function wrapper is allocated once with the thing it rides and is identity-stable for that thing's life — module-level for every occasion but a factory result's member wrappers (#876; Part 9 §4); per-value adapters and per-crossing conversion wrappers may be fresh** — the wrapper/adapter distinction is never blurred. — Part 6 §1; Part 3 §2.1; Part 6 §5.5.
5. **V1 callback signatures are representation-direct**, recursively, and cross as the same JS function object both ways — or name a captured collection and cross through a per-crossing conversion wrapper with no identity promise (#876); adapter-requiring callback signatures are hard errors. — Part 6 §5.
6. **A captured foreign collection is copied at every declared crossing, in both directions, by the type-directed walk; through the published boundary, foreign mutation cannot change a collection value Hexagon retains; positions the walk cannot reach are refused** (the one route outside the published boundary — importing a reserved internal name — is a boundary-contract violation, Part 1 §5.4; separately, until #875, storage reachable through a borrowed `JsMap`/`JsSet` stays under Part 10 §2's temporary borrow contract in both crossing directions — supported use, one exception to enforced snapshot isolation and not to purity). — Part 1 §2.2, §5.4; Part 2 §6 (#876). *For `JsMap`/`JsSet` until #875:* borrowed views expose no mutation and require foreign stability; under valid use, live and snapshot observation coincide, licensing native iteration and two-step lowerings — Part 10 §2.
...
9. **All nominal opaque `.d.ts` faces use the one non-exported-`unique symbol` brand mechanism** — opaque records, opaque unions, extern types, extern class types; dictionary brands are the same mechanism with an inference-bearing slot — save the Part 7 §2.3-pinned types, which take no brand: a pinned type's declaration seat is the pin's transparent alias (#622; Part 7 §14.5). — Part 7 §5; Part 9 §2.2.
...
13. **Every callable extern row writes its effect arrow** — `->`, `->!`, or `->?` before its result, on `fun`, `method`, `get`, `set`, `new as`, the `static` forms, and intrinsic rows alike; the keyword says how JavaScript is invoked and the arrow what a caller accommodates, independently; setters are `->!` only; a user row's arrow is trusted, an intrinsic row's verified. — Part 4 §4.5; Part 5 §2.1, §4.1; Intrinsics §4.2; Effects §6.1.
...
| `JsMap(k,v)` / `JsSet(a)` | borrowed native `Map`/`Set` (captured under #875, pending) | `ReadonlyMap<k,v>` / `ReadonlySet<a>` | Part 10 |
...
| `extern enum` | the enum object's own member values, read at initialization, or the declaration's own literals (the literal form) | per `ffi-foreign-enums.md` §7.2 — brand for the object-reading form, the literal union for the literal form | Foreign Enums |
...
**Unsupported nesting and callbacks (compile):** adapter-requiring type in a direct aggregate or foreign collection → Part 1 §5.3 (single owner; Parts 2/3/8/10 inherit); adapter-requiring callback signature → Part 6 §5.4 (three named rewrites); captured collection beneath one of the five Hexagon runtime containers the walk cannot enter (`Vector`, `Map`, `Set`, `Seq`, `Stream`), inside a Hexagon opaque representation, in an exception payload (Exceptions §2), as an exported value binding (Part 7 §7), or `JsValue.from` at an unresolved type variable (Part 11 §2) → Part 1 §5.4 (single owner of the six refusals; #876).
...
**Foreign collections and brackets (compile + runtime):** `.length` habit → Part 2 §6.3; write-position brackets → Collections Part 1 §3.3 (Parts 2/10 inherit); `jsSet[x]` → Part 10 §5; absent key → `KeyError`/`None` per Part 10 §4; out-of-bounds/`IndexError`, slice behavior → Part 2 §6.3 (Collections Part 3 doctrine).
...
**Deliberately not diagnosed (contract violations; unspecified observations):** numeric contract violations; foreign mutation racing a capture copy (unspecified for that acquisition alone, Part 1 §5.4); `JsMap`/`JsSet` mutation during a borrow (incl. between `has`/`get`, under an escaped `Seq`), and foreign mutation of a Hexagon-retained `Array` reachable through a borrowed `JsMap`/`JsSet`, until #875; wrong-arity/ill-typed JS calls into exports; `extern let` reassignment; fabricated dictionaries/brands; resource-owning iterators fed to `Seq` — Part 1 §3.1 owns the doctrine; Parts 2/3/4/6/9/10 record the instances.
...
| Capture walk (#876) | at every declared position whose type names a captured collection the value is copied, both directions; Hexagon-to-Hexagon calls, intrinsic rows, and runtime-module calls do not copy; the `Array` copy has the source's `length`, reads each index once in index order, and is dense (a source hole becomes a stored `undefined`); aggregates rebuilt component-wise, other elements by identity; callback conversion wrappers per crossing; exported functions export the stable export wrapper publicly and the internal edition under a reserved name; extern bindings denote a copying wrapper; the six refusals fire at declaration/export with their rewrites; the trigger does not descend through a borrowed `JsMap`/`JsSet` until #875, so nothing beneath one copies, wraps, or is refused (Part 1 §2.2, §5.4); `JsValue.from` refused at a type variable; `toArray` copies | Part 1 §5.4; Part 2 §6.2; Part 6 §5.5; Part 7 §7; Part 11 §2/§4.2 |
...
| Identity & wrapper allocation | representation-direct exports/functions are the raw objects; each wrapper occasion allocates exactly one wrapper with stable identity for the life of what it rides — module-level for every occasion but a factory result's member wrappers; per-crossing conversion wrappers for callback values carry no identity promise; direct export of matching trailing-evidence functions except where a captured position puts the stable export wrapper under the public name | Parts 6 §1, §5.5, 7 §7, 9 §4, §9; Part 5 §2.3 |
...
| Callbacks & exceptions | same function object both directions for a representation-direct signature; a per-crossing conversion wrapper copying captured arguments and results, with no identity promise, for a signature naming a captured collection (#876); extra JS args harmless; Hexagon throws stay branded through foreign frames; foreign throws land in `JsError`'s branch; `Unit` results discarded inbound, genuine `undefined` outbound | Part 6 §§2–5, §10 |
| Opaque branding & constructors | brand-only faces for all four opaque families (Part 7 §2.3-pinned types excluded, #622); no runtime artifact; constructors materialized at export (records, unions, exceptions — nullary exceptions function-shaped with fresh stacks) | Part 7 §§3–6 |
...
4. **`ffi-part1-boundary.md` §10 and `ffi-part7-exports.md` §2.1** — the `Hex`-alias scheme is **fixed** at §11.1 here (first-free numeric suffix probing over all top-level `.d.ts` identifiers a module's items can contribute; generated alias only, user exports never renamed): close Part 1 §10.1 with a pointer, and add the one-line algorithm reference to Part 7 §2.1. The row-finalization markers for `JsMap`/`JsSet`/`JsValue` are already cleared.
