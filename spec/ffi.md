...
| **captured foreign collection** | a foreign collection copied at every declared crossing, both directions, type-directed and recursive; the copy is Hexagon's stable value and no foreign code can reach it ("captured" in this sense only — an `extern enum`'s member values are read at initialization, not captured in this sense) | Part 1 §2.2, §5.4 (instances: Part 2 §6, Part 10 §2) |
| **borrowed foreign view** *(retired)* | the former zero-copy category under a foreign stability contract; no member remains | Part 2 §13.2, Part 10 §14 (records) |
...
| **Hexagon-owned value vs. foreign collection** | `Hex.*` persistent values cross by identity; `Array`/`JsMap`/`JsSet` are captured copies of foreign storage | Part 1 §4.1 |
