...
3. **Wrapper occasions are a closed set:** adapted top-level positions; receiver members; constrained generic editions *only when public ABI plumbing requires one* (a matching internal trailing-evidence function exports directly); exported Hexagon functions and extern bindings whose signatures name a captured collection (#876; Part 7 §7 occasion 4, Part 4 §4.3). — Part 6 §1 (the set); Part 7 §7 + Part 9 §9 (the direct-export correction).
...
6. **A captured foreign collection is copied at every declared crossing, in both directions, by the type-directed walk; through the published boundary, foreign mutation cannot change a collection value Hexagon retains; positions the walk cannot reach are refused** (the one route outside the published boundary — importing a reserved internal name — is a boundary-contract violation, Part 1 §5.4).** — Part 1 §2.2, §5.4; Part 2 §6 (#876). *For `JsMap`/`JsSet` until #875:* borrowed views expose no mutation and require foreign stability; under valid use, live and snapshot observation coincide, licensing native iteration and two-step lowerings — Part 10 §2.
