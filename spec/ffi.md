...
| **captured foreign collection** | a foreign collection copied at every declared crossing, both directions, type-directed and recursive; the copy is Hexagon's stable value and no foreign code can reach it ("captured" in this sense only — an `extern enum`'s member values are read at initialization, not captured in this sense) | Part 1 §2.2, §5.4 (instance: Part 2 §6; `JsMap`/`JsSet` under #875) |
