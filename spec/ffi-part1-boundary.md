...
- **`Vector`'s face narrows on purpose — and the price is bigger than a migration note, because the named exits do not exist yet.** Today's `Vector` representation is a plain JavaScript array, so the currently emitted `ReadonlyArray<a>` face is accidentally honest at runtime — and freezing it into the public contract would make Collections Part 3's decided trie representation a breaking `.d.ts` change. `Hex.Vector<a>` is what makes that migration invisible, the API-representation-silence the Collections corpus already promises. But stated plainly: `Vector.toArray` (§5.1's own example) is not among `stdlib/Vector.hex`'s exports; `Map.toJsMap`/`Set.toJsSet` are unimplemented (`JsMap`/`JsSet` are not yet compiler types); and no `Map`/`Set` stdlib module exists at all, so no `Map`/`Set` *operation* is importable by a JS consumer either. The **v1 interim surface of a crossed collection is therefore iteration plus whatever the module author exports** — a consumer reading `map.size` (which happens to work today) or indexing a crossed `Vector` loses it at the typecheck the moment this lands, with spread/`Array.from` as the universal rewrite until the conversions ship. This ruling deliberately does **not** sequence the narrowing behind the conversions: an honest narrow face now beats a false wide one, and the conversion obligations stay owned where they are (FFI Part 2 for `Vector.toArray`; FFI Part 10 for the `JsMap`/`JsSet` doors).

**Conformance obligations** (the #128 implementation discharges these; no new diagnostics, so the Rewrite Rule is not engaged):

1. `renderType` emits `Hex.Vector<a>`, `Hex.Set<a>`, `Hex.Map<k, v>`, and `Hex.Range` (alias as probed) for the four §4.1 rows this section governs.
2. A generated `.d.ts` mentioning any `Hex.*` face carries exactly one type-only import of the runtime declaration module; one mentioning none carries none. §10's alias probe already runs over every top-level identifier the file emits — which **includes the type-only import aliases the emitter writes today for source-level Hexagon imports** (`import type * as Json from "./tiny-json.js"`): a source module importing under the alias `Hex` forces `Hex1`. That collision class is live before this ruling, not created by it; the implementation must probe against it.
...
4. Unchanged rows stay unchanged: `Seq(a)` faces as structural `Iterable<a>` (§8.2's carve-out); `Node` reaches no shipped `.d.ts`. `JsMap`/`JsSet` are **unimplemented** — they are not `Typed.Type` kinds, and `renderType`'s `Map`/`Set` cases *are* the persistent collections obligation 1 changes — so there is nothing to leave alone yet; their `ReadonlyMap`/`ReadonlySet` faces (borrowed natives whose shapes are genuinely theirs) bind Part 10's implementation when it lands.
5. Acceptance: `tsc --noEmit --strict` over an emitted program's declaration files exercising all four faces in parameter, result, and `declare const` positions — modulo the separately filed defects the run may still trip (#227 cross-module type names, #228 `Array(a)` mutability), which this section neither fixes nor licenses.
...
**Scope fences.** #227 (a generated `.d.ts` names other Hexagon modules' types with no import) is its own ruling; nothing here decides it, but its fix must ride the relative type-only import discipline the emitter **already has** — the `.hex` → `.js` specifier rewrite behind today's `import type * as Json from "./tiny-json.js"` lines — which §8.3's runtime-declaration import also joins; this section joins that discipline, it does not establish it, and the two fixes belong to one emitter pass. #228 (`Array(a)` faces as mutable `Array<a>`) is pure conformance to §4.1's existing row — no ruling needed, not this section's. #132 is fixed and orthogonal.
...
> **Edit note (compiler source, applied when that file is next touched; added 2026-08-02, round 4):** the §5.3 clause falsified above is not only in the corpus. `compiler/src/passes/checker/variance.ts`'s claim-table doc comment carries it verbatim on the `Vector(+a)` row — "*`Vector` ships from `@hexagon/runtime`'s JS trie today (Collections Part 3 §4), so there is nothing for §6.3 to check yet*" — which is the first thing an implementer reads before touching that row, in the file that implements it. It takes the same repair: no package, no trie, `Vector` is a copy-on-write native array, and the row's conclusion (nothing for §6.3 to check before the emitter-wiring milestone) survives strengthened. **This ruling's sweep is over the specification corpus and does not edit compiler source**; the note is issued so the divergence is recorded rather than left for the next reader to find. Second defect in the same comment, pre-existing and unrelated to this ruling: it cites "`intrinsics.md` §4.2's parametricity obligation" where the corresponding spec text (`decisions-ml-dialect-generalization-2026-08.md` §5.3) says §7's.

> **Edit note (`book/`, tracked as issue #235 — recorded here so this sweep's scope is complete):** the book carries three `@hexagon/runtime` imports that the blanket reading does **not** rescue, because they are presented as emitted output rather than as provenance. `book/chapters/22-javascript-output.md:243`'s `import { Vector } from "@hexagon/runtime"` is the worst: a *value* import, in a JavaScript-output sample, doubly false since `Vector` operations are inlined natively and no such import is ever emitted. `book/chapters/20-collections.md:295` and `book/chapters/23-typescript-output.md:265` spell the type-only form and take §8.3's re-grounding. The book has no edit-note convention, which is why #235 exists; it is named here so a future sweeper does not read this note's silence as a decision that the book was clean.
...
**Provenance:** Part 3 §9.5 item 5's filed debt → `spec/notes/compiler-conformance-defects.md` entry 12's resolution block → issue #128 → this section. The ledger sentence in Part 3 §9.5 ("remain filed debt owed their own defect entry") stays literally true: #128 is that entry, and it now carries this ruling.
