...
5. Acceptance: `tsc --noEmit --strict` over an emitted program's declaration files exercising all four faces in parameter, result, and `declare const` positions — modulo the separately filed defect the run may still trip (#228 `Array(a)` mutability), which this section neither fixes nor licenses. *(#227, formerly also listed here, was ruled and fixed 2026-08-04 — Part 7 §2.4/§14.2.)*
