...
- **Filename collision, deterministically, user wins.** A project module whose own emission claims `hex.js`/`hex.d.ts` **at the output root** — only the project's modules emit there, Packages §6 — keeps its name; the runtime declaration module takes the first free of `hex.d.ts`, `hex1.d.ts`, `hex2.d.ts`, … — §10's probing discipline lifted to filenames (the first-free search, not §10's underscore suffix: a filename is not an identifier), probed **over the source basenames at that root** — a superset of the emitted filenames, and the superset is the safe direction; it is also what makes the stem defined before any module is emitted, and defined even for a program owing `hex.js` alone *(respelled 2026-08-30, #666: this clause read "over the emitted filenames"; the probe's input was always the source paths, and Part 7 §1.2's stem argument now leans on that. **Respelled again, #829:** the probe's input is the **emitted filenames at the output root**, which are the declared module names (Modules §11.1) — known once every header is parsed, so the stem is still defined before any module is emitted and for a program owing `hex.js` alone; the source basenames are no superset of them any more, a source file's name bearing no relation to the modules it declares — a `util.hex` declaring `module Hex` emits `Hex.js`, which the probe must see and the basenames would not. Read "source basenames at that root" above as "emitted filenames at the output root")* — compared case-insensitively (case-colliding filesystems exist) — and every generated import spells whichever name won. Only the generated file moves; a user module is never renamed.
- **The placement's price, recorded — and discharged by #829.** *(Historical: the source common root was the longest shared directory prefix over *all* the project's source paths (`project.ts`'s `commonRoot`), so adding one distant source file shortened the root, moved `hex.d.ts`, and rewrote the relative specifier in every generated `.d.ts` that imported it. Under #829 the artifact sits at the output root, which no source path derives, and the instability is gone; the paragraph stands as the record of a price once accepted; the rest of it is that record.)* This was **accepted, not overlooked**: the prelude modules then injected at that same root and had exactly this property, so the ruling added no new instability class — it inherited one the compiled-project model had chosen. Any future ruling that stabilizes prelude placement stabilizes this with it.
