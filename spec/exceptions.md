...
- **No captured foreign collection in a payload** *(#876; FFI Part 1 §5.4 item 3)*. `exception Bad(rows: Array(Int))` — or a payload whose type names `Array(a)` anywhere inside it, and `JsMap(k, v)`/`JsSet(a)` once #875 captures them; until then a `JsMap(String, Array(Int))` payload is legal, the trigger stopping at the borrowed map (FFI Part 1 §5.4) — is a hard error at the declaration. An exception crosses the foreign boundary wherever a throw travels: out of an exported function into a JavaScript `catch`, through foreign frames and back (FFI Part 6 §4), and none of those is a declared position at which FFI Part 1 §5.4's capture walk could copy the payload, so a captured collection there would be Hexagon-retained storage reachable from foreign code. The refusal is **unconditional** — a property of the declaration, not of any use, because an exception's travel is not statically bounded — so it fires in a program with no `extern` at all; the diagnostic says so in its own words ("an exception may be thrown through foreign code, so its payload cannot hold a foreign collection; carry a `Vector`") rather than sending the reader to the FFI. The declaration is the one place safety can be established, and establishing it there is what lets `Exn` — the abstract type, whose payloads no position can inspect — keep crossing by identity everywhere it does today (FFI Part 1 §4.1). Rewrite: carry a `Vector` (`Array.toVector` at the construction site), which crosses by identity with nothing to protect.
...
| payload names a captured foreign collection (`Array(a)`) | hard error at the declaration; rewrite: carry a `Vector` (§2; FFI Part 1 §5.4 item 3) |
