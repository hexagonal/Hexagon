...
- **No open row in a payload** (Products §4; FFI Part 1 §5.4 item 7). `exception Bad(r: {n: Int, ...})` — or a payload whose type carries `...`, bare or named, at any depth and through any `type` alias — is a hard error at the declaration, on the ground Products §4 states for every declaration's row: it is one row for every value of the type, so a construction that widened it would widen every `Bad`, and an exception crosses wherever a throw travels, so nothing at a crossing could read the widened row. Diagnostic, in Products §4's vocabulary: "an exception's payload names every field of its values; this slot's type says the record may have more fields — name them, or give the slot the type `JsValue`".
