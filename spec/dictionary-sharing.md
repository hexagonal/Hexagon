...
**Collisions.** When more than one dictionary in a module contests one spelling — flattening is not injective (underscores are legal in constraint and type-constructor names), and an imported dictionary's interface name can match a local one — **all** contestants take numeric suffixes, `_1` upward, and none keeps the bare spelling *(replaces the occupant-keeps probe this section previously cited; #425)*. A bare dictionary name therefore certifies its spelling is uncontested in that module; a suffixed one marks a collision. Suffixes are assigned in a canonical order over the contestants — declared instances in declaration order, then hoisted bindings in this section's emission order, then imports in specifier order — so the assignment is as deterministic as the names themselves. Lexer §3.2's probe discipline is unchanged for what it governs, occupancy by a name the compiler cannot rename (a foreign face): the foreign name keeps its spelling and the dictionary family numbers around it, which is the same rule — rename everything renameable, never the thing that is not.

**Imported names.** A consumer binds an imported dictionary under the exporter's interface name read from the resolved interface, unaliased, whenever that spelling is uncontested in the consumer; a contest falls under the collision rule above. Aliasing is therefore collision-only — the unconditional per-file alias prefix is gone, and a re-export chain re-binds the same interface name at each hop, so transit names no longer compound *(the dead-code-elimination plan's §3.2 measured the compounding; #425)*.
...
**Exported spellings under collision.** An instance's exported name is its bare flattened spelling; §5's collision rule renames local `const`s, not the interface. When the collision that renamed an instance's `const` involves only internal names (a hoisted binding, an import), the export re-binds the bare spelling over the suffixed local — `export { __Eq_Rat_1 as __Eq_Rat }` — so consumers see the uncontested interface name *(#425)*. Only when two **exported** dictionaries of one module contest one spelling do the suffixes reach the interface: both exports carry their §5-assigned suffixed names, and consumers read them from the resolved interface as with any exported name — no consumer-side prediction, per §5's imported-names rule.
