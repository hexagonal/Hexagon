...
**Edit note → Constraints §6.3** (apply on the same touch as the note above): its emission-order sentence "Dictionary-sharing's hoisted ground applications ride with them" generalizes to the hoisted bindings of §3.1 *and* §3.4 — a structural binding equally initializes before the term bindings that reference it.
