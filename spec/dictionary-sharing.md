...
Evidence containing a free dictionary parameter (`Core`'s `Dictionary` kind) is not ground and never hoists (§3.3); error evidence never hoists. Structural evidence joins the key through §3.4: a structural node contributes a canonical serialization of its ground type together with its components' trees, in component order, so two demands of one anonymous shape over the same component evidence share one binding per module like any other tree. Equality stays structural and no type comparison is introduced — the serialization is computed from the checker's already-selected type, is internal to the emitter, and any deterministic spelling serves; §5's name is the only visible face.
...
8. **Structural sharing:** a module demanding one ground anonymous shape at two use sites emits one §3.4 binding and two references, tuples and `Unit` both covered; a declined literal-member reduction at a ground shape reads its member off the binding (`__Show_Unit.show(…)`), and the inline literal shape appears at no ground site. Obligations 4 (determinism) and 5 (no export growth) cover the §3.4 family as written.
