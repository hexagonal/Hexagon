...
| A `pattern` head ending its logical item — the name, with or without a binder list and a component list (`pattern rat(top: BigInt, bottom: BigInt): Rat`, `pattern parts`; Pattern Declarations §2.1). The alias form `pattern rgb = Color.rgb` is a one-line item and opens nothing: its right side is a name, and a wrapped right side is refused by the alias rule (Pattern Declarations §3.4), not opened by the binding row below | `pattern` member block |
