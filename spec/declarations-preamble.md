...
- **`type` is the transparent half of a deliberate pair.** The language now has both tools and the docs must present them together: `type` for "same type, better name" (expanded away by the unifier, no instances, no runtime anything) and `record`/single-constructor `union` for "genuinely new type" (nominal wall, instance-capable, pattern-destructurable). Half the diagnostics in §5 exist to route users from the first tool to the second at the moments they reach for nominal power through an alias. *(#927.)* The `type` row of the intrinsic block (Intrinsics §3.3) is not this `type`: under that block's grammar, as under FFI Part 4 §5's, `type` names a nominal opaque type — the block's reuse of the extern grammar, not a third meaning of the keyword here.
- **Module-level declarations are order-insensitive and may be mutually recursive** where their semantics permit (§7) — types do not evaluate, so no initialization order constrains them; term bindings, which do, are read top-down (Functions §7.2). Aliases are the exception that proves it: their expansion-based semantics is exactly why *their* recursion is banned (§5.4).
...
- **Form:** a single constraint name (`derives Eq`) or the tuple form (`derives (Eq, Show)`) — deliberately mirroring the `<a: Eq>` / `<a: (Eq, Show)>` convention. A duplicate errors at its second occurrence: "`Eq` appears more than once in `derives`; remove the duplicate `Eq`." That refusal reads spellings, and a repeated spelling is always a repetition of one thing, whatever the name denotes — an entry is a single unqualified name, resolved once — so the message is never wrong about what it refuses, and unlike the base-list refusal's identity claim (Constraints §6.2) it has no unknown-spelling stand-down to need. It is not the whole rule in principle: were an entry ever to admit a second spelling of one declaration — no import binds a constraint under a second name (Modules §3, #762), so today none does — the pair would pass the spelling test. What refuses such a pair is the slot the sugar's instances occupy: the clause elaborates to `honor` declarations (below), each holding its identity-keyed coherence slot (Constraints §5.1), so two entries naming one declaration collide there under any pair of spellings — the parse-time message is the tailored early report of the repeated-spelling case, never the only guard.
- **Meaning (unchanged, restated):** pure sugar. `record Pair(a, b) derives (Eq, Show) = ...` elaborates to the declaration plus `honor Eq<Pair> = derive` and `honor Show<Pair> = derive`, each a full citizen of the instance system — coherence slot, orphan rule (trivially satisfied: the declaration site is the home module), base constraint checks, parameter obligations as instance context per Constraints §4.3. All Decisions Batch §2 semantics and §2.6 diagnostics stand; the "add `derives Eq` to the declaration of `Point`" fixit now points at an insertion site adjacent to the name the error already mentions.
...
- The RHS is any type expression: primitives, tuples, records (structural rows, including `...` forms — legal here, and read through at the use: a `record` field, union slot or `exception` payload slot whose type expands to an open row is refused there, Products §4), function types, applied nominal constructors, other aliases (acyclically — §5.4). Function types in an alias body carry the **constant** arrows only, `->` or `->!`; `->?` is refused there (§5.1.1).
...
- **The check is syntactic occurrence in the RHS as written — and that is provably sufficient.** The worry: `type Weird(x) = Const(Int, x)` "uses" `x` syntactically while expansion could erase it, *if* `type Const(a, b) = a` existed. It cannot: `Const` fails this same check first (`b` unused). Inductively, if every alias in scope uses all its parameters syntactically, syntactic occurrence guarantees post-expansion occurrence — the local check delivers the semantic guarantee. Implement the cheap check; record the induction as why it's complete.
...
Module level only: `record`, `union`, `type`, `constraint`, `honor`, `exception`, `widens` (Constraints §4.7), `pattern` (Pattern Declarations). Any of these inside a function body or block is a hard error, one message family: "declarations live at module level" (generalizing Exceptions §2's rule; the exceptions-specific phrasing may remain as its instance). Block-local binders remain exactly `let` / `var` / `fun` (Statements §1). Of the eight, `widens` alone *is* a term binding — the declaration is itself the binding it introduces, read top-down at its own line like the member definitions it stands among (§7.2; Constraints §4.6) — where the remaining six are order-insensitive declarations whose constructor and member names straddle the term line per §7.2 — and a `pattern` stands apart: its name lives in the pattern namespace and is read top-down from its line, like `widens`, its head's types alone order-insensitive (Pattern Declarations §2.1, §3.3).

### 7.2 Order-insensitivity and mutual recursion

Module-level declarations are **order-insensitive among themselves**: a declaration may reference names declared later in the module. Unions may be mutually recursive through payloads (Unions §2); records through field types; aliases may reference later nominal names (§5.4); `honor` may precede or follow the declarations it mentions. Term bindings do **not** share this policy: they evaluate, and are read top-down (Functions §7.2) — the only forward visibility among terms is a `fun` block's mutual recursion (Functions §7.3). The term names a declaration binds straddle the line: a constructor or constraint member obeys the declaration rule in *type position* and the term rule in *value or pattern position* (Functions §7.2); evidence from `honor` is ordered by emission instead (Constraints §6.3). An `import` item straddles identically — the types and constraint names it binds follow the declaration rule, while its term-namespace names are read top-down at the item (Modules §3). The only ordering-adjacent error among declarations is the alias SCC check (§5.4). Checking strategy (declaration-graph SCC processing vs. two-pass name collection) is the implementer's; the observable rule is: order never matters, cycles-through-aliases-alone are errors, everything else that each form's spec permits is legal in any arrangement.
...
| Variance sigil on a transparent declaration (`record Pair(+a, b)` not headed `opaque`) | parse error: "variance is inferred for transparent types; remove the `+`" (§2.1, #205; the intrinsic block's `type` rows excepted, #927) |
...
| The `derives` duplicate refusal reads spellings — never wrong (one unqualified name, resolved once) but not the whole rule (a qualified spelling beside the bare one is a second spelling); the governing refusal is the elaborated instances' identity-keyed coherence slot, which catches any spelling pair naming one declaration | §2.3 |
