...
- **Form:** a single constraint name (`derives Eq`) or the tuple form (`derives (Eq, Show)`) — deliberately mirroring the `<a: Eq>` / `<a: (Eq, Show)>` convention. A duplicate errors at its second occurrence: "`Eq` appears more than once in `derives`; remove the duplicate `Eq`." That refusal reads spellings, and a repeated spelling is always a repetition of one thing, whatever the name denotes — an entry is a single bare name, resolved once — so the message is never wrong about what it refuses, and unlike the base-list refusal's identity claim (Constraints §6.2) it has no unknown-spelling stand-down to need. It is not the whole rule: a constraint reached under an importer's alias (`import { Show as S }` — Modules §3.2) is a second spelling of one declaration, and the pair `derives (Show, S)` passes the spelling test. What refuses that pair is the slot the sugar's instances occupy: the clause elaborates to `honor` declarations (below), each holding its identity-keyed coherence slot (Constraints §5.1), so two entries naming one declaration collide there under any pair of spellings — the parse-time message is the tailored early report of the repeated-spelling case, never the only guard.
