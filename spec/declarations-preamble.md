...
Module level only: `record`, `union`, `type`, `constraint`, `honor`, `exception`, `widens` (Constraints §4.7), `pattern` (Pattern Declarations). Any of these inside a function body or block is a hard error, one message family: "declarations live at module level" (generalizing Exceptions §2's rule; the exceptions-specific phrasing may remain as its instance). Block-local binders remain exactly `let` / `var` / `fun` (Statements §1). Of the eight, `widens` alone *is* a term binding — the declaration is itself the binding it introduces, read top-down at its own line like the member definitions it stands among (§7.2; Constraints §4.6) — where the other seven are order-insensitive declarations whose constructor and member names straddle the term line per §7.2 — a `pattern` standing apart: its name lives in the pattern namespace and is read top-down from its line, like `widens`, its head's types alone order-insensitive (Pattern Declarations §2.1, §3.3).
