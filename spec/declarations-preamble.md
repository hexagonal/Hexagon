...
- **Form:** a single constraint name (`derives Eq`) or the tuple form (`derives (Eq, Show)`) — deliberately mirroring the `<a: Eq>` / `<a: (Eq, Show)>` convention. A duplicate errors at its second occurrence: "`Eq` appears more than once in `derives`; remove the duplicate `Eq`." That refusal reads spellings, and a repeated spelling is always a repeated declaration — an entry is a single bare name, and a name resolves to one declaration — so the message is never wrong about what it refuses. It is not the whole rule: a constraint reached under an importer's alias (Modules §3.2) is a second spelling of one declaration, and a pair like `derives (Show, S)` passes the spelling test. What refuses that pair is the slot the sugar's instances occupy: the clause elaborates to `honor` declarations (below), each holding its identity-keyed coherence slot (Constraints §5.1), so two entries naming one declaration collide there under any pair of spellings — the parse-time message is the tailored early report of the repeated-spelling case, never the only guard.
...
| The `derives` duplicate refusal reads spellings — never wrong (one bare name, one declaration per name) but not the whole rule (an importer's alias is a second spelling); the governing refusal is the elaborated instances' identity-keyed coherence slot, which catches any spelling pair naming one declaration | §2.3 |
