...
- The RHS is any type expression: primitives, tuples, records (structural rows, including `...` forms — legal here, and read through at the use: a `record` field or union slot whose type expands to an open row is refused there, Products §4), function types, applied nominal constructors, other aliases (acyclically — §5.4). Function types in an alias body carry the **constant** arrows only, `->` or `->!`; `->?` is refused there (§5.1.1).
