...
**Scope:** The shared header grammar for the type-introducing declarations (`record`, `union`, `type`): parameter form, the `derives` clause's position (moved to the header — supersedes Decisions Batch §2.5 placement), layout continuation for multi-line declarations. The `type` alias declaration in full: transparency, parameterization, the recursion ban, the no-instances rule, display stickiness, `.d.ts` emission. Consolidation: the module-level declaration inventory and site rule, order-insensitivity and mutual recursion among declarations, the type-namespace duplicate rule.
**Not in scope:** `derives` *semantics* — the derivable set, superconstraint checks, coherence, per-constraint generated code (Decisions Batch §2, unchanged; this doc moves only the clause's position), the constraint mechanism and `implement` (Constraints spec), constructor semantics and payload grammar (Unions §2, Products §5 — this doc fixes only their shared header), cross-module name management, imports, qualification (modules spec), associated `type` members in `constraint`/`implement` bodies (v2 — Loops §7.2 fixed the keyword sharing; nothing here conflicts).
**Companions:** Products §5 (nominal `record` semantics; the header was deferred here), Unions §2 (union semantics; header and layout-continuation note deferred here), Type System Overview §3 (the `type` alias row: "parameterisation, recursion ban, alias-vs-expansion display" — all discharged here), Constraints (angle-brackets-vs-parens kind distinction §1; instance machinery), Decisions Batch 2026-07 §2 (derivation mechanism; placement superseded), Exceptions §2 (module-level-only rule, generalized here), Lexer & Layout (offside continuation).
...
- **Header parameters are bare type variables.** No constraints in data-declaration headers, permanently (§2.2). Constraints do their work at functions, instances, and `constraint` declarations; parens after an uppercase name remain exclusively "type parameters of a type constructor" (Constraints §1's kind distinction), and `<>` never appears in a data header.
- **`derives` lives in the header, before `=` — the only position.** Capabilities read before representation; the clause is a statement about what you can *do* with the type and precedes the statement of what the type *is*. This supersedes the body-trailing placement of Decisions Batch §2.5 (edit note §10). Semantics are untouched: the clause remains pure sugar for a bundle of `implement C<Name> = derive` instances.
...
- `Name` is uppercase-initial and enters the **type namespace** (constructors introduced by the body enter the term namespace per Unions §2 / Products §5.1; constraint names live in their own namespace per Constraints §2.2).
- Parameters are **parenthesised, comma-separated, lowercase-initial type variables**, matching use-site shape exactly: you declare `Option(a)` the way you write `Option(Int)`. Parameters must be distinct (`Pair(a, a)` — hard error, duplicate parameter). Arity is fixed; every use site must apply the constructor to exactly that many arguments (the existing use-site rule, now anchored to its declaration).
...
- **Form:** a single constraint name (`derives Eq`) or the tuple form (`derives (Eq, Show)`) — deliberately mirroring the `<a: Eq>` / `<a: (Eq, Show)>` convention. Duplicate names in one clause: hard error.
- **Meaning (unchanged, restated):** pure sugar. `record Pair(a, b) derives (Eq, Show) = ...` elaborates to the declaration plus `implement Eq<Pair> = derive` and `implement Show<Pair> = derive`, each a full citizen of the instance system — coherence slot, orphan rule (trivially satisfied: the declaration site is the home module), superconstraint checks, parameter obligations as instance context per Constraints §4.3. All Decisions Batch §2 semantics and §2.6 diagnostics stand; the "add `derives Eq` to the declaration of `Point`" fixit now points at an insertion site adjacent to the name the error already mentions.
...
- Consistency note: this is the same policy family as the associated-types display rule (Loops §7.2 — `Elem(List(Int))` expands in hovers once resolved): expansion whenever resolution is complete and the user didn't write the name; the written name wherever they did.
...
```
type UserName = String
fun greet(name: UserName): String = ...
```

```ts
type UserName = string;
declare function greet(name: UserName): string;
```

TS aliases are equally transparent, so semantics are preserved exactly; the emitted declarations are what a TS author would hand-write, and the output is diff-stable when an alias's expansion changes. Positions where inference produced the type (no alias written) emit expanded — same stickiness. Non-exported aliases referenced by exported signatures: emit the alias declaration anyway (TS requires the name to be in scope); the modules spec may refine export visibility, flagged there.
...
| Duplicate header parameter (`Pair(a, a)`) | hard error, duplicate parameter (§2.1) |
| `derives` after the body | parse error + move-to-header fixit (§2.3) |
| Duplicate constraint in one `derives` list | hard error (§2.3) |
...
| `implement C<Alias>` | "`UserName` is an alias of `String`; aliases cannot carry their own implementations — use a `record` or a single-constructor `union`" (§4) |
...
| Duplicate type-namespace name in one module | hard error at the second, naming the first (§7.3) |
...
| `type` = fully transparent alias; no instances; `implement`-on-alias and `derives`-on-alias both redirect to newtypes | §4 |
...
| `.d.ts`: exported aliases emitted and used per stickiness; expansion where inference produced the type | §6 |
...
1. **Decisions Batch §2.5** → the `derives` placement sentence ("must follow the complete type body…") is superseded: the clause precedes `=`, header position, sole position (§2.3 here). The contextual-keyword note, the `derive`-as-`implement`-body core form, the derivable set, and all §2.6 diagnostics stand. **§10.4** ("`derives` clause placement…") → resolved, strike.
...
4. **Type System Overview §3** → the `type` row's "owed — parameterisation, recursion ban, alias-vs-expansion display" is fully discharged (§4–§6 here); update the row to **decided** with a pointer.
5. **Exceptions §2** → the module-level-only rule is now an instance of §7.1's general family; optional cross-reference.
6. **Constraints** → no semantic change; the instance-on-alias error (§4 here) joins its diagnostics neighborhood on next touch.
7. **Loops §7.2 / future associated-types spec** → keyword sharing confirmed compatible: module-level `type` (here), `constraint`-body `type Elem`, `implement`-body `type Elem = ...` — three positions, one keyword, position disambiguates. The alias recursion ban's non-transfer to associated types (recorded there) is consistent with §5.4's rationale.
8. **hexagon-for-typescript-coders** → introduce the alias/nominal pairing ("`type` renames; `record`/`union` create") and frame `derives` as "like `implements`, except the compiler writes the body."
9. **Modules spec (future)** → owed from here: export visibility of aliases referenced by exported signatures (§6); cross-module type-name shadowing (§7.3); everything §7 already deferred.
10. **Spec roadmap** → strike the Declarations Preamble entry; migrate the §9-item above into the Modules entry.
...
-- ≡ record Point = {...}; implement Eq<Point> = derive
-- coherence: a later hand-written `implement Eq<Point> = ...` is the duplicate-instance error

-- (e) Alias transparency round trip
type UserName = String
fun greet(name: UserName): String = "hi ${name}"
greet("ada")                         -- OK: String is UserName
-- .d.ts: type UserName = string; declare function greet(name: UserName): string;

-- (f) Aliases carry no instances
type Meters = Float
implement Show<Meters> = ...
-- ERROR: `Meters` is an alias of `Float`; aliases cannot carry their own
-- implementations — use a `record` or a single-constructor `union`
