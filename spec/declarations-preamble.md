...
- **Form:** a single constraint name (`derives Eq`) or the tuple form (`derives (Eq, Show)`) — deliberately mirroring the `<a: Eq>` / `<a: (Eq, Show)>` convention. A duplicate errors at its second occurrence: "`Eq` appears more than once in `derives`; remove the duplicate `Eq`." That refusal reads spellings, and the spelling is decisive at this seat: an entry is a single bare name, a name resolves to one declaration, and Constraints §5.1.1's redeclaration ban leaves a derivable constraint no second spelling — so the spelling test refuses exactly the programs an identity test would. The governing duplicate rule is not this message in any case, but the slot the sugar's instances occupy: the clause elaborates to `honor` declarations (below), each holding its identity-keyed coherence slot (Constraints §5.1), so two entries naming one declaration collide there under any pair of spellings, and the parse-time message is the tailored early report of the common case, never the only guard.
...
| The `derives` duplicate refusal reads spellings — decisive because an entry is one bare name and a derivable constraint has one spelling; the governing rule is the elaborated instances' identity-keyed coherence slot, which refuses any spelling pair naming one declaration | §2.3 |
