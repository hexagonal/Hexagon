...
| Pattern-declaration deferrals — partial views (count for nothing, like guards; field-evidence-gated), multi-case and parameterised patterns (not planned), constrained heads (deferred; the dictionary-taking factory is the named design), a `catch`-arm pattern over `Exn` (not planned) | `pattern-declarations.md` §9 |
