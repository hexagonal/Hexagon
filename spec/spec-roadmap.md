...
- **#159 Unit-as-empty-tuple ripple** (2026-07-30) — owner: `decisions-ml-dialect-unit-2026-07.md` §8 (the ledger); targets: Collections Part 2 §2.5 (`Hash<Unit>` provenance), Collections Part 4 §10.1 + §18 notes 3–4 (primitive counts and the add-only procedure) + FFI Part 10 §4.3 (primitive-inventory lead-ins, change-controlled together), FFI Part 1 §4.1 (Tuple-row arity exemption), `ffi.md` §6, Method Syntax §3.4/§4.1, Collections Part 5 §4, Functions §5.3 (optional respelling). No example respelling exists — `Unit` and `()` spell as before.
