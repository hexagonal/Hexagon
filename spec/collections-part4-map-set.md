...
| `m[k]` where `k`'s type lacks `Hash` | the unsatisfied-`Hash` report (Part 2 §9, under Modules §7.6's replacement composition; base-complete — `derives (Eq, Hash)` where the key type also lacks `Eq`) | §4.1 |
...
## 18. Correction record — §10.1's primitive-key enumeration (2026-07-28, #141)

> **Correction (2026-07-28, #141).** §10.1's lead-in named four types — `Int`, `Float`, `String`, `Bool` — under a bolded normative clause that has always claimed more: "no **primitive-keyed** entry", unqualified. The clause's intent was exhaustive over primitive keys; the lead-in never was, and it was short from the day it was written — the same initial import that carried this document carried Part 2 §2.5 with its `Hash<BigInt>` and `Hash<Unit>` rows, so `BigInt`- and `Unit`-keyed maps were legal from birth while the lead-in omitted them. `Nat` was additionally missing for #139's reason: it postdates the document (v1.1). The fix refreshes the enumeration in place to the seven-name list in Primitive Types §1's order, marked *(corrected 2026-07-28, #141)* — the list is now every primitive, and exactly Part 2 §2.5's `Hash`-bearing primitive inventory. FFI Part 10's mirroring bullet (its §4.3) is refreshed in the same correction with a marker pointing here; Part 4 leads and Part 10 follows, as its citation has always had it.

Four notes complete the record:

1. **Deliberate exclusion was checked and ruled out before extending.** A silently-restored exclusion would be a worse defect than a short list, so the negative was established first: the drafting history behind this document (`spec/notes/chatgpt-vector-map-set.md`) never discusses which primitives may be keys; §12's rejected alternatives, Part 1 §4's rationale and rejected designs, and both closure documents mention neither `BigInt` nor `Unit` as a key, excluded or otherwise; before this correction, `BigInt` appeared nowhere in this document or in FFI Part 10 at all. The four-name list has the shape of the `Eq<Float>` = SameValueZero decision's cast of characters — `Float` plus the everyday key types that discussion ran against — not of a list audited against §2.5's inventory. Nothing was excluded; the lead-in was simply never checked against the table it needed to agree with.

2. **Faithfulness was verified per added type, in both directions, before being made normative.**
