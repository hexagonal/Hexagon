...
- **Emitter:** the tuple representation decision point gains the arity-0 → `undefined` branch (today the constant lives in the `Primitive` branches); `.d.ts` writer unchanged. The `Primitive`-branch fast paths in the four derivations may stay — they compute the §5 constants — or fold into the tuple branches; either way the outputs are pinned.
