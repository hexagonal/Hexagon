...
**Status:** Decided (July 2026; every member row writes its effect arrow, setters `->!` only, constructors a full contract — #869; receiver members' slots are FFI Part 1 §5.4 capture positions and a captured receiver is copied at the call — #876; importers emit receiver calls inline with a `__class_<Type>` re-export for static members and constructors — #982; keyword-named members bind unaliased, every use dotted — #1014), revised in place after external review (Sol) before landing. Normative promotion of `spec/notes/ffi-proto-spec-questions.md` §5's receiver-member and class material (`method`, `get`/`set`, `class`, static members, visibility). The draft's three promotion questions were resolved in §13: foreign inheritance remains flat in v1; Method Syntax covers extern nominal types; and class-versus-standalone choice receives cultural guidance only. Inherits Part 4's landed resolutions: foreign-name-first aliases; monomorphic v1 extern declarations (Part 4 §12.4 — extern classes included); raw identity for representation-direct plain `fun` versus wrappers where convention demands them (Part 4 §4.3); generated opaque brands for exported extern types (Part 4 §12.3); `create` as cultural guidance, never a compiler-special name.
...
**The foreign side is a JavaScript property name** *(#982)*. A member is reached by property access, and JavaScript admits any IdentifierName there, its reserved words included — `map.delete(key)`, `promise.catch(handler)`. So the name before `as` on every member row of this part — `method`, `get`, `set`, instance or static — may be any IdentifierName, **a Hexagon hard keyword included** (the parser also reads the `_` token there as the name it spells): `then`, `catch`, `finally`, and `match` are everyday JavaScript members. **A keyword-named member needs no alias** *(#1014)*: the row's name is a declaration-name seat and every use of a member is dotted — `task.then(f)`, `Task.then(task, f)` — both name seats (Lexer §4.4), so the Hexagon name is the JavaScript name:
...
`as` stays the author's way to choose a different name. The class header is not a member row: its foreign name is an ESM export name, Part 4 §3.2's position, and takes that part's rule. An exported member named `then` publishes no JavaScript export of that name (Part 7 §7).
...
Hexagon calls emit property reads (plus, for a result type naming a captured foreign collection, FFI Part 1 §5.4's copy of what was read — `Array(Int)` from a `get` is Hexagon's snapshot, #876):
...
**The class re-export** *(#982)*. An importing module emits a static member or constructor call as the binding module does — `URL.canParse(text)`, `new URL(text)` (§6.2, §6.3) — and for that it needs the foreign constructor object. It cannot import the foreign module itself: a relative specifier is emitted verbatim and resolves from the binding module's emitted place, not the importer's (Part 4 §2.1). So a binding module whose exported class declares a `new` or `static` member re-exports the constructor object under the reserved compiler name **`__class_<Type>`** (Lexer §3.2's `__<category>_<name>` family; `<Type>` the class's local type name). The spelling is **fixed** — an importer computes it from the type name — so, like `__patt_<name>`, it is reserved before internal allocation and never suffixed. An importer that calls such a member binds it under a **minted import local** (FFI Part 7 §1.2 rule 1), and so does the binding module's own import of the class. The local **mirrors the foreign class name** — the local type name for a `default class`, whose foreign side is unnamed (§6.4) — wherever that spelling is free. Where it is not — it is in the runtime vocabulary or is one JavaScript refuses as a binding (`Map`, `Error`, `delete`), or a binding of the module already holds it (§8's idiom imports the binding module under the type's own name) — the local is **`__class_<Type>` itself**, the name the class is re-exported under: descriptive, derived from the Hexagon type the author chose, the same spelling in the binding module and every importer, and never a numeric probe. `class Map as MapboxMap` imports `{ Map as __class_MapboxMap }` and emits `new __class_MapboxMap(zoom)`, so no compiler-written `new Map(...)` in either module reaches the foreign class. (An importer reaching two binding modules' classes of one type name is the one seat where the second local takes Lexer §3.2's suffix; the exported spelling never does.)
...
| *(#982, #1014)* A member's foreign name is a JavaScript property name — any IdentifierName, Hexagon hard keywords included; a keyword-named member binds unaliased, every use of it dotted (Lexer §4.4), and `as` stays the author's choice | §2.4 |
| *(#982)* Importers emit instance-member calls inline as the binding module does; static members and constructors reach the foreign constructor object through the binding module's `__class_<Type>` re-export (fixed spelling), bound under a minted import local mirroring the foreign class name where that spelling is free and `__class_<Type>` itself where it is not — never a numeric probe (Part 7 §1.2 rule 1); first-class references import the stable wrapper; `__class_<Type>` is linkage, in no `.d.ts` | §2.2, §7 |
...
| *(#876)* Receiver-member slots (`get` result, `method` params/result, `set` argument, `new` arguments) are FFI Part 1 §5.4 capture positions; the §2.3 convention-preserving wrapper performs the walk for such a member — the third walk-carrying binding beside Part 7 §7 occasion 4 and Part 4 §4.3; a captured receiver type is copied at the call; the plain-`fun` identity contrast in §2.3 excepts captured-collection signatures | §2.3, §3.1, §5 |
