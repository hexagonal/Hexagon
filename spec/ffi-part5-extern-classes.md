...
| *(#876)* Receiver-member slots (`get` result, `method` params/result, `set` argument, `new` arguments) are FFI Part 1 §5.4 capture positions; a captured receiver type is copied at the call; the plain-`fun` identity contrast in §2.3 excepts captured-collection signatures | §2.3, §5 |
