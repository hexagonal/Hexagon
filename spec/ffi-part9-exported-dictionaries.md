...
- *(#876)* A factory whose **result** dictionary type names a captured foreign collection returns the public shape §3.4 describes — affected members as stable copying wrappers, constructed with the result — and the result-identity contract above is unchanged; its dictionary **inputs** take no inbound walk (§3.4).
