...
**Status:** Decided (July 2026), revised in place after external review (Sol) before landing. **Amended (#876):** a public dictionary handle, a public factory's result, and a published generic edition whose types name a captured foreign collection take stable copying wrappers beside the unwrapped internal dictionary, internal factory, and internal edition ordinary Hexagon evidence uses; inbound evidence is not walked; the costs are stated — §3.4, §4, §9, §10. **Implementation status:** the public-evidence surface of §2–§5 (the `Constraint.Dictionary<a>` interface, handles, factories, the closure) is not yet emitted (#886, #725), and Part 8's base-name generic edition is not yet published (#423); the amended wrappers land with them (#992). The checker's half of §3.4 — Part 1 §5.4's refusals at an exported constraint's member annotations — is implemented. Normative promotion of `spec/notes/ffi-exported-dictionaries.md` (§§3–10 of which Part 8 defers to by name). The note's two delegated completions were confirmed in §13: handles and factories live in the instance declaration's home module under the lowercased other-party name, and evidence suffixes retain maximal constraints per variable before ordering. Preserves Part 7's corrected wrapper rule: a constrained generic edition exports the matching internal trailing-evidence function directly; a stable wrapper exists only when public ABI plumbing requires one (§9).
...
**Captured foreign collections in member types** *(#876)*. A handle is a record of functions crossing outbound, and FFI Part 1 §5.4's walk applies to it as to any aggregate: when the dictionary type names a captured foreign collection (`Array(a)`, `JsMap(k, v)`, `JsSet(a)`) — a member returning or taking one — the **public handle's affected members are stable copying wrappers**, allocated once with the handle, each running FFI Part 1 §5.4's walk over its captured parameters on entry and its captured result on exit at every invocation; members naming none are the instance functions themselves; a nested base-dictionary slot (§7.1) inside a public handle **is** that base instance's own public handle, already wrapped by this paragraph — never a second wrapping, so `ord.Eq` and the base's handle are one object. An exported constraint's member parameters and result are boundary positions in their own right (FFI Part 1 §5.4's table), so Part 1 §5.4's refusals 1 and 2 apply at the member annotation, whether or not §5's closure publishes a handle (and item 7 as well: the member's face is the constraint's declaration while its body is an `honor` checked against that declared row, so an open row in a member signature would let an honor body read a field no face lists — a JavaScript caller through the public handle then hands it a live foreign value; every exported member signature is closed). The public handle keeps this section's stable identity and may still be frozen. It is then a **distinct object from the internal dictionary** — the unwrapped record ordinary Hexagon evidence uses (Constraints §6), which cross-module Hexagon polymorphic calls continue to pass and which copies nothing. **Public evidence passed back into a generic edition is accepted without an inbound dictionary walk**: lawful evidence is Hexagon's own linkage returning, and a fabricated dictionary is already §10's contract violation. The public wrappers inside such evidence remain active, so **an edition executing under public evidence pays the wrappers' copies at each member call** — a cost linear in the collection at every invocation, repeated as often as the edition invokes the member, never constant-time — accepted as a stated qualification of FFI Part 1 §5.4's copy-free internal-call principle: ordinary internal evidence is unwrapped, and execution under public evidence bears its boundary-wrapper costs. The §5 closure and Part 8's Algorithm G are unaffected; FFI Part 1 §5.4's nesting refusals and `JsValue.from` refusal apply to member types exactly as to any declared type, and dictionary generation bypasses none of them. This two-object shape is the compiler's specifically defined treatment of its own handles and does not lift Part 7 §7's refusal of ordinary exported non-function values (FFI Part 1 §5.4 item 4).
...
- Factories are exported functions with stable ESM identity, exported directly per §9 (except the captured-collection case below, where the public factory is its own binding).
- *(#876)* A factory whose **result** dictionary type names a captured foreign collection has two bindings, as a handle has two objects (§3.4): the **public factory**, exported under the home rule's name, returns the public shape — affected members as stable copying wrappers, constructed with the result, so the wrappers live as long as the result and no longer — and the **internal factory** ordinary Hexagon evidence applies (Constraints §6.1; `dictionary-sharing.md` §3) is a separate function returning the unwrapped dictionary. A captured-collection result is therefore where the two stop being one binding, and the public name is not §9's direct-export case. The result-identity contract above is unchanged; dictionary **inputs** take no inbound walk (§3.4), so a memoizing factory may see two lawful input identities for one instance — the public handle and the internal dictionary — which the identity clause above already declines to promise anything about — and memoization, where used, is per binding, since a table shared across the two would hand a public caller an unwrapped result. A public factory result's **nested base-dictionary slot** (§7.1) is the base instance's own public handle, or that base's public factory result, already wrapped — never a second wrapping and never the unwrapped internal base; where the base instance has no public evidence (§5 condition 3 failing), the composed evidence stays private with §12's explanatory diagnostic (via §5).
...
> When the internal trailing-evidence function already has the public convention — source parameters in order, canonical evidence suffix — **it is exported directly**, with raw identity and stable ESM binding (Part 7 §1). A **stable module-level wrapper** is generated **only when public ABI plumbing requires one**: a supported top-level adapted position in the same signature (an `Iterable<a>` parameter declared `Seq(a)`, Part 3), a named conversion's required validation, an internal form that no longer matches the public convention (e.g. the optimizer split or specialized the only internal edition under Part 8 §8.2's freedoms), or *(#876)* **a signature naming a captured foreign collection** (FFI Part 1 §5.4; Part 7 §7 occasion 4) — the public base name then binds the stable export wrapper, which walks the captured source parameters and result, and the unwrapped edition retains a reserved internal spelling for Hexagon importers (Part 7 §7 occasion 4) — the direct-export emission below is the no-split case, exactly as occasion 4 does for an unconstrained export. The edition's *evidence* parameters are not walked (§3.4, §10).
...
- *(#876)* **Inbound evidence is not walked.** A dictionary arriving in the trailing suffix is accepted as it is (§3.4): lawful public evidence is Hexagon's own; a fabricated one is the contract violation below.
...
| exported constraint member whose parameter or result names a captured foreign collection beneath one of Part 1 §5.4 item 1's containers or inside an opaque representation | Part 1 §5.4 items 1, 2 at the member annotation | §3.4 |
...
| *(#876)* Captured foreign collections in dictionary types: public handle members, public factory results (per-result stable copying wrappers, a separate public factory binding), and published generic editions (Part 7 §7 occasion 4's stable export wrapper on source positions) are wrapped; internal dictionary, internal factory, and internal edition stay unwrapped; a nested base slot is the base's own handle; inbound evidence is never walked; an edition executing under public evidence pays the wrappers' copies at each member call — a stated, non-constant-time qualification of the copy-free internal-call principle; closure and Algorithm G unchanged; nesting and `JsValue.from` refusals not bypassed; D15's refusal of ordinary exported values not lifted | §3.4, §4, §5, §9, §10 |
