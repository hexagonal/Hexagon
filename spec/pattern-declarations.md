...
- **The suffix binds tightest.** It is a structural form, so `(r, g, b)rgb as colour` binds `colour` to the `Color` — `as` binds the **subject**, never the component tuple — and `(_, 0.0, _)hsl | (255, 255, 255)rgb` is an or-pattern of two suffixed forms under the same-bindings rule. The name follows the closing parenthesis within the same logical item; canonical formatting writes it adjacent (`notes/canonical-formatting-and-naming.md`).
...
- **No seat has an occupant.** Nothing else in the language stands after a closing parenthesis, so the form contends with nothing: not the function header at `let` (`let plus(x, y) = …` keeps its one meaning, and `let (n, d)rat = r` is unmistakably a destructure), not a call, not a tuple. The case rule is untouched: uppercase-start heads are constructors, non-uppercase-start names without parentheses are binders, and the suffix is neither.
...
- **The door answers, never ranks.** The namespace is read first, module-wide, and the door answers only where it has nothing; where the home *is* imported the two agree, and the namespace answered. Nothing is searched: the door reads one module the type names.
...
- **An expression use** emits the call `rat.build(1n, 2n)`. A pattern of another module is reached through that module's emitted namespace import (Modules §11): `Rat.rat.view(r)`, `Rat.rat.build(1n, 2n)`. A pattern the **door** resolved from a home this module never imported is reached the same way, through a namespace import the emitter adds for the home — the import line standing for a dependency the type carried in, as Modules §11.3 has every emitted import stand for a dependency; load order is unaffected, since the home is already in the graph of whichever module produced the value.
- **A term of the same name.** `let rat` and `pattern rat` in one module are two JavaScript bindings, and the pattern's takes the emitter's collision-only alias (`rat_1`), the term keeping the plain name — the one case a JavaScript reader meets it, and only because the Hexagon reader wrote both.
...
| Suffix in scope nowhere; expected type undetermined (pattern position) | "no `rgb` here: its type is not determined at this pattern — `import Color`, or bind the function with its own annotated `let`"; "or ascribe the scrutinee" at a nested slot (§3.3) |
...
| Rejection | Reasoning |
|---|---|
| **The prefix spelling** `rat(n, d)`, constructor-shaped, in both positions | Its seat has an occupant: `let rat(n, d) = r` is already the header sugar for a function named `rat` (Functions §3.2), and the destructure could be read there only by a scope rule, with the door shut at `let` for good — the seat the importer wanted most. The suffix has no occupant anywhere (§3.1). The resemblance to a constructor was the argument for it, and it was an aesthetic one. |
...
| The suffix spelling `(p1, …, pn)name` in both positions, bare always — `5n`'s seat with a name in it; the parentheses are the component list's; `(p)name` is one component; `as` binds the subject; irrefutable iff the components are; no seat has an occupant | §1, §3.1, §3.2 |
| The pattern namespace: a namespace of its own, filled by the module's declarations and by every imported module's exported patterns; own occludes, contests refused at the use; the one seat where an import's exports appear bare (Modules §3's carve) | §3.3 |
...
| One emitted object per pattern, `{view, build?}`, the collision-only alias where a term shares the name; a door-resolved unimported home reached through an emitted namespace import; the `.d.ts` faces the object, a polymorphic pattern's binders bound per member | §6 |
...
fun bad(c: Color): String =
    match c
        (0, 0, 0)rgb => "black"
        (_, _, _)hsl => "any"                -- ERROR: match is missing cases: (_, _, _)rgb
...
match c                                      -- ERROR: cannot destructure opaque record Color;
    Color({channels}) => ...                 --   match it with (r, g, b)rgb or (h, s, l)hsl

-- (d) The door is closed where the type is undetermined; a contest; the alias
let f = c => match c
    (r, _, _)rgb => r                        -- ERROR: no rgb here: its type is not
...
pattern rgb = Color.rgb                      -- own: the contest is over
fun h(c: Color): Int =
    match c
