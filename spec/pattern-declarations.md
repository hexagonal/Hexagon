...
| A contextual word against the parenthesis with no pattern of that name in scope — `(0, 0)when g`, `(r)as s`, `(a, b)derives` | "a guard's `when` stands off the parenthesis; write `(0, 0) when g`" — the near-miss row the adjacency rule creates, for `when`, `as`, `derives`, and `with` (§3.1) |
...
| Arm dead beneath a catch-all of another view, or beneath a complete constructor signature | Pattern Matching §7.2's report, naming the shadowing arm where one subsumes alone (§4) |
