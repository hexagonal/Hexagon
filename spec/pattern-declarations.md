...
- **With a head, members are checked, not inferred** — the `honor` posture (Constraints §4.1): the expected type of each member is fixed by the head, the body checks against it, and the member's lambda parameters take their types from the head before the body is inferred (Functions §4.3's supplying seat). Annotations on a member are optional and must match the head's type exactly. A `build` whose parameter count differs from the component list or whose result is not the subject, or a `view` whose result is not the head's component tuple, is an error at the member naming the head's shape. A mismatch inside the result — a literal's component, a vector's element, a form's branch — is refused where it stands, the head's type reaching it as any expected type reaches inward (Functions §4.3): `view(v) = ["wrong"]` at a `Vector(Int)` component is refused at `"wrong"`, as `("wrong", 1)` at `(Int, Int)` is.
