...
| Imported pattern whose declared subject's home exports a pattern of the same name | "`rgb` here is `Paint.rgb`, over `Color`, whose home exports its own `rgb` — declare `pattern rgb = Color.rgb` or `pattern rgb = Paint.rgb`" at every use; with several imports contesting, the home's joins the list (§3.3) |
...
fun k(c: Color): Int =
    match c
        (r, _, _)rgb => r                    -- ERROR: rgb here is Paint.rgb, over Color, whose
                                             --   home exports its own rgb — declare
                                             --   pattern rgb = Color.rgb or pattern rgb = Paint.rgb
-- module Named:
import Color
import Paint
