...
- The union name and every constructor name are **uppercase-start** (Functions §2: uppercase is reserved for types and constructors). The union name enters the type namespace; each constructor enters the **term namespace, unqualified, in module scope** (Elm/Gleam style) — in the declaring module. Abroad, no import binds a constructor bare (Modules §3, #762): a constructor is reached qualified through the module's alias in an expression and a pattern alike (`Shape.Circle(1.0)`, Modules §3.1); bare in a **pattern** through the scrutinee's type (Pattern Matching §2.2's door, #763); and bare in term position only where it is spelled like the alias itself, the companion fallback (Modules §5.1 rule 3). Two constructors with the same name among in-scope unions is an error at the point that makes it ambiguous — a rule with no reachable instance now that no import binds a constructor bare: the unions in scope are the module's own, whose declaration-site case (two unions in one module sharing a constructor name) is a hard error at the second declaration, and the prelude's, which occlusion governs (next). A constructor name that collides with a **prelude** constructor is neither: the declaration occludes the prelude's, module-wide and in pattern position too (Modules §5.4) — the ambiguity and hard-error rules above govern unions the module itself declares, never the prelude layer. Qualified access is the modules spec's business.
...
- A constructor application's expected type reaches its arguments: where it is headed by the union, the instantiated result type is unified with it before the arguments are checked (Functions §4.3, #1066), so `let a: Option(Dec) = Some(n)` checks `n` against `Dec`. The declaration's parameters carry no constraint (Declarations Preamble §2.2), so the instantiation this fixes selects no evidence. As a call's argument, a constructor application is read into the call's argument list as a literal is: its callbacks wait for the call's second pass, and, where its head is the parameter's, its arguments at the callee's type variables join the call's sibling groups (Functions §4.3's argument spine, #1096).
...
> **The representation principle.** Where JavaScript already has the concept — the same concept, semantics included — the thing Hexagon must give a runtime form becomes that JavaScript value: `Bool` is a `boolean` (§6.2), a record is a plain object (Products §3.5), a tuple is an array (Products §2.6), a function is a function, a `Seq` is an iterable (FFI Part 3), and `Unit` is the `undefined` a function that returns nothing already returns (Products §2.6). Where JavaScript lacks the concept, Hexagon picks one representation of its own and uses it everywhere: every union is a string-tagged plain object (this section), constraint evidence is a dictionary (Constraints; FFI Part 9), and `Option` is a union like any other. Where JavaScript has a near concept with different semantics, Hexagon keeps its own representation and gives the JavaScript concept an explicit door of its own: `Vector`, `Map` and `Set` are persistent tries (Collections Parts 3–4) and cross as themselves (FFI Part 1 §4.1's identity rows), while a JavaScript array, `Map` or `Set` arrives as `Array(a)` (FFI Part 2 §6), `JsMap` or `JsSet` (FFI Part 10); JavaScript nullability arrives as `Nullable(a)` (FFI Part 2 §2), which is why `Option` never becomes `null` or `undefined`. The test is the concept, not the word: a JavaScript method dispatches on a value where a constraint dispatches on a type, and TypeScript's string-literal union is a type-level convention over strings, not a runtime value for "one of these alternatives" — so neither borrows the JavaScript form. A borrowed form is taken at the seat where the concept matches and carried to the rest by uniformity: `Unit` in a data position coexists with `undefined`-as-absence because Hexagon never reads `undefined` as absence (Primitive Types §9). The type-system overview §4 lists the commitments this principle produces.

**One representation, every union.** A union's runtime shape does not depend on its declaration: the string-tagged plain object of §6.1 serves a union of payload constructors, a union of nullary constructors (§6.2), and every mixture, so adding a constructor never changes the shape of the constructors already declared. The one pinned exception, the prelude `Bool`, is the principle's borrowing clause at work — JavaScript has the concept — and not an exception to the union rule (§6.2; #147).

### 6.1 The shape: string-tagged POJOs
...
- **Nullary constructors emit as one shared frozen-by-convention constant**: `const Point = {tag: "Point"};` — constructions reference it, allocating nothing. Sound because everything is immutable and `Eq` is structural (§7); identity is never observed. Generic nullary constructors (`None`) are a single constant shared across all instantiations — types erase. The constant is shared within the module that emits it: a module that makes the value without importing it — generated code, or a prelude module seated before its union's implementation (Modules §5.5, §11) — mints its own, and the two are the same value to every Hexagon operation.
...
### 6.2 All-nullary unions: the same shape, and the one pinned exception

A union in which **every** constructor is nullary is no special case. It takes §6.1's shape with no payload fields, each constructor the shared constant:
...
| Hexagon | JS | `.d.ts` |
|---|---|---|
| `Red` | `{tag: "Red"}` — the shared constant `const Red = {tag: "Red"};` | `type Color = {tag: "Red"} \| {tag: "Green"} \| {tag: "Blue"};` |
| `match c` arms | `switch (c.tag) { case "Red": ... }` | — |

**What one shape buys: adding a constructor is additive at the JavaScript boundary.** Whether the new constructor carries a payload or not, every existing value keeps its shape and every existing `.d.ts` arm keeps its text. What a consumer's code can still lose is the exhaustiveness of a `switch`, which a new case costs in any language; and derived `Ord` still follows declaration order (§7), so inserting or reordering constructors changes what an exported `compare` means (the same event FFI Foreign Enums §7.3 records for its form) — a change of meaning, never of shape. Nothing in the declaration has to be read as changing the representation of what stands beside it, and no generated documentation has to warn about it (FFI Part 7 §4).

This case formerly emitted its values as bare strings — `"Red"`, with `type Color = "Red" | "Green" | "Blue"` — bought for the TypeScript string-enum idiom and paid for with a *representation cliff*: the first payload-bearing constructor added to such a union changed the shape of every constructor already declared, silently for JavaScript consumers, and the language had to shout about it. The string form was retired for the cliff and by §6's principle: a string-literal union is a TypeScript convention, not a JavaScript concept — at runtime JavaScript has no value for "one of these names" — so the case falls to the principle's one-shape clause and takes the union's one shape. A *foreign* set of strings (`"up" | "down"` in a JavaScript API) is a foreign concept and crosses through `extern enum` (`ffi-foreign-enums.md`; the object-free literal form is its §2.4), never through an ordinary union.

> **The one language-side exception: the prelude's `Bool` (#147).** `union Bool = False | True` (§8) does **not** take the tagged-object representation. Its representation is **intrinsically pinned to JS `boolean`**: `True` emits `true`, `False` emits `false`, `.d.ts` says `boolean`, and `match` emits on the boolean itself (`if`/ternary or `switch` with `case true:` — §6.3's judgment license applies). This is §6's principle's borrowing clause: JavaScript has the concept, so the Hexagon value becomes it. The pin is a compiler representation commitment granted to exactly this one prelude declaration — no user syntax requests it, and it is not the intrinsic door (Intrinsics §1 links operations, not representations). Its constructor set is compiler-verified prelude source, so no later constructor can ever contest the pin. Derived-instance emission simplifies accordingly: `Eq` is `===`; `Ord` needs no declaration-index table, because JS `<` on booleans agrees with the declaration order `False | True` by construction; `Hash` hashes the boolean; `Show` is the two-way lookup to `"True"`/`"False"`. Full ruling: `decisions-ml-dialect-bool-2026-07.md` §3.
...
Source `match` compiles to a JS `switch` on `s.tag`, with pattern binders emitted as `const` bindings from the payload fields — using the *declared* field names (`const r = s.radius;`), since patterns are positional in the source but the representation is named:
...
- **Applied directly, the constructor erases into the object literal, at every seat** — `Circle(2.0)` at home and `Shape.Circle(2.0)` abroad (Modules §3.1), or a constructor reached through Modules §5.1 rule 3's fallback, all emit `{tag: "Circle", radius: 2.0}`, never a function call. Same doctrine as `record`'s constructor (Products §5.4).
- **Referenced as a value**, the emitter materialises the function on demand: `const Circle = radius => ({tag: "Circle", radius});` — emitted only where the module references the constructor as a value or exports it; direct applications still erase. A record's constructor follows the same demand rule (Products §5.4). This follows the general JavaScript-output rules: a unary arrow omits redundant parameter parentheses, and an object field uses shorthand when its key and emitted value are the same identifier. **Export of a non-opaque union is a mandatory demand site**: it materialises each payload constructor once as a stable named ESM function; nullary constructors export their shared constants (FFI Part 7 §4). Direct applications still erase, at every seat. `opaque union` exports the type only and does not materialise or expose its constructors merely because the type is exported (FFI Part 7 §5).
...
- An exported **non-opaque** union declares every constructor in `.d.ts` (FFI Part 7 §4): payload constructors as functions returning the union type, nullary constructors as shared constants. A generic shared nullary constant uses the `never` instantiation (`None: Option<never>`). An `opaque union` instead has FFI Part 7 §5's brand-only type face and exposes no constructors.
...
- **`Bool` is the sole exception to §1's match-only eliminator doctrine** *(added 2026-07-29, PR #148 review)*: `and`/`or`/`not`/`implies`/`iff` (Operators §4) and `if`/`while` conditions (Operators §11) eliminate a `Bool` without `match`, with mandated native emission (`&&`, `||`, `!`, `!a || b`, `===`, direct branch). That emission is **legal only because of the §6.2 pin** — against the tagged-object representation, `&&` on `{tag: "False"}` would be truthy nonsense — so the operators are silent dependents of the pin, recorded here and in the ruling's ledger so a pin revisit knows what breaks. The eliminators are representation-blind at the source level; nothing about them exposes fields, predicates, or casts, and the §5 dot-access ban applies to `Bool` like any union.
...
| Every union takes the tagged-object shape; the bare-string all-nullary case retired with its representation cliff (#771); the representation principle stated at the head of §6 | §6, §6.2 |
| Mixed-union string nullaries rejected; moot since #771 (no string case exists) | §6.2 |
...
| `.d.ts` = hand-written-style discriminated union for non-opaque exports, with constructors in their representation shapes; opaque export = brand-only type face | §6.5; FFI Part 7 §§4–5 |
...
| `Bool` is a prelude union (`False \| True`), representation intrinsically pinned to JS `boolean` — sole language-side exception to the union shape, an instance of §6's principle; no user-reachable pin | §6.2, §8; decisions-ml-dialect-bool-2026-07.md (#147) |
| Ord by declaration order (indices by tag, never `<` on names); Show positional | §7 |
