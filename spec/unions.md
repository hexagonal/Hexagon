...
- An exported **non-opaque** union declares every constructor in `.d.ts` (FFI Part 7 §4): payload constructors as functions returning the union type, mixed-union nullaries as shared constants, and all-nullary constructors as string constants. A generic shared nullary constant uses the `never` instantiation (`None: Option<never>`). An `opaque union` instead has FFI Part 7 §5's brand-only type face and exposes no constructors.
