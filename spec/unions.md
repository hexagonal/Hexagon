...
- A `match` may take a **`catch` clause** at the match head's own column, after the arm block — the match catch expression, whose arms handle exceptions thrown by the scrutinee's evaluation only. Owned by Exceptions §5.4; nothing in this section changes when the clause is present.
