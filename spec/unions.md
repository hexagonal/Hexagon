...
> **The representation principle.** Where JavaScript already has the concept, the thing Hexagon must give a runtime form becomes that JavaScript value: `Bool` is a `boolean` (§6.2), a record is a plain object (Products §5.4), a tuple is an array (Products §2.6), a function is a function, a `Seq` is an iterable (FFI Part 3), and `Unit` — which n-ary functions confine to the output seat — is the `undefined` a function that returns nothing already returns (Products §2.6). Where JavaScript lacks the concept, Hexagon picks one representation of its own and uses it everywhere: every union is a string-tagged plain object (this section), constraint evidence is a dictionary (Constraints; FFI Part 9), and `Option` is a union that never becomes `null` or `undefined` — `Nullable(a)` is the explicit door (FFI Part 2 §2). The test is the concept, not the word: a JavaScript method dispatches on a value where a constraint dispatches on a type, and TypeScript's string-literal union is a type-level convention over strings, not a runtime value for "one of these alternatives" — so neither borrows the JavaScript form. A borrowed form is taken at the seat where the concept matches exactly and carried to the rest by uniformity: `Unit` in a data position coexists with `undefined`-as-absence because Hexagon never reads `undefined` as absence (Primitive Types §9). The type-system overview §4 lists the commitments this principle produces.
...
**What one shape buys: adding a constructor is additive at the JavaScript boundary.** Whether the new constructor carries a payload or not, every existing value keeps its shape and every existing `.d.ts` arm keeps its text; the only thing a JavaScript consumer can lose is the exhaustiveness of a `switch`, which a new case costs in any language. Nothing in the declaration has to be read as changing the representation of what stands beside it, and no generated documentation has to warn about it (FFI Part 7 §4).
