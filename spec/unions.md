...
> **The representation principle.** Where JavaScript already has the concept — the same concept, semantics included — the thing Hexagon must give a runtime form becomes that JavaScript value: `Bool` is a `boolean` (§6.2), a record is a plain object (Products §3.5), a tuple is an array (Products §2.6), a function is a function, a `Seq` is an iterable (FFI Part 3), and `Unit` is the `undefined` a function that returns nothing already returns (Products §2.6) — n-ary functions take no unit argument (Functions §5.3), so a `Unit` function borrows the JavaScript shape at both ends. Where JavaScript lacks the concept, Hexagon picks one representation of its own and uses it everywhere: every union is a string-tagged plain object (this section), constraint evidence is a dictionary (Constraints; FFI Part 9), and `Option` is a union that never becomes `null` or `undefined`. Where JavaScript has a near concept with different semantics, Hexagon keeps its own representation and gives the JavaScript one an explicit door: `Vector`, `Map` and `Set` are persistent tries (Collections Parts 3–4), not arrays, `Map`s and `Set`s, which cross as `Array(a)`, `JsMap` and `JsSet` (FFI Parts 2, 10); nullability crosses as `Nullable(a)` (FFI Part 2 §2). The test is the concept, not the word: a JavaScript method dispatches on a value where a constraint dispatches on a type, and TypeScript's string-literal union is a type-level convention over strings, not a runtime value for "one of these alternatives" — so neither borrows the JavaScript form. A borrowed form is taken at the seat where the concept matches and carried to the rest by uniformity: `Unit` in a data position coexists with `undefined`-as-absence because Hexagon never reads `undefined` as absence (Primitive Types §9). The type-system overview §4 lists the commitments this principle produces.
