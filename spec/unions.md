...
This case formerly emitted its values as bare strings — `"Red"`, with `type Color = "Red" | "Green" | "Blue"` — bought for the TypeScript string-enum idiom and paid for with a *representation cliff*: the first payload-bearing constructor added to such a union changed the shape of every constructor already declared, silently for JavaScript consumers, and the language had to shout about it. The string form was retired (#771) for the cliff and by §6's principle: a string-literal union is a TypeScript convention, not a JavaScript concept — at runtime JavaScript has no value for "one of these names" — so the case falls to the principle's one-shape clause and takes the union's one shape. A *foreign* set of strings (`"up" | "down"` in a JavaScript API) is a foreign concept and crosses through `extern enum` (`ffi-foreign-enums.md`; the object-free literal form is #773), never through an ordinary union.
