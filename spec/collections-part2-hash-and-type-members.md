...
| Member-block `honor Hash<T>` whose subject annotation does not resolve, or whose head is a bare type variable | this seat is **silent**: the unknown-name error or Constraints §5.4's head refusal is the whole answer — no advice is owed about a subject the checker cannot name, or that cannot host an instance at all (#647) | §4.1; Constraints §5.4 |
