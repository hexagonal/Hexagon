...
Dictionary shape, per Constraints §6.2: `Hash_T = { Eq: Eq_T, hash: x => ... }` — the base slot carries the base declaration’s name verbatim, so it can never collide with a member name (start classes partition them; §6.2, #718).
...
This table is the exhaustive **status map** of core-type `Hash`; each row records its own provenance, and no row remains in §4.4's provided **category** (compiler/runtime-provided, specified normatively with no source form): every primitive's row records a *hand-written source* instance under the companion privilege carve-out (§4.4; Constraints §4.5), `Bool`'s records a *derived* instance through the ordinary §4.3 door (§18), and `Unit`'s records the *automatic structural* arity-0 derivation — never in the category, though it shares the no-source-form status every structural instance has always had (§4.4). Among `Hash` instances, the category now holds exactly the collection instances (Part 3 §8, Part 4 §8.4):
...
| `Hash<Float>` | §2.3: `-0` ≡ `+0`, all NaNs one value — SameValueZero-consistent *(provenance corrected, #344: hand-written source in `stdlib/Float.hex` under the §4.4 carve-out — row retained as the status map)* |
...
| `Hash<Unit>` | constant; the automatic structural arity-0 derivation — `Unit` is the empty tuple (Products §2.7), so this is the structural-types rule below at the vacuous arity, never in §4.4's provided category — row retained as the status map |
...
The rule constrains the instance form and nothing else. `Hash` as a **base constraint** of a user-declared constraint — `constraint Wide<a: Hash>` — is ordinary and unrestricted: extension mints no instance, and every ground `honor` of the extending constraint still existence-checks (Constraints §4.2) a `Hash` instance that no user source could have written except through this section's door (§4.4 for the provided and carve-out instances).
...
"Derivable-only" constrains **user source**. Hexagon-owned source may supply a member-block `Hash` instance for a nominal record or union declared by that same module, including an opaque type. Primitive companions retain their existing permission to supply `Hash` for their own primitive (#344). No permission is granted to honor another module's subject, and the ordinary orphan, coherence, member-signature, and base-constraint checks remain in force.

**Ownership is compiler-established provenance** (intrinsics §5.2, #843): the source is embedded as a registered standard-library member, or the compiler host explicitly supplies it as that member's trusted replacement. A module name, filename, directory, manifest, package name, or import of a trusted module confers no trust. A user module occluding a standard-library name is still user source and receives the ordinary §4.1 refusal. Trust for this instance rule is separate from intrinsic access, runtime-only capabilities, and prelude visibility; admitting a source-written hash opens none of those capabilities.

The standard-library author and reviewer must verify §2.3's equality agreement and §2.4's determinism. A member-block hash may accompany a member-block equality when both satisfy those laws. **Derived hashes retain §4.3's requirement of derived equality, even in trusted source**: trust permits a deliberately implemented pair, not an incompatible structural derivation. For example, a future decimal type retaining display scale but comparing numerically would need equality and hashing to agree on a normalized numerical representation; hashing the retained fields structurally would not suffice.

Compiler/runtime-provided collection instances remain specified in their owning collection parts. This permission does not migrate them, change primitive hashes, or introduce user-defined hashing or `derive via`.

**Conformance:** the same nominal declaration with member-block `Eq` and `Hash` is accepted when supplied as a trusted standard-library member and refused as ordinary project source. Test both records and unions, including opaque declarations and qualified `Hash` heads. Derived `Hash` beside member-block `Eq` remains refused in either origin. A trusted representation-independent equality fixture must give equal hashes and successful map/set lookups for distinct stored representations that its `Eq` treats as equal; existing primitive-companion instances remain accepted.

### 4.5 The wrapper-key pattern

The sanctioned v1 answer to "I need non-structural key equality" is a **wrapper key type with a standard-library-provided, mutually consistent `Eq`/`Hash` pair**. First customer, owed to the stdlib listing: a case-insensitive string wrapper (working name `CiString` — name *and* folding semantics decided there, §12.1), whose `Eq` and `Hash` both operate on the folded form. Such a standard-library wrapper may now supply the pair in its own trusted source under §4.4; user wrappers may instead store only canonical fields and derive both instances. This is the Rust `UniCase` / Haskell `Data.CaseInsensitive` pattern: the variant equality lives in a *type*, where the instance system can see it, rather than in an instance variant, where §4.3 would have to police it.
...
**The declaring module is `stdlib/Iterable.hex`**, seated after `Seq.hex` — the member signature names `Seq(Item)`, and the house seating principle is after-what-it-names. The declaration above is that module's source; the Constraints §5.1.1 redeclaration ban covers the name like every pre-registered constraint's, and `Iterable.toSeq` is the member's qualified home (a constraint member is an export of its declaring module). The seat forces one continuing consequence: `Seq.hex` cannot honor `Iterable` — the name is not in scope before `Iterable.hex` seats, and the reverse ordering is a genuine cycle — so its identity row remains provided. It does not prevent a later companion from owning its instance: `String.hex` sits after `Iterable.hex` and contains the authoritative source `honor Iterable<String>` (Part 5 §5).
...
| Member-block `honor Hash<T>` at a project-source nominal subject whose `Eq` is absent or derived | refused with Constraints §8's advice family (#647): "`Hash` instances must be derived; use `derives (Eq, Hash)` on the declaration of `Weird`" — list-aware ("…add `(Eq, Hash)` to `Weird`'s `derives` list") and base-complete in either dialect; the declaration's module is named ("…in module `Types`") only when it is not the reporting module, the refused honor itself being the anchor otherwise — that branch is reachable only in an already-orphan program (a member-block honor outside `T`'s module, `Hash`'s other home being the prelude), and the orphan error stands beside it, both pointing at the same declaration | §4.1; Constraints §8; Modules §7.6 |
...
| Member-block `honor Hash<T>` in untrusted source at a prelude nominal subject | the head plus the one true sentence — "`Hash` instances must be derived; derivation is spelled on the subject's declaration, which is not in project source" — no advice a reader cannot follow (Modules §7.6's offering discipline) | §4.1 |
| Member-block `honor Hash<T>` at a subject with no `derives` seat | primitive outside the carve-out, extern, or structural: "`Hash` instances must be derived, and this subject has no declaration that could carry a `derives` clause" *(carve-out, #344: not emitted for privileged prelude companion source honoring at its own primitive — `BigInt.hex`'s `honor Hash<BigInt>`, `Int.hex`'s `honor Hash<Int>`, `Nat.hex`'s `honor Hash<Nat>`, `Float.hex`'s `honor Hash<Float>`, and `String.hex`'s `honor Hash<String>` are legal there and nowhere else; Constraints §4.5)* | §4.1; Constraints §4.5 |
| Member-block `honor Hash<T>` whose subject annotation does not resolve, or whose head is a type variable (bare or prefix-bound, constrained or not) | this seat is **silent**: the unknown-name error or Constraints §5.4's head refusal is the whole answer — no advice is owed about a subject the checker cannot name, or that is not a subject at all: a type variable has no instance of any kind, where a structural type has automatic ones and merely lacks a `derives` seat (the row above) (#647) | §4.1; Constraints §5.4 |
| Member-block `honor Hash<T>` in trusted standard-library source at its own nominal record/union, or its own companion primitive | accepted subject to ordinary instance checks; equality agreement and determinism are author/reviewer obligations | §4.4 |
