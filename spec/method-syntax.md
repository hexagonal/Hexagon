...
-- the receiver's own type decides, once, before the face descends (#821):
let n: BigInt = i.add(p).gcd(s)       -- OK — the receiver's own type is Foo (i widens into it); the
                                   --   face is withheld; Foo's gcd answers — identical to the
                                   --   let-bound control   [was refused twice]
let n: BigInt = (p + (i + j)).gcd(s)  -- OK — own type Foo; no face reaches (i + j), which widens
                                   --   into Foo: Foo.gcd(add(p, fromInt(i + j)), s)   [was refused]
let n: BigInt = (if c then i + j else p).gcd(s)
                                   -- OK — own type is the branches' join, Foo; identical to the
                                   --   control   [was refused twice]
let n: BigInt = (if c then i + j else i).gcd(s)
                                   -- ERROR — own type Int reaches; the if lifts to BigInt; then
                                   --   Integral<BigInt>'s gcd meets s : Foo — refused in the
                                   --   let-bound form too; no divergence
let result: Float = (i / j).pow(k) -- OK — the probe reads the provisional own type Int, which
                                   --   reaches Float, and commits nothing: Frac<Int>'s absence
                                   --   never reports; i / j runs at Float and the dot addresses
                                   --   Float's door with k injected. The control
                                   --   `let value = (i / j).pow(k)` refuses — the face admits
                                   --   more than the control, never less
```
