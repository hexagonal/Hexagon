...
| **Primitive** (`Int`, `Nat`, `BigInt`, `Float`, `String`, and compiler-known nominals like `Range`, `Seq`) *(`Bool` moved to the nominal row — reclassified as a prelude union 2026-07-29, #147; see the note under §4.1. `Nat` and `BigInt` added to this row with #304/#335 — the rows ascription.md §6.1 recorded as this arc's debt. `Unit` retired to the tuple row with #344 — it is the empty tuple (#159), and its listing here was a pre-#159 anomaly; no `Unit.hex` exists or ever will)* | As the nominal case, with `CompanionOf` the fixed prelude companion (§4.3). No fields exist, so the collision surface is export-vs-member only. A migrated companion supplies both halves as ordinary source (`BigInt.hex`, then `Int.hex` and `Nat.hex` — #344): `42n.show()` is `Show`'s member at `BigInt`, `n.div(2)` is `Integral`'s member at `Int`. For the companions not yet migrated (`Float`, `String`), the wired primitive instances supply members exactly as source `honor` blocks do — `1.5.show()` is `Show`'s member at `Float`, companion module or not. |
...
| `Int`, `Nat`, `BigInt`, `Float`, `String` | The fixed prelude companion of the same name (Modules §5.3: `Int.div` etc. — "one mechanism, not a special prelude device"). *(`Nat`/`BigInt` rows added with #304/#335; `Unit` left this row with #344 — the empty tuple takes the tuple story, §3.4.)* `BigInt.hex`, `Int.hex`, and `Nat.hex` exist as source (#344) and are the ordinary nominal case in every respect. Where a companion does not yet exist as source (`Float`, `String` — migration order fixed, stdlib-roadmap §5.2), the wired constraint instances still supply the honored-member half of §4.2's operation set, and Modules §5.3's transitional note supplies the qualified member spellings — §6.4's qualified-home guarantee holds throughout. |
...
| Primitive companions become source per companion (#344): a migrated companion (`BigInt.hex` first, `Int.hex`+`Nat.hex` second) is the ordinary nominal case; wired instances serve the rest (`Float`, `String`) until their milestones, in the fixed migration order | §3.4, §5; Modules §5.3; Constraints §5.3 |
