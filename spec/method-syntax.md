...
let n: BigInt = (i + p).gcd(s)        -- ERROR (row 16) — the operator receiver; the same report
...
let n: BigInt = (p + (i + j)).gcd(s)  -- ERROR — the nested i + j lifts on the way down; the outer
                                   --   addition's operand-driven elaboration then has no common type
                                   --   for Foo and BigInt, and its own report is the one report,
                                   --   carrying the boundary fixit
let n: BigInt = (if c then i + j else p).gcd(s)
                                   -- ERROR (row 16) — a forwarding form is one receiver; the part
                                   --   named is the else branch p; one report   [was refused twice]
