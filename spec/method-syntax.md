...
| **Primitive** (`Int`, `Nat`, `BigInt`, `Float`, `String`, and compiler-known nominals like `Range`, `Seq`) *(`Bool` moved to the nominal row — reclassified as a prelude union 2026-07-29, #147; see the note under §4.1. `Nat` and `BigInt` added to this row with #304/#335 — the rows ascription.md §6.1 recorded as this arc's debt. `Unit` retired to the tuple row with #344 — it is the empty tuple (#159), and its listing here was a pre-#159 anomaly; no `Unit.hex` exists or ever will)* | As the nominal case, with `CompanionOf` the fixed prelude companion (§4.3). No fields exist, so the collision surface is export-vs-member only. Every companion supplies both halves as ordinary source (`BigInt.hex`, then `Int.hex` and `Nat.hex`, then `Float.hex` and `String.hex` — #344, migration complete): `42n.show()` is `Show`'s member at `BigInt`, `n.div(2)` is `Integral`'s member at `Int`, `1.5.show()` is `Show`'s member at `Float` through `stdlib/Float.hex`'s own `honor` block. |
...
The refusal above counts **claimants**, and Modules §5.3's generalisation law changes what counts as one: a companion export and that same companion's honored member related by the law — the export signature-generalises the member, the member is its restriction (Constraints §4.6's exemption) — are **one claimant, not two**. They are one operation wearing two widths, so there is nothing to disambiguate and nothing to rank: the dot call resolves as the companion-operation rewrite, `CompanionOf(T).name(e, args…)`, which under Modules §5.3's resolution order is the **export** — the operation's widest face. `x.pow(0.5)` at `Float` and `x.pow(2n)` at `BigInt` are therefore ordinary dot calls, not collisions (Operators §6.3.1). The no-ranking doctrine (§1) is untouched: it refuses genuinely rival sources, and the law's whole content is that these two are not rivals — a pair *not* related by the law (an export beside a member of the same spelling from some other honored constraint, or any same-spelled pair the law's declaration-time check did not license) counts two claimants and is refused exactly as before.
...
Two footnotes the old text carried, updated: a companion may still export a monomorphic subject-first `show(x: Int): String` — but inside the honoring module that spelling is the rebinding error (Constraints §4.6) **unless the export signature-generalises the member under the generalisation law**, in which case it is the member's own wider face and the dot reaches it as one claimant (§6.1); a same-spelled export *outside* the law, where a legal cross-module split arises, keeps the §6 refusal. The stdlib-listing question "monomorphic `show` companions?" (§12.4) is settled by construction either way: the member **is** the operation — identically, or as the lawful export's restriction — and a *duplicate* (rather than generalising) export is either unwritable or unreachable through the dot. The guide's teaching gains a spelling rather than changing: bare (`show(x)`), piped (`x |> show`), qualified (`Show.show`, `Int.show`), and — on known types and bounded variables — the dot.
