...
| 15 | *(#808)* Owned tower member at a `Nat`/`Int` receiver with no written face (`n.subtract(m)`, `i.divide(j)` unannotated) | exactly the operator's refusal — `n - m`'s no-`Signed` diagnostic; `i / j`'s no-`Frac` diagnostic with the `Int.div`/`Int.mod` fixit (Operators §15) — carrying the rider both spellings owe since the lift admits them: name the written face that runs the operation (`let d: Int = …` for `Signed`'s members at `Nat`; a `Float` or `Rat` face for `Frac`'s at `Int`). The rider is an operation seat's — it names a face the lift would run an *operation* at — and it rides only there: every other seat that draws the same missing-instance report (Modules §7.6) takes the report without it and with nothing offered in its place, a literal pattern at `Nat` among them (Pattern Matching §2.5), since a repair that is not valid at the reported position is not named |
