...
| 17 | *(#934)* Dot call on a receiver head-known at the dot whose head has **no companion module** and honors no subject-first member of the name — `Range` today (§3.4's Primitive row, §5) | "`Range` has no companion module, so `r.toSeq()` has nothing to dispatch to; write `Iterable.toSeq(r)`" — the receiver spelled as written; the offer is the qualified call of the provided-row member that answers the name, and the sentence stops at its first clause where none does (`r.spin()`). Refused at the dot, before the arguments are elaborated (§3.1): the call never becomes a row-field read, so row 8's rescue — written for a receiver that was *never* head-known — is not reached and its "type was unknown" clause is never said of a written type |
