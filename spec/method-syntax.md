...
| **The dot's receiver seat forwards the expectation** when the member name is a tower spelling (#808, post-review ruling): the receiver is an operand seat of the operation the call resolves to, so a dot chain lifts like the operator chain and the pipe; a spelling decides that an expectation travels, never what is dispatched; closed list | §2.2; Functions §4.3; Numeric Literals §5.1 |
...
-- (v) The receiver seat forwards (#808, ruled after #815's review) — a, b, c : Int,
--     a = 9007199254740991
let r: BigInt = a.add(b).multiply(c)  -- OK — both operations at BigInt; value-checked past 2^53:
                                   --   27021597764222979n, as (a + b) * c and
                                   --   a.add(b) |> Num.multiply(c) give   [was …976n: inner at Int]
let r: BigInt = (a + b).multiply(c)   -- OK — the same; emits (BigInt(a) + BigInt(b)) * BigInt(c)
let r: BigInt = (a + b).pow(c)        -- OK — the base seat lifts, the exponent stays Int
let f: Float = (a + b).toFloat()      -- OK — `toFloat` is no tower spelling: nothing forwards,
                                   --   a + b runs at Int, the companion call follows
...
- **Rejected alternative (do not relitigate):** widening the dot's *argument* seat alone, leaving the receiver pinned (#783's first reading). It repairs `b.add(i)` and leaves `i.add(b)`, `i.compare(b)`, and the lift untouched — three spellings of one member still disagreeing. Also rejected, after #815's review found the dot chain's receiver running at its own type (`a.add(b).multiply(c)` under a `BigInt` face gave a wrong value where every other spelling was exact): re-elaborating the receiver once the member is known (needs unification undo Algorithm J does not have), accepting the asymmetry (one spelling keeps the silent overflow), and refusing the shape (refuses a program every type admits) — the ruling is that the receiver seat forwards by spelling (§2.2), the pipe's own rule. Also rejected: an open tower, where any user constraint bounded on `Num` joins the rungs and its members are owned at `Nat`/`Int` — a user member spelled `add`, declared anywhere in the program and honored nowhere, would then make `i.add(j)` a two-claimant refusal for every module. (An author who *honors* such a constraint at `Int` takes that collision under §6 today, closed tower or open; closure keeps the collision the author's own declaration.)
