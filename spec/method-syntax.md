...
| **Primitive** (`Int`, `Nat`, `BigInt`, `Float`, `String`, `Unit`, and compiler-known nominals like `Range`, `Seq`) *(`Bool` moved to the nominal row — reclassified as a prelude union 2026-07-29, #147; see the note under §4.1. `Nat` and `BigInt` added to this row with #304/#335 — the rows ascription.md §6.1 recorded as this arc's debt)* | As the nominal case, with `CompanionOf` the fixed prelude companion (§4.3). No fields exist, so the collision surface is export-vs-member only — and the wired primitive instances (`Show<Int>`, `Integral<Int>`, …) supply members exactly as source `honor` blocks do: `42n.show()` is `Show`'s member at `BigInt`, `n.div(2)` is `Integral`'s member at `Int`, no companion module required. |
...
| `Int`, `Nat`, `BigInt`, `Float`, `String`, `Unit` | The fixed prelude companion of the same name (Modules §5.3: `Int.div` etc. — "one mechanism, not a special prelude device"). *(`Nat`/`BigInt` rows added with #304/#335.)* Where the companion module does not yet exist as source (`Int`, `Nat`, `BigInt`, `Float`, `String` await the companion arc), the wired constraint instances still supply the honored-member half of §4.2's operation set, and Modules §5.3's transitional note supplies the qualified member spellings — §6.4's qualified-home guarantee holds throughout. |
