...
*(#808.)* **A member call at a primitive instance emits what a person would write in JavaScript** for that operation at that type — in every spelling: operator, bare, constraint-qualified, pipe stage, or dot; a companion-qualified spelling is a written face (§1) and emits what its face's operation emits. The principle is general, above the numeric tower: **where a person would write a JavaScript operator, the compiler writes that operator** (Constraints §6.1 owns the rule). So wherever an operator spelling of a member has a lowering of its own at the type — the arithmetic family, `Eq`'s `equals` and `notEquals`, `Concat`'s `concat` at `String` — every other spelling of that member produces the operator spelling's text: `i.add(b)` emits `BigInt(i) + b`; `Num.add(i, j)` at `Int` emits `i + j`, where before this amendment only the operator spelling did and the member spellings emitted `add(i, j)`; `Float.multiply(count, price)` emits `count * price`; `i.equals(j)` emits `i === j` and `i.equals(b)` emits `BigInt(i) === b`; `s.concat(t)` at `String` emits `s + t`. The text is whatever the operator's lowering is at that type, helper and all — `f.equals(g)` at `Float` emits what `f == g` emits — never a second lowering invented for the member spelling. Where no operator carries the member's meaning — `compare`, whose result is an `Ordering`; `pow` at `Int`, a guarded function; `div`, `rem`, and the rest of `Integral`, Hexagon-defined conventions — every spelling emits the direct call to the member seat, `pow(i, j)`, which is what the member spellings emit today; that the operator spelling `i ** j` reads a dictionary slot instead is #810's business, not this clause's. The rule is about text, never meaning — the elaboration is one whichever spelling was written — and the conformance check at these sites is one text per operation across its spellings, and one value. Emission at nominal instances (`Rat`, user types) is untouched by this clause.
...
| A member call at a primitive instance emits what a person would write in JavaScript, in every spelling — the operator spelling's lowering wherever an operator carries the member's meaning (arithmetic, `equals`/`notEquals`, `concat` at `String`), the seat call otherwise; one text per operation across its spellings is the check; a general principle above the tower | §8.1; Constraints §6.1 |
...
i.equals(j)                        -- OK : Bool — emits i === j   [was equals(i, j); §8.1]
