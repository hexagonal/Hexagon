...
| Prelude nominal types (`Vector`, `Map`, `Set`, `Option`, `Result`, `Bool`, `Range`, `Seq`, `Ordering`, …) | Their prelude companion modules — the same rule as user nominals for a prelude type declared in source (`Option`, `Result`, `Bool`, `Range`, `Seq`, `Ordering`, …), whose home is the prelude module that declares it; and, for `Vector`, `Map`, and `Set`, which have **no declaration site yet** (Intrinsics §3.3, §9.2 — #930), a tie by name to the module addressable under the name, the boundary row's own mechanism. Listed separately only to record that built-ins are *not* special-cased; no dispatch outcome turns on which of the two ties applies. The authoritative inventory is the stdlib listing's. |
