...
| **Primitive** (`Int`, `Nat`, `BigInt`, `Float`, `String`, and compiler-known nominals like `Range`, `Seq`) *(`Bool` moved to the nominal row — reclassified as a prelude union 2026-07-29, #147; see the note under §4.1. `Nat` and `BigInt` added to this row with #304/#335 — the rows ascription.md §6.1 recorded as this arc's debt. `Unit` retired to the tuple row with #344 — it is the empty tuple (#159), and its listing here was a pre-#159 anomaly; no `Unit.hex` exists or ever will)* | As the nominal case, with `CompanionOf` the fixed prelude companion (§4.3). No fields exist, so the collision surface is export-vs-member only. A migrated companion supplies both halves as ordinary source (`BigInt.hex`, then `Int.hex` and `Nat.hex` — #344); for the companions not yet migrated (`Float`, `String`), the wired primitive instances supply members exactly as source `honor` blocks do: `42n.show()` is `Show`'s member at `BigInt`, `n.div(2)` is `Integral`'s member at `Int`, companion module or not. |
