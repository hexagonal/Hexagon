...
*(#808.)* **A tower member call at a primitive instance emits what a person would write in JavaScript** for that operation at that type — in every spelling: operator, bare, constraint-qualified, pipe stage, or dot; a companion-qualified spelling is a written face (§1) and emits what its face's operation emits. Where the operator lowering inlines the slot as a native operator (Constraints §6.1: `add` at `Int` emits `x + y`), every spelling produces that text: `i.add(b)` emits `BigInt(i) + b`; `Num.add(i, j)` at `Int` emits `i + j`, where before this amendment only the operator spelling did and the member spellings emitted `add(i, j)`; `Float.multiply(count, price)` emits `count * price`. Where JavaScript has no operator of the same meaning — `pow` at `Int`, a guarded function; `div`, `rem`, and the rest of `Integral`, Hexagon-defined conventions — every spelling emits the direct call to the member seat, `pow(i, j)`, which is what the member spellings emit today; that the operator spelling `i ** j` reads a dictionary slot instead is #810's business, not this clause's. The rule is about text, never meaning — the elaboration is one whichever spelling was written — and the conformance check at these sites is one text per operation across its spellings, and one value. Emission at nominal instances (`Rat`, user types) is untouched by this clause.
...
| A tower member call at a primitive instance emits what a person would write in JavaScript, in every spelling — the operator where one has the same meaning, the seat call otherwise; one text per operation across its spellings is the check | §8.1 |
