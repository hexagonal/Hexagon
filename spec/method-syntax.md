...
**Status:** Decided (July 2026), corrected in place after the second Sol review — see **correction record §16.1** (goal ownership) — and **amended August 2026 for constraint-member dispatch** (#304/#335, the members-as-values ruling) — see **reversal record §16.2** — and **amended September 2026 for tower-member widening** (#808: the dot on a tower member is the open member call, widened and lifted as the operator is) — see **amendment record §16.3** — and its receiver stand-down **retracted** (#821: the forwarded face is binding — §16.3's second record). A **hanging-questions** section (§12) remains; nothing there blocks implementation of §1–§11.
...
| Prelude nominal types (`Vector`, `Map`, `Set`, `Option`, `Result`, `Bool`, `Range`, `Seq`, `Ordering`, …) | Their prelude companion modules — the same rule as user nominals for a prelude type declared in source (`Option`, `Result`, `Bool`, `Seq`, `Ordering`, …), whose home is the prelude module that declares it; and, for `Vector`, `Map`, and `Set`, which have **no declaration site yet** *(#930; Intrinsics §3.3, §9.2)*, a tie by name to the module addressable under the name, the boundary row's own mechanism. `Range`, though listed among the heads, is head-known with no companion module today, so `CompanionOf(Range)` is undefined until `Range.hex` ships and no dot fires on it (§5; `stdlib-roadmap.md` §5.1 keeps the row). Listed separately only to record that built-ins are *not* special-cased in dispatch; no dispatch outcome turns on which of the two ties applies. The authoritative inventory is the stdlib listing's. |
| `JsValue` *(#511)* — and `Array(a)`, already eligible per Part 2 §13.1; `JsMap(k, v)` and `JsSet(a)` *(#792)*; non-absorbed `Nullable(a)` *(#786)* | The fixed prelude companion of the same name. These boundary types are compiler-provided with no declaration site, so the tie is by rule — the primitive row's pattern: `v.kind()` and `v.toInt()` are `JsValue.kind(v)` and `JsValue.toInt(v)`; `xs.length()` is `Array.length(xs)`, and each further `Array` companion operation joins the set as it ships (Part 2 §9.1); `m.get(k)` and `s.contains(x)` are `JsMap.get(m, k)` and `JsSet.contains(s, x)` (FFI Part 10 §3), while `m.toSeq()` is the provided `Iterable` row's member, not an export. At a non-absorbed `Nullable(a)`, `value.toOption()` is `Nullable.toOption(value)` (#786). If the type has collapsed to `JsValue` or a both-nullish enum, dispatch uses that resulting type's ordinary companion; write `Nullable.toOption(value)` for the nullish projection. No fields exist on these boundary values, so the collision surface is export-vs-member only. |
...
- `JsValue`, `Array(a)`, `JsMap(k, v)`, `JsSet(a)`, and non-absorbed `Nullable(a)` — compiler-provided boundary types tied to their fixed prelude companions by rule *(#511, #792, #786; §4.1's row)*: `v.kind()`, `v.toInt()`, `xs.length()` (Part 2 §13.1 — companion dispatch today; each further `Array` companion operation joins as it ships, Part 2 §9.1), `m.get(k)`, `s.contains(x)` (FFI Part 10 §3's surfaces, exported by `JsMap.hex` and `JsSet.hex` — except `toSeq`, which is the provided `Iterable` row's member and reaches the same walk by the dot); their values expose no Hexagon fields, so no collision surface; `value.toOption()` at a non-absorbed `Nullable(a)` calls its fixed `Nullable` companion (#786), with absorption following §4.1's resulting-type rule;
...
| 16 | *(#808, #821)* Tower-spelled dot call under a face honoring the spelling's rung, whose receiver is, or through Functions §4.3's forwarding forms hands the face to, a tower member call whose lift stands down — no ascription, binding, or companion-qualified spelling between — **and whose dot then resolves to a claimant outside the spelling's rung**: a companion export (`let n: BigInt = p.add(q).gcd(s)`, `Foo` honoring `Num`, its companion exporting `gcd(Foo, Foo): BigInt`), an honored member of a user constraint, or a function-typed field | the **receiver refusal** — one report, three facts, at the span of the outermost stood-down call the face reached, the fixit ascribing that call at the type it kept (the receiver itself where they coincide): "`gcd` is a member of `Integral` and `BigInt` honors `Integral`, so the `BigInt` here reached the receiver `p.add(q)`; `p` is a `Foo` and cannot enter `BigInt`, so the addition could not run at `BigInt`. To keep the receiver at `Foo`, ascribe it — `(p.add(q): Foo).gcd(s)` — or bind it first." Inside a form the ascribed expression is the stood-down call, `(if c then (p.add(q): Foo) else p).gcd(s)`, a repair by construction inside this row's scope. Never a second report at the binding, and none from inside the receiver (§2.2: the subject seats are established after every operand is in). The operand named is the one Numeric Literals §6's stand-down note carries; the type is spelled as it is spelled at the site (`Foo.Foo` across modules), and where it has no spelling in scope only the binding repair is offered. Where the claimant is the rung's own member (`p.add(q).multiply(s)`) the enclosing seat's ordinary stand-down report fires as today and no receiver fixit is offered — the ascription would repair nothing. At a nested receiver (`(p + (i + j)).gcd(s)`) the one report is the outer operation's own — the face descended into `i + j`, which ran at `BigInt`, and the addition then had no algebra for `Foo` and `BigInt` — carrying the same boundary fixit. A forwarding-form receiver whose parts disagree is not this row, whether or not a lift stood down inside it: the form's own report (Operators §11's, at the `if`; Pattern Matching §6.2's, at the arm body) names the branch that cannot enter the face and carries the boundary fixit under §2.2's licence only where that ascription compiles (`(if c then i + j else p: Foo)` does, `(if c then p.add(q) else b: Foo)` does not) — this row's branch case fires only where the form joins at the kept type |
| 17 | *(#934)* Dot call whose receiver is head-known — at the dot, or by the deadline fixpoint (§3.1) — with a head that has **no companion module** and for which §4.2's member clause does not fire: `Range` today (§3.4's Primitive row, §5) | "`Range` has no companion module, so `r.toSeq()` has nothing to dispatch to; write `Iterable.toSeq(r)`" — the call spelled as written, receiver and arguments (`r.fold(0, add)`), the receiver's outer grouping shed in the offer; the offer is the spelling of the honored subject-first member that answers the name where one does (`Iterable.toSeq(r)`; a member of a constraint declared in this module bare, §6), only where the member takes the subject alone and the call wrote no other argument; the sentence stops at its first clause where no member answers (`r.spin()`), and where the receiver has no source spelling to paste (Modules §7.6: a fixit the reader cannot paste is worse than none) the first clause names the member instead of quoting the call — "so `toSeq` after the dot has nothing to dispatch to" — and where an argument has no source spelling the call is quoted as receiver and member with no argument list (`r.fold`); no ellipsis is printed in either seat. The offer sheds only parentheses that are the receiver's own grouping, never an ascription's (`(r: Range).toSeq()` offers `Iterable.toSeq((r: Range))`, as row 16 spells every ascription fixit). The arguments elaborate without expectations, their own reports standing, and no second report about the call follows, however the call is nested: the call never becomes a row-field read, so row 8's rescue — written for a receiver that was *never* head-known — is not reached and its "type was unknown" clause is never said of a written type. The access form `r.toSeq`, no call, is not this row and still reaches row 8: a recorded defect, #936. The offer qualifies through the constraint's home alias and is unpasteable where a project module shadows that alias — a defect shared with rows 6 and 8, recorded as #937 |
...
let n: BigInt = (p + (i + j)).gcd(s)  -- ERROR — the nested i + j lifts on the way down; the outer
                                   --   addition then has no algebra for Foo and BigInt, and its own
                                   --   report is the one report — saying the face descended into i + j —
                                   --   carrying the boundary fixit
...
let n: BigInt = (p.add(q) + q).gcd(s)  -- ERROR (row 16) — the face reaches the inner call as an operand;
                                   ...
                                   --   outermost stood-down call — compare the nested row above, whose
                                   --   outer has no algebra and reports itself   [was OK]
