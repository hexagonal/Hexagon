...
- **One operation, its own elaboration.** *(#808.)* `receiver.name(args…)` is the one operation `name` the receiver's type owns, applied to `(receiver, args…)` and elaborated exactly as that operation's own spelling would be. For a companion export the subject seat is written, so the call runs at the receiver's type and the rewrite above is the whole story. For a constraint member the subject is polymorphic, so the call runs where its operands and the seat's written face establish it, the receiver being one operand among the others. At a **tower member** — a subject-first member of `Num`, `Signed`, `Frac`, `Pow`, or `Integral`, the closed list Numeric Literals §5.1 fixes — that is the open member call with the tower's contextual widening and expected-type lift: `count.multiply(price)` is `Float` multiplication for `count : Int` and `price : Float`, and `let r: BigInt = i.add(j)` is `BigInt` addition of two injected `Int`s, exactly as `count * price` and `let r: BigInt = i + j` are. **The receiver decides *what* is called; the operation decides *where it runs*.** Before this amendment §3.4's nominal row pre-selected the instance at the receiver's type, which read the dot as the *pinned* companion spelling `Int.add(i, b)` and refused a widening every other spelling of the same member performed (§16.3).
...
*(#513.)* **When the receiver's type is head-known at the dot** — already solved to a §3.2 trigger shape when the receiver's own elaboration returns — **resolution runs before the arguments are elaborated**: the goal is created and resolved per §3.4 on the spot, and the call then checks as a named call to the resolved member, its signature supplying each argument's expected type (Functions §4.3), pointwise. *(#808.)* Where the resolved member is a **tower member**, the named call it checks as is the member's *open* call over `(receiver, args…)`: the receiver, already elaborated, stands as the first operand, and the operation's home is established from its operands and the seat's face by Numeric Literals §5.1 — operand-driven where no expectation lands, the written face where one does — so the argument seats expect the home type, not the receiver's type. The dot inherits Functions §4.3's ordering pin exactly as the operator has it: receiver first, then the arguments on the schedule; a sibling sharing an undetermined variable may widen differently by position, by design. Which member is chosen never rides on any of this — the receiver's head selects the name as before. This ordering is what the argument seat requires — an expectation is useless after its argument has been checked — and it changes nothing about *which* member is chosen: §3.2's monotonicity argument is exactly the proof that resolving at creation and resolving at the trigger or deadline agree. A receiver still unsolved at the dot keeps the path above unchanged — arguments elaborated first, the goal pending on the trigger — and those arguments synthesize, Functions §4.3's known-callee condition being unmet. One resolution algorithm (§3.4), two entry moments; and only the arguments' expected types ride on which moment fires — dispatch never does (§11.3's rejection is untouched — see the note there). Evaluation order (§2.3) is inference-independent and unchanged.
...
| **Nominal type `T`** (record, union, opaque, built-in collection) | **Collision check first** (§6): if `name` is claimed by more than one of — a field of `T` *visible at this site*, an exported companion operation of `T`, an honored constraint member of `T` (§4.2) — hard error naming every claimant. Exactly a visible field → field call, as the structural case. Exactly a companion operation → **rewrite**: `CompanionOf(T).name(e, args…)`, thereafter an ordinary qualified call — arity, types, constraints, dictionaries all proceed as if the user had written it. Exactly an honored member → **member dispatch**: the call elaborates as the member's own call applied to `(e, args…)` — the elaboration the bare or qualified spelling of that member takes with `e` in the subject seat (Constraints §6.1). For a member outside the tower that is the `C<T>` instance selected by coherence at `T`. For a **tower member** *(#808)* it is the open call: the instance is established by the operands and the seat's written face (Numeric Literals §5.1), `e` being one operand, so a `Rat` receiver's `r.add(i)` injects the `Int`, and a `Float` operand or face carries an `Int` receiver up with it. The receiver names the operation; it does not pin the algebra. None → the neither-error (§9, row 4), with near-miss suggestions drawn from the field, companion, and member sets. |
...
*(#808.)* **A tower member call at a primitive instance emits what a person would write in JavaScript** for that operation at that type — in every spelling: operator, bare, qualified, pipe stage, or dot. Where JavaScript has an operator of the same meaning on the representation, that operator: `i.add(b)` emits `BigInt(i) + b`; `Num.add(i, j)` at `Int` emits `i + j`, as `i + j` always did. Where it has none, the direct call to the member seat, as before: `pow` at `Int` is a guarded function and stays `pow(i, j)`; `div` and `rem` are Hexagon-defined conventions and stay calls. The rule is about text, never meaning — the elaboration is one whichever spelling was written — and it is Constraints §6.1's operator lowering extended to the member spellings of the same operation. Byte-identity between the dot and the qualified spelling is therefore not the conformance check at these sites; value-identity is, and the operator lowering (Primitive Types §7) is the text every spelling must produce. Emission at nominal instances (`Rat`, user types) is untouched by this clause.
...
| 14 | *(#808)* Tower member dot call whose argument seat cannot widen — the member's written `Int` exponent seat at `i.pow(2n)`, `i.pow(0.5)` | Operators §6.3's mandatory fixit, branched on the exponent's type and naming the door (`BigInt.pow(value, exponent)` / `Float.pow(value, exponent)`), identical to `i ** 2n`'s; the span is the offending argument and the message names the seat and the conversion that could not apply — never the whole dot call, never a bare "type mismatch" (#783's second finding) |
| 15 | *(#808)* Owned tower member at a `Nat`/`Int` receiver with no written face (`n.subtract(m)`, `i.divide(j)` unannotated) | exactly the operator's refusal: `n - m`'s no-`Signed` diagnostic; `i / j`'s no-`Frac` diagnostic with the `Int.div`/`Int.mod` fixit (Operators §15) |
...
| **A dot call is the receiver's one operation, elaborated as its own spelling would be** (#808): companion export → written subject seat, runs at the receiver's type; tower member → the open call, home established by operands and face (Numeric Literals §5.1) — dot, bare, qualified, pipe, and operator spellings of a tower member are one elaboration; the companion-qualified spelling is a written face | §1, §2.2, §3.4, §7, §16.3 |
| `Nat`/`Int` receivers own every tower member, honored or not — the ownership clause; owned spellings reserved in `Nat.hex`/`Int.hex`; the rungs are closed (Numeric Literals §5.1), so the owned set is finite | §3.4, §4.2, §8.4 |
...
| A tower member call at a primitive instance emits what a person would write in JavaScript, in every spelling — the operator where one has the same meaning, the seat call otherwise; value-identity, not byte-identity with the qualified spelling, is the check | §8.1 |
...
let r: BigInt = i |> Num.add(j)    -- OK — the same
...
- **What changed:** §3.4's nominal and primitive rows no longer pre-select the `C<T>` instance at the receiver for a tower member; the call is the member's open call, its home established by operands and face (Numeric Literals §5.1, which now fixes the tower as a closed list and extends the expected-type lift to every spelling of a tower member call). §1 states the one-operation sentence; §2.2 the entry moment and the inherited ordering pin; §4.2 gains the ownership clause; §6.1 the member/door principle; §8.1 the primitive-instance emission rule; §9 rows 14–15. #783 (the door's argument seat) is folded in.
- **The defect, measured** (`28d3761`; `i, j : Int`, `b : BigInt`): `i + b` and `Num.add(i, b)` widened; `i.add(b)` and `b.add(i)` refused "type mismatch". `let r: BigInt = i + j` ran at `BigInt`; the same binding spelled `Num.add(i, j)`, `i |> Num.add(j)`, or `i.add(j)` ran at `Int` and injected the finished sum — the silent overflow Numeric Literals §5.1 exists to prevent, reachable through three spellings the lift did not govern.
- **Origin:** the nominal row's "with the `C<T>` instance selected by coherence" read the dot as the *pinned* companion spelling `Int.add(i, b)`, a written face; but the dot writes no face — it names an operation, and a member's operation runs where its operands establish it. The row was right for every member outside the tower, where the receiver's instance is the only one in question, and wrong for exactly the members whose subject ranges over a widening chain.
- **The ruling** (#808): numeric widening is the canonical interpretation of a tower member call, whatever its spelling; the receiver decides *what*, the operation decides *where it runs*. The companion-qualified spelling is excluded as a written face, on Modules §5.3's migration principle. The tower's rungs are the language's own, a closed list — a type joins by honoring them, never by adding one — which is what keeps the ownership clause finite and the tower's vocabulary conflict-free at the source types. Emission was ruled as JavaScript text only: where a person would write an operator, the compiler writes an operator (§8.1).
- **Rejected alternative (do not relitigate):** widening the dot's *argument* seat alone, leaving the receiver pinned (#783's first reading). It repairs `b.add(i)` and leaves `i.add(b)`, `i.compare(b)`, and the lift untouched — three spellings of one member still disagreeing. Also rejected: an open tower, where any user constraint bounded on `Num` joins the rungs — a user member spelled `add` would then make `i.add(j)` a two-claimant refusal for every module in the program.
