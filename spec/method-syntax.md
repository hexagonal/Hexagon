...
- `JsValue` and `Array(a)` — compiler-provided boundary types tied to their fixed prelude companions by rule *(#511; §4.1's row)*: `v.kind()`, `v.toInt()`, `xs.length()` (Part 2 §13.1 — companion dispatch today; each further `Array` companion operation joins as it ships, Part 2 §9.1); their values expose no Hexagon fields, so no collision surface; `Nullable`, `JsMap`, and `JsSet` are not settled here;
