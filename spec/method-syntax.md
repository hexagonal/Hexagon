...
— the receiver's tyvar, the member name, the argument types (each elaborated normally, in source order, after the receiver), and a fresh result tyvar that stands for the whole expression's type. **The goal is owned by the inference region of its receiver tyvar** — the level, in the level-based-generalisation sense, at which that variable lives (§3.1).
