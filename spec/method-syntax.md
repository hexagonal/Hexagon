...
honor (`Num`, `Signed`, `Frac`, `Pow`, `Integral`). Thus Nat gains Signed's
`subtract`/`negate` and Frac's `divide`; Int and BigInt gain Frac's `divide`.
