...
- `JsValue` and `Array(a)` — compiler-provided boundary types tied to their fixed prelude companions by rule *(#511; §4.1's row)*: `v.kind()`, `v.toInt()`, `xs.length()` (Part 2 §13.1 — companion dispatch today; each further companion operation joins as it ships, Part 2 §9.1); their values expose no Hexagon fields, so no collision surface. (`Nullable`, `JsMap`, and `JsSet` are not settled here.);
...
| `JsValue` is an eligible receiver through its fixed prelude companion (#511) — these two compiler-provided boundary types tie to companions by rule, the primitive pattern; `Array(a)`'s clause records the eligibility Part 2 §13.1 already reads; `Nullable`/`JsMap`/`JsSet` not settled | §3.4, §4.1, §5 |
