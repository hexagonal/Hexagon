...
| `JsValue` *(#511)* — and `Array(a)`, whose eligibility Part 2 §13.1 already records, as its companion ships | The fixed prelude companion of the same name. These boundary types are compiler-provided with no declaration site, so the tie is by rule — the primitive row's pattern: `v.kind()` and `v.toInt()` are `JsValue.kind(v)` and `JsValue.toInt(v)`. No fields exist on such values, so the collision surface is export-vs-member only. |
...
- foreign boundary types with fixed prelude companions *(#511)*: `JsValue` (`v.kind()`, `v.toInt()`), and `Array(a)` per Part 2 §13.1 as its companion ships — their values expose no Hexagon fields, so no collision surface;
...
| `JsValue` is an eligible receiver through its fixed prelude companion (#511) — compiler-provided boundary types tie to companions by rule, the primitive pattern; `Array(a)`'s row operative as its companion ships (Part 2 §13.1) | §3.4, §4.1, §5 |
