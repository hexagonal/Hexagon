...
| 16 | *(#808, #821)* Tower-spelled dot call under a face honoring the spelling's rung, whose receiver is a tower expression that cannot enter the forwarded face — `let n: BigInt = p.add(q).gcd(s)`, `Foo` honoring `Num`, its companion exporting `gcd(Foo, Foo): BigInt`; an honored member of a user constraint or a function-typed field bearing the spelling takes the same row | the **receiver refusal** — one report, three facts, at the receiver's span: "`.gcd` is an `Integral` spelling, so the `BigInt` face reached its receiver `p.add(q)`; `p` is a `Foo` and cannot enter `BigInt`, so the addition ran at `Foo`. To keep the receiver at `Foo`, ascribe it — `(p.add(q): Foo).gcd(s)` — or bind it first." Never a second report at the binding; the operand named is the one Numeric Literals §6's stand-down note carries; where the claimant the boundary reaches is a companion operation the fixit may name it (`Foo.gcd`) |
...
let n: BigInt = p.add(q).gcd(s)       -- ERROR (row 16) — BigInt honors Integral, so BigInt is forwarded
                                   --   and is binding; Foo cannot enter it, p.add(q) runs at Foo, and
                                   --   the receiver seat refuses: one report, three facts   [was OK]
let n: BigInt = (p.add(q): Foo).gcd(s) -- OK — the ascription is the boundary; Foo.gcd answers
...
let n: BigInt = (p + (i + j)).gcd(s)  -- ERROR (row 16) — the nested i + j lifts on the way down;
                                   --   the verdict is the same whichever operand declines
let n: BigInt = (if c then i + j else p).gcd(s)
                                   -- ERROR (row 16) — a forwarding form is one receiver; one report
let n: BigInt = (1 + 2).gcd(s)        -- ERROR — literals reach anything: the receiver lifts to BigInt
                                   ...
                                   --   and neither is row 16: the receiver entered the face
...
let n: BigInt = (z1.add(z2): Baz).gcd(z3) -- OK — (add(z1, z2).gcd)(z3), the field call
