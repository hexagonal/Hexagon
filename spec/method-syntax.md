...
| **Primitive** (`Int`, `Float`, `String`, `Unit`, and compiler-known nominals like `Range`, `Seq`) *(`Bool` moved to the nominal row — reclassified as a prelude union 2026-07-29, #147; see the note under §4.1)* | As the nominal case, with `CompanionOf` the fixed prelude companion (§4.3). No fields exist, so no collision surface. |
