...
let n: BigInt = (if c then p.add(q) else p).gcd(s)
                                   ...
                                   --   the fixit ascribing the whole receiver   [was OK: the branches
                                   --   joined at Foo and Foo.gcd answered — the retracted behaviour]
let n: BigInt = (if c then p.add(q) else p: Foo).gcd(s)
                                   -- OK — the ascription is the boundary
let n: BigInt = (if c then (p.add(q): Foo) else p).gcd(s)
                                   -- OK — a boundary inside the branch composes; the smaller repair
