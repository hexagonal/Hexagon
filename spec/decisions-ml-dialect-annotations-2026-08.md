...
Functions §4's preamble already carries the principle at annotation scope — annotations "never change what inference could derive except by restricting it" — and this section records it as doctrine at type-system scope. The claim is about derivation power, not legality: the corpus does require annotations at boundaries — an export's complete signature (Modules §4.1.1), a match function's parameter (Functions §4.3) — and there the requirement is a boundary or determinacy rule, and the type the annotation supplies is one inference works with wherever the same information arrives by any other route. What no written claim ever does is inject an assumption inference could not have reconstructed — above all, a *scheme* where inference holds a monotype, which is Haskell's signature rule for recursive groups and annotated polymorphic recursion with it. Such features are rejected in §9.11: each makes typability depend on which annotations are present, and one admitted case is the thin edge of the wedge toward a second, annotation-driven type system standing beside the inferred one. (Functions §4.2's rank-2 pathway — annotation-gated, deferred, never designed — is outside this ruling: the deferral stands as recorded there and in Functions §11, and any future rank-2 design must answer to this section before it opens the gate.) Roc draws the same line: decidable principal inference, under which no annotation can make a type more flexible than deleting it would. The principle is generative, not merely defensive: complete inference is the platform whole-program features stand on, and the effects system is the standing exemplar — a definition's own effect colour is inferred from its body, never written in its header (Functions §4.1; Effects §2), a design coherent only because the checker reconstructs every type unaided. Features of that kind are meant to grow, and each one compounds the value of keeping the guarantee absolute. Functions §7.4's monomorphic knot leans on the principle directly, and dictionary-sharing §6.2 states its emission-side face: a recursive occurrence sees a monotype and cannot demand new instantiations.
...
15. *(#368.)* The identity suffix at the knot (Functions §7.4), pinned on **emitted JS plus runtime execution**, never emitted JS alone — the arc began as a silent miscompile: self-recursion (single- and multi-constraint, the suffix order agreeing with the head's), unannotated mutual cross-calls, a value-position recursive occurrence, and the asymmetric knot (a member whose suffix is empty calling a sibling whose suffix is not — the callee-only variable resolving by Functions §8's ordinary rules, not an internal error).
...
- **§2.4** records the derivability principle (anchored on Functions §4's preamble sentence; rank-2's recorded deferral explicitly left standing); **§9.11** the annotation-enabled-typability rejection; **§8** items 15–16 the conformance obligations.
- **Functions §7.4** gains the two knot corollaries (identity evidence suffix over the shared variables; a declared head admits no instantiation inside the knot, wrapper-only at an export boundary); **Functions §10**'s polymorphic-recursion row pins the SCC hint and both message fences.
...
- **Modules §4.1.1**: its complete-signature advice ("write `<a: Eq>`") needs a carve-out for members of a recursive knot, where the advised spelling is refused and the wrapper is the legal one (Functions §7.4).

- **Modules §4.1.1**: one sentence — a hole does not satisfy the completeness requirement (this document §5.4).
