...
Draw: the skolem-checking technique for rigid variables (the literature anchor Functions §4.1 already names once); the hole-elaboration model (wildcards become metavariables); hole reporting mapped to hover rather than warnings (§7). Decline: named wildcards (`_a` — §9.4); `ScopedTypeVariables`-style scoping accretion; higher-rank machinery (rejected — §9.12). Diagnostic vocabulary is unchanged: Hexagon says **declared type variable**, never "skolem" or "rigid type variable," in user-facing text.
...
15. *(#368.)* The identity suffix at the knot (Functions §7.4), pinned on **emitted JS plus runtime execution**, never emitted JS alone — the arc began as a silent miscompile: self-recursion (single- and multi-constraint, the suffix read in the callee's own order), unannotated mutual cross-calls, a value-position recursive occurrence, and the asymmetric knot both ways (a member whose suffix is empty calling a sibling whose suffix is not). For the asymmetric knot the normative outcomes are pinned outcome-by-outcome: where the callee-only variable's constraints admit defaulting — and the closed defaultable set is every constraint whose `Int` instance the prelude supplies, `Eq` included (Numeric Literals §4) — the module compiles and **runs**, the cross-call passing the defaulted ground dictionary; where they do not (a variable bound by a user constraint, never defaultable), the module is **refused** on the ambiguity path (`decisions-ml-dialect-generalization-2026-08.md` §4.1's Step 2 and §13; Numeric Literals §4/§6), the refusal's wording the implementation's to supply within that path's family — the cited bullets are literal-centric and this shape has no literal. The two shapes #368 records as nonconforming — an internal compiler error on a `Num`-bound variant, a clean compile with an under-applied cross-call on an `Eq`-bound one, both in the defaultable clause — lived in different passes (checker vs. emitter), so a fix to one did not discharge the other; the refusal clause is exercised by the user-constraint shape.
16. *(#368.)* Both §10 message fences (Functions §10's polymorphic-recursion row): the rigid-vs-concrete message surfaces no type the body did not demand; the rigid-vs-rigid message carries the SCC hint with per-member qualification and never the same-name advice. *(Amended under #700: a side whose variable is declared on a `fun` block head qualifies by the head, located by its span — the head binds no name; member sides qualify as before.)* The concrete-instantiation refusal inside a knot (a sibling calling a headed member at a concrete type) keeps the rigid-vs-concrete message, correct in kind.
...
- **Functions §4.2** and **§11**: the "annotation-gated pathway" sentences are rewritten from deferral to rejection, each citing §9.12; the Not-in-scope line here drops its rank-2 clause; §3.2's higher-rank decline points at §9.12.
- **Type-System Overview**: pillar 3's "if they arrive" sentence rewritten to the retirement (citing §2.4/§9.12); the owed-list's "rank-2 pathway" item dropped.
- **Functions §7.4** gains the two knot corollaries (identity evidence suffix over the shared variables; a declared head admits no instantiation inside the knot — with, at the time, wrapper-only export where two or more functions of one knot export, a rule #700's `fun` block later dissolved); **Functions §10**'s polymorphic-recursion row pins the SCC hint and both message fences.
...
- **Modules §4.1.1**: (i) one sentence — a hole does not satisfy the completeness requirement (this document §5.4). *(Item (ii), the knot carve-out for the complete-signature advice, was discharged by #700's ruling in reshaped form: the advice names the `fun` block-head spelling — Modules §4.1.1 carries it.)*
