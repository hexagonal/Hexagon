...
- ***(#700.)* A `var` may not have a function type.** Vars are data accumulators, not functions. The rule is **type-level**, not a spelling ban (a lambda-RHS check would be a leaky speed bump — `var f = identity(x => x)` walks past it): it refuses the `var` wherever its monotype **resolves to an arrow** — at the declaration, whether the arrow is annotated or inferred from the initializer, or at the pinning use that settles an unsolved variable to a function type (first-use pinning, above). The ban is on the `var`'s own top-level type only: **functions inside data are data** — `var handlers = [onOpen, onClose]` (a `Vector` of functions) is legal, as is a record field holding one. The rewrite models changing behavior as data: declare a union of the states, `var mode = Fast`, and `match mode` at the use site (§9.3's row). Non-foreclosing — refused in v1, reopenable on field evidence. One consequence lands in Functions §4.3: with no function-typed `var`, the annotated-`var`/`:=` supplying seat is gone from its list (the assignment boundary's numeric channel is Numeric Literals §5.1's own, untouched).
