...
**Status:** Decided (July 2026); **§5 revised July 2026** — `let`-pattern binders reclassified as sequential; correction record and rejected alternative in §5.4. With a **hanging-questions** section (§10); nothing there blocks implementation of §1–§9.
...
- **Everything is an expression, except bindings.** Hexagon adopts the F# model: there is no statement category in the semantics. What reads as a "statement" (`print(x)`, `x := 5`) is an ordinary expression of type `Unit`. The only non-expressions in the language are **bindings** — `let`, `var`, `fun` in blocks, and the module-level declarations (`record`, `union`, `type`, `constraint`, `implement`, `exception`). A binding introduces a name; it has no value and no type.
...
| `record` / `union` / `type` / `constraint` / `implement` / `exception` | module top level only (their own specs) |
...
- The rule is a **hard error**. Hexagon has no warning tier; the F# precedent (a warning) is deliberately upgraded, consistent with exhaustiveness and reachability being hard errors elsewhere.
...
- An `if` **without** `else` (if the grammar permits one; conditionals are the operators/expressions spec's final word) would necessarily be `Unit`-typed with the then-block unifying against `Unit`. Flagged to that spec; not decided here.
...
*(Revised July 2026: `let`-pattern binders reclassified from head to sequential. Correction record, rationale, and the rejected alternative: §5.4.)*
...
- The sequential-binder ban is what makes `:=` resolution trivial: within any function, a `var` name is globally unique among sequential binders, so an assignment target resolves to exactly one candidate, always. The §5.4 reclassification only strengthens this: moving `let`-pattern names *into* the sequential class adds rejected collisions, never a second live candidate.
- **Binder class is positional, not pattern-determined.** The full pattern grammar (Pattern Matching spec) is legal at both sequential and head positions; the pattern contributes the *names*, the position contributes the *class*. Still exactly two classes; no pattern form may ever create a third. (Supersedes this section's earlier "pattern forms inherit head-binder status wholesale" — see §5.4.)
- **Module aliases can never be shadowed by any binder**, head or sequential — not by a new restriction, but by the case rule: module aliases are mandatorily uppercase (Modules §3.3) and an uppercase name in any pattern position is a constructor reference, never a binder (Pattern Matching §2.1). `let f(Json) = ...` therefore binds nothing; it resolves `Json` as a constructor and fails as unknown. The near-miss diagnostic for that failure is owed to the Modules diagnostics table (edit note §9.2).
...
### 5.4 Correction record (July 2026): `let`-pattern binders are sequential

**What was wrong.** This section's original enumeration listed "`let`-destructuring component names (Products §2.4)" among the head binders — contradicting its own definition three lines above: `let (x, y) = pair` governs no delimited region; its binders scope to the tail of the enclosing block, exactly like two consecutive `let`s. The mistake was filing by syntax ("is it a pattern?") rather than by scope shape ("where does it scope?"), and it propagated into the Pattern Matching spec ("all pattern binders are head binders"). Under the erroneous classification, `let x = 10; let (x, y) = getPair(); use(x)` compiled — from the destructuring line onward, `x` had silently changed meaning: precisely the hazard the sequential ban exists to prevent. (Defect surfaced in external review, July 2026.)

**Why the correction is forced, not preferred:**

1. **Refactoring invariance.** Replacing `let x = ...` with `let (x, y) = ...` produces the identical scope for `x`; it must not change whether shadowing is legal. This is the same argument that motivated the sequential ban, turned back on the rule itself.
2. **Punning is the aggravating case.** With record-pattern punning (Pattern Matching §2.4), `let {name, total} = order` binds a name the programmer *never wrote* — inherited from a field of a type declared elsewhere. Every genuine head binder has an explicitly-typed name at the binding site; only `let` combined an implicit name with an unbounded scope. (§5 predates punning, which is how the combination slipped through.)
3. The proper-subterm criterion (§5) classifies every position correctly with no exceptions, including module level (destructuring names become module-layer sequential binders, so Modules §5.4 occlusion applies to them unchanged).

**Rejected alternative — keep `let`-pattern names head-class for state-threading.** The idiom this correction bans:

```
let (state, a) = step1(state0)
let (state, b) = step2(state)      -- now an error
```

OCaml/F# bread-and-butter, and *more* common with tuples than the scalar rebinding Hexagon already banned, because tuple returns are how state is threaded without mutation. The usual scalar escape hatch is also unavailable: `var` forbids destructuring (§6.1). Rejected anyway: the alternative preserves the silent-rebinding hazard of the motivating example, which is worse; the scalar analogue was already an error, so the doctrine already demands fresh names (`state1`, `state2`) or restructuring into a `for`/fold; and the cost is honest friction where the hazard is real, not new friction. Whether state-threading pressure eventually justifies `var` destructuring is a field-evidence question, recorded in §10.6 — **do not relitigate the classification itself.**
...
### 7.4 Constraint on the forthcoming loops spec (binding)

A `for` / `for..in` loop is coming (own spec). This spec imposes one requirement, recorded now because §6.2 makes it existential: **loop bodies must be blocks, not lambdas** — if a loop body were a lambda, §6.2 would forbid the loop from touching any `var`, and the accumulate-in-a-loop idiom that motivates `var` (the Roc motivation) would be impossible. The loop variable of `for..in` is a **head binder** (§5). Whatever else that spec decides, these two points are fixed.
...
```
-- (a) The accumulator shape (pending loops; recursion for now)
fun sumTo(n) =
  var total = 0
...
*Added July 2026, from the §5.4 correction (apply on next touch; until then this doc governs):*

5. **Pattern Matching** — companions line "head-binder status of *all* pattern binders" → "binder class is determined by position (Statements §5), not by pattern-ness." §6.3 (`let` patterns) gains: "names bound here are **sequential binders** (Statements §5.4) — they may not reuse any name in scope; the arm/lambda positions bind head binders as before." §14.4's discharge note re-worded: the no-third-class flag *is* discharged, but via the positional rule, not via blanket head-binder status. Decisions-log row "All pattern binders are head binders" → "Binder class is positional; `let`-pattern binders sequential (corrected, Statements §5.4)."
...
7. **Modules §10** diagnostics table — new near-miss row: uppercase name in a binder-pattern position resolving to no constructor but matching an in-scope module alias → "`Json` is a module alias; binders are lowercase — did you mean `json`?" (resolution-time hint, same family as "`Shape` is a type, not a module," Modules §5.1).
...
| `var` outside a function body | "`var` is only allowed inside a function" (§6.1) |
...
| Lambda references an outer `var` (read or `:=`) | "`shift` is a `var` and cannot be used inside a lambda; copy it to a `let` first" (§6.2) |
...
| `t.itemN := e` on a tuple | "tuples are immutable" (§6.3) |
...
2. **Head Binder Shadowing vs module/prelude names** — **discharged** by Modules §5.4 (module-level bindings may occlude the prelude; function-local binders occlude nothing). Retained as a stub so cross-references to §10.2 resolve; the interim function-local-only enforcement rule is retired.
3. **`else`-less `if`** (§4): if the conditional grammar permits it, its typing is forced (`Unit`, then-block unifies with `Unit`); whether it exists is the expressions/operators spec's call.
...
| Lambda boundary: no read, no write of an outer `var` from any lambda; copy-to-`let` idiom; `fun` therefore never touches vars | §6.2 |
...
| Loops spec constraint: loop bodies are blocks, not lambdas; loop variable is a head binder | §7.4 |
