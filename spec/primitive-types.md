...
`stdlib/Float.hex` also declares `FloatRangeError` — the range guard thrown by `Rat.toFloat` (`rat.md` §6): that door's result must be finite, and nonzero when the input is nonzero. The declaration sits here, at the target type, so that any future door from the exact world *can* share it; which doors carry the guard is each door's own ruling.

**`Float.pow(value: Float, exponent: Float): Float`** is the analytic power — `exp(y·ln x)`, total, honestly IEEE with every `NaN` edge, `Float.pow(2.0, 0.5)` the nearest double to `√2`. It is the generalising export over `Pow<Float>`'s member (Operators §6.3.1; Modules §5.3's generalisation law): the operator `**` takes the member's `Int` exponent, the qualified spelling and the dot call take this door, and the member is the door restricted to integer exponents. A fractional exponent at `**` draws the mandatory fixit pointing here.
