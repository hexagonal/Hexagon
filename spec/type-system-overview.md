...
3. **No `forall` binder.** Non-uppercase-start = type variable, uppercase-start = type name; implicit quantification falls out of the start-class rule (Primitive Types §1; Lexer §3). Rank-1 is the whole language: the once-recorded rank-2 annotation-gated pathway is retired, rejected rather than deferred (`decisions-ml-dialect-annotations-2026-08.md` §2.4/§9.12).
...
5. **Type-system internals** — Algorithm J details, levels, row unification.
