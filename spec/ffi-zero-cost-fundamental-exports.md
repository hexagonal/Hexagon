...
2. **Candidates per specializing variable:**
   ...
   Lawfulness is the ordinary Constraints §4/§5 judgment over the fundamental instances — e.g. `Num → {Nat, Int, Float, BigInt}` and `Signed → {Int, Float, BigInt}`; `Show`/`Eq` → whichever members of the closed set carry those instances per the prelude. *(Provenance note, #344: "compiler/runtime-provided" is per-companion history now — the pre-registered constraints' instances this judgment consults at the primitives are prelude source in their companions; `Iterable<String>` alone keeps the old status, and the binder ban (Collections Part 2 §7.2) keeps `Iterable` out of every `C(vi)`. The judgment and the sets are unchanged.)* No new instance judgment is introduced.
