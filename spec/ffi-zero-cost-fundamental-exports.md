...
2. **Candidates per specializing variable:**
   ...
   Lawfulness is the ordinary Constraints §4/§5 judgment over the fundamental instances — e.g. `Num → {Nat, Int, Float, BigInt}` and `Signed → {Int, Float, BigInt}`; `Show`/`Eq` → whichever members of the closed set carry those instances per the prelude. *(Provenance note, #344: "compiler/runtime-provided" is per-companion history now — `BigInt`'s instances are prelude source in `stdlib/BigInt.hex`; the judgment and the sets are unchanged.)* No new instance judgment is introduced.
