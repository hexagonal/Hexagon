...
2. **Candidates per specializing variable:**
   ...
   Lawfulness is the ordinary Constraints §4/§5 judgment over the fundamental instances — e.g. `Num → {Nat, Int, Float, BigInt}` and `Signed → {Int, Float, BigInt}`; `Show`/`Eq` → whichever members of the closed set carry those instances per the prelude. *(Provenance note, #344: "compiler/runtime-provided" is per-companion history now — `BigInt`'s, `Int`'s, and `Nat`'s instances are prelude source in their companions; the judgment and the sets are unchanged.)* No new instance judgment is introduced.
...
- **`Bool`** is a prelude union: its own four instances derive in its declaring module (Constraints §5.3), and for every other constraint it is an ordinary instance head. A lawful `honor K<Bool>` — for a user constraint `K`, necessarily in `K`'s own module, `Bool`'s declaring module being prelude source no user file can add to (Constraints §5.3) — puts `Bool` in `candidates(vi)` for `K`-constrained variables exactly as an instance at any other fundamental would. `Bool` is not distinguished here: each of the six non-`Unit` fundamentals has a home module no user file can add to, so a user constraint's fundamental instances are the ones written in the constraint's own module.
...
| 19 | **The candidates judgment at `Unit` and `Bool`, stated:** `Unit`'s lawful instances are exactly the automatic structural ones (today `Eq`/`Ord`/`Show`/`Hash`) and no `honor` can name it — never a candidate for a user constraint; `Bool` derives its own four in its declaring module and is otherwise an ordinary instance head — like every non-`Unit` fundamental, a user constraint reaches it from the constraint's own module | §3.2 |
