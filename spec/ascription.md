...
- There is **no coercion of the ascription's own**. An ascription admits exactly what the equivalent `let` annotation admits and nothing besides — Numeric Literals §1/§5.1's contextual numeric widening included, which reaches every annotated position identically: `(x : Float)` with `x : Int` compiles here exactly because `let y: Float = x` does, through `Signed`'s injection at an independently established target, never by the ascription itself. The expected-type lift arrives the same way: `(count + count : Rat)` runs its addition at `Rat` exactly as the annotated `let` does — the ascribed face is the arithmetic's home (Numeric Literals §5.1), by the seat's supplying, not by any power of the parentheses. What no annotation admits is a type mismatch, not a conversion — conversions are named functions, as everywhere: `("s" : Float)` is an error.
...
- **An ascription is a supplying seat for expected-type propagation** (Functions §4.3, #513): the written face reaches a lambda inside it before the body is inferred, so `(match … : (Int) -> String)` hands the match function its parameter type ahead of arm checking. This is ordering, plus the one channel ordering feeds: the ascribed face also selects a lifted operation's algebra (the widening bullet above; Numeric Literals §5.1 owns the rule). The unifications performed are the ones this section already requires, and everything else here (rigidity, accumulation, holes) is untouched.
...
let inc = (x => x + 1 : a -> a) -- fine: Num is inferred for a; inc : <a: Num> a -> a
...
One corner is newly reachable and ruled here: a declared variable that **occurs nowhere in the declaration's type** — `let f() = ignore((42 : a))` — accumulates a constraint that would quantify with the function, yet no call site could ever determine its evidence. Ruled: an error at the declaration, in §4.1's declared-type-variable family — the checker must surface it, never quietly default a declared variable to escape it. The same corner is reachable through a binder list (`let f<a: Num>(x: Int): Int = x`), and the rule is one rule across both spellings (Functions §10's unmentioned-variable row, #712).
...
- **Declared-variable errors:** rejections arising from a rigid variable report in §4.1's declared-type-variable family (the diagnostics say **declared type variable**, never *skolem*), at the position the corpus already assigns — the structure-mismatch case at the ascription; the defaulting-refusal case at the binding, Functions §10's *forced-to-a-concrete-type* row (§3.1 here) — **worded for this spelling**: the report says what the claim got wrong and names this form's rewrites — "`a` is a declared type variable, but `42` can be only a `Num` type; ascribe the concrete type you mean — `(42 : Int)` — or remove the ascription to let the literal default to `Int`" — never the binder spelling's "the body requires `Int`": the Rewrite Rule requires this form's own edits, and Numeric Literals §6's literal-voice rule — report in the literal's terms — is the authority for this report's voice; the declined-seat case for constraints defaulting cannot discharge at a non-function value binding (Functions §8); the orphaned-variable case at the declaration (§3.1).
...
An ascription is **independent evidence** in Method Syntax §3.1's sense: `(42: Nat)` is head-known `Nat` at the dot, so `(42: Nat).show()` resolves per Method Syntax §3.4's Primitive row — since #304/#335 that row holds `Nat`, and the operation set reaches the instances' members (`Show<Nat>` — wired when this was written, a source `honor` block in `stdlib/Nat.hex` since #344; the row reads identically either way). This spec only guarantees the receiver spelling exists and pins.
...
1. **Unifiable ascription variables** (OCaml: `(3 : 'a)` succeeds, `'a` silently `int`). Rejected, on consistency: every other Hexagon annotation variable is rigid, and one spelling must not carry two meanings by position. The comparison is sharpened by accumulation: OCaml's `(fun x -> x + 1 : 'a -> 'a)` *looks* like a polymorphism claim while the compiler silently monomorphizes `'a` — the classic footgun. Hexagon's `(x => x + 1 : a -> a)` is a checked structure claim that elaborates genuinely polymorphic (`<a: Num> a -> a`, §3.1): the demand becomes a constraint, never a quiet concretization. The unifiable reading's lone power — linking two positions without claiming generality — is discovered by inference anyway, and the doctrine record (closure doc §2.3, §9.6) rejects a linking-only form permanently.
2. **General postfix ascription** (`e : T` anywhere). **Reserved, not rejected** — v1 is parens-only (§2.4); revisit on demonstrated safe contexts. The `match` head (`match d: Direction`) was the first candidate context, proposed as the door for Pattern Matching §2.2's constructor resolution and declined (#763): that door reads the scrutinee's already-determined type and needs no new seat, and the parenthesised form serves where a type must be supplied.
...
8. **Tests:** the §3.1 four-example block verbatim, including the scheme assertion `inc : <a: Num> a -> a` and calls at two numeric types; the §2.2 eats-right pair; the §2.3 tuple/lambda pair; `(42: Nat)` end-to-end (type, emission erasure, `.d.ts` absence — under the export's own signature where the specimen is exported: an ascription is a body position and does not satisfy Modules §4.1.1's mandatory annotation); a tuple with mixed ascribed/plain components; a hole ascription `(e : Vector(_))` (compile + hover fill), a constrained-hole ascription exercising the floor, and the hole-ascribed literal `let n = (42 : _)` defaulting to `Int`; the four §9.3 error paths; the retired parse error's replacement behavior; mutation coverage on the rigid-scope join (a test that fails if ascription variables are fresh-per-ascription instead of declaration-scoped).
