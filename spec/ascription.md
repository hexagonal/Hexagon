...
Numeric Literals §4's rule is untouched, and its existing arms already cover every ascribed literal. Ascribed to a concrete type — `(42: Nat)` — unification settled the literal's variable at the ascription, so defaulting has nothing to consult. Ascribed to a hole — `let n = (42 : _)` — the hole is an ordinary inference variable and the literal defaults through it exactly as `let n: _ = 42` does (closure doc §4.1): `n : Int`. Ascribed to a declared variable, the closure doc's proof pair answers (§3.1 here): at a value binding defaulting reaches the rigid variable and rigidity refuses the narrowing; at a function declaration the variable generalizes with its accumulated constraints (Functions §4.1's `numeric`), and defaulting never narrowed anything. No new ordering rule anywhere — one defaulting rule, met by *written is claimed, unwritten is inferred*.
...
- **Functions §8 items 3 and 7, and §10's two annotated-binding rows**: the arms that key on an *annotated* binding's declared variable extend to variables an RHS ascription declared — declaredness, not the binder's own annotation, is the key (§3.1 here).
...
3. **Checker:** reuse the annotation path — elaborate the written type in the enclosing declaration's annotation-variable scope (introducing unseen names rigid, per §3.1), unify with the expression's type; accumulation then proceeds as for any rigid variable, and what the accumulated constraints become is Functions §8's existing business at the declaration. Three error paths must exist and be tested: the defaulting-refusal case (`let n = (42 : a)`, the closure doc §6.2 proof pair's second half at expression granularity — hard error at the binding, Functions §10's forced-to-a-concrete-type row); the declined-seat case for a non-defaultable accumulated constraint at a non-function value binding (Functions §8's arm must recognize ascription-declared variables as declared); and the orphaned-variable case (`let f() = ignore((42 : a))`, error at the declaration; must not default). Hole seeding uses the existing floor mechanism unchanged. No new unification machinery.
4. **Emitter:** erase (§4).
...
8. **Tests:** the §3.1 four-example block verbatim, including the scheme assertion `inc : Num a => a -> a` and calls at two numeric types; the §2.2 eats-right pair; the §2.3 tuple/lambda pair; `(42: Nat)` end-to-end (type, emission erasure, `.d.ts` absence); a tuple with mixed ascribed/plain components; a hole ascription `(e : Vector(_))` (compile + hover fill), a constrained-hole ascription exercising the floor, and the hole-ascribed literal `let n = (42 : _)` defaulting to `Int`; the three §9.3 error paths; the retired parse error's replacement behavior; mutation coverage on the rigid-scope join (a test that fails if ascription variables are fresh-per-ascription instead of declaration-scoped).
