...
**An argument's spine** *(#1096)*. A call sees through exactly the forms its expected types reach inward through. An argument's **spine** is the argument and every expression it reaches through grouping, the other forwarding forms' value paths — both branches of `if`, every arm body of `match` and `try`, a block's final expression, a `try`'s body block — the components of the literal forms, a `with` update's overrides included, and the arguments of a constructor application the constructor-application bullet above qualifies. Anything else ends it: a call's result — every other call, an operator included, elaborating its own arguments on its own schedule — an ascription, a lambda's body, and the parts of a form that are not its value: a condition, a scrutinee, a pattern, a guard, a block's earlier items. A call reads each argument's spine as part of its argument list:

- **Its lambda literals are the deferred class, at any depth.** Each waits for the call's second pass, in source order across the whole list. In the first, its written parameter and result types land (ruling A2), and it stands in the form around it as a function type built from what it writes and fresh variables, its annotations read once; the second pass checks its body against that type. So `g6(m, Some((v) => v + n))` at `g6<t>(x: t, p: Option((t) -> t))` is refused as `apply2(m, (v) => v + n)` is, and so is the callback as a tuple component, a vector element, a record field, or an `if` branch beside `ident`; while `apply2(m, if c then (v) => v + n else incInt)` (`incInt : (Int) -> Int`) meets at `Int`, as `apply2(m, incInt)` does, the named function being a value of the first pass.
- **Its values join the call's sibling groups.** Before any argument is checked, each argument's spine is read against the callee's parameter type as the call finds it — the type the sibling groups are keyed on — through the literals' written shapes: a tuple literal against a tuple type of its arity, component by component; a record literal against a structural record type, field by field; a vector literal against `Vector(t)`, every element against `t`; a constructor application against a type headed by the constructor's own, its instantiated result unified with that type there — the unification that bullet performs, performed at the call's start, where it cannot fail, the constructor's side being fresh — and its arguments against its parameter types; and each value path of a forwarding form against the type the form was read against. A `with` update's overrides are read against its head's field types, which no signature writes, so they join no group. A non-lambda expression read against a bare type variable of the callee's parameters is a **sibling** in that variable's group, exactly as a whole argument there is, and the literal or constructor application holding it has the variable itself as that part's type; any other expression the reading reaches is checked at its turn against the type it was read against, as an argument is. So at `both<a>(x: a, y: Vector(a))`, `both(price, [n])` and `both(price, if c then [n] else prices)` meet at `Dec`; at `both3<a>(y: Vector(a), z: Vector(a))` (`prices : Vector(Dec)`), `both3([n], prices)` compiles as `both3(prices, [n])` does; and at `pairUp<t>(p: (t, t))`, `pairUp((n, price))` meets at `Dec` in either component order. The groups are read once, from the callee's parameter types and the written shapes, and never revisited as the schedule solves variables: at `k<a, b>(f: (a) -> b, p: (a, b))`, `k(ident, (n, price))` keeps `n` and `price` in the groups of `a` and `b`, and is refused, as `k2(ident, n, price)` at `k2<a, b>(f: (a) -> b, x: a, y: b)` is — `ident` makes the two variables one only at its turn.
- **Where the shapes part, the literal is a value.** A literal or constructor application read against a type without its shape — a bare type variable, another head — is one value at that seat, a sibling where the seat is a bare variable: `pick([n], [price])` at `pick<a>(x: a, y: a)` is refused, the signature writing no element for the two to share. Its non-lambda components close within it at its turn — a literal's as the component schedule below closes them, a constructor's arguments on its own call's schedule — and its lambda literals still wait for the enclosing call's second pass.
...
**The component schedule** *(#1066)*. A tuple, record, or vector literal on no call argument's spine — at a binding's right-hand side, an ascription, a lambda's body, an assignment's right-hand side, a dot call's receiver — elaborates its components — a `with` update its overrides — as a call elaborates its arguments: non-lambda components in source order, then lambda-literal components — the deferred class above — in source order. A tuple component or record field at a seat its expectation writes as a bare type variable closes after the first pass, alone and in source order, as a sibling group of one does, and a vector literal's non-lambda elements close there as one sibling group; a lambda literal's written parameter and result types land in the first pass, and its body is checked in the second. So a callback's body chooses no home for the components beside it (ruling A2); and, the expectation's variables reaching a literal here through a landing or a forwarding form, as the schedule has solved them, never from a signature it is read against, its components never share a home as sibling arguments do: at `f<a>(g: (Int) -> (a, a))`, `f((x) => (price, n))` is `Dec`, `price` settling `a` first, while `f((x) => (n, price))` is refused; and at `f2<t>(g: (Int) -> ((t) -> t, t))`, `f2((x) => ((v) => v * price, n))` is refused, `n` having settled `t` at `Int` before the callback's body is checked (Numeric Literals §6), while `f2((x) => ((v: Dec) => v * price, n))` compiles. A literal nested in such a literal is one of its components, elaborated at that turn — its components join none of the enclosing groups, and a callback within it is checked within that turn (the ordering pin's residue across trees). A literal on a spine is never elaborated so: the spine reads it into its call's argument list.
