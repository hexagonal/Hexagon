...
| Declared type variable that carries a constraint and occurs nowhere in the type it was declared for: a binder's function, a `fun` block head's members, or an ascription's enclosing declaration *(#712)* | hard error at the declaration, before any defaulting: "`a` is a declared type variable, but this declaration's type does not mention it, so no call can choose it or supply its `Num` evidence; use `a` in a parameter or result type, or remove `a` from the binder list" — a body demand the written list does not entail is §4.2's own refusal, so the binder's constraints here are always written ones and removing the binder always compiles; an ascription-declared variable takes Ascription §3.1's rewrites. The type is the only place a call chooses a variable, and Hexagon has no call-site type application, so no call could ever supply the evidence (GHC's ambiguity check, for the same reason). Never defaulting's "requires `Int`", which names a type no body demanded. An **unconstrained** unused binder (`fun f<a>(x: Int): Int = x`) is legal: it needs no evidence, and a zero-information form misleads no one (Constraints §3) |
