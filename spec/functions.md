...
- **A declared head admits no instantiation inside the knot.** A declared type variable is rigid and scoped to its own declaration (§4.1), and within the SCC every use of a member is at its not-yet-general type — so a sibling reaches a headed member only at the declared variables themselves, through inference variables that unify into them. A concrete use from inside the knot is refused (the ordinary §7.4 failure), and two heads the knot links ask two rigids to be one variable and are refused too. Generic recursion itself is unaffected; the legal spellings are: leave the heads off, keep a single head that every sibling reaches generically, or hang the contract on a non-recursive wrapper. Where two or more functions of one knot must be exported, the wrapper is the only spelling — Modules §4.1.1 requires a complete signature on every exported function, and two heads cannot meet — so the pair exports as headed non-recursive `let` faces over an unexported headless knot; a knot exporting a single function keeps the single-head spelling. The refusal takes §10's SCC hint. The acceptance it declines — a member participating at its declared *scheme*, Haskell's signature rule, which would also license polymorphic recursion — is rejected doctrine, not a gap: `decisions-ml-dialect-annotations-2026-08.md` §2.4 (derivability) and §9.11 record it.
...
| Polymorphic recursion — any SCC-internal use of a member at other than its own not-yet-general type (§7.4): an occurs-check failure (`grow([x], n - 1)`), a concrete instantiation of a headed member, or a second member's rigid | ordinary unification failure (occurs-check included), carrying the SCC hint: name the rule — members of a recursive knot are checked together at not-yet-general types (the knot is the SCC, never the mere §7.3 group). Two message families, each with its own fence. Rigid-vs-concrete (the forced-to-a-concrete-type row above) stays and is correct in kind, but must never surface a type the body did not demand — defaulting's proposal on the errored variable is not a body demand (the pinning-use row's own law). Rigid-vs-rigid ("`a` and `a` are distinct declared type variables") qualifies each side by its declaring member ("`a` declared on `isEven`, `a` declared on `isOdd`") and offers §7.4's spellings — a headless knot, or a non-recursive wrapper (the only spelling where two or more functions of one knot must export) — never "use one type variable name in both annotations", which the refused program may already satisfy |
