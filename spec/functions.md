...
- The component schedule, on no spine: `f2((x) => ((v) => v * price, n))` refused once with Numeric Literals §6's settled-callback report in its component wording, and `f2((x) => ((v: Dec) => v * price, n))` accepted, at `f2<t>(g: (Int) -> ((t) -> t, t))`; `f((x) => (price, n))` at `Dec` and `f((x) => (n, price))` refused, at `f<a>(g: (Int) -> (a, a))`; the residue one level down, `f4((x) => (Some((v) => v * price), n))` accepted at `Dec`; and a tuple literal of the wrong arity at a tuple expectation reported as the arity mismatch it is (Products §2.1).
