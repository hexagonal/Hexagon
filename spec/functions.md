...
- **Evidence reaches a body only through a declaration that quantifies the variable** *(#1048)*. A declared variable's dictionary exists exactly where a declaration's generalization quantifies it: that declaration's scheme takes it as a trailing parameter, and every body its right-hand side encloses closes over it. So a demand on a declared variable is admissible only inside such a declaration. A block head's variable is quantified by the members whose types mention it, and by no other member, nor by a `fun` nested in one (the variable is the head's, not the nested function's to quantify). A knot member's own variable is quantified by that member alone, so a sibling that reaches it through the shared type has no dictionary for it. A demand no enclosing declaration carries is refused at the demand (§10), never left for emission to find missing: on a module the checker accepts, no demand reaches emission without its evidence (closure doc §13.6's invariant).
