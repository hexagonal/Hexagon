...
**The forwarding forms.** A form that returns a subexpression's value hands its expected type to that subexpression: grouping parentheses (§3.1), a block's final expression (Statements §3; a right-hand side's layout block is the one-item case, read through as ever — §8.2), **both** branches of `if` (Operators §11), a `try`'s body block (Exceptions §5.1 — the construct's value path), and every arm body of `match` and `try` — catch arms included (Pattern Matching §6.2). **The literal forms** *(#1066)* hand each component the matching part of their expected type: a tuple literal, where the expectation is a tuple of its arity, component by component; a record literal, where it is a record type — a nominal record's definition row included, at the record's constructor (Products §5.1) — field by field, and a `with` update's overrides the head's type at each field (Products §3.3); and a vector literal, where it is `Vector(t)`, `t` to every element. A part that is still an inference variable is handed on as what it is. A literal form is not a node of an expression tree, as the forms above are: its components stand as a call's arguments do, each the root of a tree of its own whose seat's expected type is the part handed on, or, at one bare type variable, a sibling group — a vector literal's elements always one (Numeric Literals §5.1; the component schedule below). **No other form forwards.** A call's own result — a constructor application's excepted (above) — and every other position synthesize exactly as before — an operator's operands included, with one channel that is not forwarding: at a tower member call — an operator, or the same member spelled bare, qualified, piped, or by the dot (#808) — the expected-type lift governs, the operands take the operation's home type as their expected type — a dot call's receiver among them, under Method Syntax §2.2's receiver rule, which hands the receiver the call's expectation before it elaborates when the spelling's rung is honored at the face — a widening channel Numeric Literals §5.1 owns entire. Numerically, the forms listed here and the tower member calls are the interior of one expression tree, whose home Numeric Literals §5.1 chooses once for the whole tree (#1062): the expectation a form forwards is that home where it is concrete, and each form's own type is the home — a `match` or `try` arm widens into it exactly as an `if` branch does. Forwarding through more forms is a compatible liberalization under the ordering pin below, adopted only on field evidence.

**The component schedule** *(#1066)*. A tuple, record, or vector literal elaborates its components as a call elaborates its arguments: non-lambda components in source order, then lambda-literal components — the deferred class above — in source order. Components at the seats its expectation writes as one bare type variable, and every element of a vector literal, are a sibling group, closed after the first pass as sibling arguments are; a lambda literal's written parameter and result types land in the first pass, and its body is checked in the second. So a callback's body chooses no home for the components beside it, exactly as for arguments (ruling A2): at `g<t>(p: ((t) -> t, t))`, `g(((v) => v * price, n))` (`n : Int`, `price : Dec`) is refused, `n` having settled `t` at `Int` before the body is checked (Numeric Literals §6), while `g(((v: Dec) => v * price, n))` compiles; and at `pairUp<t>(p: (t, t))`, `pairUp((n, price))` meets at `Dec`, as `h(n, price)` does at `h<t>(x: t, y: t)`. The literal is itself one argument of any call it sits in, elaborated at that argument's turn — its components join none of that call's groups, and a callback within it is checked within that turn (the ordering pin's residue across trees). So a literal reads its expectation as the schedule has solved it by then: at `both3<a>(y: Vector(a), z: Vector(a))` (`prices : Vector(Dec)`), `both3(prices, [n])` compiles and `both3([n], prices)` is refused.
...
- A constructor application's own expected type reaches its arguments (#1066): `let a: Option(Dec) = Some(n)`, `let a: Result(Dec, String) = Ok(n)`, `let a: Option(Dec) = Some(n * 1.5)` with its multiplication at `Dec`, `let a: Option(Dec) = if c then Some(n) else None`, and `let w: Wrapper(Dec) = Wrapper({value = n})` at `record Wrapper(a) = {value: a}` compile; `let a: Option(Dec) = wrap(n)` at `wrap<a>(x: a): Option(a)` is refused, and `wrap((n : Dec))` compiles.
- The component schedule: `g(((v) => v * price, n))` refused once with Numeric Literals §6's settled-callback report in its component wording, and `g(((v: Dec) => v * price, n))` accepted, at `g<t>(p: ((t) -> t, t))`; `pairUp((n, price))` at `Dec`, at `pairUp<t>(p: (t, t))`.
...
- **The elaboration schedule** (#517): §4.3's schedule is normative — non-lambda arguments first, lambda literals last, a lambda-literal callee after its arguments, and a tuple, record, or vector literal's components on the same schedule (#1066) — so the pipe supplies in both §6.7 spellings, and contextual widening's targets include the written face at tower member calls, operators included (Numeric Literals §5.1's expected-type lift; every spelling since #808). The design ledger, the order-migration record, and the declined alternative (widening removal) live on #517.
