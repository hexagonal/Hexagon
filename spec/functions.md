...
**The ordering pin — the schedule is the semantics.** Propagation performs early exactly the unifications the seat's final check performs anyway; that final check remains the typing authority, and propagation never invents types and never reports errors of its own. What it rides is a **normative elaboration schedule**: statements top-down (Statements §2), an application's callee first *unless the callee is a lambda literal*, then non-lambda arguments in source order, then lambda-literal arguments in source order, then a lambda-literal callee; operands left to right. The schedule is a checking order, not an evaluation order — runtime evaluation stays source order everywhere. Elaboration order is observable through exactly one judgment: contextual widening (Numeric Literals §5.1), whose targets read the substitution as the schedule built it — so two programs differing only in the order of non-lambda siblings that share an undetermined variable may type differently, by design. That trade is made where F# made it — type-directed conversions defined against a fixed known-type checking order — and this schedule is the smallest observable surface a widening language can have: a lambda literal's position is *never* observable (the deferred class elaborates last from any position), which no source-order schedule can say. Three consequences are normative:
...
- Schedule pins, both directions each: the **deferred-lambda pair** — `fun outer(p) = f(x => useFloat(p), p + one)` and its argument-swapped mirror behave **identically**: the first pass types `p + one` at `Int`, and the lambda's `useFloat(p)` reports the ordinary mismatch (with §4.1's annotate-the-parameter guidance), whichever argument is written first — a lambda literal's position is unobservable. The **callee-position flavours** likewise reject identically: `(p + one) |> (x => useFloat(p))` and `(show(p + one)) |> match` with an arm body reading `useFloat(p)` — arguments elaborate first. The **non-lambda sibling pair** pins the observable residue as spec: `g(p + half, p + one)` types `p` at `Float` (the second argument's `one` widening in) while the swapped `g(p + one, p + half)` rejects (`p` pinned exactly at `Int`, `half` reaching no target) — source order within a pass is semantics. A bare unannotated binding — `let f = match` with its arms on the following lines — still refuses with the rider (no seat, and defaulting does not rescue a dispatch that fired mid-inference).
- The written-face lift's landing pair: `let g: (Int) -> Float = x => x + x` compiles, its body's addition running at `Float` and emitting what the `Int` addition would; `let g = (x: Int): Float => x + x` compiles identically (Numeric Literals §5.1's lift, both faces written).
...
- **The pipe seat and the two-pass argument order** (#517): **landed** — §4.3's elaboration schedule is normative (non-lambda arguments first, lambda literals last, a lambda-literal callee after its arguments), the pipe supplies in both §6.7 spellings, and contextual widening stays, its targets now including the written face at operations (Numeric Literals §5.1's expected-type lift). The design ledger, the order-migration record, and the retired alternative (widening removal) live on #517.
