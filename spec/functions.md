...
> An **expected type** flows in from the seats that write one, through the forms that return a subexpression's value, and lands at lambdas.
...
- **Each argument of a call whose callee's function type is known** when the call is checked: the callee's parameter types, pointwise — constructor applications included, a constructor being a function with a known type; **dot calls included**: a dot call whose receiver is head-known at the dot resolves its member before elaborating its arguments precisely so the member's signature can supply here (Method Syntax §2.2). Arguments are checked **in source order, left to right**, each argument's expectation read as resolved at its turn — the subject-first convention (§5.4) is what makes the order pay: the subject resolves the instantiation, then the callback's expectation arrives concrete. The two-pass order (non-lambda arguments first, then lambdas — F#'s known-type delay, rustc's closures-last) is deliberately **not** adopted: contextual widening's exact-match priority makes elaboration order observable through variables shared between sibling arguments (Numeric Literals §5.1), so deferring a lambda can lose a program the source order accepts — which the monotonicity pin below forbids. That order becomes a pure liberalization if contextual widening is ever retired (#517 records the ledger and the mandatory sequence), and it arrives whole or not at all: partial deferrals reopen the race. A callee whose type is still an undetermined variable supplies nothing, and a dot-call goal whose receiver is still unsolved at the dot keeps its pending path, its arguments synthesizing (Method Syntax §2.2, §3.3).
- **The pipe seat is deferred whole, its mechanism recorded** *(#517)*. `value |> match …` rewrites to `(match …)(value)` before inference (Operators §8; the checker never knows pipes exist), so the pipe's seat is the application whose callee is a lambda literal — and that seat requires elaborating the argument before the callee's body, a reorder of the order otherwise in force (**an application elaborates its callee before its arguments**). Under contextual widening the reorder is lossy for both lambda-literal callee kinds — the plain lambda and the match function alike: a free reference in the callee's body can exactly pin an enclosing variable that the argument's arithmetic would, argument-first, exactly pin to something else — `(show(p + one)) |> match` with an arm body reading `useFloat(p)` types `p` at `Float` under the callee-first order and would re-type at `Int` argument-first, stranding accepted callers — and since bodies are arbitrary, no syntactic class of callee escapes and the seat admits no carve the monotonicity pin would allow. In callee position both spellings of Pattern Matching §6.7 therefore synthesize alike: literal-heavy pipe matches work as they always have, patterns fixing the type; guard-only ones take §6.1's refusal with its rider, whose first rewrite (a preceding annotated `let`, then `value |> classify`) is the working pipe spelling. The seat arrives with #517's sequence, where retiring contextual widening makes argument-first a pure liberalization for every callee, plain lambdas included.
...
**The forwarding forms.** A form that returns a subexpression's value hands its expected type to that subexpression: grouping parentheses (§3.1), a block's final expression (Statements §3; a right-hand side's layout block is the one-item case, read through as ever — §8.2), **both** branches of `if` (Operators §11), a `try`'s body block (Exceptions §5.1 — the construct's value path), and every arm body of `match` and `try` — catch arms included (Pattern Matching §6.2). **No other form forwards.** A tuple or record literal's components, an operator's operands, a call's own result, and every other position synthesize exactly as before; forwarding through more forms is a compatible liberalization under the monotonicity pin below, adopted only on field evidence.

**Landing at a lambda.** The expectation does nothing until it reaches a lambda literal — a match function included, being a lambda by desugar (Pattern Matching §6.7). There:
...
**The ordering pin — an ordering device, not a new judgment.** Propagation performs early exactly the unifications the seat's final check performs anyway; that final check remains the typing authority, and propagation never invents types, never converts, and never reports errors of its own. Two consequences are normative:

- **Monotone, with one rider.** Every program accepted without propagation is accepted with it, at the same types — except where the pre-propagation scheme itself breached §4.1's rigidity contract: a written annotation whose row tail was silently closed to a concrete row, the face narrowed with no diagnostic (`export fun m(): {...a} -> {...a} = (r) => {...r}` faced `() -> {} -> {}`). There the written face wins and the scheme becomes the one the annotation states. The rider strands no caller — the faithful scheme is strictly more general — and it reaches only programs whose old scheme rested on the checker ignoring a written contract; at a seat propagation does not reach, the collapse persists as the pre-existing defect it always was, owned by #520, never by this section. Otherwise propagation can only *add* accepted programs, by giving the order-sensitive judgments — match dispatch (Pattern Matching §6.1), dot dispatch (Method Syntax §3), literal defaulting (Numeric Literals §4), contextual widening (Numeric Literals §5.1) — inputs at least as resolved as they had before.
- **No new boundaries.** A propagated expectation is not an annotation: it establishes no contextual-widening target of its own (Numeric Literals §5.1's "independently established" list is untouched), and it neither adds nor removes a defaulting decision — a variable propagation resolves is resolved to exactly what the seat's final unification would have imposed, so Numeric Literals §4 sees the same residue either way. The pair that pins the widening edge: `let g: (Int) -> Float = x => x + x` fails without propagation — the body's `Int` meets the expected `Float` at the seat's ordinary function-type unification, which admits no widening — and keeps failing with it, while `let g = (x: Int): Float => x + x` compiles either way, the *written* return annotation being a widening boundary (Numeric Literals §5.1) as it always was. Propagation moves unifications earlier; it does not move `Float` into the body's arithmetic.
...
- Each supplying seat accepts the guard-only numeric match function: the annotated `let` (`let sign: (Int) -> String = match …`), the argument seat subject-first (`Seq.map(xs, match …)`), the **dot call at a head-known receiver** (`xs.map(match …)` — Method Syntax §2.2's resolve-before-arguments order, which adds expectations without reordering elaboration), the ascription, the annotated `var` and `:=`, the constraint member body.
...
- Decline paths: arity mismatch at a supplied seat yields §5's ordinary arity error with no propagation artifact; a dot call whose receiver is **still unsolved at the dot** (a bare parameter receiver: `(v) => v.map(match …)`) declines — the goal pends, the arguments synthesize, and the arms see a variable, with §6.1's rider; a data-last call to a **generic** callee (`f(match …, xs)` with `f : ((a) -> b, Vector(a)) -> …`) declines — at the callback's turn the instantiation is unresolved and the arms see a variable — while the same shape at concrete parameter types supplies and succeeds.
- Ordering: both members of the widening pair above; the **order-race specimen** keeps compiling — a source-first lambda that exactly pins an enclosing parameter (`x => useFloat(p)`) ahead of a sibling whose widening then follows it (`p + one`) — the pin that guards the left-to-right order; its **callee-position flavours** likewise — `(p + one) |> (x => useFloat(p))` and `(show(p + one)) |> match` with an arm body reading `useFloat(p)` both keep compiling at `p : Float`, the pins that guard the callee-first order (#517 records the ledger); a bare unannotated binding — `let f = match` with its arms on the following lines — still refuses with the rider (no seat, and defaulting does not rescue a dispatch that fired mid-inference).
- The pipe seat's deferral: a guard-only `value |> match …` refuses with the rider in **both** §6.7 spellings alike, and a literal-heavy pipe match compiles exactly as it does without propagation.
- Monotonicity spot-checks: representative pre-#513 programs keep their inferred schemes verbatim.
...
- **The pipe seat and the two-pass argument order** (#517): deferred alongside contextual widening's reconsideration — §4.3 records the loss mechanism, the specimens, and the mandatory sequence (widening's retirement first, the order liberalization second, never a partial step).
