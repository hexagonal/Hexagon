...
- A concrete expectation outside the tower meets each value path at its own turn (#1107; `o1, o2 : Option(Int)`, `od : Option(Dec)`, `f : Float`): `let r: Option(Dec) = if c then o1 else od` and its branch-swapped mirror are each refused once, at `o1`, "type mismatch: expected Dec, found Int" — and so are the pair as `match` arms, as a `try` body and its catch arm, nested one `if` inside another, and as a block's final expression — while `if c then o1 else o2` is refused twice, once at each branch; `if c then Some(price) else wrap(n)` and its mirror draw Numeric Literals §6's function-result report at `wrap(n)`, as does each of a `match`'s arms `wrap(n)` and `wrap(m)`; `let r: String = if c then n + 1 else "x"` and its mirror are refused at `n + 1`, the gated operation a value of its path; a vector literal's elements alike — `[o1, od]` and `[od, o1]` at `Vector(Option(Dec))` each refused once, at `o1`, `[if c then o1 else od]` at `o1`, and `[wrap(n), od]` with the function-result report; and the colours meet where they met: `let p: Option(() -> Unit) = if c then Some(impure) else None` is refused at the annotation, as `Some(impure)` alone is, and every Effects §13.2 merge-form pin reads its merge at the `if`, `match`, or `try` it read before.
