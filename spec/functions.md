...
| Declared type variable that carries a constraint and occurs nowhere in the type it was declared for: a binder's function, a `fun` block head's members, a list on a binding's name (§4.2), or an ascription's enclosing declaration *(#712)* | hard error at the declaration, before any defaulting: "`a` is a declared type variable, but this declaration's type does not mention it, so no call can choose it or supply its `Num` evidence; use `a` in a parameter or result type, or remove `a` from the binder list"; where the body names the variable, the second rewrite adds "and write a concrete type where the body names `a`", since removing the binder alone would leave those names to meet §4.1's forced-to-a-concrete-type row. A list on a binding's name whose value is not a lambda written without a binding annotation has no parameter list to point at: its first rewrite reads "use `a` in the binding's type", and where that type is not a function the first rewrite is dropped, since a constrained variable there would meet §8.2's evidence-seat refusal (`let v<a: Num>: Int = 3` → "…; remove `a` from the binder list"). An ascription-declared variable's rewrites are "ascribe a concrete type, or name a type variable the declaration uses" — at a value binding only the first, since a constrained variable there meets §8.2's evidence-seat refusal. A declaration whose own written parameter or return type failed to resolve (`x: Nope(a)`) takes that failure's report alone — the unresolved type may be where the variable was meant to occur; an error anywhere else (an inferred result, another variable's refusal) excuses nothing. The type is the only place a call chooses a variable, and Hexagon has no call-site type application, so no call could ever supply the evidence (GHC's ambiguity check, for the same reason). Never defaulting's "requires `Int`", which names a type no body demanded. An **unconstrained** unused binder (`fun f<a>(x: Int): Int = x`) is legal: it needs no evidence, and a zero-information form misleads no one (Constraints §5.4) |
