...
- **A declared head admits no instantiation inside the knot.** A declared type variable is rigid — scoped to its declaring member, or, written on the head, to its block (§4.1, §7.3) — and within the SCC every use of a member is at its not-yet-general type: a sibling reaches a headed member only at the declared variables themselves, through inference variables that unify into them. A concrete use from inside the knot is refused (the ordinary §7.4 failure). Two rigids the knot links are refused too — two members' own annotation variables meeting through the knot (the same spelling or two; a spelling shares nothing, §7.3), or a nested block's head entangled with an outer variable. *(#700.)* The legal spellings are: leave the head off, or declare **one head on the block** that every member sharing the variable writes — and that spelling exports directly: the head's binder satisfies Modules §4.1.1's constraint-writing requirement for every exported member (Modules §4.1.1), so a knot exporting two or more functions no longer needs the non-recursive wrapper #698 recorded as its only spelling; the wrapper remains legal. The refusal takes §10's SCC hint. The acceptance it declines — a member participating at its declared *scheme*, Haskell's signature rule, which would also license polymorphic recursion — is rejected doctrine, not a gap: `decisions-ml-dialect-annotations-2026-08.md` §2.4 (derivability) and §9.11 record it; the block adds no typing power (it is pure scoping — the one binder was always able to be written on a fused `fun`, and the block only lets several members stand under it).
