...
*(#700.)* An annotated `var` and the `:=` right-hand side left this list when the `var` function-type ban landed (Statements §6.1): a `var` cannot hold a function, so no lambda can land at that seat. The assignment boundary's **numeric** channel is unaffected — it belongs to Numeric Literals §5.1's own list, which is about data.

**Sibling arguments** *(#1062)*. The first pass's arguments at the seats a callee's
signature writes as one type variable are the parts of one expression tree (Numeric
Literals §5.1's expression home): none settles the shared variable before the others
