...
- A lambda is a *syntactic value*: constructing it evaluates nothing. This property is load-bearing for §7 (`fun` blocks) and §8 (value restriction).
- **The match function** (Pattern Matching §6.7): `match` ending its logical item, followed by an arm block, is a unary lambda — `$x => match $x …` by definition, with exhaustiveness demanded over the parameter type. It is a lambda literal and a syntactic value like any other lambda (§8.2). *(#700.)* It has no `fun` seat: `fun` is header-only (§7.1), and the recursive matcher names its parameter and scrutinee — `fun size(shape) = match shape …`.
...
*(#700.)* `fun` is **header-only**: unlike `let`, it has no `name = lambda` spelling — a `fun` always shows its parameters on its own line (§7.1 owns the rule and the retired forms' rewrites). The differences from `let` are scoping and recursion, specified in §7 — including the `fun` **block**, mutual recursion's one spelling (§7.3).
...
- Return annotation: **colon after the parameter list** — TypeScript/C#/Scala/Kotlin style. There is no `->` in definition headers; arrow notation is the canonical displayed type form (§5.1). *(#867, #869.)* The headers that write an outer arrow are not definitions': a **constraint member header** (Constraints §2; Effects §13.1) and a **callable extern row** (FFI Part 4 §4.5) are contracts with no body to infer from, and they write `-> R`, `->! R`, or `->? R` where a definition writes `: R`. An `honor` member and a `widens` door are definitions in this bullet's sense — `:` and inference — and the contract they are compared against is the member header's (Effects §13.2).
...
- A written binder's constraint list is also checked as a contract. Every constraint
  ...
  A refusal advises the **whole** list: every constraint the body demands of
...
- **Position restriction:** `<...>` type parameters are syntactically permitted only on lambdas in `let` RHS position (equivalently, in `let` or `fun` header sugar), on the name of a `let` binding with no parameter list *(#1047)*, and — *(#700)* — on a `fun` block head (`fun<a: Eq>`, §7.3), where the list binds block-wide and a *block's* member lines take none — the fused spelling's binder is the head's (§7.3). A `<...>`-annotated lambda anywhere else is a parse error. This prevents rank-2 types from being *expressed* here; there is no other pathway — rank-2 is rejected, not deferred (`decisions-ml-dialect-annotations-2026-08.md` §2.4 and §9.12).
- **A binder list on a binding's name** *(#1047)*. A `let` with no parameter list may write its list on its name:

  ```
  let describe<a: Show>(x: a): String = show(x)
  export let alias<a: Show>: (a) -> String = describe
  ```

  The list is declared over the binding's annotation and right-hand side, and it is the same contract a header's list is. The body's demands must be entailed by it, so `let alias<a: Eq>: (a) -> String = describe` is refused under the entailment bullet above. A constrained variable it declares must occur in the binding's type, under §10's unmentioned-variable row. This is the one spelling of a constraint on a binding whose value is not a lambda literal. It is the counterpart of a Haskell signature's context on a pattern binding (`alias :: Show a => a -> String`), and it is also where a displayed binder list pastes (§5.1): the display `<a: Show> a -> String` gives the bracket to the name and the type to the annotation after `:`. The binder sits on the binding, outermost, so it is rank-1 like every other list, and the rank-2 fence is untouched. The evidence-seat rule still governs what such a binding can carry: a constrained variable in the type of a value that is not a function is refused (§8.2). §8.2's two refusals then name the list's own exit, since a concrete annotation, or none, would leave the listed variable unmentioned (§10). A binding takes **one** list: `let f<a: Eq> = <b: Show>(x: b) => …` writes two spellings of one declaration and is a parse error at the lambda's list.
...
> An **expected type** flows in from the seats that write one, through the forms that return a subexpression's value and into the components of the forms that build one, and lands at lambdas — and, as a widening home, at tower member calls, the arithmetic operators included (Numeric Literals §5.1's expected-type lift; #808).

Inference is Hindley–Milner, Algorithm J (§8) — a lambda's unannotated parameters begin as inference variables, and a type written *around* the lambda reaches them by unification. This section fixes **when**: the unification a seat would perform anyway is performed **before the lambda's body is inferred**, so every judgment that reads a type during body inference — the match dispatch above all (Pattern Matching §6.1) — reads the resolved one. The everyday beneficiary is the match function (Pattern Matching §6.7), whose parameter type is often fixed by nothing *inside* it: guard-only arms over a numeric type constrain without resolving, and before this rule the §6.1 refusal fired on a type that was written one token away. Half of this channel predates the rule — a body has always been checked knowing its enclosing signature's written face (a local `->?` names the nearest enclosing signature that can own a variable, Effects §2.2.2; the face check reads both sides, Effects §4.2) — and the doctrine sentence is now whole: **the annotation's face flows inward entire**, types and colours alike, colours riding as ordinary components of the type (Effects §3.4). One seat propagates types without their colours *(#867)*: at a constraint member's honor seat the contract's arrows reach the body with every effect slot replaced by a fresh variable of the body's own, because Effects §13.2 compares the colours after inference instead of unifying them before it.
...
- **Each argument of a call whose callee's function type is known** when the call is checked: the callee's parameter types, pointwise — constructor applications included, a constructor being a function with a known type; **dot calls included**: a dot call whose receiver is head-known at the dot resolves its member before elaborating its arguments precisely so the member's signature can supply here (Method Syntax §2.2) — every non-subject seat of a constraint member, and every seat of a companion operation; a member's subject seats are established together after their operands are in (#808, Method Syntax §2.2). Arguments are elaborated in **two passes — non-lambda arguments in source order first, then lambda literals in source order** (F#'s known-type order, rustc's closures-last; the deferred class is exactly the landing class below — the arrow form and the scrutinee-less `match`, Pattern Matching §6.7, annotated parameters or not, **read through grouping parentheses** (§3.1): `(x => e)` defers as `x => e` does — and so does a lambda literal anywhere on the argument's **spine**: a literal's component, a constructor's argument, a branch or arm body, a block's final expression (the argument's spine, below; #1096). Nothing else defers: a name bound to a lambda, a call producing one, an **ascribed** lambda — whose face the ascription itself supplies, so deferral would buy it nothing — and every other expression elaborate in the first pass). Each expectation is read as resolved at the argument's turn, so the schedule pays twice over: the subject-first convention (§5.4) resolves the instantiation at the first argument, and a callback *anywhere* in the list — ahead of its subject included, the callback-first signature `f : ((a) -> b, Vector(a)) -> …` — reads its expectation off an instantiation the first pass has already resolved. The schedule is an **elaboration order only**: evaluation order remains the rewritten form's source order (§5.4, Method Syntax §2.2) — which for a bare pipe stage differs from the pipe's written order, Operators §8's recorded footnote — and the schedule reorders no runtime effect. A callee whose type is still an undetermined variable supplies nothing, and a dot-call goal whose receiver is still unsolved at the dot keeps its pending path, its arguments synthesizing (Method Syntax §2.2, §3.3).
- **A constructor application's own expected type** *(#1066)*. At a constructor a `union` or `record` declaration makes — the callee resolving to it by name, bare or qualified; which calls qualify is decided by resolution, never by inference, so a name bound to a constructor (`let mk = Some`) is called as any function is — the instantiated result type is unified with the application's expected type before the first argument elaborates, where both are headed by the constructor's type, so the parameter types the bullet above supplies are the expectation's parts: `let a: Option(Dec) = Some(n)` checks `n` against `Dec`, as `let a: (Dec, Dec) = (n, 0.5)` checks each component (the literal forms, below). This is the unification the seat's final check performs anyway, since a constructed value is never converted as a whole — Numeric Literals §5.1 converts numeric values only. Its colours are the exception (Effects §3.4): the result meets the expectation with the expectation's colours replaced by fresh variables, so they are compared where the application meets its seat, while each argument is handed its part as the expectation writes it, colours included — so `let h: Option((() ->! Unit) ->! Unit) = Some((f) => f!())` lands `f` at `() ->! Unit`. And the instantiation it fixes chooses nothing else: a data declaration's parameters carry no constraint (Declarations Preamble §2.2) and a constructor has no body, so no evidence is selected at it and no code runs at it. Where the expectation is a variable, or is headed by another type, nothing is unified early, and the seat's ordinary diagnostic stands. **No other call unifies its result with its expected type ahead of its arguments.** A function's type parameters choose the instance its body runs at, and its type may itself be inferred, so a face reaching an argument through a function's result would choose what the body runs through a type the reader may not see — a home travelling through a function's type, which Numeric Literals §5.1 keeps out and its §7 rejects. (A tower member call takes its expectation as its tree's home, by Numeric Literals §5.1's lift, never by unifying its result.) `let a: Option(Dec) = wrap(n)` at `wrap<a>(x: a): Option(a)` is refused with Numeric Literals §6's function-result report, which names the repair, `wrap((n: Dec))`.
...
*(#700.)* An annotated `var` and the `:=` right-hand side left this list when the `var` function-type ban landed (Statements §6.1): a `var` cannot hold a function, so no lambda can land at that seat. The assignment boundary's **numeric** channel is unaffected — the `:=` right-hand side is a seat whose expected type, the `var`'s, is its expression tree's home (Numeric Literals §5.1, #1062), which is about data.

**Sibling arguments** *(#1062)*. The first pass's arguments at the seats a callee's
signature writes as one bare type variable — alone or with others, and with the values
on the arguments' spines read against such seats (below) — are the parts of one
...
its body is not — what a callback's body does never chooses the home of the values
beside it, wherever on an argument's spine the callback stands. So `apply2(m, (v: Int) =>
v + n)` (`m : Nat`) meets at `Int`, while `apply2(m, (v) => v + n)` is refused, the body
finding `a` settled at `Nat` (Numeric Literals §6 names both repairs), and so is
`apply2(m, if c then (v) => v + n else ident)`. A body still unifies where no conversion is chosen:
...
**The forwarding forms.** A form that returns a subexpression's value hands its expected type to that subexpression: grouping parentheses (§3.1), a block's final expression (Statements §3; a right-hand side's layout block is the one-item case, read through as ever — §8.2), **both** branches of `if` (Operators §11), a `try`'s body block (Exceptions §5.1 — the construct's value path), and every arm body of `match` and `try` — catch arms included (Pattern Matching §6.2). **No other form forwards**; the literal forms below hand their components parts of an expectation, never the whole. A call's own result — a constructor application excepted (above) — and every other position synthesize exactly as before — an operator's operands included, with one channel that is not forwarding: at a tower member call — an operator, or the same member spelled bare, qualified, piped, or by the dot (#808) — the expected-type lift governs, the operands take the operation's home type as their expected type — a dot call's receiver among them, under Method Syntax §2.2's receiver rule, which hands the receiver the call's expectation before it elaborates when the spelling's rung is honored at the face — a widening channel Numeric Literals §5.1 owns entire. Numerically, the forms listed here and the tower member calls are the interior of one expression tree, whose home Numeric Literals §5.1 chooses once for the whole tree (#1062): the expectation a form forwards is that home where it is concrete, and each form's own type is the home — a `match` or `try` arm widens into it exactly as an `if` branch does. Outside the tower the same holds of types *(#1107)*: where the expectation a form forwards leaves its paths nothing to fill in — every part written, a declared variable counting as written, since no path can solve it — each value path meets it **at its own turn** — its types, with the expectation's colours replaced by fresh variables (Effects §3.4) — so no path is measured against another, whichever is written first, and each that cannot meet it is refused where it stands — several in one report, at the first checked, the others labels on it (Numeric Literals §6); the paths then join, merging their colours at the form (Effects §13.2), and the form meets its seat. So `let r: Option(Dec) = if c then o1 else od` (`o1 : Option(Int)`, `od : Option(Dec)`) is refused once, at `o1`, in either branch order, and a branch that is a function's call draws Numeric Literals §6's function-result report wherever it stands. Where the expectation leaves a part open — a hole, `Option(_)`, or a variable the schedule has not solved, `takeA<a>(x: Option(a))` — no written type says which path is wrong: the paths join with one another first, in source order, as ever, and a disagreement is reported where the join always reported it — at an `if`, the `if`; at a `match` or `try`, each later arm that disagrees. A part the form's own paths fill as they elaborate — a constructor application on a path, unifying its result first (above) — is still open: `let r: Option(_) = if c then Some(price) else o1` joins, as its mirror does. Forwarding through more forms is adopted only on field evidence, and is not a pure liberalization: a form that forwards is on an argument's spine (below), so a callback on its value path would then wait for its call's second pass, and a program whose callback's body had chosen its call's home would be refused.

**The literal forms** *(#1066)* hand each component the matching part of their expected type: a tuple literal, where the expectation is a tuple of its arity, component by component; a record literal, where it is a structural record type, each field the expectation names — a nominal record's definition row among them, at the record's constructor (Products §5.1) — and a `with` update's overrides the head's type at each field (Products §3.3); and a vector literal, where it is `Vector(t)`, `t` to every element. A part that is still an inference variable is handed on as what it is. A part reaches a lambda literal whole, its colours included; every other component meets its part with the part's colours replaced by fresh variables (Effects §3.4) — the types at the component's turn (the component schedule, below), the colours where the literal meets its seat. A literal form is not a node of an expression tree, as the forwarding forms are: each tuple component and record field is the root of a tree of its own, whose seat's expected type is the part handed on, and a vector literal's elements are one tree of siblings (Numeric Literals §5.1), each meeting a part outside the tower at its own turn, as a form's value paths do (#1107) — a part left open filled by the first element to meet it on the schedule, as a tuple's components fill one (the component schedule, below): a constructor application as it elaborates, the other values at the literal's close in source order, lambda literals last; the elements being a literal's components rather than a form's alternatives — except on an argument's spine, where a component read against a bare type variable of the callee's parameters is a sibling in that variable's group, and a vector literal's elements read against one join it (the argument's spine and the component schedule, below).

**An argument's spine** *(#1096)*. A call sees through exactly the forms its expected types reach inward through. An argument's **spine** is the argument and every expression it reaches through grouping, the other forwarding forms' value paths — both branches of `if`, every arm body of `match` and `try`, a block's final expression, a `try`'s body block — the components of the literal forms, a `with` update's overrides included, and the arguments of a constructor application the constructor-application bullet above qualifies; a constructor application on a spine whose shape the reading that reaches it matches — its call's, or a shapes-part constructor's own (below) — has no spines of its own, its arguments standing on the enclosing argument's. Anything else ends it: an ascription, a lambda's body, a `with` update's head, the parts of a form that are not its value — a condition, a scrutinee, a pattern, a guard, a block's earlier items — and every other call, whose arguments it reads on its own schedule (a tower member call's operands remaining, as ever, parts of the tree the call sits in, Numeric Literals §5.1). A call reads each argument's spine as part of its argument list:

- **Its lambda literals are the deferred class, at any depth.** Each waits for the call's second pass, in source order across the whole list — at a call whose callee's type supplies nothing too, the two passes running there as ever. In the first pass — its call's, or, within a constructor application whose shape the spine does not read, that constructor's own (below) — its written parameter and result types land (ruling A2), and it stands in the form around it as a function type of its own arity, built from what it writes and fresh variables, its annotations read once, its parameters taking the parameter types the form hands it, colours included (Effects §3.4); the second pass checks its body against that type. The stand-in's colour is not the lambda's: a demand, argument, or branch it meets in the first pass is held until the body closes, and checked against the colour the body gives it (Effects §2.6), so `optBang(Some(() => ()))` at `optBang(o: Option(() ->! Unit))` is refused as `runBang(() => ())` is. Where the stand-in's arity differs from the function type it meets, the mismatch is §5's arity error, reported where the form holding it meets its seat, as for a direct argument — at the lambda itself where it is a forwarding form's value path meeting a concrete expectation (the forwarding forms, below; #1107). So `g6(m, Some((v) => v + n))` at `g6<t>(x: t, p: Option((t) -> t))` is refused as `apply2(m, (v) => v + n)` is, and so is the callback as a tuple component, a vector element, a record field, or an `if` branch beside `ident`; while `apply2(m, if c then (v) => v + n else incInt)` (`incInt : (Int) -> Int`) meets at `Int`, as `apply2(m, incInt)` does, the named function being a value of the first pass.
- **Its values join the call's sibling groups.** Before any argument is checked, each argument's spine is read against the callee's parameter type as the call finds it — the type the sibling groups are keyed on — through the written shapes of its literals and constructor applications: a tuple literal against a tuple type of its arity, component by component; a record literal against a structural record type, each field the type names (a field it does not name is read against nothing); a vector literal against `Vector(t)`, every element against `t`; a constructor application of its constructor's arity against a type headed by the constructor's own, its instantiated result unified with that type there and its arguments read against its parameter types — the unification the constructor-application bullet performs, performed at the call's start, where it cannot fail and binds none of the callee's variables, the constructor's side being distinct fresh variables; and each value path of a forwarding form against the type the form was read against. A `with` update's overrides are not read against the callee's types: each is checked at the update's turn against its head's field type (Products §3.3), so its values join no group, while a lambda literal among them waits like any other. A non-lambda expression read against a bare type variable of the callee's parameters is a **sibling** in that variable's group, exactly as a whole argument there is, and the literal or constructor application holding it has the variable itself as that part's type. Any other value the reading reaches closes at its turn at the type it was read against, as an argument's tree does — the elements of one vector literal, and the value paths of one forwarding form, remaining the parts of one tree. So at `both<a>(x: a, y: Vector(a))`, `both(price, [n])` and `both(price, if c then [n] else prices)` meet at `Dec`; at `both3<a>(y: Vector(a), z: Vector(a))` (`prices : Vector(Dec)`), `both3([n], prices)` compiles as `both3(prices, [n])` does; and at `pairUp<t>(p: (t, t))`, `pairUp((n, price))` meets at `Dec` in either component order. The groups are read once, from the callee's parameter types and the written shapes, and never revisited as the schedule solves variables: at `k<a, b>(f: (a) -> b, p: (a, b))`, `k(ident, (n, price))` keeps `n` and `price` in the groups of `a` and `b`, and is refused, as `k2(ident, n, price)` at `k2<a, b>(f: (a) -> b, x: a, y: b)` is — `ident` makes the two variables one only at its turn.
- **Where the shapes part, the literal is a value.** A literal or constructor application on a spine whose shape the reading does not match — read against a bare type variable, against another head, as a constructor of the wrong arity, as a `with` update's override (checked against its head's field type), or against nothing, the call supplying no parameter type (a callee whose type is still undetermined, a pending dot-call goal, a lambda-literal callee's application) — is one value at its seat, a sibling where the seat is a bare type variable of the callee's parameters: `pick([n], [price])` at `pick<a>(x: a, y: a)` is refused, the signature writing no element for the two to share. It is elaborated at its turn, its non-lambda parts closing within it — a literal's components as the component schedule below closes them; a constructor application as its own call, reading its own arguments' spines into its own sibling groups, the written types of the lambda literals on those spines landing in its own first pass before its groups close, so `useV([Point2({x = n, y = price})])` at `useV<b>(v: Vector(b))` meets at `Dec` as `let p = Point2({x = n, y = price})` does (`record Point2(a) = {x: a, y: a}`) — while every lambda literal on the spine within it, at any depth, still waits for the enclosing call's second pass. What it receives at its turn is its seat's type as the schedule has solved it by then, the ordering pin's residue across trees: at `pickF<a>(f: (a) -> Int, y: a)` (`vecDecToInt : (Vector(Dec)) -> Int`), `pickF(vecDecToInt, [n])` compiles, `vecDecToInt` having solved `a` at its turn and the literal handing `n` its face's `Dec`, while `pickF2([n], vecDecToInt)` at `pickF2<a>(y: a, f: (a) -> Int)` is refused.

Nothing on a spine is elaborated twice: each part is checked once, at its turn in the first pass or in the second; each group's home is chosen once, from its parts' types as a set (Numeric Literals §5.1); and nothing is tried and undone. A dot call's receiver ends its tree before the dot resolves (Method Syntax §2.2) and is on no spine, so `[m].append(n)` is refused while `Vector.append([m], n)` meets at `Int`.

**The component schedule** *(#1066)*. A tuple, record, or vector literal on no call argument's spine — at a binding's right-hand side, an ascription, a lambda's body, an assignment's right-hand side, a dot call's receiver — elaborates its components — a `with` update its overrides — as a call elaborates its arguments: non-lambda components in source order, then its lambda-literal components, read through grouping (§3.1), in source order. A tuple component or record field at a seat its expectation writes as a bare type variable closes after the first pass, alone and in source order, as a sibling group of one does, and a vector literal's non-lambda elements close there as one sibling group; a lambda literal's written parameter and result types land in the first pass, and its body is checked in the second. So a callback's body chooses no home for the components beside it (ruling A2); and, the expectation's variables reaching a literal here through a landing or a forwarding form, as the schedule has solved them, never from a signature it is read against, its components never share a home as sibling arguments do: at `f<a>(g: (Int) -> (a, a))`, `f((x) => (price, n))` is `Dec`, `price` settling `a` first, while `f((x) => (n, price))` is refused; and at `f2<t>(g: (Int) -> ((t) -> t, t))`, `f2((x) => ((v) => v * price, n))` is refused, `n` having settled `t` at `Int` before the callback's body is checked (Numeric Literals §6), while `f2((x) => ((v: Dec) => v * price, n))` compiles. A literal, constructor application, or forwarding form nested in such a literal is one non-lambda component of it, elaborated at its turn — its components join none of the enclosing groups, and a callback within it is checked within that turn — so there a callback one level down is checked before the components beside it close: `f4((x) => (Some((v) => v * price), n))` at `f4<t>(g: (Int) -> (Option((t) -> t), t))` compiles at `Dec`, where `f2`'s program is refused (the ordering pin's residue across trees). The same closing of non-lambda components serves a literal on a spine whose shape the spine does not read (above), its lambda literals, at any depth, waiting for the call instead; a literal whose shape the spine reads is never elaborated so, the spine reading it into its call's argument list.

**Landing at a lambda.** Besides the operation home above (Numeric Literals §5.1's business) and a constructor application's early unification (above), the expectation does nothing until it reaches a lambda literal — a match function included, being a lambda by desugar (Pattern Matching §6.7). There:
...
**The ordering pin — the schedule is the semantics.** Propagation performs early exactly the unifications the seat's final check performs anyway; that final check remains the typing authority, and propagation never invents types and never reports errors of its own. What it rides is a **normative elaboration schedule**: term bindings top-down (§7.2 — in blocks as at module level), an application's callee first *unless the callee is a lambda literal*, a constructor application's result unified with its expectation, then non-lambda arguments in source order, then lambda-literal arguments in source order — each argument's spine read into the list, its lambda literals among the second pass and its values read against the callee's bare type variables among the groups (#1096) — then a lambda-literal callee; a literal on no spine, its components as arguments, non-lambda components first — a literal or constructor application on a spine whose shape the spine does not read, its non-lambda components so (a constructor's as its own call's), and its lambda literals in its call's second pass; operands left to right. The schedule is a checking order, not an evaluation order — runtime evaluation is the rewritten form's source order (for a bare pipe stage that differs from the pipe's written order, Operators §8's recorded footnote), and the schedule reorders no effect. Elaboration order is observable through the judgments that read the substitution mid-schedule: **contextual widening** (Numeric Literals §5.1), whose homes read the substitution as the schedule built it — once per expression tree, when its last part is in — and Pattern Matching §6.1's abstract-scrutinee **refusal**, which reads the scrutinee's type at dispatch — so two programs differing only in the order of sibling expressions that share an undetermined variable may type differently, by design. Within one expression tree the order of the parts never decides the home: it is chosen from their types as a set, whatever their written order (#1033, #1062). What a part received at its own turn — a sibling argument's parameter the schedule had solved, the expectation a dot call's receiver was handed (Method Syntax §2.2) — is this pin's residue within the tree; trees closing at one moment close in source order, and across trees the residue stands. That trade is made where F# made it — type-directed conversions defined against a fixed known-type checking order — and this schedule keeps the observable surface as small as a widening language's can be: a lambda literal's position **relative to its non-lambda siblings** is never observable (the deferred class elaborates after the first pass from any position, at any depth of its argument's spine), which no source-order schedule can say; among lambda literals, pass-2 source order is semantics, the same residue as the first pass's. Three consequences are normative:
...
- **One boundary at a tree's seat, none elsewhere.** A propagated expectation that is concrete when an expression tree closes — at the tree's seat, and handed through its forwarding forms — is the tree's home, every value widening in (Numeric Literals §5.1's expression home, #1062; the lift is its written-face case, at every spelling of a tower member call, #808). Everywhere else an expectation is not an annotation: it establishes no value-widening target of its own and neither adds nor removes a defaulting decision — a variable propagation resolves is resolved to exactly what the seat's final unification would have imposed, so Numeric Literals §4 sees the same residue either way. The landing pair, deliberate: `let g: (Int) -> Float = x => x + x` compiles — the body's addition runs at `Float`, the written return type being the arithmetic's home — and `let g = (x: Int): Float => x + x` compiles identically, both emitting the JavaScript the `Int` addition would emit (`Float` erases to the same representation; Numeric Literals §5.2).
...
- Each supplying seat accepts the guard-only numeric match function: the annotated `let` (`let sign: (Int) -> String = match …`), the argument seat subject-first (`Seq.map(xs, match …)`), the argument seat **callback-first** (`f(match …, xs)` at `f : ((a) -> b, Vector(a)) -> …` — the first pass resolves the instantiation from `xs`, the second hands the arms `Int`), the **dot call at a head-known receiver** (`xs.map(match …)` — Method Syntax §2.2's resolve-before-arguments order), the **pipe** (`total |> match …` at a determined `total` — in **both** §6.7 spellings), the ascription, the constraint member body.
- Each forwarding form conducts: `if` branches, the `try` body block, `match`/`try` arm bodies, block-final, grouping — and joins as one: `let a: Int = match c` with arms `n` and `m` (`n : Int`, `m : Nat`) accepts exactly as `if c then n else m` does, in either arm order (#824, #1062).
- An expectation outside the tower that leaves nothing to fill in meets each value path at its own turn (#1107; `o1, o2 : Option(Int)`, `od : Option(Dec)`, `f : Float`): `let r: Option(Dec) = if c then o1 else od` and its branch-swapped mirror are each refused once, at `o1`, "type mismatch: expected Dec, found Int" — and so are the pair as `match` arms, as a `try` body and its catch arm, nested one `if` inside another, and as a block's final expression — while `if c then o1 else o2` is refused once, at `o1`, labelled at `o2`, and a `match` with several refused arms likewise; `if c then Some(price) else wrap(n)` and its mirror draw Numeric Literals §6's function-result report at `wrap(n)`, as does each of a `match`'s arms `wrap(n)` and `wrap(m)`; `let r: String = if c then n + 1 else "x"` and its mirror are refused at `n + 1`, the gated operation a value of its path; a vector literal's elements alike, lambda literals among them — `[o1, od]` and `[od, o1]` at `Vector(Option(Dec))` each refused once, at `o1`, as are `[(x) => o1, (x) => od]` and its mirror at `Vector((Int) -> Option(Dec))`, at `(x) => o1`, `[o1, od, o2]` once, at `o1`, labelled at `o2`, `[if c then o1 else od]` at `o1`, and `[wrap(n), od]` with the function-result report; and the colours meet where they met: `let p: Option(() -> Unit) = if c then Some(impure) else None` is refused at the annotation, as `Some(impure)` alone is, a path refused for its type says nothing more of its colour (`if c then impureI else pureD` at `(Int) -> Option(Dec)`, `impureI` performing an effect and returning `Option(Int)`, refused once, at `impureI`, in either order), and every Effects §13.2 merge-form pin reads its merge at the `if`, `match`, or `try` it read before; a declared variable counts as written — `fun gk<a>(o: Option(a)): Option(a) = if c then o1 else o` and its mirror are refused at `o1` — while a part left open keeps the join: `let r: Option(_) = if c then o1 else od`, `takeA(if c then o1 else od)` at `takeA<a>(x: Option(a))`, and each mirror are refused once, at the `if`, and the same as `match` arms at the later arm, as is `let r: Option(_) = if c then Some(price) else o1`, the constructor having filled the hole; a vector literal's elements fill it in turn — `[o1, od]` at `Vector(Option(_))` refused once, at `od`, and `[o1, od, od2]` once, at `od`, labelled at `od2`.
- Each literal form conducts (#1066; `n : Int`, `price : Dec`), each at `Dec`: `let a: (Dec, Dec) = (n, 0.5)`, `let a: {x: Dec} = {x = n}`, `Point({x = n, y = price})` and `{p with x = n}` at `record Point = {x: Dec, y: Dec}`, `let a: Vector(Dec) = [n, 0.5]` and `[n]`, and nested, `let a: Vector((Dec, Dec)) = [(n, 1), (2, price)]`; and a match function as a **tuple component** of an annotated binding lands — `let p: ((Int) -> String, Int) = (match …, 1)` compiles.
- A constructor application's own expected type reaches its arguments (#1066): `let a: Option(Dec) = Some(n)`, `let a: Result(Dec, String) = Ok(n)`, `let a: Option(Dec) = Some(n * 1.5)` with its multiplication at `Dec`, `let a: Option(Dec) = if c then Some(n) else None`, and `let w: Wrapper(Dec) = Wrapper({value = n})` at `record Wrapper(a) = {value: a}` compile; `let a: Option(Dec) = wrap(n)` at `wrap<a>(x: a): Option(a)` is refused with Numeric Literals §6's function-result report, and `wrap((n: Dec))` compiles; `let a: Result(Dec, String) = Some(n)` draws the ordinary mismatch, with no propagation artifact. Its colours (Effects §3.4): `let h: Option((() ->! Unit) ->! Unit) = Some((f) => f!())` compiles, the callback landing `f` at the written `() ->! Unit`, as do the same callback held in a record constructor's field and in a tuple or vector a constructor holds; while `let p: Option(() -> Unit) = Some(impure)` (`impure` performing an effect) is refused at the annotation, as `let o = Some(impure)` then `let p: Option(() -> Unit) = o` is, and a type refused where the colours are held back is reported with the colours written (`optBang(Some(Some(() => ())))` names `() ->! Unit`).
- The argument's spine (#1096; `prices : Vector(Dec)`, `ints : Vector(Int)`, `incInt : (Int) -> Int`): each nested callback refused once with Numeric Literals §6's settled-callback report, as `apply2(m, (v) => v + n)` is — `g5(m, ((v) => v + n, 1))` at `g5<t>(x: t, p: ((t) -> t, Int))` and its argument-swapped mirror, `g6(m, Some((v) => v + n))`, `g7(m, [(v) => v + n])` at `Vector((t) -> t)`, `g9(m, {f = (v) => v + n})` at `{f: (t) -> t}`, `g10(m, [(1, (v) => v + n)])` at `Vector((Int, (t) -> t))`, `apply2(m, if c then (v) => v + n else ident)`, and the callback as a `match` arm body and as a block's final expression — while each spelled `(v: Int) =>` is accepted at `Int`, as are `apply2(m, if c then (v) => v + n else incInt)` and `apply2(m, ((v: Int) => v + n))`; `g(((v) => v * price, n))` at `g<t>(p: ((t) -> t, t))` refused with the report naming the call and `n`, and `g(((v: Dec) => v * price, n))` accepted; a match function as a tuple component at a callback-first signature, `cbFirst((match …, 1), ints)` at `cbFirst<a>(p: ((a) -> String, Int), xs: Vector(a))`, accepted, its arms reading `Int`; the siblings on a spine, each at `Dec` — `both(price, [n])`, `bothR([0.5], price)` at `bothR<a>(y: Vector(a), x: a)`, its literal promoted, `both3(prices, [n])` and `both3([n], prices)`, `pairUp((n, price))` and `pairUp((price, n))`, `bothO(price, Some(n))` at `bothO<a>(x: a, o: Option(a))`, `bothW(price, Wrapper({value = n}))` at `bothW<a>(x: a, w: Wrapper(a))`, and `both(price, if c then [n] else prices)`; the groups read before the first argument, `k(ident, (n, price))` refused as `k2(ident, n, price)` is; the shapes parting, `pick([n], [price])` refused; a `with` update's override joining no group; and the receiver on no spine, `[m].append(n)` refused while `Vector.append([m], n)` is accepted at `Int`; a `with` update's lambda override waiting — `gw(m, {rf with f = (v) => v + n})` at `gw<t>(x: t, r: {f: (t) -> t})` (`rf = {f = (q) => q}`) refused; the shapes-part residue pair, `pickF(vecDecToInt, [n])` accepted and `pickF2([n], vecDecToInt)` refused; a callback whose call a value checked at its turn had settled, `apply2(m, if c then (v) => v + n else natF)` (`natF : (Nat) -> Nat`), drawing the ordinary mismatch, not the settled-callback report; the stand-in's colour, `optBang(Some(() => ()))` refused as `runBang(() => ())` is; a nested stand-in of the wrong arity, `g6(m, Some((v, w) => v + w))`, reported as §5's arity error, as the direct argument is; and a constructor whose shape the spine does not read grouping its own literal's fields — `useV([Point2({x = n, y = price})])`, `pickP(Point2({x = n, y = price}), p0)` at `pickP<a>(x: a, y: a)` (`p0 : Point2(Dec)`), and `fun outer(h) = h(Point2({x = n, y = price}))` each at `Dec`, as `let p = Point2({x = n, y = price})` is; and a waiting lambda's written face landing in such a constructor's own first pass — `pickP2(Holder2((v: Int) => v + n, m), h0)` at `pickP2<a>(x: a, y: a)` (`union Holder2(a) = Holder2((a) -> a, a)`, `h0 : Holder2(Int)`) accepted at `Int`, as `let h = Holder2((v: Int) => v + n, m)` is.
- The component schedule, on no spine: `f2((x) => ((v) => v * price, n))` refused once with Numeric Literals §6's settled-callback report naming the tuple, and `f2((x) => ((v: Dec) => v * price, n))` accepted, at `f2<t>(g: (Int) -> ((t) -> t, t))`; `f((x) => (price, n))` at `Dec` and `f((x) => (n, price))` refused, at `f<a>(g: (Int) -> (a, a))`; the residue one level down, `f4((x) => (Some((v) => v * price), n))` accepted at `Dec`; and a tuple literal of the wrong arity at a tuple expectation reported as the arity mismatch it is (Products §2.1).
...
- Schedule pins, both directions each, all at **generic** callees so no expectation lands and the schedule alone decides. The pins come in fixture pairs, because Numeric Literals §5.1 itself shapes what is observable: at `useFloat : (Float) -> Float`, a pinned `Int` still *accepts* — the argument seat value-widens `Int` into the `Float` parameter, and a `Float` sibling operand recovers a pinned `Int` by operand-driven selection — so the observable there is the **scheme**; at `useNat : (Nat) -> Nat`, `Int -> Nat` is the direction §5.1 refuses, so the observable is the **verdict**. Fixtures throughout: `one : Int`, `half : Float`. The **deferred-lambda pair** — `fun outer(p) = f(x => useFloat(p), p + one)` at `f : ((a) -> b, c) -> …` and its argument-swapped mirror behave **identically**, whichever argument is written first: the first pass types `p + one` at `Int`, both spell `outer : (Int) -> …`, and the `useNat` variant rejects at the lambda in both orders alike — a lambda's position relative to non-lambda siblings is unobservable; so do the same callbacks as tuple components, `fun outer(p) = f5((x => useFloat(p), 1), p + one)` at `f5 : (((a) -> b, Int), c) -> …` and its mirror, at any depth of the spine. The **callee-position flavours** agree the same way: `(p + one) |> (x => useFloat(p))` and `(show(p + one)) |> match` with an arm body reading `useFloat(p)` type `p` at `Int` (their `useNat` variants reject) — arguments elaborate first. The **non-lambda sibling pair** pins the first pass's residue as spec: at `g : (a, b) -> …`, `g(useNat(p), p + half)` and the swapped `g(p + half, useNat(p))` both accept at `p : Nat` — `p + half` is a tree at its variable's seat (`b`'s, or `a`'s once swapped) that closes after the pass's last argument, when `useNat(p)` has fixed `p`, and the addition widens the `Nat` toward `half`'s `Float` (#1062) — while the `useFloat`-only spelling `g(p + half, p + one)` / swapped accepts in **both** orders, at `p : Float` and `p : Int` schemes respectively: its two trees close at one moment, in source order, and the first to close fixes `p`. The **lambda sibling pair** pins pass 2's residue the same way: at `f : ((a) -> b, (c) -> d) -> …`, `f(x => useNat(p), y => p + one)` accepts at `p : Nat` while the lambda-swapped mirror rejects — source order within each pass is semantics. A bare unannotated binding — `let f = match` with its arms on the following lines — still refuses with the rider (no seat, and defaulting does not rescue a dispatch that fired mid-inference).
...
- *(#410.)* **A constrained scheme displays its constraints source-shaped**: §4.2's binder bracket, prefixing the type, set off by one space.
  ...
  Grouping uses §4.2's conjunction form, so one variable's several constraints share one entry. Entries follow the display's variable-letter order — the order the variables occur in the type, which is where the letters come from — and conjuncts within an entry order **alphabetically by constraint name**, matching the evidence suffix's second key (FFI Part 9 §6.2). The order constraints happened to be accumulated in is visible in neither face, and the written conjunction order is invisible here too; the suffix reads it in exactly one case — a same-named pair of distinct declarations (Constraints §5.1.1), whose alphabetical tie resolves positionally (FFI Part 9 §6.2) — and the display loses nothing by not tracking it, two same-spelled conjuncts rendering alike.

  **A binder list the compiler generates for a function or a `fun` block head (§7.3) renders its conjuncts the same way** — a fixit's `write <a: (…)>` (§10's row; Modules §10's), Modules §4.1.1's advised list; a rewritten head keeps the reader's entry order, the declared type-parameter list being the suffix's ordinal (FFI Part 9 §6.2) — with one step before the ordering: redundant bases are removed first (a generated list never names a constraint beside one that already provides it — `Signed` provides `Num`), the elimination-then-ordering FFI Part 9 §7.2 already performs for the evidence suffix. The key is the constraint's declared name, a qualified spelling sorting among its name's letter (`Lib.Heft` among the `H`s). A same-named pair of distinct declarations (Constraints §5.1.1) compares equal under that key, and the tie is settled by where the pair's order is written: at a fixit rewriting a binder the reader wrote, the written entries hold their relative order and the new demands follow them — FFI Part 9 §6.2's positional tie, by the written order of the declared conjunction, since reordering a written same-named conjunction is an ABI event (FFI Part 9 §11) — and where the pair's order is not written — the list first written by the fixit — the pair takes Modules §4.1.1's order, the implementation's own and stable. All of this is independent of the order the constraints were accumulated in and of the order their own failure reports stand in: a literal pattern's demand and `x == 0`'s both suggest `<a: (Eq, Num)>`, and `<a: (Eq, Signed)>` where `Signed` is required, and the failure reports themselves, with their positions, are untouched by the rendering. The other constraint-bearing heads are not reached — a `derives` clause (the derivation fixit is base-complete by its own rule, Constraints §8), a constraint's base list, an `honor` header (Constraints §8's subject and header rows; the other block heads of Lexer & Layout §2.1's inventory): there a written list's order is the reader's, and a base list's is ABI-relevant (Constraints §6.2). A refusal that adds constraints to either list keeps the written entries where they are, dropping one that another entry entails, and appends the new ones alphabetically (Constraints §8).
...
## 7. `fun`: recursive, blocked, header-only

### 7.1 `fun` is header-only *(#700)*

A `fun` defines its members in **header syntax only**: `name(params)` with optional annotations, `=`, body — fused (`fun f(n) = body`) or as a member of a block (§7.3). There is no `fun name =` production; every former RHS spelling is a parse error with a mechanical rewrite:

```
fun f(n) = body                  -- allowed (the one form)
fun f = (n) => body              -- ERROR; write: fun f(n) = body
fun size = match ...             -- ERROR; write: fun size(shape) = match shape ...
fun x = 5                        -- ERROR; not a function definition — use let
fun fib = memoize(...)           -- ERROR (see §7.5)
```

Rationale, unchanged in substance: a block's members are all bound before any member's body can run (§7.3 — mutual visibility requires it), which is only sound if creating each member requires **zero evaluation**. The old rule asked whether a written RHS was a lambda literal; the header form has the property **by construction** — it can only denote a lambda — so the check is now the grammar itself. (SML's precedent: `fun` is clausal-only; the lambda-RHS spelling belongs to `let`, which keeps it, along with the match function — the special-purpose keyword is the one that always shows its header.)

The match-function rewrite names the parameter and the scrutinee. Match functions remain legal at every *expression* seat — a `let` RHS, a call argument, a pipe stage, an ascription — where §4.3's supplying seats hand them their parameter type; the retirement touches only the `fun` seat (Pattern Matching §6.7).

### 7.2 Declarations are read top-down

Every term binding — `let`, `var`, `fun`, and a module alias in term position (Modules §3) alike — is usable only **after** its declaration. There is no hoisting: dependencies sit above their dependents, and a file reads in the order it evaluates. The one exception is the inside of a `fun` block (§7.3), whose members see each other.
...
- **Members are header-syntax definitions** (§7.1), one per block item — no `fun` repetition, no `name = lambda` lines. Each member's `=` opens its ordinary binding-body block. An empty block is a parse error.
- **Export is per member**: an `export` marker at the member's left margin exports that member (Modules §4.1/§4.1.1) — the common shape is a public face over private workers, and the marker sits on the line that names what crosses. `export` before the block head is a parse error with the per-member advice (§10), and markers are legal only in a module-level block — a member of an inner block's `fun` takes none, drawing Modules §4.1's below-module-level refusal. Doc comments attach per member (Doc Comments §4.2).
- **Member names bind at the enclosing scope** — module level or the enclosing block — usable *after* the block per §7.2. The block binds no name of its own; to code outside it, a block is ordinary.
- **The head's binder list is the block's.** Its type variables are declared variables (§4.1) — rigid, scoped over every member — and its constraint list is the §4.2 contract for every member that mentions the variable. Sharing is **opt-in by placement**: two members that write `a` share one rigid, which is what makes a constrained knot spellable (§7.4); a member that does not write it is untouched. Members take no binder lists of their own — a member-line `<...>` is refused with the head advice (§10); refused, not foreclosed. A type variable a member's own annotations write that the head does **not** declare is the member's own — rigid, scoped to that member (§4.1's scoped-to-the-declaration rule, the member being the declaration) — so two members that coincidentally write one spelling share nothing: the head is the only sharing route, which is what keeps a shared spelling from silently strengthening anyone's contract. A head variable no member's type mentions takes §10's unmentioned-variable row, exactly as a fused `fun`'s binder its type does not mention does. SML's ground: `fun … and …` is one declaration whose tyvarseq scopes the chain — this is that construct with layout playing the role of `and`.
- **A lone `fun f(…) = …` is the fused spelling of the one-member block** — identity on every axis: visibility (self-reference legal, §3.3), typing, binder (the §4.2 list after the name is the head's list), emission (§9 — identical up to vertical separation, the head's own source line), and §7.1. A single-member block is also legal; canonical formatting rewrites neither spelling into the other (`notes/canonical-formatting-and-naming.md`).
- **Non-cross-referencing members are legal** (SML: one declaration regardless). **The block bounds visibility, not typing**: the monomorphic knot of §7.4 is still the strongly-connected component of actual references, computed within the block and typed dependencies-first; independent members do not restrict each other's generality.
- **Adjacency runs are retired.** Two adjacent `fun`s are two blocks. A forward reference between them gets §7.2's declared-later diagnostic, extended with the block rule — only members of one `fun` block recurse together; wrap both definitions as its members (§10). Backward references between separate `fun`s stay legal, as between any generalized bindings: outside the knot, functions meet through schemes; inside it, through the block's live monotypes.
...
- **Evidence at the knot is the identity suffix.** An SCC-internal reference is at the definition's own monomorphic type, so for every constrained variable the caller's and callee's schemes share, the call passes the caller's own dictionary parameter, unchanged — the identity instantiation. A recursive occurrence in value position closes over the same dictionaries (Constraints §6.1's evidence-in-scope shape). The suffix is read in the callee's own order (FFI Part 9 §6.2; for a variable introduced by first use, §6.2.1's implementation-chosen placement — the guarantee is that both ends of the knot read one order, not which). A constrained variable of the callee's scheme that is *not* among the caller's own resolves as at any external call site — pinned, defaulted, or refused on the ambiguity path where nothing fixes it and its constraints admit no default (a variable occurring only in constraints is never quantified silently: `decisions-ml-dialect-generalization-2026-08.md` §4.1's Step 2, the defaulting/ambiguity path of its §13; Numeric Literals §4/§6) — and never by minting a new instantiation: dictionary-sharing §6.2 states the same fact from the emitter's side (a recursive occurrence sees a monotype and cannot demand new instantiations).

- **A declared head admits no instantiation inside the knot.** A declared type variable is rigid — scoped to its declaring member, or, written on the head, to its block (§4.1, §7.3) — and within the SCC every use of a member is at its not-yet-general type: a sibling reaches a headed member only at the declared variables themselves, through inference variables that unify into them. A concrete use from inside the knot is refused (the ordinary §7.4 failure). Two rigids the knot links are refused too — two members' own annotation variables meeting through the knot (the same spelling or two; a spelling shares nothing, §7.3), or a nested block's head entangled with an outer variable. *(#700.)* The legal spellings are: drop the members' own variable annotations and let inference link the knot, or declare **one head on the block** that every member sharing the variable writes — and that spelling exports directly: the head's binder satisfies Modules §4.1.1's constraint-writing requirement for every exported member (Modules §4.1.1), so a knot exporting two or more functions no longer needs the non-recursive wrapper #698 recorded as its only spelling; the wrapper remains legal. The refusal takes §10's SCC hint. The acceptance it declines — a member participating at its declared *scheme*, Haskell's signature rule, which would also license polymorphic recursion — is rejected doctrine, not a gap: `decisions-ml-dialect-annotations-2026-08.md` §2.4 (derivability) and §9.11 record it; the block adds no typing power (it is pure scoping — the one binder was always able to be written on a fused `fun`, and the block only lets several members stand under it).

**Evidence reaches a body only through a declaration that quantifies the variable** *(#1048)*. A declared variable's dictionary exists exactly where a declaration's generalization quantifies it: that declaration's scheme takes it as a trailing parameter, and every body its right-hand side encloses closes over it. So a demand on a declared variable is admissible only inside such a declaration. A block head's variable is quantified by the members whose types mention it, and by no other member, nor by a `fun` nested in one (the variable is the head's, not the nested function's to quantify). A knot member's own variable is quantified only by the members whose types mention it, so a sibling that reaches it through the shared type without its own type mentioning it has no dictionary for it. A demand no enclosing declaration carries is refused at the demand (§10) — once per declaration and constraint, at its first such demand, never left for emission to find missing: on a module the checker accepts, no demand reaches emission without its evidence (closure doc §13.6's invariant).

### 7.5 Memoized recursion (the one restricted pattern)

`memoize` itself is an ordinary higher-order function and fully writable in Hexagon; `let cheap = memoize(expensive)` is unremarkable. The only restricted pattern is the self-referential one-liner `fun fib = memoize((n) => ... fib ...)` — rejected by §7.1 (there is no `fun name =` production), for the same reason OCaml's `let rec` rejects non-lambda RHSes: creating a recursive binding must evaluate nothing. The blessed idiom is **open recursion**: write the function taking "itself" as a parameter, and tie the knot with a `memoFix` combinator (a v1 listing obligation at `stdlib-roadmap.md` §2; fully expressible in Hexagon):
...
2. **Value restriction (ML-style; list completed and rationale corrected 2026-08-01, #205 — closure doc `decisions-ml-dialect-generalization-2026-08.md`):** a `let` RHS is generalized in full **only if it is a syntactic value** — a lambda literal, a literal, a **reference (possibly module-qualified) to an immutable term binding** (a `let`, a `fun`, a parameter, a pattern binder, or an import — never a `var` read, which is a state observation, not a value; closure doc §2.3), a constructor application of values, a **record literal whose field values are values**, or a tuple of values. A function *call* is not a value. Given any generic producer — say a local `fun makeEmpty() = []`, of type `() -> Vector(a)` —
   ...
   **The test reads through pure wrappers.** It asks what the RHS *means*, not what punctuation surrounds it. Parentheses only group, an ascription only pins a type without evaluating (Ascription spec §3 — an ascription of a syntactic value is a syntactic value), and a RHS written on the following line only opens a block holding that one expression (Lexer & Layout §2.1) — so
   ...
   are one binding written three ways, and all three generalize. Every other rule that reads what a RHS *means* reads it the same way: the exported-signature check (§4.1), the evidence a constrained binding carries (Constraints §6.1), and the emitted shape (§9). Layout is layout; it does not decide whether a binding is polymorphic. *(#700.)* No rule declines to read through any more: the once-recorded exception, §7.1's written-form check on a `fun` RHS, is retired with the RHS itself — `fun` is header-only, and the grammar admits no wrapper to read through.
...
| Hexagon | JS |
|---|---|
| `fun f(n) = body` | `function f(n) { ... }` — a function declaration |
| a `fun` block (§7.3) | invisible: each member a `function` declaration at the block's textual position, in member order; `export` members take the ordinary export emission. The one-member block and the fused spelling emit identically up to vertical separation — the head's source line becomes a blank line where items precede the block |
...
| `fun f = (n) => …` (the retired lambda-RHS spelling) | parse error: "`fun` defines functions by header; write `fun f(n) = …`" — the rewrite is mechanical from the lambda's own parameters (§7.1) |
| `fun f = match …` (the retired match-function RHS) | parse error, the header rewrite naming parameter and scrutinee: "write `fun size(shape) = match shape …` — a match function stays legal on a `let` and at call sites" (§7.1; Pattern Matching §6.7) |
| Any other `fun name = …` (`fun x = 5`, `fun fib = memoize(...)`) | parse error: "`fun` defines functions by header; write `fun f(params) = …`, or bind the value with `let`" (§7.1, §7.5) |
| `export` before a `fun` block head | parse error: "`export` marks members: put it on each member to export" (§7.3) |
| Binder list on a member line inside a block | parse error: "members take no binder lists; declare the variable on the block head: `fun<b: Show>`" (§7.3) |
| `fun` block head with no members | parse error: "a `fun` block needs at least one member; write one, or remove the head" (§7.3) |
...
| Mutual reference between two separate `fun`s (the retired adjacency run included) | the declared-later family, extended: "only members of one `fun` block recurse together; wrap both definitions as its members" (§7.3) |
| Dot call targeting a member of the caller's own `fun` block | Method Syntax §4.4/§9 own it — "a dot call cannot target its own `fun` block; spell the call by name: `twice(b)`" |
...
| Polymorphic recursion — any SCC-internal use of a member at other than its own not-yet-general type (§7.4): an occurs-check failure (`grow([x], n - 1)`), a concrete instantiation of a headed member, or another declared variable's rigid | ordinary unification failure (occurs-check included), carrying the SCC hint: name the rule — members of a recursive knot are checked together at not-yet-general types (the knot is the SCC, never the whole §7.3 block). Two message families, each with its own fence. Rigid-vs-concrete (the forced-to-a-concrete-type row above) stays and is correct in kind, but must never surface a type the body did not demand — defaulting's proposal on the errored variable is not a body demand (the pinning-use row's own law). The knot refuses as one: its other declared variables, left unquantified because it failed, are its **survivors**, and take no report of their own — defaulting never proposes a type for a declared variable (Numeric Literals §4, #704), and the evidence row does not speak for them. Rigid-vs-rigid ("`a` and `a` are distinct declared type variables") qualifies each side by its declaring site — "`a` declared on `isEven`, `a` declared on `isOdd`"; a side declared on a block head is qualified by the head, located by its span (the head binds no name) — and offers §7.4's spellings — one head on the block, annotation-free members inference links, or a non-recursive wrapper — never "use one type variable name in both annotations", which the refused program may already satisfy |
...
| Annotated (incl. exported; ascription-declared variables included — Ascription spec §3.1) expansive binding whose declared variable item 7 declines | hard error at the declaration, naming the clause that fired: "`a` is a declared type variable, but this right-hand side is a computation that cannot be generalized in `a` (`a` is constrained by `Num`)" / "(`a` occurs in argument position)"; "bind where the type is known, or remove the annotation" (§8.7, §4.1 family; Modules §4.1.1; #205). A constrained variable declared by a list on the binding's name (§4.2) reads "remove `a` from the binder list, and bind where the type is known or remove the annotation": either exit alone leaves it unmentioned, which the #712 row refuses. Where the right-hand side names the variable, the list's exit adds "write a concrete type where the body names `a`", as that row's does. An unconstrained listed variable keeps the plain exits, since the #712 row does not refuse it |
| Annotated (incl. exported; ascription-declared variables included — Ascription spec §3.1) **value** binding whose declared variable the evidence-seat rule declines (§8.2) | hard error at the declaration: "`a` is a declared type variable, but a binding whose type is not a function cannot carry its `Tag` constraint — evidence rides only a function's trailing parameters; annotate at a concrete type, or remove the annotation" (§8.2, §4.1 family; Modules §4.1.1; #205/#207, closure doc §13.6). A variable declared by a list on the binding's name (§4.2) reads "remove `a` from the binder list, and annotate at a concrete type or remove the annotation" — with "write a concrete type where the body names `a`" added where the right-hand side names it — for the same reason as the row above; the binding's own annotation is where the list is used, not a body name |
| A demand on a declared variable that no enclosing declaration quantifies (§7.4's evidence bullet, #1048) | refused at the demand: "`u` is a declared type variable, and this needs its `Frac` evidence, but `a`'s type does not mention `u`, so no call of `a` can supply it; use `u` in `a`'s parameter or result types", naming the member of the variable's own block (or the declaration that introduced it) that encloses the demand. A knot sibling's own variable takes the head advice instead, the head spelled whole so the advice compiles as written: "`u` is declared on `b`, and this in `a` needs its `FromBigInt` evidence, but `a`'s type does not mention it, and a knot member cannot name a sibling's variable; declare it on the block's head, `fun<u: FromBigInt>`, and write `u` in `a`'s signature too". The declaration named is never a local binding inside the member: the repair belongs in the member's type. A demand is attributed to where it was **made** — a dot call settled at a later generalization, or a requirement's components minted when its type became concrete, belong to the binding that wrote them. A demand whose declaration *does* mention the variable but was stopped from quantifying it by another demand is that demand's casualty, and is not reported separately; were such a demand ever the only one on its variable, it is reported as "`u` is a declared type variable, and this needs its `Num` evidence, but `c` cannot be generalized over `u`, so no call of `c` can supply it" rather than left for emission. A demand on a variable the unmentioned-variable row below has refused is not reported separately: that row, at the declaration, is the whole report |
| Declared type variable that carries a constraint, declared or demanded by the body, and occurs nowhere in the type it was declared for: a binder's function, a `fun` block head's members, a list on a binding's name (§4.2), or an ascription's enclosing declaration *(#712)* | hard error at the declaration, before any defaulting: "`a` is a declared type variable, but this declaration's type does not mention it, so no call can choose it or supply its `Num` evidence; use `a` in a parameter or result type, or remove `a` from the binder list"; where the body names the variable, the second rewrite adds "and write a concrete type where the body names `a`", since removing the binder alone would leave those names to meet §4.1's forced-to-a-concrete-type row. Where the body demands a constraint the list does not entail, this row absorbs §4.2's refusal (the row below): the evidence it names is the whole list that row would advise — two of one declared name each qualified by its declaring module, whatever words were written, as Constraints §5.1.1's disambiguation bullet requires ("…supply its evidence for this module's `Heft` and the `Heft` declared in module `Lib`") — and its first rewrite adds the list, since using the variable alone would meet that refusal next — "…so no call can choose it or supply its `Eq`, `Num` evidence; use `a` in a parameter or result type and write `<a: (Eq, Num)>`, or remove `a` from the binder list and write a concrete type where the body names `a`"; at a block head, "and widen the head to `fun<a: (Eq, Num)>`". Where one of those demands has no spelling here (that row's fourth tier), no list can be written, and the first rewrite is dropped. A list on a binding's name whose value is not a lambda written without a binding annotation has no parameter list to point at: its first rewrite reads "use `a` in the binding's type", and where that type is not a function the first rewrite is dropped, since a constrained variable there would meet §8.2's evidence-seat refusal (`let v<a: Num>: Int = 3` → "…; remove `a` from the binder list"). An ascription-declared variable's rewrites are "ascribe a concrete type, or name a type variable the declaration uses" — at a value binding only the first, since a constrained variable there meets §8.2's evidence-seat refusal. A declaration whose own written parameter or return type failed to resolve (`x: Nope(a)`) takes that failure's report alone — the unresolved type may be where the variable was meant to occur; an error anywhere else (an inferred result, another variable's refusal) excuses nothing. The type is the only place a call chooses a variable, and Hexagon has no call-site type application, so no call could ever supply the evidence (GHC's ambiguity check, for the same reason). Never defaulting's "requires `Int`", which names a type no body demanded. An **unconstrained** unused binder that the body demands nothing of (`fun f<a>(x: Int): Int = x`) is legal: it needs no evidence, and a zero-information form misleads no one (Constraints §5.4) |
...
| Body requires a constraint not entailed by the written constraint list | "`a` is declared to honor `Eq`, but the body requires `Hash`; write `<a: Hash>`, or remove the constraint annotation to let it be inferred" (§4.2) — the list rendered as §5.1 renders a generated binder list: bases removed, alphabetical by declared name. The list is **whole**: every constraint the body demands of the variable, merged with the declared ones, so applying the rewrite leaves no second refusal of this row behind. Each demand keeps its own report at its own caret, except that reports which read alike at one caret are one ("Once per place", above), and every report about one variable advises the same list: `export let f<a: Eq>(x: a): String = if x < x then show(x) else ""` is refused for `Ord` at `x < x` and for `Show` at `show`, and both advise `write <a: (Ord, Show)>`. So the report is decided when the module's declarations have all been checked: a demand made later in the body would otherwise be missing from the list advised at an earlier one. A variable with a demand that no list here can spell (the fourth tier, below) has no whole list to write, so every refusal of that variable names that constraint too, drops the `write` rewrite, and keeps only the exits the fourth tier leaves: "`a` is declared to honor `Ord`, but the body requires `Show`, and also the constraint `Gate`, declared in module `Lib` and not exported; no constraint list here can name it — remove the constraint annotation to let it be inferred". A variable the unmentioned-variable row above refuses takes no report here: that row is the whole report. Where the variable is declared on a **block head**, the advice respells to the head and names the exceeding member — "`a` is declared to honor `Eq` on the block head, but `containsAll`'s body requires `Hash`; widen the head: `fun<a: Hash>`, or remove the head's constraint to let it be inferred" — never a member binder, which the block refuses (§7.3); the widen advice is the fused arm's maximal respell (entailed bases unrestated). Where the required constraint shares a spelling with a declared one but not a declaration (Constraints §5.1.1), each side qualifies by its declaring module — "`a` is declared to honor this module's `Heft`, but the body requires the `Heft` declared in module `Lib`" — the rigid-vs-rigid row's discipline for constraints, and the rewrite spells the required declaration per the advised-spelling law, module-routed where no spelling resolves here: "write `<a: (Heft, Lib.Heft)>` — `import Lib` and spell it `Lib.Heft`" — the route clause's "declared in" half elided because this message has already named the declaring module (the law's elision licence, Constraints §5.1.1); never the bare word already written, whose retry re-reports this row against itself. The block-head arm composes identically: the same law respells to the head — "widen the head: `fun<a: (Heft, Lib.Heft)>`" — carrying the route clause, never a member binder; and a required constraint with no spelling and no route takes the law's fourth tier — no rewrite: the report names the declaring module and, at a private function, the row's own standing exit ("remove the constraint annotation to let it be inferred" — inference carries what no binder here can spell); Modules §4.1.1 owns the export seat's exits (#715) |
| `<...>` type parameters on a lambda outside `let` RHS position, or anywhere other than a header, a binding's name, or a `fun` block head | parse error (§4.2) |
| A binder list on a binding's name **and** on the lambda it binds (`let f<a: Eq> = <b: Show>(x: b) => …`) | parse error at the lambda's list: "`f` already writes its binders on the name; a binding takes one binder list" (§4.2) |
...
- **Type-system internals** (Algorithm J, levels, union-find): compiler architecture, not additional language surface. §8 owns the observable generalization rules, and §4.3 the observable checking-mode rules — expected-type propagation is language surface (#513). Rank-2 types do not arrive: the once-recorded annotation-gated pathway is retired by the derivability ruling (`decisions-ml-dialect-annotations-2026-08.md` §9.12), and §4.3's machinery hosts no such gate.
- **The elaboration schedule** (#517): §4.3's schedule is normative — non-lambda arguments first, lambda literals last, a lambda-literal callee after its arguments — so the pipe supplies in both §6.7 spellings, and contextual widening's targets include the written face at tower member calls, operators included (Numeric Literals §5.1's expected-type lift; every spelling since #808); an argument's spine — its literals, constructor applications, and forwarding forms — is read into its call's argument list (#1096), and a literal on no spine schedules its components the same way (#1066). The design ledger, the order-migration record, and the declined alternative (widening removal) live on #517.
...
own answer. *(#700 later retired §7.1's written-form check along with the `fun`
RHS itself; this record describes the rules as they stood.)* That much is a compiler defect correction. The one specification
