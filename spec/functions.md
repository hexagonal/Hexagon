...
- *(#410.)* **A constrained scheme displays its constraints source-shaped**: §4.2's binder bracket, prefixing the type, set off by one space.
  ...
  Grouping uses §4.2's conjunction form, so one variable's several constraints share one entry. Entries follow the display's variable-letter order — the order the variables occur in the type, which is where the letters come from — and conjuncts within an entry order **alphabetically by constraint name**, matching the evidence suffix's second key (FFI Part 9 §6.2). **A function's or block head's binder list the compiler generates renders the same way** — a fixit's `write <a: (…)>` (§10's row; Modules §10's), Modules §4.1.1's advised list — with one step before the ordering: redundant bases are removed first (a generated list never names a constraint beside one that already provides it — `Signed` provides `Num`), the elimination-then-ordering FFI Part 9 §7.2 already performs for the evidence suffix. The key is the constraint's declared name, a qualified spelling sorting among its name's letter (`Lib.Heft` among the `H`s); a same-named pair keeps the written order where the fixit rewrites a binder the reader wrote (FFI Part 9 §6.2's positional tie — reordering a written same-named conjunction is an ABI event, FFI Part 9 §11) and takes Modules §4.1.1's order, the implementation's own and stable, where the list is first written here. All of this is independent of the order the constraints were accumulated in and of the order their own failure reports stand in: a literal pattern's demand and `x == 0`'s both suggest `<a: (Eq, Num)>`, and `<a: (Eq, Signed)>` where `Signed` is required, and the failure reports themselves, with their positions, are untouched by the rendering. Other homes are not reached — a `derives` clause (the derivation fixit is base-complete by its own rule, Constraints §8), a constraint's base list, an `honor` header (Constraints §8's subject and header rows): there a written list's order is the reader's, and a base list's is ABI-relevant (Constraints §6.2). The order constraints happened to be accumulated in is visible in neither face, and the written conjunction order is invisible here too; the suffix reads it in exactly one case — a same-named pair of distinct declarations (Constraints §5.1.1), whose alphabetical tie resolves positionally (FFI Part 9 §6.2) — and the display loses nothing by not tracking it, two same-spelled conjuncts rendering alike.
