...
- **Each argument of a call whose callee's function type is known** when the call is checked: the callee's parameter types, pointwise — constructor applications included, a constructor being a function with a known type. Arguments are checked **left to right**, and each argument's expectation is the parameter type as resolved at that argument's turn — the subject-first convention (§5.4) is what makes this order pay: the subject resolves the instantiation, then the callback's expectation arrives concrete. A callee whose type is still an undetermined variable supplies nothing, and a pending dot-call goal synthesizes its arguments exactly as before (Method Syntax §3.3/§3.5).
- **The callee of an application that is syntactically a lambda literal**: the arguments synthesize first and their types supply the lambda's parameter expectations, with the application's own expected type, if any, as the result expectation. This is the pipe's seat — `value |> match …` rewrites to `(match …)(value)` before inference (Operators §8; the checker never knows pipes exist), and the subject's type reaches the match function's parameter through this clause. A hand-written `(x => …)(value)` is the same shape and gets the same treatment.
...
- If the expectation is a function type of the lambda's arity, each **unannotated** parameter takes the corresponding parameter component — an ordinary type, **never rigid**: rigidity remains the written variable's contract (§4.1), and a propagated component is inference's, not a declaration. The expectation's result component becomes the body's expected type, forwarded per the rules above. Components that are still undetermined variables land as what they are; landing is unification, not resolution.
...
- Each supplying seat accepts the guard-only numeric match function: the annotated `let` (`let sign: (Int) -> String = match …`), the argument seat subject-first (`Vector.map(xs, match …)`), the pipe (`value |> match …`), the ascription, the annotated `var` and `:=`, the constraint member body.
- Each forwarding form conducts: `if` branches, `match`/`try` arm bodies, block-final, grouping.
...
- Decline paths: arity mismatch at a supplied seat yields §5's ordinary arity error with no propagation artifact; a data-last call to a **generic** callee (`f(match …, xs)` with `f : ((a) -> b, Vector(a)) -> …`) still refuses — at the callback's turn the instantiation is unresolved and the arms see a variable — while the same shape at concrete parameter types supplies and succeeds.
- Ordering: both members of the widening pair above; a bare unannotated `let f = match n when n < 0 => …` still refuses with the rider (no seat, and defaulting does not rescue a dispatch that fired mid-inference); a pending dot-call goal's arguments still synthesize.
