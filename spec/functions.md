...
- **Evidence at the knot is the identity suffix.** An SCC-internal reference is at the definition's own monomorphic type, so a recursive call passes the definition's own trailing evidence — its dictionary parameters, unchanged, in the parameter list's own canonical order (Constraints §6.1; FFI Part 9 §6.2). A recursive occurrence in value position closes over the same dictionaries (Constraints §6.1's evidence-in-scope shape). Nothing here is special-cased: the recursive call is an ordinary call site whose instantiation happens to be the identity, and dictionary-sharing §6.2's finiteness argument leans on exactly this — a recursive occurrence can demand no new instantiation.

- **Declared heads cannot meet across the knot.** A declared type variable is rigid and scoped to its own declaration (§4.1), so a sibling's body reaches a generic member's type only through inference variables, which unify with that member's rigids. Where the knot forces two members' types to share a variable — the ordinary mutual shape, arguments flowing through the cross-calls — a declared head on both members asks two rigids to be one variable, and is refused. Generic mutual recursion itself is unaffected, and the refused spelling has legal neighbors of identical meaning: leave the heads off, annotate at most one member, or hang the contract on a non-recursive wrapper. The refusal takes §10's SCC hint. The acceptance it declines — a member with a head participating at its declared *scheme*, Haskell's signature rule, which would also license polymorphic recursion — is rejected doctrine, not a gap: `decisions-ml-dialect-annotations-2026-08.md` §2.4 (derivability) and §9.11 record it.
...
| Polymorphic recursion | ordinary unification failure at the recursive call site, carrying the SCC hint (§7.4) whenever a failing side belongs to a group member's not-yet-generalized type: name the rule ("a recursive `fun` group is checked at one shared type"). When both sides are declared type variables from different members, qualify each by its declaring member ("`a` declared on `isEven`, `a` declared on `isOdd`") and offer §7.4's working spellings — annotate at most one member of the group, or move the contract to a non-recursive wrapper. The generic declared-variable advice must not fire here: it reads the collision as a body demand and advises annotation edits that cannot help |
