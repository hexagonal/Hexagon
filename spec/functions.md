...
- *(#410.)* **A constrained scheme displays its constraints source-shaped**: §4.2's binder bracket, prefixing the type, set off by one space.
  ...
  Grouping uses §4.2's conjunction form, so one variable's several constraints share one entry. Entries follow the display's variable-letter order — the order the variables occur in the type, which is where the letters come from — and conjuncts within an entry order **alphabetically by constraint name**, matching the evidence suffix's second key (FFI Part 9 §6.2). **Every compiler-generated binder list renders this way** — a fixit's `write <a: (…)>` (§10's row; Modules §10's), the lists Modules §4.1.1's maximality rule governs, as much as a displayed scheme: redundant bases removed first (a binder list never names a constraint beside one that already provides it — `Signed` provides `Num`), then alphabetical by the constraint's declared name, a qualified spelling sorting on its name (`Lib.Heft` among the `H`s) and a same-named pair ordered as Modules §4.1.1 orders it (the implementation's own, unspecified but stable) — whatever order the constraints were accumulated in and whatever order their own failure reports stand in. So a literal pattern's demand and `x == 0`'s both suggest `<a: (Eq, Num)>`, and `<a: (Eq, Signed)>` where `Signed` is required; the failure reports themselves, and their positions, are untouched by the rendering. A `derives` list is not a binder list: the derivation fixit is base-complete by its own rule (Constraints §8) and appends to what the reader wrote, and this sentence does not reach it. The order constraints happened to be accumulated in is visible in neither face, and the written conjunction order is invisible here too; the suffix reads it in exactly one case — a same-named pair of distinct declarations (Constraints §5.1.1), whose alphabetical tie resolves positionally (FFI Part 9 §6.2) — and the display loses nothing by not tracking it, two same-spelled conjuncts rendering alike.
