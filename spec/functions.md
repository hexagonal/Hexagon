...
**The forwarding forms.** A form that returns a subexpression's value hands its expected type to that subexpression: grouping parentheses (§3.1), a block's final expression (Statements §3; a right-hand side's layout block is the one-item case, read through as ever — §8.2), **both** branches of `if` (Operators §11), a `try`'s body block (Exceptions §5.1 — the construct's value path), and every arm body of `match` and `try` — catch arms included (Pattern Matching §6.2). **No other form forwards** — with one further form since #808: **the receiver of a dot call whose member name is a tower member's spelling** (Numeric Literals §5.1's closed list) hands the call's expected type to the receiver before it elaborates, the receiver being an operand seat of the operation the call resolves to (Method Syntax §2.2); a spelling decides that the expectation travels, never what is dispatched, and at any receiver that is not a tower operation the forwarded expectation lands on nothing. A tuple or record literal's components, a call's own result, and every other position synthesize exactly as before — an operator's operands included, with one channel that is not forwarding: at a tower member call — an operator, or the same member spelled bare, qualified, piped, or by the dot (#808) — the expected-type lift governs, the operands take the operation's home type as their expected type, a widening channel Numeric Literals §5.1 owns entire. Forwarding through more forms is a compatible liberalization under the ordering pin below, adopted only on field evidence.
