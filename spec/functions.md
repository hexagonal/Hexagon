...
- **Where the shapes part, the literal is a value.** A literal or constructor application on a spine whose shape the reading does not match — read against a bare type variable, against another head, as a constructor of the wrong arity, or against nothing, the call supplying no parameter type (a callee whose type is still undetermined, a pending dot-call goal, a lambda-literal callee's application) — is one value at its seat, a sibling where the seat is a bare type variable: `pick([n], [price])` at `pick<a>(x: a, y: a)` is refused, the signature writing no element for the two to share. It is elaborated at its turn, its non-lambda parts closing within it — a literal's components as the component schedule below closes them, a constructor's arguments as its own call's siblings — while every lambda literal on the spine within it, at any depth, still waits for the enclosing call's second pass. What it receives at its turn is its seat's type as the schedule has solved it by then, the ordering pin's residue across trees: at `pickF<a>(f: (a) -> Int, y: a)` (`vecDecToInt : (Vector(Dec)) -> Int`), `pickF(vecDecToInt, [n])` compiles, `vecDecToInt` having solved `a` at its turn and the literal handing `n` its face's `Dec`, while `pickF2([n], vecDecToInt)` at `pickF2<a>(y: a, f: (a) -> Int)` is refused.
