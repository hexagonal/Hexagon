...
- **Monotone.** Every program accepted without propagation is accepted with it, at the same types; propagation can only *add* accepted programs, by giving the order-sensitive judgments — match dispatch (Pattern Matching §6.1), dot dispatch (Method Syntax §3), literal defaulting (Numeric Literals §4), contextual widening (Numeric Literals §5.1) — inputs at least as resolved as they had before.
