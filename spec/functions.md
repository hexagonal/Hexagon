...
- A written binder's constraint list is also checked as a contract. Every constraint
  ...
  the variable, merged with the declared ones, so that the list compiles as
  written (§10's row). It is therefore decided once the body is checked, and not
