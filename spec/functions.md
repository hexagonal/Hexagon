...
| a `fun` block (§7.3) | invisible: each member a `function` declaration at the block's textual position, in member order; `export` members take the ordinary export emission |
