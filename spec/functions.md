...
> An **expected type** flows in from the seats that write one, through the forms that return a subexpression's value, and lands at lambdas — and, as a widening home, at tower member calls, the arithmetic operators included (Numeric Literals §5.1's expected-type lift; #808) — and, as an exact home, at decimal-point literals (Numeric Literals §5.1's promotion; #525).
...
**Exact-integer argument deferral.** Within the existing numeric argument pass,
fixed integer sources leave room for other already-elaborated arguments to
establish a permitted destination at the same shared parameter. BigInt uses
independent `FromBigInt` evidence; the fresh callee parameter's own bound cannot
establish that destination. With no independently established target, BigInt
unifies exactly. Numeric Literals §5.1 and Exact Integer Widening §4 own the
rule; it changes neither the expression elaboration schedule nor runtime
evaluation order.
...
**Landing at a decimal-point literal** *(#525)*. A forwarding form's value path that is itself an unsuffixed decimal-point literal takes a concrete expected type that is an exact home for it — `Dec`, or a type honoring `Frac` and `FromBigInt` — and is promoted there, carrying its written digits (Numeric Literals §5.1 owns the rule). This is the reach an integer literal already has, its type variable meeting the expectation by unification: `if waived then 0.0 else 1.25` at a `Dec` seat compiles as `if waived then 0 else 1` does. An expectation that is a variable, or `Float`, or any other type leaves the literal `Float`.

**Landing at a lambda.** Besides the operation home above and the decimal-point literal (Numeric Literals §5.1's business), the expectation does nothing until it reaches a lambda literal — a match function included, being a lambda by desugar (Pattern Matching §6.7). There:
...
- **One boundary at operations, none elsewhere.** A propagated expectation that lands at a *tower member call* — an arithmetic operator, or the same member in any other spelling (#808) — as a concrete type is an established widening target — the operation's home type, operands widening in (Numeric Literals §5.1 owns the rule). Everywhere else an expectation is not an annotation: it establishes no value-widening target of its own — a decimal-point literal taking it is a literal read exactly, not a value widened — and neither adds nor removes a defaulting decision — a variable propagation resolves is resolved to exactly what the seat's final unification would have imposed, so Numeric Literals §4 sees the same residue either way. The landing pair, deliberate: `let g: (Int) -> Float = x => x + x` compiles — the body's addition runs at `Float`, the written return type being the arithmetic's home — and `let g = (x: Int): Float => x + x` compiles identically, both emitting the JavaScript the `Int` addition would emit (`Float` erases to the same representation; Numeric Literals §5.2).
