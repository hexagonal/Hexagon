...
A **`fun` head** — the keyword alone, or carrying a §4.2 binder list glued to it, `fun<a: Eq>` — ending its logical item opens a layout block of **members**. Inside the block, every member's body sees every member, earlier and later alike. This is the language's only forward visibility among terms, and it exists because mutual recursion cannot be spelled without it.
...
- **A declared head admits no instantiation inside the knot.** A declared type variable is rigid and scoped to its block (§4.1, §7.3), and within the SCC every use of a member is at its not-yet-general type — so a sibling reaches a headed member only at the declared variables themselves, through inference variables that unify into them. A concrete use from inside the knot is refused (the ordinary §7.4 failure). Two rigids the knot links are refused too — reachable now only through *nested* blocks (a local block inside a member's body, its own head entangled with the outer's through the knot), since one block has one head and mutual recursion demands the block (§7.3). *(#700.)* The legal spellings are: leave the head off, or declare **one head on the block** that every member sharing the variable writes — and that spelling exports directly: the head's binder satisfies Modules §4.1.1's constraint-writing requirement for every exported member (Modules §4.1.1), so a knot exporting two or more functions no longer needs the non-recursive wrapper #698 recorded as its only spelling; the wrapper remains legal. The refusal takes §10's SCC hint. The acceptance it declines — a member participating at its declared *scheme*, Haskell's signature rule, which would also license polymorphic recursion — is rejected doctrine, not a gap: `decisions-ml-dialect-annotations-2026-08.md` §2.4 (derivability) and §9.11 record it; the block adds no typing power (it is pure scoping — the one binder was always able to be written on a fused `fun`, and the block only lets several members stand under it).
...
| `fun` block head with no members | parse error (§7.3) |
...
| Polymorphic recursion — any SCC-internal use of a member at other than its own not-yet-general type (§7.4): an occurs-check failure (`grow([x], n - 1)`), a concrete instantiation of a headed member, or a nested block head's rigid | ordinary unification failure (occurs-check included), carrying the SCC hint: name the rule — members of a recursive knot are checked together at not-yet-general types (the knot is the SCC, never the whole §7.3 block). Two message families, each with its own fence. Rigid-vs-concrete (the forced-to-a-concrete-type row above) stays and is correct in kind, but must never surface a type the body did not demand — defaulting's proposal on the errored variable is not a body demand (the pinning-use row's own law). Rigid-vs-rigid ("`a` and `b` are distinct declared type variables" — reachable only through nested blocks, one block having one head) qualifies each side by its declaring block head and offers §7.4's spellings — one head on one block, or a headless knot, or a non-recursive wrapper |
