...
- The component schedule: `g(((v) => v * price, n))` refused once with Numeric Literals §6's settled-callback report in its component wording, and `g(((v: Dec) => v * price, n))` accepted, at `g<t>(p: ((t) -> t, t))`; `pairUp((price, n))` at `Dec` and `pairUp((n, price))` refused, at `pairUp<t>(p: (t, t))`; the literal as one argument, `both3(prices, [n])` accepted and `both3([n], prices)` refused, at `both3<a>(y: Vector(a), z: Vector(a))`; and a tuple literal of the wrong arity at a tuple expectation reported as the arity mismatch it is (Products §2.1).
