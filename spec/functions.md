...
- Return annotation: **colon after the parameter list** — TypeScript/C#/Scala/Kotlin style. There is no `->` in definition headers; arrow notation is the canonical displayed type form (§5.1). *(#867.)* The one header that writes an outer arrow is not a definition's: a **constraint member header** is a contract with no body to infer from, and it writes `-> R`, `->! R`, or `->? R` where a definition writes `: R` (Constraints §2; Effects §13.1). An `honor` member and a `widens` door are definitions in this bullet's sense — `:` and inference — and the contract they are compared against is the member header's (Effects §13.2).
