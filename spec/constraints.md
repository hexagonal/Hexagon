...
- **A constrained scheme is a function scheme** *(added 2026-08-01, #205/#207 — closure doc §13.6)*. The trailing suffix is the language's **only** evidence representation, and Functions §8.2's evidence-seat rule keeps generalization inside it: a variable still constrained after defaulting is quantified only at a binding whose type is a function type; at any other value binding it declines and is pinned by its first use. Consequence for this section, normative and mutation-testable: every scheme that reaches emission carrying a residual constraint set is function-typed, so the bare-name and eta-expansion shapes above are total over their domain. An emitter path asked for evidence at a non-function value is a compiler conformance defect and may assert; it is never a user diagnostic — the two emission-time messages the pre-ruling implementation surfaced there are retired (closure doc §13.6).
...
| A constrained scheme is a function scheme: the trailing suffix is the only evidence representation, Functions §8.2's evidence-seat rule keeps generalization inside it, and evidence-at-a-non-function during emission is a conformance assertion, never a user diagnostic (2026-08-01, #205/#207) | §6.1; Functions §8.2; closure doc §13.6 |
