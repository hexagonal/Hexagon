...
**A member definition is a module-level binding.** One sentence; every rule below is an existing law applied to it — plus one licensed exemption (#541). There is only one `multiply` at `Rat` — bare-in-module, `Rat.multiply`, and evidence dispatch all reach the same binding. Where the first bullet's generalising-export exemption applies, the sentence holds one level up: there is one *operation*, whose export and member are its two widths, the member the export's restriction — never two rival implementations of one spelling.

- **Its spelling is claimed against ordinary bindings** *(in force since PR β, #339; generalising-export exemption added with #541)*. Honoring a constraint claims each member's name in the module's term space: an ordinary module-level binding of that spelling — `let`, exported or private — is the ordinary rebinding error. The claim is against ordinary bindings only: member definitions from **distinct honor blocks coexist** (one module honoring two constraints with same-spelled members stays legal, disambiguated by evidence, by qualification, or by dot dispatch where unambiguous), and the declaring module's own declaration-plus-honor coexists the same way. Where the coexistence puts **two member bindings of one spelling** in the module, a bare in-module use of that spelling is refused naming the routes that remain — the refusal family Modules §5.5 uses for collided prelude exports, owned here for the in-module case, since neither binding outranks the other and the fourth bullet's "this binding" has no single referent. The delegation pattern (`add(l, r) = add(l, r)` beside a module-level `let add`) is therefore ill-formed, not unfashionable; a shared helper takes a different name.

  **One exemption — the generalising export** (Modules §5.3's generalisation law, #541). An **exported** module-level binding of a member's spelling is legal when its signature **properly generalises** the member's: same subject seat, same result, every remaining member parameter accepted at the export's corresponding seat exactly or through Numeric Literals §5.1's exact conversions, and **at least one seat properly wider** — an identical signature generalises nothing, is exactly the delegation pattern the bullet above rules ill-formed, and stays the rebinding error. The signature half is **checked at the declaration**; the behavioural half — the export agrees with the member on the whole shared domain, the member is its restriction — is a stated law of the same standing as instance lawfulness (§3), reviewed rather than machine-checked. A same-spelled export that does *not* signature-generalise stays the rebinding error, now with the law named in the diagnostic. **The generalising export shares the member's visibility rule** (the last bullet below): it is *qualifiable, not a bare export* — it does not enter consumers' bare scope and is not an exporter for Modules §5.5's collision count, so bare `pow` keeps its one exporter (`Pow.hex`'s member) however many companions widen their `pow` — reached, like the member it generalises, qualified and through the dot. The coexistence is **not** the two-bindings refusal above, because a lawful pair holds no rivals: there is one operation, so a bare in-module use of the spelling **resolves to the export**, the operation's widest face — the same face the qualified spelling and the dot show — and every call the member would have answered is answered identically, by the behavioural law. The blessed idiom still holds the implementation under a private in-module name, exports the door as its thin face, and defines the member as the same implementation restricted — one implementation, three spellings (`Float.pow` and `x.pow(…)` the door, `**` the member; Operators §6.3.1 holds the standing instances).

  **The exemption carves this bullet's claim, and nothing else.** Its reach is decided by the claim's own geography, not by the constraint's layer: it is available in any honoring module where the member's spelling is not already an *ordinary* binding, so a prelude constraint and one reached through a namespace import (Modules §3.3, which binds the alias alone and leaves every member spelling free) stand identically — the prelude doors are the law's first instances, not its extent. Where the spelling **is** an ordinary binding, the prior refusals stand — verdicts unamended, the law never consulted, though the named-import message carries the repair hint (the diagnostics table; Modules §3.1): in the constraint's own declaring module the spelling is the member's polymorphic forwarder, the declaration's exported bare face (declaration-plus-honor holds it on purpose, and a door there would be a second export of one name); under a named constraint import the members arrive as ordinary bindings with the constraint (Modules §3.1), and a same-spelled export is that section's collision. The exemption never unseats an ordinary binding, and the forwarder case shows why the line sits exactly here: a forwarder and a door are wide on **different axes** — the forwarder across types, the door across seats at one type — so a bare spelling shared between them could resolve only by ranking against the arguments, which the language refuses on principle. The door-builder's form is therefore the namespace import: to widen a member, import the module, not the constraint.
...
- **A bare use elsewhere in the module means this binding** — monomorphic, at the honored type — occluding the prelude's polymorphic export exactly as Modules §5.4 already occludes any prelude name a module binds. `divide(left, right) = multiply(left, reciprocal(right))` inside `Frac<Rat>` means the `Num<Rat>` member. (Two member bindings of one spelling → the first bullet's refusal; under a lawful generalising pair, the bare use resolves to the export, the operation's widest face — the first bullet's exemption; "this binding" presumes the ordinary single-binding case.)
...
| Ordinary binding takes an honored member's spelling | the ordinary rebinding error, in `#reportRebinding`'s house form (§4.6, in force since #339); where the honor-claim itself is the refuser, an **exported** binding draws the refusal with the generalisation law named — legal exactly where it properly generalises the member (§4.6's exemption, #541). The declaring-module collision keeps the plain form; the named-import collision keeps the plain refusal but carries the door-builder's repair hint, naming the namespace-import route and Modules §5.3 as a signpost — the law is not consulted at either seat (§4.6's boundary; Modules §3.1) |
...
| **A member definition is a module-level binding** (#304/#335): spelling claimed against ordinary bindings (distinct-block coexistence carved out); top-down order at its own line, evidence routes exempt; own body cannot call its own name (#293's non-`fun` law — rewrite diagnostic names dot-call and qualified forms; default bodies exempt); bare use in-module = this binding, monomorphic (Modules §5.4 occlusion; under a lawful generalising pair, the export — the operation's widest face, #541/#544); qualifiable but not a bare export (one-exporter law); the generalising-export exemption carves the claim only, never an ordinary binding — layer-blind reach, boundary at the declaring module's forwarder and named-import members (#544) | §4.6 |
