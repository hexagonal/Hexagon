...
- *(#355.)* **Constraint members are pure — demanded, not conventional.** Every member's arrows are `->` throughout (its own face and any function-typed parameter), an `honor` declaration's member bodies must check pure, and derived instances are pure by construction. The ground (Effects §5): operators elaborate to member calls (Operators §1.1), brackets to `at`, `for` heads to `iterate`, interpolation to `show` — four call forms with no seat for a call mark, so what they dispatch to must never need one. Consequently **`Iterable` can have no effectful instance**: `iterate` is a member; a type whose traversal performs effects cannot honor it (the effectful traversal type is `Stream`, which deliberately has no instance — `stream.md` §4.5).
