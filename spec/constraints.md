...
| `widens` where the spelling is an ordinary binding (declaring module unspellable; named-import members) | the standing §4.6-boundary refusals, verdicts unamended, the named-import seat carrying the namespace-import repair hint (§4.6, #544) |
...
| `widened` outside the member-accounting position | parse error, `derive`-style: "`widened` is only legal as the complete RHS of a member line in an `honor` block" (§4.7; Lexer §4.2) |
