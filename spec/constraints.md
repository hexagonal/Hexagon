...
| `widens` head whose qualifier names no module alias — nothing of the spelling in scope | ``unknown module `Pow` `` + the route: "a `widens` head names its member through a module alias; import the member's home module with `import module Pow`" — the not-yet-imported author's front door onto Modules §5.3's law, same repair family as Modules §5.1 (§4.7; #577) |
| Bare constraint name resolving nowhere (`honor Pow<T>`, a binder's `<a: Pow>`), no same-spelled alias visible | ``unknown constraint `Pow` `` + the route: "import its home module with `import module Pow` for qualified access, or import the constraint by name" — the alias-bearing arm keeps its §5.1-family realias repair (Modules §10) (§2.2; #577) |
