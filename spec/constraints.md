...
- **Pre-registered constraints are declarations too** (Modules §6.4): each pre-registered name denotes exactly one compiler-global identity, in every module, with no import. Where the compiler holds the declaration itself (`Num`, `Signed`, `Frac`, `Pow`, `Concat`, `Eq`, `Ord`, `Show`, `Hash`), the name is non-redeclarable — a module-level `constraint Eq<a> = ...` is an error naming the pre-registered constraint, not a second `Eq`; the wired-in machinery (operator routing, derivation, collection contracts) reaches these constraints by their one identity, and a user-spelled twin would be unreachable by that machinery, so it is refused rather than admitted as a trap. Two pre-registrations are **name-only** in v1: for `Iterable` and `Integral` the compiler holds the name but no declaration, so a source `constraint Iterable<c> = ...` *supplies* the one pre-registered declaration's members rather than minting a rival — it lands on the same identity. The redeclaration ban extends to them when the compiler holds their declarations; the name-only state is a compiler gap, not a spec freedom, and no third kind of pre-registration exists.
