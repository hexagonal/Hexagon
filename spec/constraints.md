...
- A **ground instance** is a module-level record of its completed member set: supplied members plus inherited defaults. Each member's implementation hoists to its own module-level binding — the instance's **member seat**, a generated family under the `__` reservation keyed (constraint, subject, member), never exported under the member's source name — and the record's slots reference the seats by name: `const __Num_Int_add = (left, right) => left + right; const __Num_Int = { add: __Num_Int_add, ... };` (seat spellings representative; determinism and the collision discipline are dictionary-sharing §5's). A **defaulted** member's seat is a thin wrapper closing over the constraint's default helper and this module's own dictionary: the instance *has* the member — §5.2 qualifies `Int.notEquals` whether or not the honor block wrote it — and default-versus-override is a fact about the block's text that never reaches emitted shape or the interface. The record is materialised only if some polymorphic use actually needs it (see erasure below); a seat, when its record is or when a concrete member call routes to it (the direct-call clause below).
...
- **A constraint member's module-level seat is a forwarder** *(the shape §6.5 exports)*. Each member of a module-declared constraint emits one module-level binding — source parameters followed by the member's trailing evidence suffix, the body a single dictionary slot read: `const show = (x, dictShow) => dictShow.show(x);`. A member call at a **genuinely polymorphic** type is a call *into* that forwarder: the selection erases — the instance is chosen at compile time where it can be, the call never searches — and the evidence rides the trailing suffix. A source-written member **call** — bare `show(x)`, qualified `Int.show(x)`, or dot `x.show()` — whose head resolves to a **concrete type with a declared instance** is a call to the instance's method: it denotes the honoring module's member binding (the seat below, §5.2's qualification made emission-true) and emits as a **direct call** to that binding, the forwarder hop and the evidence both erased. At a concrete head whose instance is **structural** (no honoring module — tuples, `Unit`, `Bool`), the call reads the member off the hoisted structural dictionary instead (dictionary-sharing §3.4), which is the same erasure with a slot read in place of a named binding. This clause governs source-form member calls; elaboration-internal dispatch (interpolation, comparison fallbacks, `Hash`, iteration) keeps its own routes, and slot inlining still belongs to the operator and comparison lowerings above. The forwarder remains the seat the member name denotes as a module-scope **term** — a reference, a generalized alias, the exported ABI face — so a constraint member forwarder still has no Part 8 edition, at any type. *(Superseded in part by #335/#344, on this touch:)* every pre-registered constraint with a compiler-held declaration now **has** canonical prelude source, so the forwarder shape above is simply its shape too; what remained wired-in was per-instance, not per-constraint — and those primitive instances have all retired per companion on stdlib-roadmap §5.2's schedule (§5.3's wired-row-retirement law, complete since `Float.hex` and `String.hex` landed, #344).
