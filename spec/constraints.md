...
| Displayed schemes are source-shaped: the §4.2 binder bracket as a quantifier prefix, one space before the type; grouped per variable, entries in variable order, conjuncts alphabetical (the evidence suffix's own order); Haskell-flavored `C a =>` retired | §9.4; Functions §5.1 |
