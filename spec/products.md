...
**Implementer note (a component means its instance):** "every component has `C`" is discharged by the component type's **own instance** — for a nominal component, the unique coherent `C` instance, hand-written or derived; for a structural component, the automatic structural instance of this section, recursively. This holds wherever the structural walk starts: a `derives` body (here and Unions §7, via §3.4/§5.2's application of these rules), or a direct structural use site (`v1 == v2` at `Vector(T)`). A structural *re-derivation* that reaches into a nominal component's representation is not an implementation of these rules — it silently ignores a hand-written component instance and pierces `opaque` (#278). The checker selects the component instances when it discharges the derivation's demands; emission must render that same selection, never re-walk the type independently. Shortcuts that remain licensed, because the structural answer provably *is* the instance's answer: primitives (compiler-provided instances only), `Bool` (the #147 representation pin; a competing hand-written instance is unwritable), and `Hash` (derivable-only with derived `Eq` required, Collections Part 2 §4.3 — agreement by construction).
