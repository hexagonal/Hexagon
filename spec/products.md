...
- **A tuple literal takes its components' types from the type it meets** (Functions §4.3's literal forms, #1066): where the expected type is a tuple of the literal's arity, each component is checked against its part, so `let p: (Dec, Dec) = (n, 0.5)` widens `n : Int` and promotes `0.5` (Numeric Literals §5.1). As a call's argument, a tuple literal is read into the call's argument list, its components at the callee's type variables joining the call's sibling groups (Functions §4.3's argument spine, #1096).
...
- **Arity 0 (`Unit`, §2.7) is the vacuous case of each rule** *(added 2026-07-30, #159)*: `Eq` is constantly `true`, `Ord` constantly `Equal`, `show ()` is `"()"`, `Hash` a constant (`0`). These are the same answers the primitive-era fiat rows decreed — verified per operation at ruling time (`decisions-ml-dialect-unit-2026-07.md` §5) — so the reclassification changed no observable output. "Defined iff every component has the constraint" is vacuously satisfied, so `Unit` carries all four unconditionally; neither `Num` nor `Signed`, as for every tuple.

**Implementer note (a component means its instance):** "every component has `C`" is discharged by the component type's **own instance** — for a nominal component, the unique coherent `C` instance, whether hand-written, derived, or compiler/runtime-provided (Collections Part 2 §4.4); for a structural component, the automatic structural instance of this section, recursively. This holds wherever the componentwise rules apply: a `derives` body (records via §3.4/§5.2, unions via Unions §7's mirroring of these rules) or a structural use site (`(m1, n) == (m2, n)`). A structural *re-derivation* that reaches into a nominal component's representation is not an implementation of these rules — it silently ignores the component's own instance and pierces `opaque` (#278). The checker selects the component instances when it discharges the demands; emission must render that same selection. Shortcuts are licensed by Operators §5.1's fast-path criterion — only where the emitted operation provably preserves the selected instance's semantics. That covers: primitives (no user instance is writable, Constraints §5.3); `Bool` (likewise unwritable, and the #147 pin makes the native answers the derived instances' answers); and inlining a component instance that is itself *derived* (the expansion is that instance's own body). `Hash` is no exemption: component hashes come from component instances too — a `Set(a)` or `Map(k, v)` component hashes by its provided permutation-invariant instance (Part 4 §8.4), never by a representation walk. What Part 2 §3.2 separately licenses is the *unspecified combine*, which is what permits flattening across components whose `Hash` is itself derived.
...
| `()` | `undefined` | `void` (return position), `undefined` elsewhere (§2.7, #159) |
...
**The representation rule is arity-indexed** *(added 2026-07-30, #159)*: at arity ≥ 2, the array above; at arity 0, `undefined` — a positional product with no positions has nothing to box, and `undefined` is JavaScript's spelling of nothing, so a `Unit`-returning function is a JS function that returns nothing. This is a clause of the structural representation rule, not a #147-style pin: there is no declaration to privilege and no exception being carved (`decisions-ml-dialect-unit-2026-07.md` §4). Its safety condition — nothing consumes a tuple arity-generically at runtime — was audited at ruling time; parameter-list polymorphism (Functions §5.2, post-v1) is the pre-registered reopener.

### 2.7 The empty tuple is `Unit` (#159)

*(Added 2026-07-30; ruling `decisions-ml-dialect-unit-2026-07.md`, which governs until consolidated.)* `Unit` — formerly a primitive (Primitive Types §9, now the pointer) — is the **arity-0 tuple**: an ordinary member of this section's family in every semantic respect. `()` is its literal and its (irrefutable, arity-0) tuple pattern; its four constraints are §2.5's automatic structural instances at the vacuous arity; `.itemN` on it reports §2.3's arity error with K = 0; a `match` on it is exhausted by the `()` arm through the ordinary tuple clause (Pattern Matching §7.1); its representation is §2.6's arity-0 clause.

Two rules about the *name*:

- **`Unit` is the type's only spelling, and `()` is not a type expression.** In type syntax, parens remain exactly what they are today: grouping `(T)`, tuple types at arity ≥ 2, and function-type parameter lists — where `() -> T` is the zero-parameter domain, *not* a function taking `Unit` (Functions §5.3, whose thunk/`Unit`-taking distinction this rule is load-bearing for; SML-style single-parameter functions were re-rejected 2026-07-30). A bare `()` in type position gets the §6 redirect. The value-spelled-`()`/type-spelled-`Unit` asymmetry is SML's and OCaml's exactly.
- **`Unit` is a compiler-known name, not a prelude declaration.** Structural types have no declarations and no home module (Modules §2), so nothing here uses or needs the `Bool.hex` machinery of #147 §3.5. Diagnostics and the pretty-printer say `Unit`, never "0-tuple" or "empty tuple" — the classification is spec vocabulary, like "row" (§4).
...
- `name: Type` in type position / `name = expr` in term position, comma-separated, braces (§1 doctrine; §8 correction record). Field names are non-uppercase-start identifiers (term-level names, Functions spec §2 case rule), and a hard keyword is one: a field name is a label seat, and its access a dotted seat (Lexer §4.4), so `{type: String}`, `{type = "click"}`, and `ev.type` describe the JavaScript objects that carry such keys *(#1014)*.
- **Construction punning ships** (Pattern Matching §9): `{x, y}` in value position is `{x = x, y = y}`, and it composes with update — `{p with x}` is `{p with x = x}` (legal OCaml, same expansion). A pun names a binding bare, so a keyword field is written out — `{type}` is refused naming `{type = …}` (Lexer §4.4). Term-level only; `{x}` in *type* position remains an error ("record types need field types"). The emitter uses JS shorthand whenever a field key and its emitted value are the same identifier, whether the Hexagon source used `{x}` or the equivalent `{x = x}`. **No** computed keys, methods, getters, or spread forms in term braces beyond the bare copy `{...p}` (§3.3/§5.3).
...
- **A record literal takes its fields' types from the structural record type it meets** — an annotation's, a parameter's, one handed on, or a nominal record's definition row at its constructor (Functions §4.3's literal forms, #1066): `Point({x = n, y = price})` at `record Point = {x: Dec, y: Dec}` widens `n : Int` into the `Dec` field. As a call's argument, a record literal is read into the call's argument list likewise (Functions §4.3's argument spine, #1096).
...
- **Update semantics (unchanged by the re-spelling):** every overridden field must already exist in the head's type, and its override meets the field's type there: the head's field type is the override's expected type (Functions §4.3, #1066), so a value enters it as at any seat — `{p with x = n}` widens `n : Int` into a `Dec` field. Field *addition* is a compile error: "record update cannot add fields; `p` has no field `z`". Result type = the head's type — structural in, same structural type out; nominal in, nominal out (§5.3). Duplicate override fields: the §3.1 duplicate-field error.
...
- **Where `...` may be written.** In the annotations of a definition Hexagon compiles — a function's parameters and result, a `let`, an ascription, a lambda — where the face a consumer sees is the solved row after the body is checked; and in a `type` alias (Declarations Preamble §5), which is a type fragment read wherever it is expanded. Two declarations name every field of their values instead, and `...` — bare or named — **anywhere in their field or slot types, at any depth, and through any alias** (applied or not, and however many aliases deep) is refused at the declaration: a nominal `record`'s field types (§5.1; the definition row itself is closed by its grammar) and a union constructor's payload types (Unions §2.1) — an `exception`'s payload with them, being a freestanding constructor (Exceptions §2). `type Row = {n: Int, ...}` is legal, and `fun f(r: Row)` uses it as written; `record H = { r: Row }` is refused at `r`. A declaration's row is **one row for every value of the type**, so a construction that widened it would widen every value of the type at once — including a foreign value of the type arriving at a boundary, where the fields the tail was widened to would cross unseen (FFI Part 1 §5.4 item 7, which refuses `...` at every position of an *extern* declaration and in an exported constraint member's signature on the same ground: a face not derived from a body). A record parameterised by a remainder is a feature this section does not have; today the rewrite is to name the fields, or to give the field the type `JsValue`. Diagnostic, in this section's vocabulary: "a `record` names every field of its values; `r`'s type says the record may have more fields — name them, or give the field the type `JsValue`" (for a payload: "a constructor's payload names every field of its values; this slot's type says the record may have more fields — name them, or give the slot the type `JsValue`"), reported at the field or slot, naming the alias when one carried the tail.
...
2. **A constructor function `Point : {x: Float, y: Float} -> Point`** in the term namespace (uppercase-start, per the Functions spec's reservation for constructors). Argument is the exact closed row — a definition row is always closed (§4) — and ordinary record-literal checking applies (all fields, no extras). At a parameterised record the application's expected type fixes the row's parameters before the argument is checked (Functions §4.3, #1066): `let w: Wrapper(Dec) = Wrapper({value = n})` at `record Wrapper(a) = {value: a}` widens `n : Int` into the field.
...
**Recorded v2 option (considered, not adopted):** implicit nominal→structural coercion inserted in *checking* mode only (where the expected type is annotation-known — decidable, no inference pollution; Functions §4.3's expected-type propagation is the machinery that would host it, #513 — note that §4.3 itself inserts nothing: it orders unifications and performs no coercion of its own. The one conversion machinery expectations feed is Numeric Literals §5.1's pre-existing widening, whose targets an expectation may establish (the expected-type lift); a nominal→structural coercion has no such pre-existing judgment to feed, so adopting this option would be a genuine extension of §4.3's contract, not a consequence of it). Declined for v1 because "compiles with an annotation, fails without" is a confusing failure mode for exactly our audience. Revisit only with field evidence that `{...p}` ceremony is a real pain point.

### 5.4 Emission

- A `Point` **is** the POJO. No wrapper, no brand, no tag.
- `Point({x = 1.0, y = 2.0})` **applied directly erases**: emits `{x: 1.0, y: 2.0}`.
- The constructor is first-class (`map(rows, Point)` is legal); when *referenced* rather than applied, the emitter materialises an identity function — `const Point = r => r;` — emitted only where the module references the constructor as a value or exports it (the same demand rule as a union's constructor, Unions §6.4). **Export is a mandatory demand site**: `export record Point` emits one stable named ESM constructor for JavaScript consumers (FFI Part 7 §3). Direct applications still erase, including internal applications of an exported constructor.
...
| `...` (bare or named, at any depth, through any alias) in a nominal `record`'s field types, a union payload type, or an `exception`'s payload type | hard error at the field or slot: "a `record` names every field of its values; `r`'s type says the record may have more fields — name them, or give the field the type `JsValue`" (§4; Unions §2.1; FFI Part 1 §5.4 item 7 for the extern-declaration and constraint-member twins) |
...
| `Unit` reclassified as the arity-0 tuple (2026-07-30, #159): tuple family = every arity except one; representation arity-indexed (`undefined` at 0); constraints = §2.5's vacuous case, no observable change; `()` never a type — `Unit` is the sole type spelling; `() -> T` untouched | §2.1, §2.5–§2.7, §10; `decisions-ml-dialect-unit-2026-07.md` |
...
### 9.6 Edit notes to other specs — **all discharged** (implementation PR, July 2026)

Every edit below was applied in the implementation PR that landed `with` in the parser, the book, and the playground; the list is kept as the decision trail, not as outstanding work. The only spread forms remaining anywhere in the corpus are the bare copy `{...p}`, vector rest patterns, and emitted JavaScript — which is where `...` belongs.
...
6. **Implementation (Opus's PR, before Phase 5)** — `with` as a parser-recognized contextual word (no lexer token change), the head restriction with recognition one token past the head path, the four new §6 diagnostics (spread-update and merge fixits permanent), migration of the update sites above; stdlib and runtime carry **zero** update forms (verified at ruling time), so no `.hex` migration exists. Landed: the update and the bare copy share one AST node — the head *is* the emitted spread — so the change is confined to the surface grammar, and the emission tests pinning exact JavaScript output passed unedited, which is §9.5's byte-identity claim discharged rather than asserted.

---

## 10. Correction record: `Unit` is the arity-0 tuple (2026-07-30, #159)

> **Correction (2026-07-30, #159).** Under the ML-dialect doctrine (`decisions-ml-dialect-bool-2026-07.md` §1), `Unit` was reclassified from Primitive Types' primitive set to this document's tuple family: the empty tuple, with `()` its literal and pattern, the automatic structural instances its constraints (vacuous at arity 0 — same outputs as the former fiat rows, verified), and an arity-indexed representation rule whose arity-0 clause is `undefined`. The full ruling is `decisions-ml-dialect-unit-2026-07.md`, authoritative until consolidated. Edits applied in place here: §2.1's arity sentence, §2.5's vacuous-case bullet, §2.6's emission row and arity-indexed clause, new §2.7 (the normative home), the §6 type-position redirect row, and the §7 log row.

Two standing rules accompany the reclassification, mirroring Primitive Types §12's:

1. **Section numbers did not move.** §2's subsections were appended (§2.7), never renumbered; "Products §2.1" citations resolve unchanged.
2. **Ownership transferred.** `Unit`'s normative home is this document (§2.7) and the decisions doc; Primitive Types retains only the pointer (§9 there) and its §1 representation-fact row. Where older text says "Primitive Types owns `Unit`," the decisions doc's ledger (§8 there) governs the fix-on-next-touch.
