...
- **Where `...` may be written.** In the annotations of a definition Hexagon compiles — a function's parameters and result, a `let`, an ascription, a lambda — where the face a consumer sees is the row after the body is checked. Two declarations name every field of their values instead, and `...` in them is refused at the declaration: a nominal `record`'s definition row (§5.1), and a union constructor's payload types (Unions §2.1). A declaration's row is **one row for every value of the type**, so a construction that widened it would widen every value of the type at once — including a foreign value of the type arriving at a boundary, where the fields the tail was widened to would cross unseen (FFI Part 1 §5.4 item 7 — which refuses `...` at every position of an *extern* declaration on the same ground). Diagnostic, in this section's vocabulary: "a `record` names every field of its values; `r`'s type says the record may have more fields — name them, or give the field the type `JsValue`" (for a payload: "a constructor's payload names every field of its values …").
...
| `...` in a nominal `record` definition row or a union payload type | hard error at the declaration: "a `record` names every field of its values; `r`'s type says the record may have more fields — name them, or give the field the type `JsValue`" (§4; Unions §2.1; FFI Part 1 §5.4 item 7 for the extern-declaration twin) |
