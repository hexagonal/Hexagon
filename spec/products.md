...
- **A tuple literal takes its components' types from the type it meets** (Functions §4.3's literal forms, #1066): where the expected type is a tuple of the literal's arity, each component is checked against its part, so `let p: (Dec, Dec) = (n, 0.5)` widens `n : Int` and promotes `0.5` (Numeric Literals §5.1).
...
- **A record literal takes its fields' types from the structural record type it meets** — an annotation's, a parameter's, one handed on, or a nominal record's definition row at its constructor (Functions §4.3's literal forms, #1066): `Point({x = n, y = price})` at `record Point = {x: Dec, y: Dec}` widens `n : Int` into the `Dec` field.
