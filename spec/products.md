...
| `...` (bare or named, at any depth, through any alias) in a nominal `record`'s field types or a union payload type | hard error at the field or slot: "a `record` names every field of its values; `r`'s type says the record may have more fields — name them, or give the field the type `JsValue`" (§4; Unions §2.1; FFI Part 1 §5.4 item 7 for the extern-declaration and constraint-member twins) |
