...
- **A record literal takes its fields' types from the record type it meets** — an annotation's, or a nominal record's definition row at its constructor (Functions §4.3's literal forms, #1066): `Point({x = n, y = price})` at `record Point = {x: Dec, y: Dec}` widens `n : Int` into the `Dec` field.
...
2. **A constructor function `Point : {x: Float, y: Float} -> Point`** in the term namespace (uppercase-start, per the Functions spec's reservation for constructors). Argument is the exact closed row — a definition row is always closed (§4) — and ordinary record-literal checking applies (all fields, no extras).
